...
public record ExtractedImage(
     ...
     * ({@code crates/kreuzberg/src/plugins/processor/builtin/captioning.rs}), which routes
     * each image through {@code crate.llm.region_extractor.extract_region_with_vlm} in
     * caption mode. {@code None} when captioning is disabled or the VLM declined to caption.
     */
    @Nullable @JsonProperty("caption") String caption,
    /**
     * QR codes decoded from this image, when QR detection is enabled.
     *
     * Populated by the QR post-processor ({@code crates/kreuzberg/src/extractors/qr.rs}) via
     * the pure-Rust {@code rqrr} decoder. {@code None} when QR detection is disabled; an empty
     * {@code Some(vec![])} when detection ran but found nothing.
     */
