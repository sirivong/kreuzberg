...
public final class KreuzbergRs {
    ...
    public static java.nio.file.Path downloadModel(
        final String name,
        final @Nullable java.nio.file.Path cacheDir
    ) throws KreuzbergRsException {
        try (var arena = Arena.ofShared()) {
    ...
    /**
     * Apply {@code strategy} to {@code original} for {@code category} and return the replacement token.
     *
     * The optional {@code counter} is required for RedactionStrategy.TokenReplace;
     * other strategies ignore it.
     */
    public static String applyStrategy(
        ...
        try (var arena = Arena.ofShared()) {
            var cstrategyJson = strategy != null ? MAPPER.writeValueAsString(strategy) : null;
            var cstrategyJsonSeg = cstrategyJson != null ? arena.allocateFrom(cstrategyJson) : MemorySegment.NULL;
            var cstrategy = cstrategyJson != null
                ? (MemorySegment) NativeLib.KREUZBERG_REDACTION_STRATEGY_FROM_JSON.invoke(cstrategyJsonSeg)
                : MemorySegment.NULL;
            if (cstrategyJson != null && cstrategy.equals(MemorySegment.NULL)) {
                checkLastError();
                throw new IllegalStateException("failed to create strategy from JSON");
            }
            var coriginal = arena.allocateFrom(original);
            var ccategoryJson = category != null ? MAPPER.writeValueAsString(category) : null;
            var ccategoryJsonSeg = ccategoryJson != null ? arena.allocateFrom(ccategoryJson) : MemorySegment.NULL;
            var ccategory = ccategoryJson != null
                ? (MemorySegment) NativeLib.KREUZBERG_PII_CATEGORY_FROM_JSON.invoke(ccategoryJsonSeg)
                : MemorySegment.NULL;
            if (ccategoryJson != null && ccategory.equals(MemorySegment.NULL)) {
                checkLastError();
                throw new IllegalStateException("failed to create category from JSON");
            }
            var ccounter = counter.handle();
            var resultPtr = (MemorySegment) NativeLib.KREUZBERG_APPLY_STRATEGY.invoke(cstrategy, coriginal, ccategory, ccounter);
            if (!cstrategy.equals(MemorySegment.NULL)) {
                NativeLib.KREUZBERG_REDACTION_STRATEGY_FREE.invoke(cstrategy);
            }
            if (!ccategory.equals(MemorySegment.NULL)) {
                NativeLib.KREUZBERG_PII_CATEGORY_FREE.invoke(ccategory);
            }
            if (resultPtr.equals(MemorySegment.NULL)) {
                checkLastError();                return null;            }
            long resultLen = (long) NativeLib.KREUZBERG_APPLY_STRATEGY_LEN.invoke(cstrategy, coriginal, ccategory, ccounter);
            String str = readCString(resultPtr, resultLen);
            NativeLib.KREUZBERG_FREE_STRING.invoke(resultPtr);
            return str;
        } catch (Throwable e) {
     ...
     * {@literal @}param image_bytes — Raw bytes of the **pre-cropped** region image.
     *
     * {@literal @}param image_mime — MIME type of the image ({@code "image/png"}, {@code "image/jpeg"}, etc.).
     *
     * {@literal @}param region_kind — Content type of the region, used to select the default prompt.
     *
     * {@literal @}param llm_config — LLM provider and model configuration.
     *
     * {@literal @}param custom_prompt — Optional override for the default per-region prompt template.
     *
