...
public final class KreuzbergRs {
     ...
     * {@code tokio.task.spawn_blocking} when on an async runtime.
     */
    public static PcmAudio decodeAudioToPcm(final byte[] bytes, final @Nullable Long maxBytes) throws KreuzbergRsException {
        try (var arena = Arena.ofShared()) {
            var cbytes = arena.allocateFrom(ValueLayout.JAVA_BYTE, bytes);
            var cbytesLen = (long) bytes.length;
            long cmaxBytes = (maxBytes == null) ? -1L : maxBytes;
            var resultPtr = (MemorySegment) NativeLib.KREUZBERG_DECODE_AUDIO_TO_PCM.invoke(cbytes, cbytesLen, cmaxBytes);
            if (resultPtr.equals(MemorySegment.NULL)) {
                checkLastError();                return null;            }
            // CPD-OFF
            var jsonPtr = (MemorySegment) NativeLib.KREUZBERG_PCM_AUDIO_TO_JSON.invoke(resultPtr);
            NativeLib.KREUZBERG_PCM_AUDIO_FREE.invoke(resultPtr);
            if (jsonPtr.equals(MemorySegment.NULL)) {
                checkLastError();
                throw new KreuzbergRsException("decodeAudioToPcm: failed to serialize response", null);
            }
            String json = jsonPtr.reinterpret(Long.MAX_VALUE).getString(0);
            NativeLib.KREUZBERG_FREE_STRING.invoke(jsonPtr);
            return MAPPER.readValue(json, PcmAudio.class);
            // CPD-ON
        } catch (Throwable e) {
            throw new KreuzbergRsException("FFI call failed", e);
        }
     ...
     * # Behaviour
     *
     * 1. Compute {@code target_dir = {cache_root}/whisper/{model_size}/} (see module docs for
     *    the full resolution order).
     * 2. If every required file already exists locally, return the paths immediately.
     * 3. If {@code allow_network} is {@code false} and the model is not cached, return
     ...
     * 5. Download each file via {@code hf-hub} into {@code target_dir} under canonical local names.
     * 6. When {@code verify_hash} is {@code true} emit a warning that the hash table is not yet
     ...
     * {@literal @}throws KreuzbergRsException Returns WhisperModelError on I/O failures, download failures, or when the
     * model is unavailable and {@code allow_network} is {@code false}.
     */
    public static WhisperModelPaths ensureWhisperModel(
        final WhisperModel model,
        final java.nio.file.@Nullable Path cacheDir,
        final boolean allowNetwork,
        final boolean verifyHash
    ) throws KreuzbergRsException {
        try (var arena = Arena.ofShared()) {
            var cmodelJson = model != null ? MAPPER.writeValueAsString(model) : null;
            var cmodelJsonSeg = cmodelJson != null ? arena.allocateFrom(cmodelJson) : MemorySegment.NULL;
            var cmodel = cmodelJson != null
                ? (MemorySegment) NativeLib.KREUZBERG_WHISPER_MODEL_FROM_JSON.invoke(cmodelJsonSeg)
                : MemorySegment.NULL;
            if (cmodelJson != null && cmodel.equals(MemorySegment.NULL)) {
                checkLastError();
                throw new IllegalStateException("failed to create model from JSON");
            }
            var ccacheDir = cacheDir != null ? arena.allocateFrom(cacheDir.toString()) : MemorySegment.NULL;
            var resultPtr = (MemorySegment) NativeLib.KREUZBERG_ENSURE_WHISPER_MODEL.invoke(cmodel, ccacheDir, (allowNetwork
                ? 1
                : 0), (verifyHash ? 1 : 0));
            if (!cmodel.equals(MemorySegment.NULL)) {
                NativeLib.KREUZBERG_WHISPER_MODEL_FREE.invoke(cmodel);
            }
            if (resultPtr.equals(MemorySegment.NULL)) {
                checkLastError();                return null;            }
            // CPD-OFF
            var jsonPtr = (MemorySegment) NativeLib.KREUZBERG_WHISPER_MODEL_PATHS_TO_JSON.invoke(resultPtr);
            NativeLib.KREUZBERG_WHISPER_MODEL_PATHS_FREE.invoke(resultPtr);
            if (jsonPtr.equals(MemorySegment.NULL)) {
                checkLastError();
                throw new KreuzbergRsException("ensureWhisperModel: failed to serialize response", null);
            }
            String json = jsonPtr.reinterpret(Long.MAX_VALUE).getString(0);
            NativeLib.KREUZBERG_FREE_STRING.invoke(jsonPtr);
            return MAPPER.readValue(json, WhisperModelPaths.class);
            // CPD-ON
        } catch (Throwable e) {
            throw new KreuzbergRsException("FFI call failed", e);
        }
     ...
     *
     * Never panics or returns an error — failures produce {@code AudioTags.default()}.
     */
    public static AudioTags readAudioTags(final byte[] bytes) throws KreuzbergRsException {
        try (var arena = Arena.ofShared()) {
            var cbytes = arena.allocateFrom(ValueLayout.JAVA_BYTE, bytes);
            var cbytesLen = (long) bytes.length;
            var resultPtr = (MemorySegment) NativeLib.KREUZBERG_READ_AUDIO_TAGS.invoke(cbytes, cbytesLen);
            if (resultPtr.equals(MemorySegment.NULL)) {
                checkLastError();                return null;            }
            // CPD-OFF
            var jsonPtr = (MemorySegment) NativeLib.KREUZBERG_AUDIO_TAGS_TO_JSON.invoke(resultPtr);
            NativeLib.KREUZBERG_AUDIO_TAGS_FREE.invoke(resultPtr);
            if (jsonPtr.equals(MemorySegment.NULL)) {
                checkLastError();
                throw new KreuzbergRsException("readAudioTags: failed to serialize response", null);
            }
            String json = jsonPtr.reinterpret(Long.MAX_VALUE).getString(0);
            NativeLib.KREUZBERG_FREE_STRING.invoke(jsonPtr);
            return MAPPER.readValue(json, AudioTags.class);
            // CPD-ON
        } catch (Throwable e) {
            throw new KreuzbergRsException("FFI call failed", e);
        }
