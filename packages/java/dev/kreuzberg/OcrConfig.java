...
public record OcrConfig(
     ...
     * OcrConfig.pipeline is {@code None}, a multi-stage pipeline is synthesised
     * automatically:
     *
     * - VlmFallbackPolicy.OnLowQuality → {@code [classical_stage, vlm_stage]} with the
     *   {@code quality_threshold} mapped onto OcrQualityThresholds.pipeline_min_quality.
     * - VlmFallbackPolicy.Always → {@code [vlm_stage]} only.
     *
     * Requires OcrConfig.vlm_config to be {@code Some} when not {@code Disabled}.
     ...
     *
     * Required when {@code backend} is {@code "vlm"} or when {@code vlm_fallback} is not
