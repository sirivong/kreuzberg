...
mod ffi {
    ...
    extern "Rust" {
        #[swift_bridge(swift_name = "extractSync")]
        fn extract_sync(input: ExtractInput, config: ExtractionConfig) -> Result<ExtractionOutput, String>;
        #[swift_bridge(swift_name = "extractBatchSync")]
        fn extract_batch_sync(inputs: Vec<ExtractInput>, config: ExtractionConfig) -> Result<ExtractionOutput, String>;
    }
...
impl ExtractionOutput {
    pub fn new(
        ...
        if let Ok(v) = ::serde_json::from_str::<::serde_json::Value>(&crawl) {
    ...
    pub fn crawl(&self) -> String {
        serde_json::to_string(&self.0.crawl).expect("serializable crawl")
    }
}
...
pub fn extract_sync(input: ExtractInput, config: ExtractionConfig) -> Result<ExtractionOutput, String> {
    xberg::extract_sync(input.0, &config.0)
        .map_err(|e| e.to_string())
...
pub fn extract_batch_sync(inputs: Vec<ExtractInput>, config: ExtractionConfig) -> Result<ExtractionOutput, String> {
    xberg::extract_batch_sync(inputs.into_iter().map(|w| w.0).collect::<Vec<_>>(), &config.0)
        .map_err(|e| e.to_string())
        .map(ExtractionOutput)
}
...
impl xberg::DocumentExtractor for SwiftDocumentExtractorWrapper {
    async fn extract_bytes(
        ...
        decode_inbound_envelope::<xberg::InternalDocument>(&envelope)
    }
