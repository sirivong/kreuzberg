...
public struct LayoutDetectionConfig: Codable, Sendable, Hashable {
    ...
    public init(confidenceThreshold: Float? = nil, applyHeuristics: Bool, tableModel: TableModel, acceleration: AccelerationConfig? = nil, enableChartUnderstanding: Bool) {
...
public struct RedactionConfig: Codable, Sendable, Hashable {
    ...
    public func validate() -> Void {
        fatalError("Not yet implemented: redaction_config_validate")
    }
...
public struct ServerConfig: Codable, Sendable, Hashable {
    ...
    public func listenAddr() -> String {
        fatalError("Not yet implemented: server_config_listen_addr")
    }

    public func corsAllowsAll() -> Bool {
        fatalError("Not yet implemented: server_config_cors_allows_all")
    }

    public func isOriginAllowed(origin: String) -> Bool {
        fatalError("Not yet implemented: server_config_is_origin_allowed")
    }

    public func maxRequestBodyMb() -> UInt {
        fatalError("Not yet implemented: server_config_max_request_body_mb")
    }

    public func maxMultipartFieldMb() -> UInt {
        fatalError("Not yet implemented: server_config_max_multipart_field_mb")
    }
...
internal extension PdfFormField {
    init(_ rb: RustBridge.PdfFormFieldRef) throws {
        ...
        self.fieldType = try { let rawValue = rb.fieldType().toString(); guard let value = FormFieldType(rawValue: rawValue) else { throw KreuzbergError.validation(message: "Unknown FormFieldType variant", source: rawValue) }; return value }()
        self.value = rb.value()?.toString()
...
public struct ChunkingResult: Codable, Sendable, Hashable {
    /// List of text chunks
...
// MARK: - Internal FFI conversions for ChunkingResult
internal extension ChunkingResult {
    init(_ rb: RustBridge.ChunkingResultRef) throws {
        self.chunks = try rb.chunks().map { try Chunk($0) }
        self.chunkCount = rb.chunkCount()
    }

    func intoRust() throws -> RustBridge.ChunkingResult {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "{}"
        return try RustBridge.chunkingResultFromJson(json)
    }
}
...
/// ```
public struct EnrichOptions: Codable, Sendable, Hashable {
    ...
    /// result.
    public let labels: [String]
    public init(keywords: Bool, entities: Bool, labels: [String]) {
        self.keywords = keywords
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.keywords = try container.decodeIfPresent(Bool.self, forKey: .keywords) ?? false
        self.entities = try container.decodeIfPresent(Bool.self, forKey: .entities) ?? false
        self.labels = try container.decodeIfPresent([String].self, forKey: .labels) ?? []
    }
}

// MARK: - Internal FFI conversions for EnrichOptions
internal extension EnrichOptions {
    init(_ rb: RustBridge.EnrichOptionsRef) throws {
        self.keywords = rb.keywords()
        self.entities = rb.entities()
        self.labels = rb.labels().map { $0.as_str().toString() }
    }

    func intoRust() throws -> RustBridge.EnrichOptions {
        let __labels = RustVec<RustString>()
        for __elem in self.labels { __labels.push(value: RustString(__elem)) }
        return RustBridge.EnrichOptions(self.keywords, self.entities, __labels)
    }
}
...
/// ```
public struct EnrichResult: Codable, Sendable, Hashable {
    ...
    public let labels: [String]
    public init(keywords: [String], entities: [Entity], labels: [String]) {
        self.keywords = keywords
        self.entities = entities
        self.labels = labels
    }
...
// MARK: - Internal FFI conversions for EnrichResult
internal extension EnrichResult {
    init(_ rb: RustBridge.EnrichResultRef) throws {
        self.keywords = rb.keywords().map { $0.as_str().toString() }
        self.entities = try rb.entities().map { try Entity($0) }
        self.labels = rb.labels().map { $0.as_str().toString() }
    }

    func intoRust() throws -> RustBridge.EnrichResult {
        let __keywords = RustVec<RustString>()
        for __elem in self.keywords { __keywords.push(value: RustString(__elem)) }
        let __entities = RustVec<RustBridge.Entity>()
        for __elem in self.entities { __entities.push(value: try __elem.intoRust()) }
        let __labels = RustVec<RustString>()
        for __elem in self.labels { __labels.push(value: RustString(__elem)) }
        return RustBridge.EnrichResult(__keywords, __entities, __labels)
    }
}
...
internal extension UserChunkConfig {
    init(_ rb: RustBridge.UserChunkConfigRef) throws {
        self.pageRanges = try rb.pageRanges()?.map { try PageRange($0) }
...
public struct ConfidenceSignals: Codable, Sendable, Hashable {
    ...
    public init(textCoverage: Float, ocrAggregate: Float? = nil, schemaCompliance: SchemaCompliance) {
...
// MARK: - Internal FFI conversions for ConfidenceSignals
internal extension ConfidenceSignals {
    init(_ rb: RustBridge.ConfidenceSignalsRef) throws {
        self.textCoverage = rb.textCoverage()
        self.ocrAggregate = rb.ocrAggregate()
        self.schemaCompliance = try { let rawValue = rb.schemaCompliance().toString(); guard let value = SchemaCompliance(rawValue: rawValue) else { throw KreuzbergError.validation(message: "Unknown SchemaCompliance variant", source: rawValue) }; return value }()
    }

    func intoRust() throws -> RustBridge.ConfidenceSignals {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "{}"
        return try RustBridge.confidenceSignalsFromJson(json)
    }
}
...
public struct ConfidenceWeights: Codable, Sendable, Hashable {
    ...
    public let schemaCompliance: Float
    public init(textCoverage: Float, ocrAggregate: Float, schemaCompliance: Float) {
        self.textCoverage = textCoverage
        self.ocrAggregate = ocrAggregate
        self.schemaCompliance = schemaCompliance
    }
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.textCoverage = try container.decodeIfPresent(Float.self, forKey: .textCoverage) ?? 0.3
        self.ocrAggregate = try container.decodeIfPresent(Float.self, forKey: .ocrAggregate) ?? 0.3
        self.schemaCompliance = try container.decodeIfPresent(Float.self, forKey: .schemaCompliance) ?? 0.4
    }
    // MARK: - Instance Methods
    public func isNormalized() -> Bool {
        fatalError("Not yet implemented: confidence_weights_is_normalized")
    }

}

// MARK: - Internal FFI conversions for ConfidenceWeights
internal extension ConfidenceWeights {
    init(_ rb: RustBridge.ConfidenceWeightsRef) throws {
        self.textCoverage = rb.textCoverage()
        self.ocrAggregate = rb.ocrAggregate()
        self.schemaCompliance = rb.schemaCompliance()
    }

    func intoRust() throws -> RustBridge.ConfidenceWeights {
        return RustBridge.ConfidenceWeights(self.textCoverage, self.ocrAggregate, self.schemaCompliance)
    }
}
...
public struct HeuristicsConfig: Codable, Sendable, Hashable {
    ...
    // MARK: - Instance Methods
    public func validate() -> Void {
        fatalError("Not yet implemented: heuristics_config_validate")
    }
...
public struct ChunkPlan: Codable, Sendable, Hashable {
    ...
    public let totalEstimatedTimeMs: UInt64
    /// Whether to use disk-based processing for large files.
    public let useDiskProcessing: Bool
    /// Reason for chunking.
    public let reason: ChunkingReason
    public init(totalChunks: UInt32, chunks: [ChunkInfo], totalEstimatedTimeMs: UInt64, useDiskProcessing: Bool, reason: ChunkingReason) {
        self.totalChunks = totalChunks
        self.chunks = chunks
        self.totalEstimatedTimeMs = totalEstimatedTimeMs
        self.useDiskProcessing = useDiskProcessing
        self.reason = reason
    }
    private enum CodingKeys: String, CodingKey {
        case totalChunks = "total_chunks"
        case chunks = "chunks"
        case totalEstimatedTimeMs = "total_estimated_time_ms"
        case useDiskProcessing = "use_disk_processing"
        case reason = "reason"
    }
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.totalChunks = try container.decodeIfPresent(UInt32.self, forKey: .totalChunks) ?? 0
        self.chunks = try container.decodeIfPresent([ChunkInfo].self, forKey: .chunks) ?? []
        self.totalEstimatedTimeMs = try container.decodeIfPresent(UInt64.self, forKey: .totalEstimatedTimeMs) ?? 0
        self.useDiskProcessing = try container.decodeIfPresent(Bool.self, forKey: .useDiskProcessing) ?? false
        self.reason = try container.decode(ChunkingReason.self, forKey: .reason)
    }
    // MARK: - Instance Methods
    public func totalPages() -> UInt32 {
        fatalError("Not yet implemented: chunk_plan_total_pages")
    }

}

// MARK: - Internal FFI conversions for ChunkPlan
internal extension ChunkPlan {
    init(_ rb: RustBridge.ChunkPlanRef) throws {
        self.totalChunks = rb.totalChunks()
        self.chunks = try rb.chunks().map { try ChunkInfo($0) }
        self.totalEstimatedTimeMs = rb.totalEstimatedTimeMs()
        self.useDiskProcessing = rb.useDiskProcessing()
        self.reason = try JSONDecoder().decode(ChunkingReason.self, from: ((rb.reason().toString()).data(using: .utf8) ?? Data("null".utf8)))
    }

    func intoRust() throws -> RustBridge.ChunkPlan {
        let __chunks = RustVec<RustBridge.ChunkInfo>()
        for __elem in self.chunks { __chunks.push(value: try __elem.intoRust()) }
        return RustBridge.ChunkPlan(self.totalChunks, __chunks, self.totalEstimatedTimeMs, self.useDiskProcessing, try self.reason.intoRust())
    }
}
...
public struct PageRange: Codable, Sendable, Hashable {
    ...
    public func pageCount() -> UInt32 {
        fatalError("Not yet implemented: page_range_page_count")
    }
...
internal extension MultidocInput {
    init(_ rb: RustBridge.MultidocInputRef) throws {
        self.pageCount = rb.pageCount()
        self.pages = try rb.pages().map { try PageSignals($0) }
    }
...
internal extension DocumentBoundary {
    init(_ rb: RustBridge.DocumentBoundaryRef) throws {
        ...
        self.confidence = rb.confidence()
        self.reason = try { let rawValue = rb.reason().toString(); guard let value = BoundaryReason(rawValue: rawValue) else { throw KreuzbergError.validation(message: "Unknown BoundaryReason variant", source: rawValue) }; return value }()
    }
...
public struct StructuredInput: Codable, Sendable, Hashable {
    ...
    public let avgCharsPerPage: Double
    /// Count of embedded images (figures, photos, signatures) discovered.
    public let embeddedImageCount: UInt32
    /// When `true`, promote the result to at least [`StructuredCallMode::TextPlusVision`].
    public let userForceVision: Bool
    public init(mimeType: String, pageCount: UInt32, textCoverage: Double, avgCharsPerPage: Double, embeddedImageCount: UInt32, userForceVision: Bool) {
        self.mimeType = mimeType
        self.pageCount = pageCount
        self.textCoverage = textCoverage
        self.avgCharsPerPage = avgCharsPerPage
        self.embeddedImageCount = embeddedImageCount
        self.userForceVision = userForceVision
    }
    private enum CodingKeys: String, CodingKey {
        case mimeType = "mime_type"
        case pageCount = "page_count"
        ...
        case userForceVision = "user_force_vision"
    }
}

// MARK: - Internal FFI conversions for StructuredInput
internal extension StructuredInput {
    init(_ rb: RustBridge.StructuredInputRef) throws {
        self.mimeType = rb.mimeType().toString()
        self.pageCount = rb.pageCount()
        self.textCoverage = rb.textCoverage()
        self.avgCharsPerPage = rb.avgCharsPerPage()
        self.embeddedImageCount = rb.embeddedImageCount()
        self.userForceVision = rb.userForceVision()
    }

    func intoRust() throws -> RustBridge.StructuredInput {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "{}"
        return try RustBridge.structuredInputFromJson(json)
    }
}
...
/// };
/// ```
public struct StructuredThresholds: Codable, Sendable, Hashable {
    ...
    public let scanMaxCoverage: Double
    ...
    /// config.
    public let enableVisionFallback: Bool
    public init(scanMaxCoverage: Double, digitalMinCoverage: Double, docxTextMinDensity: Double, enableVisionFallback: Bool) {
        self.scanMaxCoverage = scanMaxCoverage
        self.digitalMinCoverage = digitalMinCoverage
        self.docxTextMinDensity = docxTextMinDensity
        self.enableVisionFallback = enableVisionFallback
    }
    private enum CodingKeys: String, CodingKey {
        case scanMaxCoverage = "scan_max_coverage"
        case digitalMinCoverage = "digital_min_coverage"
        case docxTextMinDensity = "docx_text_min_density"
        case enableVisionFallback = "enable_vision_fallback"
    }
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.scanMaxCoverage = try container.decodeIfPresent(Double.self, forKey: .scanMaxCoverage) ?? 0.1
        self.digitalMinCoverage = try container.decodeIfPresent(Double.self, forKey: .digitalMinCoverage) ?? 0.9
        self.docxTextMinDensity = try container.decodeIfPresent(Double.self, forKey: .docxTextMinDensity) ?? 200.0
        self.enableVisionFallback = try container.decodeIfPresent(Bool.self, forKey: .enableVisionFallback) ?? false
    }
}

// MARK: - Internal FFI conversions for StructuredThresholds
internal extension StructuredThresholds {
    init(_ rb: RustBridge.StructuredThresholdsRef) throws {
        self.scanMaxCoverage = rb.scanMaxCoverage()
        self.digitalMinCoverage = rb.digitalMinCoverage()
        self.docxTextMinDensity = rb.docxTextMinDensity()
        self.enableVisionFallback = rb.enableVisionFallback()
    }

    func intoRust() throws -> RustBridge.StructuredThresholds {
        return RustBridge.StructuredThresholds(self.scanMaxCoverage, self.digitalMinCoverage, self.docxTextMinDensity, self.enableVisionFallback)
    }
}
...
internal extension PresetSummary {
    init(_ rb: RustBridge.PresetSummaryRef) throws {
        ...
        self.description = rb.description().toString()
        self.category = try { let rawValue = rb.category().toString(); guard let value = PresetCategory(rawValue: rawValue) else { throw KreuzbergError.validation(message: "Unknown PresetCategory variant", source: rawValue) }; return value }()
        self.tags = rb.tags().map { $0.as_str().toString() }
        self.preferredCallMode = try { let rawValue = rb.preferredCallMode().toString(); guard let value = CallMode(rawValue: rawValue) else { throw KreuzbergError.validation(message: "Unknown CallMode variant", source: rawValue) }; return value }()
...
public typealias ClassificationEnrichmentConfig = RustBridge.ClassificationEnrichmentConfig
...
public typealias CaptioningEnrichmentConfig = RustBridge.CaptioningEnrichmentConfig
...
public typealias ChunkingResultRef = RustBridge.ChunkingResultRef

public typealias ChunkingResultRefMut = RustBridge.ChunkingResultRefMut
...
public typealias EnrichOptionsRef = RustBridge.EnrichOptionsRef

public typealias EnrichOptionsRefMut = RustBridge.EnrichOptionsRefMut

public typealias EnrichResultRef = RustBridge.EnrichResultRef

public typealias EnrichResultRefMut = RustBridge.EnrichResultRefMut
...
public typealias ConfidenceSignalsRef = RustBridge.ConfidenceSignalsRef

public typealias ConfidenceSignalsRefMut = RustBridge.ConfidenceSignalsRefMut

public typealias ConfidenceWeightsRef = RustBridge.ConfidenceWeightsRef

public typealias ConfidenceWeightsRefMut = RustBridge.ConfidenceWeightsRefMut
...
public typealias ChunkPlanRef = RustBridge.ChunkPlanRef

public typealias ChunkPlanRefMut = RustBridge.ChunkPlanRefMut
...
public typealias StructuredInputRef = RustBridge.StructuredInputRef

public typealias StructuredInputRefMut = RustBridge.StructuredInputRefMut

public typealias StructuredThresholdsRef = RustBridge.StructuredThresholdsRef

public typealias StructuredThresholdsRefMut = RustBridge.StructuredThresholdsRefMut
...
/// ```
public enum EnrichStatus: Codable, Sendable, Hashable {
    /// Job submitted; processing has not yet started or is in progress.
    case pending
    /// Processing completed successfully.
    case completed(result: EnrichResult)
    /// Processing failed.
    ...
    public init(from decoder: Decoder) throws {
        ...
        case "pending":
            self = .pending
        case "completed":
            self = .completed(result: try container.decode(EnrichResult.self, forKey: .result))
        case "failed":
        ...
        default:
            throw DecodingError.dataCorruptedError(
                forKey: .status,
                in: container,
                debugDescription: "Unknown EnrichStatus type: \(type)"
            )
        }
    }
...
extension EnrichStatus {
    func intoRust() throws -> RustBridge.EnrichStatus {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "null"
        return try RustBridge.enrichStatusFromJson(json)
    }
}
...
public enum ChunkingDecision: Codable, Sendable, Hashable {
    /// Process without chunking (small file, text layer detected, etc.)
    case noChunking(reason: NoChunkingReason)
    /// Chunk according to plan.
    case chunk(field0: ChunkPlan)
    /// Use user-provided chunk overrides.
    case useOverrides(userChunks: [PageRange])
    ...
    public init(from decoder: Decoder) throws {
        ...
        switch type {
        case "NoChunking":
            self = .noChunking(reason: try container.decode(NoChunkingReason.self, forKey: .reason))
        case "Chunk":
            self = .chunk(field0: try container.decode(ChunkPlan.self, forKey: .field0))
        case "UseOverrides":
            self = .useOverrides(userChunks: try container.decode([PageRange].self, forKey: .userChunks))
        default:
            throw DecodingError.dataCorruptedError(
                forKey: .type,
                in: container,
                debugDescription: "Unknown ChunkingDecision type: \(type)"
            )
        }
    }
    ...
    public func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        switch self {
        case .noChunking(let reason):
            try container.encode("NoChunking", forKey: .type)
            try container.encode(reason, forKey: .reason)

        case .chunk(let field0):
            try container.encode("Chunk", forKey: .type)
            try container.encode(field0, forKey: .field0)

        case .useOverrides(let userChunks):
            try container.encode("UseOverrides", forKey: .type)
            try container.encode(userChunks, forKey: .userChunks)

        }
    }
}
extension ChunkingDecision {
    func intoRust() throws -> RustBridge.ChunkingDecision {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "null"
        return try RustBridge.chunkingDecisionFromJson(json)
    }
}
...
/// pipeline.
public enum StructuredCallMode: String, Codable, Sendable, Hashable {
...
extension StructuredCallMode {
    func intoRust() throws -> RustBridge.StructuredCallMode {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "null"
        return try RustBridge.structuredCallModeFromJson(json)
    }
}
...
public func classifyDocument(_ pages: [String], _ configJson: String) async throws -> [ClassificationLabel] {
    let config = try pageClassificationConfigFromJson(configJson)
    return try await classifyDocument(pages: pages, config: config)
}
...
public func chunkForRag(_ text: String, _ configJson: String) throws -> ChunkingResult {
    let config = try chunkingConfigFromJson(configJson)
    return try chunkForRag(text: text, config: config)
}
...
public func analyzeDocument(_ metadataJson: String, _ configJson: String, _ documentBytes: [UInt8]?) throws -> ChunkingDecision {
    let metadata = try documentMetadataFromJson(metadataJson)
    let config = try heuristicsConfigFromJson(configJson)
    return try analyzeDocument(metadata: metadata, config: config, documentBytes: documentBytes)
}
public func analyzeWithUserChunks(_ userRanges: [PageRange], _ totalPages: UInt32, _ sizeBytes: UInt64, _ configJson: String) throws -> ChunkingDecision {
    let config = try heuristicsConfigFromJson(configJson)
    return try analyzeWithUserChunks(userRanges: userRanges, totalPages: totalPages, sizeBytes: sizeBytes, config: config)
}
public func scoreConfidence(_ signalsJson: String, _ weightsJson: String) throws -> ExtractionConfidence {
    let signals = try confidenceSignalsFromJson(signalsJson)
    let weights = try confidenceWeightsFromJson(weightsJson)
    return try scoreConfidence(signals: signals, weights: weights)
}
public func checkFormatLimits(_ mimeType: String, _ sheetCount: UInt32?, _ workbookCells: UInt64?, _ embeddedCount: UInt32?, _ configJson: String) throws -> String? {
    let config = try heuristicsConfigFromJson(configJson)
    return try checkFormatLimits(mimeType: mimeType, sheetCount: sheetCount, workbookCells: workbookCells, embeddedCount: embeddedCount, config: config)
}
public func boundariesFromExtractionResult(_ resultJson: String, _ thresholdsJson: String) throws -> [DocumentBoundary] {
    let result = try extractionResultFromJson(resultJson)
    let thresholds = try multidocThresholdsFromJson(thresholdsJson)
    return try boundariesFromExtractionResult(result: result, thresholds: thresholds)
}
public func detectBoundaries(_ inputJson: String, _ thresholdsJson: String) throws -> [DocumentBoundary] {
    let input = try multidocInputFromJson(inputJson)
    let thresholds = try multidocThresholdsFromJson(thresholdsJson)
    return try detectBoundaries(input: input, thresholds: thresholds)
}
public func chooseCallMode(_ inputJson: String, _ tJson: String) throws -> StructuredCallMode {
    let input = try structuredInputFromJson(inputJson)
    let t = try structuredThresholdsFromJson(tJson)
    return try chooseCallMode(input: input, t: t)
}
public func calculateChunkPlan(_ pageCount: UInt32, _ sizeBytes: UInt64, _ needsOcr: Bool, _ configJson: String) throws -> ChunkPlan {
    let config = try heuristicsConfigFromJson(configJson)
    return try calculateChunkPlan(pageCount: pageCount, sizeBytes: sizeBytes, needsOcr: needsOcr, config: config)
}
public func calculatePlanFromOverrides(_ userChunks: [PageRange], _ totalPages: UInt32, _ sizeBytes: UInt64, _ configJson: String) throws -> ChunkPlan {
    let config = try heuristicsConfigFromJson(configJson)
    return try calculatePlanFromOverrides(userChunks: userChunks, totalPages: totalPages, sizeBytes: sizeBytes, config: config)
}
public func resolve(_ presetJson: String, _ customSchema: String?, _ context: [String: String]) throws -> ResolvedPreset {
    let preset = try presetFromJson(presetJson)
    return try resolve(preset: preset, customSchema: customSchema, context: context)
}
public func captionImage(_ imageBytes: [UInt8], _ llmConfigJson: String, _ customPrompt: String?) async throws -> String {
    let llmConfig = try llmConfigFromJson(llmConfigJson)
    return try await captionImage(imageBytes: imageBytes, llmConfig: llmConfig, customPrompt: customPrompt)
}
public func captionImageFile(_ path: String, _ llmConfigJson: String, _ customPrompt: String?) async throws -> String {
    let llmConfig = try llmConfigFromJson(llmConfigJson)
    return try await captionImageFile(path: path, llmConfig: llmConfig, customPrompt: customPrompt)
}
...
public func chunkingResultFromJson(_ json: String) throws -> ChunkingResult {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(ChunkingResult.self, from: data)
}
...
public func enrichOptionsFromJson(_ json: String) throws -> EnrichOptions {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(EnrichOptions.self, from: data)
}
public func enrichResultFromJson(_ json: String) throws -> EnrichResult {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(EnrichResult.self, from: data)
}
...
public func confidenceSignalsFromJson(_ json: String) throws -> ConfidenceSignals {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(ConfidenceSignals.self, from: data)
}
public func confidenceWeightsFromJson(_ json: String) throws -> ConfidenceWeights {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(ConfidenceWeights.self, from: data)
}
...
public func chunkPlanFromJson(_ json: String) throws -> ChunkPlan {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(ChunkPlan.self, from: data)
}
...
public func structuredInputFromJson(_ json: String) throws -> StructuredInput {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(StructuredInput.self, from: data)
}
public func structuredThresholdsFromJson(_ json: String) throws -> StructuredThresholds {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(StructuredThresholds.self, from: data)
}
...
public func enrichStatusFromJson(_ json: String) throws -> EnrichStatus {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(EnrichStatus.self, from: data)
}
...
public func chunkingDecisionFromJson(_ json: String) throws -> ChunkingDecision {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(ChunkingDecision.self, from: data)
}
...
public func structuredCallModeFromJson(_ json: String) throws -> StructuredCallMode {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(StructuredCallMode.self, from: data)
}
...
/// ```
public func classifyDocument(pages: [String], config: PageClassificationConfig) async throws -> [ClassificationLabel] {
    return try await Task.detached(priority: .userInitiated) {
        let _rb_pages: RustVec<RustString> = { let v = RustVec<RustString>(); for s in pages { v.push(value: RustString(s)) }; return v }()
        let _rb_config = try config.intoRust()
        let result = try RustBridge.classifyDocument(_rb_pages, _rb_config)
        var items: [ClassificationLabel] = []
        for ref in result {
            items.append(try ClassificationLabel(ref))
        }
...
public func chunkForRag(text: String, config: ChunkingConfig) throws -> ChunkingResult {
        let _rb_text = RustString(text)
    let _rb = try RustBridge.chunkForRag(_rb_text, config)
    return try ChunkingResult(_rb)
}
...
public func analyzeDocument(metadata: DocumentMetadata, config: HeuristicsConfig, documentBytes: [UInt8]? = nil) throws -> ChunkingDecision {
        let _rb_metadata = try metadata.intoRust()
        let _rb_config = try config.intoRust()
        let _rb_documentBytes = documentBytes.map { bytes -> RustVec<UInt8> in let v = RustVec<UInt8>(); for b in bytes { v.push(value: b) }; return v }
    let _rb = try RustBridge.analyzeDocument(_rb_metadata, _rb_config, _rb_documentBytes)
    return try ChunkingDecision(_rb)
}
/// Analyze a document with user-specified chunk ranges.
///
/// Creates a chunk plan based on user-provided page ranges.
public func analyzeWithUserChunks(userRanges: [PageRange], totalPages: UInt32, sizeBytes: UInt64, config: HeuristicsConfig) throws -> ChunkingDecision {
        let _rb_userRanges: RustVec<RustString> = try ({ () throws -> RustVec<RustString> in let v = RustVec<RustString>(); for item in userRanges { let data = try JSONEncoder().encode(item); let json = String(data: data, encoding: .utf8) ?? "null"; v.push(value: RustString(json)) }; return v }())
        let _rb_config = try config.intoRust()
    let _rb = RustBridge.analyzeWithUserChunks(_rb_userRanges, totalPages, sizeBytes, _rb_config)
    return try ChunkingDecision(_rb)
}
...
public func scoreConfidence(signals: ConfidenceSignals, weights: ConfidenceWeights) throws -> ExtractionConfidence {
        let _rb_signals = try signals.intoRust()
        let _rb_weights = try weights.intoRust()
    let _rb = RustBridge.scoreConfidence(_rb_signals, _rb_weights)
    return try ExtractionConfidence(_rb)
}
...
public func checkFormatLimits(mimeType: String, sheetCount: UInt32? = nil, workbookCells: UInt64? = nil, embeddedCount: UInt32? = nil, config: HeuristicsConfig) throws -> String? {
        let _rb_mimeType = RustString(mimeType)
        let _rb_config = try config.intoRust()
    let _rb_json = RustBridge.checkFormatLimits(_rb_mimeType, sheetCount, workbookCells, embeddedCount, _rb_config).toString()
    let _rb_data = _rb_json.data(using: .utf8) ?? Data()
    return (try? JSONDecoder().decode(String?.self, from: _rb_data)) ?? nil
}
...
public func boundariesFromExtractionResult(result: ExtractionResult, thresholds: MultidocThresholds) throws -> [DocumentBoundary] {
        let _rb_thresholds = try thresholds.intoRust()
    return try RustBridge.boundariesFromExtractionResult(result, _rb_thresholds).map { ref in try DocumentBoundary(ref) }
}
...
public func detectBoundaries(input: MultidocInput, thresholds: MultidocThresholds) throws -> [DocumentBoundary] {
        let _rb_input = try input.intoRust()
        let _rb_thresholds = try thresholds.intoRust()
    return try RustBridge.detectBoundaries(_rb_input, _rb_thresholds).map { ref in try DocumentBoundary(ref) }
}
...
///   `Skip`).
public func chooseCallMode(input: StructuredInput, t: StructuredThresholds) throws -> StructuredCallMode {
        let _rb_input = try input.intoRust()
        let _rb_t = try t.intoRust()
    let _rb = RustBridge.chooseCallMode(_rb_input, _rb_t)
    return try StructuredCallMode(_rb)
}
...
public func calculateChunkPlan(pageCount: UInt32, sizeBytes: UInt64, needsOcr: Bool, config: HeuristicsConfig) throws -> ChunkPlan {
        let _rb_config = try config.intoRust()
    let _rb = RustBridge.calculateChunkPlan(pageCount, sizeBytes, needsOcr, _rb_config)
    return try ChunkPlan(_rb)
}
/// Calculate a chunk plan from user-specified page ranges.
///
/// Validates and processes user overrides into a proper chunk plan.
public func calculatePlanFromOverrides(userChunks: [PageRange], totalPages: UInt32, sizeBytes: UInt64, config: HeuristicsConfig) throws -> ChunkPlan {
        let _rb_userChunks: RustVec<RustString> = try ({ () throws -> RustVec<RustString> in let v = RustVec<RustString>(); for item in userChunks { let data = try JSONEncoder().encode(item); let json = String(data: data, encoding: .utf8) ?? "null"; v.push(value: RustString(json)) }; return v }())
        let _rb_config = try config.intoRust()
    let _rb = RustBridge.calculatePlanFromOverrides(_rb_userChunks, totalPages, sizeBytes, _rb_config)
    return try ChunkPlan(_rb)
}
/// Stable sha256 fingerprint of `raw`, formatted as `sha256:<hex>`.
public func fingerprint(raw: [UInt8]) -> String {
        let _rb_raw: RustVec<UInt8> = { let v = RustVec<UInt8>(); for b in raw { v.push(value: b) }; return v }()
    return RustBridge.fingerprint(_rb_raw).toString()
}
...
public func resolve(preset: Preset, customSchema: String? = nil, context: [String: String]) throws -> ResolvedPreset {
    return try RustBridge.resolve(preset, customSchema, context)
}
...
public func extractStructuredJson(bytes: [UInt8], mime: String, presetSpecJson: String, optionsJson: String) throws -> String {
        let _rb_bytes: RustVec<UInt8> = { let v = RustVec<UInt8>(); for b in bytes { v.push(value: b) }; return v }()
        let _rb_mime = RustString(mime)
        let _rb_presetSpecJson = RustString(presetSpecJson)
        let _rb_optionsJson = RustString(optionsJson)
    return try RustBridge.extractStructuredJson(_rb_bytes, _rb_mime, _rb_presetSpecJson, _rb_optionsJson).toString()
}
...
/// via [`From<StructuredError>`](super::StructuredError).
public func splitAndExtractJson(bytes: [UInt8], mime: String, presetSpecJson: String, optionsJson: String) throws -> String {
        let _rb_bytes: RustVec<UInt8> = { let v = RustVec<UInt8>(); for b in bytes { v.push(value: b) }; return v }()
        let _rb_mime = RustString(mime)
        let _rb_presetSpecJson = RustString(presetSpecJson)
        let _rb_optionsJson = RustString(optionsJson)
    return try RustBridge.splitAndExtractJson(_rb_bytes, _rb_mime, _rb_presetSpecJson, _rb_optionsJson).toString()
}
...
/// # }
/// ```
public func captionImage(imageBytes: [UInt8], llmConfig: LlmConfig, customPrompt: String? = nil) async throws -> String {
    return try await Task.detached(priority: .userInitiated) {
        let _rb_imageBytes: RustVec<UInt8> = { let v = RustVec<UInt8>(); for b in imageBytes { v.push(value: b) }; return v }()
        let _rb_llmConfig = try llmConfig.intoRust()
        let _rb_customPrompt = customPrompt.map { RustString($0) }
        let result = try RustBridge.captionImage(_rb_imageBytes, _rb_llmConfig, _rb_customPrompt)
        return result.toString()
    }.value
}
...
/// # }
/// ```
public func captionImageFile(path: String, llmConfig: LlmConfig, customPrompt: String? = nil) async throws -> String {
    return try await Task.detached(priority: .userInitiated) {
        let _rb_path = RustString(path)
        let _rb_llmConfig = try llmConfig.intoRust()
        let _rb_customPrompt = customPrompt.map { RustString($0) }
        let result = try RustBridge.captionImageFile(_rb_path, _rb_llmConfig, _rb_customPrompt)
        return result.toString()
    }.value
}
...
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.ChunkingResult: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
...
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.EnrichOptions: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.EnrichResult: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.DocumentMetadata: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.UserChunkConfig: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.ConfidenceSignals: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.ConfidenceWeights: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.ExtractionConfidence: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.HeuristicsConfig: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.ChunkPlan: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
...
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.StructuredInput: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.StructuredThresholds: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
...
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.ClassificationEnrichmentConfig: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.CaptioningEnrichmentConfig: @unchecked Sendable {}
