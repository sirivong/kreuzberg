...
public struct ImageExtractionConfig: Codable, Sendable, Hashable {
    ...
    public init(extractImages: Bool, targetDpi: Int32, maxImageDimension: Int32, injectPlaceholders: Bool, autoAdjustDpi: Bool, minDpi: Int32, maxDpi: Int32, maxImagesPerPage: UInt32? = nil, classify: Bool, includePageRasters: Bool, runOcrOnImages: Bool, ocrTextOnly: Bool, appendOcrText: Bool, outputFormat: ImageOutputFormat) {
...
internal extension ImageExtractionConfig {
    ...
    func intoRust() throws -> RustBridge.ImageExtractionConfig {
        return RustBridge.ImageExtractionConfig(self.extractImages, self.targetDpi, self.maxImageDimension, self.injectPlaceholders, self.autoAdjustDpi, self.minDpi, self.maxDpi, self.maxImagesPerPage, self.classify, self.includePageRasters, self.runOcrOnImages, self.ocrTextOnly, self.appendOcrText, try self.outputFormat.intoRust())
    }
}
...
public typealias PcmAudio = RustBridge.PcmAudio

/// On-disk paths for all files needed to load a Whisper model in an ORT session.
public typealias WhisperModelPaths = RustBridge.WhisperModelPaths

/// Tag and audio-property data extracted from an audio/video file.
public typealias AudioTags = RustBridge.AudioTags
...
public func decodeAudioToPcm(bytes: [UInt8], maxBytes: UInt64? = nil) throws -> PcmAudio {
        let _rb_bytes: RustVec<UInt8> = { let v = RustVec<UInt8>(); for b in bytes { v.push(value: b) }; return v }()
    return try RustBridge.decodeAudioToPcm(_rb_bytes, maxBytes)
}
...
public func ensureWhisperModel(model: WhisperModel, cacheDir: String? = nil, allowNetwork: Bool, verifyHash: Bool) throws -> WhisperModelPaths {
        let _rb_model = try model.intoRust()
        let _rb_cacheDir = cacheDir.map { RustString($0) }
    return try RustBridge.ensureWhisperModel(_rb_model, _rb_cacheDir, allowNetwork, verifyHash)
}
...
public func readAudioTags(bytes: [UInt8]) -> AudioTags {
        let _rb_bytes: RustVec<UInt8> = { let v = RustVec<UInt8>(); for b in bytes { v.push(value: b) }; return v }()
    return RustBridge.readAudioTags(_rb_bytes)
}
...
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.PcmAudio: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.WhisperModelPaths: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.AudioTags: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
