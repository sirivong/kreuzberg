...
public typealias CaptioningConfig = RustBridge.CaptioningConfig
...
public struct ChunkClassificationDefinition: Codable, Sendable, Hashable {
...
// MARK: - Internal FFI conversions for ChunkClassificationDefinition
internal extension ChunkClassificationDefinition {
    init(_ rb: RustBridge.ChunkClassificationDefinitionRef) throws {
        self.label = rb.label().toString()
        self.description = rb.description().toString()
    }

    func intoRust() throws -> RustBridge.ChunkClassificationDefinition {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "{}"
        return try RustBridge.chunkClassificationDefinitionFromJson(json)
    }
}
...
public typealias ChunkClassificationConfig = RustBridge.ChunkClassificationConfig

/// Configuration for the page-classification post-processor.
public typealias PageClassificationConfig = RustBridge.PageClassificationConfig
...
public typealias NerConfig = RustBridge.NerConfig
...
public struct OcrQualityThresholds: Codable, Sendable, Hashable {
    ...
    public let minUndecodableRatio: Double
    ...
    /// Defaults to `true`.
    public let enableProvenanceOcrRouting: Bool
    ...
    public init(minTotalNonWhitespace: UInt, minNonWhitespacePerPage: Double, minMeaningfulWordLen: UInt, minMeaningfulWords: UInt, minAlnumRatio: Double, minGarbageChars: UInt, maxFragmentedWordRatio: Double, criticalFragmentedWordRatio: Double, minAvgWordLength: Double, minWordsForAvgLengthCheck: UInt, minConsecutiveRepeatRatio: Double, minWordsForRepeatCheck: UInt, substantiveMinChars: UInt, nonTextMinChars: UInt, alnumWsRatioThreshold: Double, pipelineMinQuality: Double, minUndecodableRatio: Double, enableProvenanceOcrRouting: Bool, minProvenanceFallbackRatio: Double) {
        ...
        self.minUndecodableRatio = minUndecodableRatio
        self.enableProvenanceOcrRouting = enableProvenanceOcrRouting
        self.minProvenanceFallbackRatio = minProvenanceFallbackRatio
    }
    private enum CodingKeys: String, CodingKey {
        ...
        case minUndecodableRatio = "min_undecodable_ratio"
        case enableProvenanceOcrRouting = "enable_provenance_ocr_routing"
        case minProvenanceFallbackRatio = "min_provenance_fallback_ratio"
    }
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        ...
        self.minUndecodableRatio = try container.decodeIfPresent(Double.self, forKey: .minUndecodableRatio) ?? 0
        self.enableProvenanceOcrRouting = try container.decodeIfPresent(Bool.self, forKey: .enableProvenanceOcrRouting) ?? false
        self.minProvenanceFallbackRatio = try container.decodeIfPresent(Double.self, forKey: .minProvenanceFallbackRatio) ?? 0
    }
}
...
public struct HierarchyConfig: Codable, Sendable, Hashable {
    ...
    public init(enabled: Bool, kClusters: UInt, includeBbox: Bool) {
        self.enabled = enabled
...
internal extension HierarchyConfig {
    ...
    func intoRust() throws -> RustBridge.HierarchyConfig {
        return RustBridge.HierarchyConfig(self.enabled, self.kClusters, self.includeBbox)
    }
}
...
public typealias RedactionConfig = RustBridge.RedactionConfig
...
public typealias SummarizationConfig = RustBridge.SummarizationConfig
...
public typealias TranslationConfig = RustBridge.TranslationConfig
...
public struct TreeSitterProcessConfig: Codable, Sendable, Hashable {
    ...
    public init(structure: Bool, imports: Bool, exports: Bool, comments: Bool, docstrings: Bool, symbols: Bool, diagnostics: Bool, dataExtraction: Bool, chunkMaxSize: UInt? = nil, contentMode: CodeContentMode) {
...
internal extension TreeSitterProcessConfig {
    ...
    func intoRust() throws -> RustBridge.TreeSitterProcessConfig {
        return RustBridge.TreeSitterProcessConfig(self.structure, self.imports, self.exports, self.comments, self.docstrings, self.symbols, self.diagnostics, self.dataExtraction, self.chunkMaxSize, try self.contentMode.intoRust())
    }
}
...
public struct ChunkMetadata: Codable, Sendable, Hashable {
    ...
    public let classifications: [ClassificationLabel]
    public init(byteStart: UInt, byteEnd: UInt, tokenCount: UInt? = nil, chunkIndex: UInt, totalChunks: UInt, firstPage: UInt32? = nil, lastPage: UInt32? = nil, headingContext: HeadingContext? = nil, headingPath: [String], imageIndices: [UInt32], nodeIds: [String], pageSpans: [PageSpan], classifications: [ClassificationLabel]) {
...
internal extension ChunkMetadata {
    init(_ rb: RustBridge.ChunkMetadataRef) throws {
        ...
        self.nodeIds = rb.nodeIds().map { $0.as_str().toString() }
        self.pageSpans = try rb.pageSpans().map { (s: RustStringRef) -> PageSpan in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(PageSpan.self, from: d) }
        self.classifications = try rb.classifications().map { (s: RustStringRef) -> ClassificationLabel in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(ClassificationLabel.self, from: d) }
    }
...
public struct PageSpan: Codable, Sendable, Hashable {
    /// Page number (1-indexed).
    public let page: UInt32
    /// Bounding box on this page, if known.
    public let bbox: BoundingBox?
    public init(page: UInt32, bbox: BoundingBox? = nil) {
        self.page = page
        self.bbox = bbox
    }
}

// MARK: - Internal FFI conversions for PageSpan
internal extension PageSpan {
    init(_ rb: RustBridge.PageSpanRef) throws {
        self.page = rb.page()
        self.bbox = try rb.bbox().map { try BoundingBox($0) }
    }

    func intoRust() throws -> RustBridge.PageSpan {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "{}"
        return try RustBridge.pageSpanFromJson(json)
    }
}
...
public typealias CodeMetadata = RustBridge.CodeMetadata
...
/// `Sequence` nodes.
public struct CodeDataAttribute: Codable, Sendable, Hashable {
    ...
    public let byteEnd: UInt
    public init(name: String, value: String, byteStart: UInt, byteEnd: UInt) {
        self.name = name
        self.value = value
        self.byteStart = byteStart
        self.byteEnd = byteEnd
...
// MARK: - Internal FFI conversions for CodeDataAttribute
internal extension CodeDataAttribute {
    init(_ rb: RustBridge.CodeDataAttributeRef) throws {
        self.name = rb.name().toString()
        self.value = rb.value().toString()
        self.byteStart = rb.byteStart()
        self.byteEnd = rb.byteEnd()
    ...
    func intoRust() throws -> RustBridge.CodeDataAttribute {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "{}"
        return try RustBridge.codeDataAttributeFromJson(json)
    }
}
...
public typealias CodeDataNode = RustBridge.CodeDataNode
...
public struct Table: Codable, Sendable, Hashable {
    ...
    public init(cells: [[String]], markdown: String, pageNumber: UInt32, boundingBox: BoundingBox? = nil, tableId: String? = nil, columns: [String]? = nil) {
        self.cells = cells
        self.markdown = markdown
        self.pageNumber = pageNumber
        self.boundingBox = boundingBox
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.tableId = try container.decodeIfPresent(String.self, forKey: .tableId) ?? nil
        self.columns = try container.decodeIfPresent([String].self, forKey: .columns) ?? nil
    }
}
...
internal extension Table {
    init(_ rb: RustBridge.TableRef) throws {
        ...
        self.pageNumber = rb.pageNumber()
        self.boundingBox = try rb.boundingBox().map { try BoundingBox($0) }
        self.tableId = rb.tableId()?.toString()
        self.columns = rb.columns()?.map { $0.as_str().toString() }
    }
...
internal extension KeywordConfig {
    init(_ rb: RustBridge.KeywordConfigRef) throws {
        self.algorithm = try { let rawValue = rb.algorithm().toString(); guard let value = KeywordAlgorithm(rawValue: rawValue) else { throw XbergError.validation(message: "Unknown KeywordAlgorithm variant", source: rawValue) }; return value }()
...
public struct PdfMetadata: Codable, Sendable, Hashable {
    ...
    public let scannedConfidence: Float?
    ...
    public let scannedPages: [UInt32]?
    public init(pdfVersion: String? = nil, producer: String? = nil, isEncrypted: Bool? = nil, width: Int64? = nil, height: Int64? = nil, pageCount: UInt32? = nil, scannedConfidence: Float? = nil, scannedPages: [UInt32]? = nil) {
        ...
        self.height = height
        self.pageCount = pageCount
        self.scannedConfidence = scannedConfidence
    ...
    private enum CodingKeys: String, CodingKey {
        ...
        case height = "height"
        case pageCount = "page_count"
        case scannedConfidence = "scanned_confidence"
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.scannedConfidence = try container.decodeIfPresent(Float.self, forKey: .scannedConfidence) ?? nil
        self.scannedPages = try container.decodeIfPresent([UInt32].self, forKey: .scannedPages) ?? nil
    }
}
...
internal extension PdfMetadata {
    init(_ rb: RustBridge.PdfMetadataRef) throws {
        ...
        self.height = rb.height()
        self.pageCount = rb.pageCount()
        self.scannedConfidence = rb.scannedConfidence()
        self.scannedPages = rb.scannedPages().map { Array($0) }
    }
...
public typealias ChunkClassificationEnrichmentConfig = RustBridge.ChunkClassificationEnrichmentConfig
...
public typealias ChunkClassificationDefinitionRef = RustBridge.ChunkClassificationDefinitionRef

public typealias ChunkClassificationDefinitionRefMut = RustBridge.ChunkClassificationDefinitionRefMut
...
public typealias PageSpanRef = RustBridge.PageSpanRef

public typealias PageSpanRefMut = RustBridge.PageSpanRefMut
...
public typealias CodeDataAttributeRef = RustBridge.CodeDataAttributeRef

public typealias CodeDataAttributeRefMut = RustBridge.CodeDataAttributeRefMut
...
public typealias YakeParamsRef = RustBridge.YakeParamsRef

public typealias YakeParamsRefMut = RustBridge.YakeParamsRefMut

public typealias RakeParamsRef = RustBridge.RakeParamsRef

public typealias RakeParamsRefMut = RustBridge.RakeParamsRefMut

public typealias KeywordConfigRef = RustBridge.KeywordConfigRef

public typealias KeywordConfigRefMut = RustBridge.KeywordConfigRefMut

public typealias KeywordRef = RustBridge.KeywordRef

public typealias KeywordRefMut = RustBridge.KeywordRefMut
...
/// ```
public enum OcrStrategy: Codable, Sendable, Hashable {
    ...
    case scannedPages(minConfidence: Double)
    ...
    public init(from decoder: Decoder) throws {
        ...
        case "auto":
            self = .auto
        case "scanned_pages":
            self = .scannedPages(minConfidence: try container.decode(Double.self, forKey: .minConfidence))
        default:
            throw DecodingError.dataCorruptedError(
                forKey: .mode,
                in: container,
                debugDescription: "Unknown OcrStrategy type: \(type)"
            )
        }
    }
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        case .scannedPages(let minConfidence):
            try container.encode("scanned_pages", forKey: .mode)
            try container.encode(minConfidence, forKey: .minConfidence)

        }
    }
}
extension OcrStrategy {
    func intoRust() throws -> RustBridge.OcrStrategy {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "null"
        return try RustBridge.ocrStrategyFromJson(json)
    }
}
...
///
/// Since v5.0.0.
public typealias RerankerModelType = RustBridge.RerankerModelType
...
public enum CodeDataNodeKind: String, Codable, Sendable, Hashable {
...
extension CodeDataNodeKind {
    func intoRust() throws -> RustBridge.CodeDataNodeKind {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "null"
        return try RustBridge.codeDataNodeKindFromJson(json)
    }
}
...
public func classifyChunks(_ resultJson: String, _ configJson: String) async throws -> Void {
    let result = try extractedDocumentFromJson(resultJson)
    let config = try chunkClassificationConfigFromJson(configJson)
    return try await classifyChunks(result: result, config: config)
}
...
public func chunkClassificationDefinitionFromJson(_ json: String) throws -> ChunkClassificationDefinition {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(ChunkClassificationDefinition.self, from: data)
}
public func chunkClassificationConfigFromJson(_ json: String) throws -> ChunkClassificationConfig {
    return try RustBridge.chunkClassificationConfigFromJson(json)
}
...
public func pageSpanFromJson(_ json: String) throws -> PageSpan {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(PageSpan.self, from: data)
}
...
public func codeMetadataFromJson(_ json: String) throws -> CodeMetadata {
    return try RustBridge.codeMetadataFromJson(json)
}
...
public func codeDataAttributeFromJson(_ json: String) throws -> CodeDataAttribute {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(CodeDataAttribute.self, from: data)
}
public func codeDataNodeFromJson(_ json: String) throws -> CodeDataNode {
    return try RustBridge.codeDataNodeFromJson(json)
}
...
public func ocrStrategyFromJson(_ json: String) throws -> OcrStrategy {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(OcrStrategy.self, from: data)
}
...
public func codeDataNodeKindFromJson(_ json: String) throws -> CodeDataNodeKind {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(CodeDataNodeKind.self, from: data)
}
...
public func classifyChunks(result: ExtractedDocument, config: ChunkClassificationConfig) async throws {
    return try await Task.detached(priority: .userInitiated) {
        try RustBridge.classifyChunks(result, config)
    }.value
}
...
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.ChunkClassificationDefinition: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.ChunkClassificationConfig: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
...
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.PageSpan: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
...
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.CodeDataAttribute: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.CodeDataNode: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
...
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.ChunkClassificationEnrichmentConfig: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
