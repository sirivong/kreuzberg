...
public typealias ExtractionOutput = RustBridge.ExtractionOutput

/// URL ingestion and crawl configuration.
public struct UrlExtractionConfig: Codable, Sendable, Hashable {
    /// URL extraction mode.
    public let mode: UrlExtractionMode
    /// Optional regex filter for document-discovered URLs.
    public let documentUrlPattern: String?
    /// Maximum URLs to follow per extraction result.
    public let maxDocumentUrlsPerResult: UInt32?
    /// Maximum URLs followed across the whole extraction call.
    public let maxTotalUrls: UInt32?
    /// Allow bare local filesystem path inputs.
    public let allowLocalFileInputs: Bool
    /// Allow local `file://` URI inputs.
    public let allowFileUris: Bool
    public init(mode: UrlExtractionMode, documentUrlPattern: String? = nil, maxDocumentUrlsPerResult: UInt32? = nil, maxTotalUrls: UInt32? = nil, allowLocalFileInputs: Bool, allowFileUris: Bool) {
        self.mode = mode
        self.documentUrlPattern = documentUrlPattern
        self.maxDocumentUrlsPerResult = maxDocumentUrlsPerResult
        self.maxTotalUrls = maxTotalUrls
        self.allowLocalFileInputs = allowLocalFileInputs
        self.allowFileUris = allowFileUris
    }
    private enum CodingKeys: String, CodingKey {
        case mode = "mode"
        case documentUrlPattern = "document_url_pattern"
        case maxDocumentUrlsPerResult = "max_document_urls_per_result"
        case maxTotalUrls = "max_total_urls"
        case allowLocalFileInputs = "allow_local_file_inputs"
        case allowFileUris = "allow_file_uris"
    }
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.mode = try container.decode(UrlExtractionMode.self, forKey: .mode)
        self.documentUrlPattern = try container.decodeIfPresent(String.self, forKey: .documentUrlPattern) ?? nil
        self.maxDocumentUrlsPerResult = try container.decodeIfPresent(UInt32.self, forKey: .maxDocumentUrlsPerResult) ?? nil
        self.maxTotalUrls = try container.decodeIfPresent(UInt32.self, forKey: .maxTotalUrls) ?? nil
        self.allowLocalFileInputs = try container.decodeIfPresent(Bool.self, forKey: .allowLocalFileInputs) ?? true
        self.allowFileUris = try container.decodeIfPresent(Bool.self, forKey: .allowFileUris) ?? true
    }
}

// MARK: - Internal FFI conversions for UrlExtractionConfig
internal extension UrlExtractionConfig {
    init(_ rb: RustBridge.UrlExtractionConfigRef) throws {
        self.mode = try { let rawValue = rb.mode().toString(); guard let value = UrlExtractionMode(rawValue: rawValue) else { throw XbergError.validation(message: "Unknown UrlExtractionMode variant", source: rawValue) }; return value }()
        self.documentUrlPattern = rb.documentUrlPattern()?.toString()
        self.maxDocumentUrlsPerResult = rb.maxDocumentUrlsPerResult()
        self.maxTotalUrls = rb.maxTotalUrls()
        self.allowLocalFileInputs = rb.allowLocalFileInputs()
        self.allowFileUris = rb.allowFileUris()
    }

    func intoRust() throws -> RustBridge.UrlExtractionConfig {
        return RustBridge.UrlExtractionConfig(try self.mode.intoRust(), self.documentUrlPattern.map(RustString.init), self.maxDocumentUrlsPerResult, self.maxTotalUrls, self.allowLocalFileInputs, self.allowFileUris)
    }
}
...
public typealias UrlExtractionConfigRef = RustBridge.UrlExtractionConfigRef

public typealias UrlExtractionConfigRefMut = RustBridge.UrlExtractionConfigRefMut
...
public func extractSync(_ inputJson: String, _ configJson: String) throws -> ExtractionOutput {
    let input = try extractInputFromJson(inputJson)
    let config = try extractionConfigFromJson(configJson)
    return try extractSync(input: input, config: config)
}
public func extractBatchSync(_ inputs: [ExtractInput], _ configJson: String) throws -> ExtractionOutput {
    let config = try extractionConfigFromJson(configJson)
    return try extractBatchSync(inputs: inputs, config: config)
}
public func extract(_ inputJson: String, _ configJson: String) async throws -> ExtractionOutput {
    let input = try extractInputFromJson(inputJson)
    let config = try extractionConfigFromJson(configJson)
    return try await extract(input: input, config: config)
}
public func extractBatch(_ inputs: [ExtractInput], _ configJson: String) async throws -> ExtractionOutput {
    let config = try extractionConfigFromJson(configJson)
...
public func embedTextsAsync(_ texts: [String], _ configJson: String) async throws -> [[Float]] {
    ...
    return try await embedTextsAsync(texts, config)
}
...
public func extractionOutputFromJson(_ json: String) throws -> ExtractionOutput {
    return try RustBridge.extractionOutputFromJson(json)
}
public func urlExtractionConfigFromJson(_ json: String) throws -> UrlExtractionConfig {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(UrlExtractionConfig.self, from: data)
}
...
public func extractSync(input: ExtractInput, config: ExtractionConfig) throws -> ExtractionOutput {
    return try RustBridge.extractSync(input, config)
}
/// Synchronous wrapper for `extract_batch`.
public func extractBatchSync(inputs: [ExtractInput], config: ExtractionConfig) throws -> ExtractionOutput {
        let _rb_inputs: RustVec<ExtractInput> = { let v = RustVec<ExtractInput>(); for x in inputs { v.push(value: x) }; return v }()
    return try RustBridge.extractBatchSync(_rb_inputs, config)
}
/// Extract content from a single bytes or URI input.
public func extract(input: ExtractInput, config: ExtractionConfig) async throws -> ExtractionOutput {
...
public func extractBatch(inputs: [ExtractInput], config: ExtractionConfig) async throws -> ExtractionOutput {
    return try await Task.detached(priority: .userInitiated) {
        let _rb_inputs: RustVec<ExtractInput> = { let v = RustVec<ExtractInput>(); for x in inputs { v.push(value: x) }; return v }()
...
public func pdfPageCount(pdfBytes: [UInt8], password: String? = nil) throws -> UInt {
        let _rb_pdfBytes: RustVec<UInt8> = { let v = RustVec<UInt8>(); for b in pdfBytes { v.push(value: b) }; return v }()
        let _rb_password = password.map { RustString($0) }
    return try RustBridge.pdfPageCount(_rb_pdfBytes, _rb_password)
}
...
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.ExtractionOutput: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
