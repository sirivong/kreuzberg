...
#[derive(Debug, Clone, Default, serde::Serialize, serde::Deserialize, rustler::NifStruct)]
#[module = "Kreuzberg.ExtractionDiff"]
...
#[derive(Debug, Clone, Default, serde::Serialize, serde::Deserialize, rustler::NifStruct)]
#[module = "Kreuzberg.EmbeddedChanges"]
...
#[derive(Debug, Clone, Default, serde::Serialize, serde::Deserialize, rustler::NifStruct)]
#[module = "Kreuzberg.WhisperModelPaths"]
...
impl AudioTags {
    pub fn new(opts: std::collections::HashMap<String, rustler::Term>) -> Self {
        Self {
            title: opts.get("title").and_then(|t| t.decode().ok()),
            artist: opts.get("artist").and_then(|t| t.decode().ok()),
            year: opts.get("year").and_then(|t| t.decode().ok()),
            language: opts.get("language").and_then(|t| t.decode().ok()),
            duration_ms: opts.get("duration_ms").and_then(|t| t.decode().ok()),
            sample_rate_hz: opts.get("sample_rate_hz").and_then(|t| t.decode().ok()),
            channels: opts.get("channels").and_then(|t| t.decode().ok()),
...
#[rustler::nif]
pub fn decode_audio_to_pcm(bytes: rustler::Binary, max_bytes: Option<u64>) -> Result<PcmAudio, String> {
    let result = kreuzberg::transcription::decode::decode_audio_to_pcm(bytes.as_slice(), max_bytes)
        .map_err(|e| e.to_string())?;
    Ok(result.into())
}
...
pub fn ensure_whisper_model(
    ...
    allow_network: bool,
    verify_hash: bool,
) -> Result<WhisperModelPaths, String> {
    let result = kreuzberg::transcription::model::ensure_whisper_model(
        model.into(),
...
#[rustler::nif]
pub fn read_audio_tags(bytes: rustler::Binary) -> AudioTags {
    kreuzberg::transcription::tags::read_audio_tags(bytes.as_slice()).into()
}
