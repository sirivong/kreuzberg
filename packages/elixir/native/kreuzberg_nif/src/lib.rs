...
pub fn download_model(name: String, cache_dir: Option<String>) -> Result<String, String> {
    let result =
        kreuzberg::text::ner::download_model(&name, std::path::PathBuf::from(cache_dir)).map_err(|e| e.to_string())?;
...
pub fn scan_text(text: String, categories: Vec<PiiCategory>) -> Vec<PatternMatch> {
    kreuzberg::text::redaction::patterns::scan_text(&text, &categories)
        .into_iter()
        .map(Into::into)
        .collect()
}
...
pub fn apply_strategy(
    ...
    counter: rustler::ResourceArc<TokenCounter>,
) -> String {
    String::from("[unimplemented: apply_strategy]")
}
...
#[rustler::nif]
pub fn glinebackend_new(repo_id: Option<String>) -> Result<GlineBackend, String> {
    let result = kreuzberg::GlineBackend::new(repo_id.as_deref()).map_err(|e| e.to_string())?;
    Ok(result.into())
}

#[rustler::nif(schedule = "DirtyCpu")]
pub fn glinebackend_detect_async(
    obj: GlineBackend,
    text: String,
    ...
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        std::thread::Builder::new()
            ...
            .spawn(move || {
                let rt = tokio::runtime::Runtime::new().map_err(|e| e.to_string())?;
                let result = rt
                    .block_on(async { kreuzberg::GlineBackend::from(obj).detect(&text, &categories).await })
                    .map_err(|e| e.to_string())?;
...
#[rustler::nif(schedule = "DirtyCpu")]
pub fn glinebackend_detect_with_custom_async(
    obj: GlineBackend,
    text: String,
    categories: Vec<EntityCategory>,
    custom_labels: Vec<String>,
) -> Result<Vec<Entity>, String> {
    Err(String::from("Not implemented: glinebackend_detect_with_custom_async"))
}
    ...
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        std::thread::Builder::new()
            ...
            .spawn(move || {
                let rt = tokio::runtime::Runtime::new().map_err(|e| e.to_string())?;
                let result = rt
                    .block_on(async { resource.inner.as_ref().detect(&text, &categories).await })
                    .map_err(|e| e.to_string())?;
                Ok(result.into_iter().map(Into::into).collect())
            })
            .map_err(|e| e.to_string())?
...
#[rustler::nif]
pub fn tokencounter_next_token(
    resource: rustler::ResourceArc<TokenCounter>,
    category: PiiCategory,
    original: String,
) -> String {
    String::from("[unimplemented: tokencounter_next_token]")
}
