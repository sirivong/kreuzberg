...
#[derive(Debug, Clone, Default, serde::Serialize, serde::Deserialize, rustler::NifStruct)]
#[module = "Xberg.ChunkClassificationDefinition"]
...
#[derive(Debug, Clone, Default, serde::Serialize, serde::Deserialize, rustler::NifStruct)]
#[module = "Xberg.ChunkClassificationConfig"]
    ...
    pub fn new(opts: std::collections::HashMap<String, rustler::Term>) -> Self {
        Self {
            ...
            output_format: opts
                ...
                .and_then(|t| t.decode().ok())
                .unwrap_or_default(),
            escape_markdown: opts
                .get("escape_markdown")
                .and_then(|t| t.decode().ok())
                .unwrap_or(true),
            table_anchors: opts.get("table_anchors").and_then(|t| t.decode().ok()).unwrap_or(false),
            ...
            chunk_classification: opts.get("chunk_classification").and_then(|t| t.decode().ok()),
...
impl FileExtractionConfig {
    pub fn new(opts: std::collections::HashMap<String, rustler::Term>) -> Self {
        Self {
            ...
            ocr_strategy: opts.get("ocr_strategy").and_then(|t| t.decode().ok()),
            force_ocr_pages: opts.get("force_ocr_pages").and_then(|t| t.decode().ok()),
...
impl LlmConfig {
    pub fn new(opts: std::collections::HashMap<String, rustler::Term>) -> Self {
        Self {
            ...
            load_env: opts.get("load_env").and_then(|t| t.decode().ok()),
...
#[derive(Debug, Clone, Default, serde::Serialize, serde::Deserialize, rustler::NifStruct)]
#[module = "Xberg.CodeDataAttribute"]
    ...
    pub fn new(opts: std::collections::HashMap<String, rustler::Term>) -> Self {
        Self {
            ...
            page_number: opts
                ...
                .unwrap_or_default(),
            bounding_box: opts.get("bounding_box").and_then(|t| t.decode().ok()),
            table_id: opts.get("table_id").and_then(|t| t.decode().ok()),
            columns: opts.get("columns").and_then(|t| t.decode().ok()),
        }
    }
}
...
impl PdfMetadata {
    pub fn new(opts: std::collections::HashMap<String, rustler::Term>) -> Self {
        Self {
            ...
            scanned_confidence: opts.get("scanned_confidence").and_then(|t| t.decode().ok()),
            scanned_pages: opts.get("scanned_pages").and_then(|t| t.decode().ok()),
        }
    }
}
...
#[allow(clippy::derivable_impls)]
impl Default for CodeDataNodeKind {
    fn default() -> Self {
        Self::KeyValue
    }
}
...
#[rustler::nif(schedule = "DirtyCpu")]
pub fn classify_chunks_async(result: Option<String>, config: ChunkClassificationConfig) -> Result<(), String> {
    let result_core: Option<xberg::ExtractedDocument> = result
        .map(|s| serde_json::from_str::<xberg::ExtractedDocument>(&s))
        .transpose()
        .map_err(|e| e.to_string())?;
    ...
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                let result = rt
                    .block_on(async {
                        xberg::text::classification::classify_chunks(&mut result_mut, &config.clone().into()).await
                    })
                    .map_err(|e| e.to_string())?;
                Ok(result)
            })
