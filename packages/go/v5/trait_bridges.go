...
func goOcrBackendFreeUserData(userData unsafe.Pointer) {
	...
	defer func() {
		if r := recover(); r != nil {
			// Handle already deleted or invalid; silently ignore.
			// This can occur if the C side double-frees or if the handle
			// was already deleted by another cleanup path.
		}
	}()
	if userData != nil {
