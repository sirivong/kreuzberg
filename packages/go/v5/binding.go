...
// PcmAudio canonical PCM format that all transcription engines receive.
...
// WhisperModelPaths on-disk paths for all files needed to load a Whisper model in an ORT session.
type WhisperModelPaths struct {
	...
	DecoderWithPast string `json:"decoder_with_past"`
	...
	NMels uint32 `json:"n_mels"`
}

// AudioTags tag and audio-property data extracted from an audio/video file.
...
// DecodeAudioToPcm decode arbitrary audio bytes (mp3, wav, m4a, webm, etc.) into 16 kHz mono f32 PCM.
//
// This is a blocking CPU-heavy operation — callers should use
// `tokio::task::spawn_blocking` when on an async runtime.
func DecodeAudioToPcm(bytes []byte, maxBytes *uint64) (*PcmAudio, error) {
	...
	var cMaxBytes C.uint64_t = C.uint64_t(^uint64(0))
	if maxBytes != nil {
		cMaxBytes = C.uint64_t(uint64(*maxBytes))
	}

	ptr := C.kreuzberg_decode_audio_to_pcm(cBytes, cBytesLen, cMaxBytes)
	if err := lastError(); err != nil {
		if ptr != nil {
			C.kreuzberg_pcm_audio_free(ptr)
		}
		return nil, err
	}
	defer C.kreuzberg_pcm_audio_free(ptr)
	jsonPtr := C.kreuzberg_pcm_audio_to_json(ptr)
	if jsonPtr == nil {
		return nil, fmt.Errorf("failed to convert to JSON")
	}
	defer C.kreuzberg_free_string(jsonPtr)
...
// EnsureWhisperModel resolve a Whisper model to on-disk ONNX paths, downloading from HF Hub if needed.
//
// # Behaviour
//
...
//
// Errors are returned when returns [`WhisperModelError`] on I/O failures, download failures, or when the
// model is unavailable and `allow_network` is `false`.
func EnsureWhisperModel(model WhisperModel, cacheDir *string, allowNetwork bool, verifyHash bool) (*WhisperModelPaths, error) {
	cModelStr := C.CString(string(model))
	defer C.free(unsafe.Pointer(cModelStr))
	cModel := C.kreuzberg_whisper_model_from_str(cModelStr)
	...
	ptr := C.kreuzberg_ensure_whisper_model(cModel, cCacheDir, cAllowNetwork, cVerifyHash)
	if err := lastError(); err != nil {
		if ptr != nil {
			C.kreuzberg_whisper_model_paths_free(ptr)
		}
		return nil, err
	}
	defer C.kreuzberg_whisper_model_paths_free(ptr)
	jsonPtr := C.kreuzberg_whisper_model_paths_to_json(ptr)
	if jsonPtr == nil {
		return nil, fmt.Errorf("failed to convert to JSON")
	}
	defer C.kreuzberg_free_string(jsonPtr)
	var result WhisperModelPaths
	if err := json.Unmarshal([]byte(C.GoString(jsonPtr)), &result); err != nil {
		return nil, fmt.Errorf("failed to unmarshal: %w", err)
	}
	return &result, nil
...
// ReadAudioTags attempt to read audio tags and properties from raw bytes.
//
// Never panics or returns an error — failures produce `AudioTags::default()`.
func ReadAudioTags(bytes []byte) *AudioTags {
	var cBytes *C.uint8_t
	if len(bytes) > 0 {
		var cBytesPinner runtime.Pinner
		cBytesPinner.Pin(&bytes[0])
	...
	ptr := C.kreuzberg_read_audio_tags(cBytes, cBytesLen)
	defer C.kreuzberg_audio_tags_free(ptr)
	return func() *AudioTags {
		jsonPtr := C.kreuzberg_audio_tags_to_json(ptr)
		if jsonPtr == nil {
			return nil
		}
		defer C.kreuzberg_free_string(jsonPtr)
