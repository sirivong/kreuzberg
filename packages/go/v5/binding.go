...
const (
	// EntityCategoryPerson is the Person variant of EntityCategory.
	EntityCategoryPerson EntityCategory = "Person"
	// EntityCategoryOrganization is the Organization variant of EntityCategory.
	EntityCategoryOrganization EntityCategory = "Organization"
	// EntityCategoryLocation is the Location variant of EntityCategory.
	EntityCategoryLocation EntityCategory = "Location"
	// EntityCategoryDate is the Date variant of EntityCategory.
	EntityCategoryDate EntityCategory = "Date"
	// EntityCategoryTime is the Time variant of EntityCategory.
	EntityCategoryTime EntityCategory = "Time"
	// EntityCategoryMoney is the Money variant of EntityCategory.
	EntityCategoryMoney EntityCategory = "Money"
	// EntityCategoryPercent is the Percent variant of EntityCategory.
	EntityCategoryPercent EntityCategory = "Percent"
	// EntityCategoryEmail is the Email variant of EntityCategory.
	EntityCategoryEmail EntityCategory = "Email"
	// EntityCategoryPhone is the Phone variant of EntityCategory.
	EntityCategoryPhone EntityCategory = "Phone"
	// EntityCategoryURL is the Url variant of EntityCategory.
...
// Variants: Pdf, Docx, Excel, Email, Pptx, Archive, Image, Xml, Text, Html, Ocr, Csv, Bibtex, Citation, FictionBook, Dbf, Jats, Epub, Pst
...
const (
	// PiiCategoryEmail is the Email variant of PiiCategory.
	PiiCategoryEmail PiiCategory = "Email"
	// PiiCategoryPhone is the Phone variant of PiiCategory.
	PiiCategoryPhone PiiCategory = "Phone"
	// PiiCategorySsn is the Ssn variant of PiiCategory.
	PiiCategorySsn PiiCategory = "Ssn"
	// PiiCategoryCreditCard is the CreditCard variant of PiiCategory.
	PiiCategoryCreditCard PiiCategory = "CreditCard"
	// PiiCategoryPostalCode is the PostalCode variant of PiiCategory.
	PiiCategoryPostalCode PiiCategory = "PostalCode"
	// PiiCategoryIPAddress is the IpAddress variant of PiiCategory.
	PiiCategoryIPAddress PiiCategory = "IpAddress"
	// PiiCategoryIban is the Iban variant of PiiCategory.
	PiiCategoryIban PiiCategory = "Iban"
	// PiiCategorySwiftBic is the SwiftBic variant of PiiCategory.
	PiiCategorySwiftBic PiiCategory = "SwiftBic"
	// PiiCategoryDateOfBirth is the DateOfBirth variant of PiiCategory.
...
// GlineBackend kreuzberg-gliner-rs ONNX backend wrapper.
//
...
// Segment text segment with its byte offset in the original document.
type Segment struct {
	Text      string `json:"text"`
	ByteStart uint   `json:"byte_start"`
}
...
// FindAll calls the FFI function.
func FindAll(text string) []PatternMatch {
	cText := C.CString(text)
	defer C.free(unsafe.Pointer(cText))

	ptr := C.kreuzberg_find_all(cText)
	return func() []PatternMatch {
		if ptr == nil {
			return nil
		}
		defer C.kreuzberg_free_string(ptr)
...
// ScanText scan `text` for every PII category in `categories` and return all matches
...
func ScanText(text string, categories []PiiCategory) ([]PatternMatch, error) {
	cText := C.CString(text)
	defer C.free(unsafe.Pointer(cText))
	...
	ptr := C.kreuzberg_scan_text(cText, cCategories)
	if ptr == nil {
		return nil, fmt.Errorf("failed to get result")
	}
	defer C.kreuzberg_free_string(ptr)
...
// New build a backend for `repo_id` (or the default model if `None`).
//
...
func GlineBackendNew(repoID *string) (*GlineBackend, error) {
	var cRepoID *C.char
	if repoID != nil {
	...
	ptr := C.kreuzberg_gline_backend_new(cRepoID)
	if err := lastError(); err != nil {
		return nil, err
	}
	defer C.kreuzberg_gline_backend_free(ptr)
	return func() *GlineBackend {
		jsonPtr := C.kreuzberg_gline_backend_to_json(ptr)
		if jsonPtr == nil {
			return nil
		}
		defer C.kreuzberg_free_string(jsonPtr)
		var result GlineBackend
		if err := json.Unmarshal([]byte(C.GoString(jsonPtr)), &result); err != nil {
			return nil
		}
		return &result
...
// Detect is a method.
func (r *GlineBackend) Detect(text string, categories []EntityCategory) ([]Entity, error) {
	cText := C.CString(text)
	defer C.free(unsafe.Pointer(cText))
	...
	tmpStrRecv := C.CString(string(jsonBytesRecv))
	cRecv := C.kreuzberg_gline_backend_from_json(tmpStrRecv)
	C.free(unsafe.Pointer(tmpStrRecv))
	if cRecv == nil {
		return nil, fmt.Errorf("failed to create receiver: %s", C.GoString(C.kreuzberg_last_error_context()))
	}
	defer C.kreuzberg_gline_backend_free(cRecv)
	ptr := C.kreuzberg_gline_backend_detect(cRecv, cText, cCategories)
	if err := lastError(); err != nil {
		return nil, err
	}
	return func() []Entity {
...
// DetectWithCustom native zero-shot multi-label inference: passes the union of `categories`
// (as label strings) and `custom_labels` to a single GLiNER inference call.
func (r *GlineBackend) DetectWithCustom(text string, categories []EntityCategory, customLabels []string) ([]Entity, error) {
	cText := C.CString(text)
	defer C.free(unsafe.Pointer(cText))
	...
	defer C.kreuzberg_gline_backend_free(cRecv)
	ptr := C.kreuzberg_gline_backend_detect_with_custom(cRecv, cText, cCategories, cCustomLabels)
	if err := lastError(); err != nil {
		return nil, err
	}
	return func() []Entity {
