...
type OcrQualityThresholds struct {
	...
	MinUndecodableRatio float64 `json:"min_undecodable_ratio"`
	...
	// Defaults to `true`.
	EnableProvenanceOcrRouting bool `json:"enable_provenance_ocr_routing"`
	...
	// is `true`.
	MinProvenanceFallbackRatio float64 `json:"min_provenance_fallback_ratio"`
}
...
type ChunkClassificationEnrichmentConfig struct {
	...
	Config ChunkClassificationConfig `json:"config"`
}
...
// ClassifyChunks run chunk classification against an extraction result.
//
...
//
// Errors are returned when returns `Validation` when `config.definitions` is empty.
...
func ClassifyChunks(resultArg ExtractedDocument, config ChunkClassificationConfig) error {
	...
	if cResult == nil {
		return fmt.Errorf("failed to create extracted_document: %s", C.GoString(C.xberg_last_error_context()))
	}
	...
	C.free(unsafe.Pointer(tmpStrcConfig))
	if cConfig == nil {
		return fmt.Errorf("failed to create chunk_classification_config: %s", C.GoString(C.xberg_last_error_context()))
	}
	defer C.xberg_chunk_classification_config_free(cConfig)

	C.xberg_classify_chunks(cResult, cConfig)
	return lastError()
}
