...
pub fn new_audiotags(
    ...
    let mut __out = <AudioTags>::default();
    if let Some(v) = title {
        __out.title = Some(v);
    }
...
impl From<PcmAudio> for kreuzberg::transcription::decode::PcmAudio {
    fn from(val: PcmAudio) -> Self {
        Self {
            ...
            sample_rate_hz: val.sample_rate_hz as u32,
...
impl From<kreuzberg::transcription::decode::PcmAudio> for PcmAudio {
    fn from(val: kreuzberg::transcription::decode::PcmAudio) -> Self {
        Self {
            ...
            sample_rate_hz: val.sample_rate_hz as i32,
            channels: val.channels as i32,
            duration_ms: val.duration_ms as f64,
        }
    }
}
...
impl From<kreuzberg::ImageOutputFormat> for ImageOutputFormat {
    fn from(val: kreuzberg::ImageOutputFormat) -> Self {
        match val {
            kreuzberg::ImageOutputFormat::Native => Self {
                r#type: "native".to_string(),
                ..Default::default()
            },
            kreuzberg::ImageOutputFormat::Png => Self {
                r#type: "png".to_string(),
                ..Default::default()
            },
            kreuzberg::ImageOutputFormat::Jpeg { .. } => Self {
                r#type: "jpeg".to_string(),
                ..Default::default()
            },
            kreuzberg::ImageOutputFormat::Webp { .. } => Self {
                r#type: "webp".to_string(),
                ..Default::default()
            },
            kreuzberg::ImageOutputFormat::Heif { .. } => Self {
                r#type: "heif".to_string(),
                ..Default::default()
            },
            kreuzberg::ImageOutputFormat::Svg => Self {
...
pub fn classify_document(pages: Vec<String>, config: &PageClassificationConfig) -> String {
    match (|| -> Result<String> {
        ...
        let result = rt.block_on(async {
            kreuzberg::classify_document(&pages, &config_core).await.map_err(|e| {
                extendr_api::Error::Other(
                    e.to_string()
                        .replace(":", "_")
                        .replace("/", "_")
...
#[allow(clippy::missing_errors_doc)]
#[extendr]
pub fn decode_audio_to_pcm(bytes: Vec<u8>, max_bytes: Option<f64>) -> Result<PcmAudio> {
...
pub fn ensure_whisper_model(
    ...
    match (|| -> Result<String> {
        let model_core: kreuzberg::WhisperModel = serde_json::from_str(&model).map_err(|e| {
            extendr_api::Error::Other(
                e.to_string()
                    .replace(":", "_")
                    .replace("/", "_")
...
#[allow(clippy::missing_errors_doc)]
#[extendr]
pub fn rerank_async(_query: String, _documents: Vec<String>, _config: &RerankerConfig) -> String {
    match (|| -> Result<String> {
        let _config_core: kreuzberg::RerankerConfig = _config.clone().into();
        let rt = tokio::runtime::Runtime::new().map_err(|e| {
            extendr_api::Error::Other(
                e.to_string()
                    .replace(":", "_")
        ...
        let result = rt.block_on(async {
            kreuzberg::rerank_async(_query, _documents, &_config_core)
                .await
                .map_err(|e| {
                    extendr_api::Error::Other(
                        e.to_string()
