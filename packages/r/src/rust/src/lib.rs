...
#[allow(clippy::missing_errors_doc)]
#[extendr]
pub fn download_model(_name: String, _cache_dir: Option<String>) -> Result<String> {
    kreuzberg::text::ner::download_model(&_name, _cache_dir.map(std::path::PathBuf::from))
        .map(|val| val.to_string_lossy().to_string())
        .map_err(|e| {
            extendr_api::Error::Other(
                e.to_string()
...
#[extendr]
pub fn get_embedding_preset(_name: String) -> Option<String> {
    let result = kreuzberg::get_embedding_preset(&_name);
...
#[allow(clippy::missing_errors_doc)]
#[extendr]
pub fn rerank(_query: String, _documents: Vec<String>, _config: &RerankerConfig) -> String {
    match (|| -> Result<String> {
        let _config_core: kreuzberg::RerankerConfig = _config.clone().into();
        let result = kreuzberg::rerank(_query, _documents, &_config_core).map_err(|e| {
            extendr_api::Error::Other(
                e.to_string()
                    .replace(":", "_")
                    .replace("/", "_")
...
#[extendr]
pub fn get_reranker_preset(_name: String) -> Option<String> {
    let result = kreuzberg::get_reranker_preset(&_name);
