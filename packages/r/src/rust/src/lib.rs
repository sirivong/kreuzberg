...
#[allow(clippy::missing_errors_doc)]
#[extendr]
pub fn extract_sync(input: &ExtractInput, config: &ExtractionConfig) -> String {
    match (|| -> Result<String> {
        let input_core: xberg::ExtractInput = (*input).clone().into();
        let config_core: xberg::ExtractionConfig = (*config).clone().into();
        let result = xberg::extract_sync(input_core, &config_core).map_err(|e| {
            extendr_api::Error::Other(
                e.to_string()
                    .replace(":", "_")
                    .replace("/", "_")
...
#[allow(clippy::missing_errors_doc)]
#[extendr]
pub fn extract_batch_sync(inputs: String, config: &ExtractionConfig) -> String {
    match (|| -> Result<String> {
        ...
        let config_core: xberg::ExtractionConfig = (*config).clone().into();
        let result = xberg::extract_batch_sync(inputs_core, &config_core).map_err(|e| {
            extendr_api::Error::Other(
                e.to_string()
                    .replace(":", "_")
                    .replace("/", "_")
    ...
    async fn extract_bytes(
        ...
        tokio::task::spawn_blocking(move || {
            ...
            match result {
                ...
                Ok(val) => {
                    let payload = val.as_str().unwrap_or("");
                    serde_json::from_str::<xberg::InternalDocument>(payload).map_err(|_| {
                        xberg::XbergError::Other(format!(
