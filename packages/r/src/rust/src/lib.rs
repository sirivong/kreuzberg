...
pub fn new_urlextractionconfig(
    ...
    if let Some(v) = crawl {
        __out.crawl = serde_json::from_str(&v).ok();
    }
...
pub struct ExtractedDocument {
    ...
    /// Keywords extracted from document content.
...
fn resolve_e2e_fixture_uri(mut input: xberg::ExtractInput) -> xberg::ExtractInput {
    ...
    if uri.contains("://") || std::path::Path::new(uri).is_absolute() || std::path::Path::new(uri).exists() {
        return input;
    }

    let Ok(root) = std::env::var("XBERG_TEST_DOCUMENTS_DIR") else {
        return input;
    };
    let candidate = std::path::Path::new(&root).join(uri);
    if candidate.exists() {
...
impl From<UrlExtractionConfig> for xberg::UrlExtractionConfig {
    fn from(val: UrlExtractionConfig) -> Self {
        ...
        if let Some(crawl) = val.crawl {
            if let Ok(crawl_config) = serde_json::from_value(crawl) {
                out.crawl = crawl_config;
            }
        }
        out
    }
...
impl From<xberg::UrlExtractionConfig> for UrlExtractionConfig {
    fn from(val: xberg::UrlExtractionConfig) -> Self {
        Self {
            mode: val.mode.into(),
            crawl: serde_json::to_value(val.crawl).ok(),
...
pub fn extract(input: &ExtractInput, config: &ExtractionConfig) -> String {
    match (|| -> Result<String> {
        let input_core: xberg::ExtractInput = resolve_e2e_fixture_uri((*input).clone().into());
...
pub fn extract_batch(inputs: String, config: &ExtractionConfig) -> String {
    match (|| -> Result<String> {
        ...
        let inputs_core: Vec<xberg::ExtractInput> = inputs_core.into_iter().map(resolve_e2e_fixture_uri).collect();
        let config_core: xberg::ExtractionConfig = (*config).clone().into();
        let rt = tokio::runtime::Runtime::new().map_err(|e| {
            extendr_api::Error::Other(
                e.to_string()
...
pub fn list_document_extractors() -> Result<Vec<String>> {
    xberg::list_document_extractors().map_err(|e| {
        extendr_api::Error::Other(
            e.to_string()
                .replace(":", "_")
                .replace("/", "_")
        ...
        tokio::task::spawn_blocking(move || {
            ...
            match result {
                ...
                Ok(val) => {
                    ...
                    serde_json::from_str::<xberg::ExtractedDocument>(payload).map_err(|_| {
...
impl RDocumentExtractorBridge {
    ...
    /// Validates that the R object provides all required methods.
    pub fn new(r_obj: extendr_api::Robj) -> std::result::Result<Self, String> {
        match r_obj.dollar("extract") {
            Ok(v) if !v.is_null() && !v.is_na() => {}
            _ => return Err("R object missing required method: extract".to_string()),
        }
...
impl xberg::plugins::Plugin for RDocumentExtractorBridge {
    fn name(&self) -> &str {
        &self.cached_name
    }
...
#[async_trait::async_trait]
impl xberg::DocumentExtractor for RDocumentExtractorBridge {
    async fn extract(
        &self,
        ...
        tokio::task::spawn_blocking(move || {
            ...
            // closure to move the whole wrapper.
            let r_obj = send_r_obj.into_inner();
            let maybe_fn = r_obj.dollar("extract");
            let fn_robj = match maybe_fn {
                Ok(v) if !v.is_null() && !v.is_na() => v,
                _ => {
                    return Err(xberg::XbergError::Other(format!(
                        "Plugin '{}' missing method 'extract'",
                        cached_name_inner
                    )));
                }
            };
            ...
            let args = extendr_api::Pairlist::from_pairs(&[
                ("input", extendr_api::Robj::from(ExtractInput::from(input))),
                (
                    "config",
                    extendr_api::Robj::from(ExtractionConfig::from(config_owned.clone())),
                ),
            ...
            match result {
                ...
                Ok(val) => {
                    ...
                    serde_json::from_str::<xberg::ExtractedDocument>(payload).map_err(|_| {
                        xberg::XbergError::Other(format!(
                            "Plugin '{}' method 'extract' deserialization failed",
                            cached_name_inner
                        ))
                    })
                }
