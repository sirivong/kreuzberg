...
impl From<kreuzberg::transcription::decode::PcmAudio> for PcmAudio {
    fn from(v: kreuzberg::transcription::decode::PcmAudio) -> Self {
        PcmAudio {
            samples: v.samples.into_iter().map(|x| x as _).collect::<Vec<_>>(),
            sample_rate_hz: v.sample_rate_hz as _,
            channels: v.channels as _,
            duration_ms: v.duration_ms as _,
        }
    }
}
...
impl From<kreuzberg::transcription::model::WhisperModelPaths> for WhisperModelPaths {
    fn from(v: kreuzberg::transcription::model::WhisperModelPaths) -> Self {
        WhisperModelPaths {
            encoder: v.encoder.to_string_lossy().into_owned(),
            decoder: v.decoder.to_string_lossy().into_owned(),
            decoder_with_past: v.decoder_with_past.to_string_lossy().into_owned(),
            tokenizer: v.tokenizer.to_string_lossy().into_owned(),
            config: v.config.to_string_lossy().into_owned(),
            n_mels: v.n_mels as _,
        }
    }
}
...
impl From<kreuzberg::transcription::tags::AudioTags> for AudioTags {
    fn from(v: kreuzberg::transcription::tags::AudioTags) -> Self {
        AudioTags {
            title: v.title.map(|s| s.into()),
            artist: v.artist.map(|s| s.into()),
            year: v.year.map(|s| s.into()),
            language: v.language.map(|s| s.into()),
            duration_ms: v.duration_ms.map(|x| x as _),
            sample_rate_hz: v.sample_rate_hz.map(|x| x as _),
            channels: v.channels.map(|x| x as _),
...
pub fn decode_audio_to_pcm(bytes: Vec<u8>, max_bytes: Option<i64>) -> Result<PcmAudio, String> {
...
pub fn ensure_whisper_model(
    ...
    kreuzberg::transcription::model::ensure_whisper_model(
        unsafe { std::mem::transmute::<WhisperModel, kreuzberg::WhisperModel>(model) },
        cache_dir.as_ref().map(std::path::Path::new),
        allow_network,
        verify_hash,
    )
    .map(WhisperModelPaths::from)
    .map_err(|e| e.to_string())
}
...
pub fn read_audio_tags(bytes: Vec<u8>) -> AudioTags {
    AudioTags::from(kreuzberg::transcription::tags::read_audio_tags(&bytes))
}
...
pub async fn rerank_async(
    ...
    kreuzberg::rerank_async(_query, _documents, &kreuzberg::RerankerConfig::from(_config))
        .await
        .map(|v| v.into_iter().map(RerankedDocument::from).collect::<Vec<_>>())
        .map_err(|e| e.to_string())
}
