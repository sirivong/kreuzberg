...
def _to_rust_token_counter(value: TokenCounter | dict[str, Any] | str | None) -> _rust.TokenCounter | None:
    """Convert Python TokenCounter to Rust binding type."""
    if isinstance(value, str):
        value = json.loads(value)
    if isinstance(value, dict):
        value = TokenCounter(**value)
    if value is None:
        return None
    value = cast(TokenCounter, value)
    return _rust.TokenCounter(
    )
...
async def classify_pages(
    ...
    if _rust_result is None:
        _rust_result = _rust.ExtractionResult()
    await _rust.classify_pages(result=_rust_result, config=config)
...
async def redact(
    ...
    if _rust_result is None:
        _rust_result = _rust.ExtractionResult()
    _rust_config = _to_rust_redaction_config(config)

    if _rust_config is None:
        _rust_config = _rust.RedactionConfig()
...
def apply_strategy(
    ...
    _rust_category = (_rust.PiiCategory(category) if not isinstance(category, _rust.PiiCategory) else category)

    _rust_counter = _to_rust_token_counter(counter)

    if _rust_counter is None:
        _rust_counter = _rust.TokenCounter()
    return _rust.apply_strategy(strategy=strategy, original=original, category=_rust_category, counter=_rust_counter)
...
async def translate_result(
    ...
    if _rust_result is None:
        _rust_result = _rust.ExtractionResult()
    await _rust.translate_result(result=_rust_result, config=config)
...
async def extract_region_with_vlm(
    ...
    _rust_llm_config = _to_rust_llm_config(llm_config)

    if _rust_llm_config is None:
        _rust_llm_config = _rust.LlmConfig()
