...
def download_model(_name: str, _cache_dir: str | None = None) -> str:
    """Download model."""
    return _rust.download_model(_name=_name, _cache_dir=_cache_dir)
...
def get_embedding_preset(_name: str) -> EmbeddingPreset | None:
    """Returns `None` for builds without the `embedding-presets` feature."""
    return _rust.get_embedding_preset(_name=_name)
...
def rerank(
    ...
    """Stub for builds without the `reranker` feature -- keeps the symbol available on no-ORT ta."""
    _rust__config = _to_rust_reranker_config(_config) if _config is not None else _rust.RerankerConfig()
    return _rust.rerank(_query=_query, _documents=_documents, _config=_rust__config)
...
def get_reranker_preset(_name: str) -> RerankerPreset | None:
    """Returns `None` for builds without the `reranker-presets` feature."""
    return _rust.get_reranker_preset(_name=_name)
