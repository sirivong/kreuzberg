...
class StructuredCallMode(str, Enum):
...
class NerConfig:
    ...
    """Override the default model -- only used by [`NerBackendKind::Onnx`]. `None` lets the backend pick its pinned default (`urchade/gliner_multi-v2.1` for gline-rs)."""
...
class ExtractionDiff(TypedDict, total=False):
    ...
    """Metadata difference, encoded as a JSON object with three top-level keys: `added` (keys present in `b` but not `a`), `removed` (keys present in `a` but not `b`), and `changed` (keys whose values differ -- each entry is `{ "from": <value-in-a>, "to": <value-in-b> }`)."""
...
class ChunkPlan(TypedDict, total=False):
...
class StructuredThresholds:
    ...
    scan_max_coverage: float = 0.1

    """PDFs with `text_coverage` strictly below this are treated as scanned."""

    digital_min_coverage: float = 0.9

    """PDFs with `text_coverage` at or above this AND zero embedded images route to [`StructuredCallMode::TextOnly`]."""

    docx_text_min_density: float = 200

    """DOCX / HTML / text documents with `avg_chars_per_page` above this route to [`StructuredCallMode::TextOnly`]."""

    enable_vision_fallback: bool = False

    """When `true`, emit [`StructuredCallMode::TextOnlyWithVisionFallback`] instead of [`StructuredCallMode::TextOnly`] so the orchestrator can escalate to vision on low confidence."""
...
RerankerModelType = (
    ...
    str |    str | None |    list[str] |    int | None |    LlmConfig)
...
FormatMetadata = (
    ...
    PdfMetadata |    DocxMetadata |    ExcelMetadata |    EmailMetadata |    PptxMetadata |    ArchiveMetadata |    ImageMetadata |    XmlMetadata |    TextMetadata |    HtmlMetadata |    OcrMetadata |    CsvMetadata |    BibtexMetadata |    CitationMetadata |    FictionBookMetadata |    DbfMetadata |    JatsMetadata |    EpubMetadata |    PstMetadata |    AudioMetadata |    str)
