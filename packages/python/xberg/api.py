...
def _to_rust_file_extraction_config(
    ...
    return _rust.FileExtractionConfig(
        ...
        **(
        ...
        chunk_classification=value.chunk_classification,
...
def _to_rust_extracted_image(value: ExtractedImage | dict[str, Any] | str | None) -> _rust.ExtractedImage | None:
    ...
    return _rust.ExtractedImage(
        ...
        **({"image_kind": _coerce_enum(_rust.ImageKind, value.image_kind)} if value.image_kind is not None else {}),
...
def _to_rust_ocr_element(value: OcrElement | dict[str, Any] | str | None) -> _rust.OcrElement | None:
    ...
    if isinstance(value, str):
        value = json.loads(value)
    if isinstance(value, dict):
        if "geometry" in value and value["geometry"] is not None:
            value["geometry"] = _coerce_enum(_rust.OcrBoundingGeometry, value["geometry"])
    ...
    return _rust.OcrElement(
        text=value.text,
        geometry=_coerce_enum(_rust.OcrBoundingGeometry, value.geometry),
...
async def classify_chunks(
    ...
    _rust_result = _to_rust_extracted_document(result) if result is not None else _rust.ExtractedDocument()
    await _rust.classify_chunks(config=config, result=_rust_result)
