...
object Kreuzberg {
     ...
     */
    fun decodeAudioToPcm(bytes: ByteArray, maxBytes: Long? = null): PcmAudio {
        val resultJson = KreuzbergBridge.nativeDecodeAudioToPcm(java.util.Base64.getEncoder().encodeToString(bytes), maxBytes ?: 0L)
        return mapper.readValue(resultJson, PcmAudio::class.java)
    }
    /**
     * Decode arbitrary audio bytes (mp3, wav, m4a, webm, etc.) into 16 kHz mono f32 PCM.
     *
     ...
     * `tokio.task.spawn_blocking` when on an async runtime.
     */
    suspend fun decodeAudioToPcmAsync(bytes: ByteArray, maxBytes: Long? = null): PcmAudio =
        withContext(Dispatchers.IO) { decodeAudioToPcm(bytes, maxBytes) }
    /**
     ...
     */
    fun ensureWhisperModel(model: WhisperModel, cacheDir: String? = null, allowNetwork: Boolean, verifyHash: Boolean): WhisperModelPaths {
        val resultJson = KreuzbergBridge.nativeEnsureWhisperModel(mapper.writeValueAsString(model), cacheDir ?: "", allowNetwork, verifyHash)
        return mapper.readValue(resultJson, WhisperModelPaths::class.java)
    }
    /**
     * Resolve a Whisper model to on-disk ONNX paths, downloading from HF Hub if needed.
     *
     ...
     * model is unavailable and `allow_network` is `false`.
     */
    suspend fun ensureWhisperModelAsync(model: WhisperModel, cacheDir: String? = null, allowNetwork: Boolean, verifyHash: Boolean): WhisperModelPaths =
        withContext(Dispatchers.IO) { ensureWhisperModel(model, cacheDir, allowNetwork, verifyHash) }
    /**
     ...
     */
    fun readAudioTags(bytes: ByteArray): AudioTags {
        val resultJson = KreuzbergBridge.nativeReadAudioTags(java.util.Base64.getEncoder().encodeToString(bytes))
        return mapper.readValue(resultJson, AudioTags::class.java)
    }
    /**
     * Attempt to read audio tags and properties from raw bytes.
     *
     * Never panics or returns an error — failures produce `AudioTags.default()`.
     */
    suspend fun readAudioTagsAsync(bytes: ByteArray): AudioTags =
        withContext(Dispatchers.IO) { readAudioTags(bytes) }
    /**
