...
object Xberg {
    ...
    fun extractSync(input: ExtractInput, config: ExtractionConfig): ExtractionOutput {
        val resultJson = XbergBridge.nativeExtractSync(mapper.writeValueAsString(input), mapper.writeValueAsString(config))
        return mapper.readValue(resultJson, ExtractionOutput::class.java)
    }
    /** Synchronous wrapper for `extract`. */
    suspend fun extractSyncAsync(input: ExtractInput, config: ExtractionConfig): ExtractionOutput =
        withContext(Dispatchers.IO) { extractSync(input, config) }
    ...
    fun extractBatchSync(inputs: List<ExtractInput>, config: ExtractionConfig): ExtractionOutput {
        val resultJson = XbergBridge.nativeExtractBatchSync(mapper.writeValueAsString(inputs), mapper.writeValueAsString(config))
        return mapper.readValue(resultJson, ExtractionOutput::class.java)
    }
    /** Synchronous wrapper for `extract_batch`. */
    suspend fun extractBatchSyncAsync(inputs: List<ExtractInput>, config: ExtractionConfig): ExtractionOutput =
        withContext(Dispatchers.IO) { extractBatchSync(inputs, config) }
