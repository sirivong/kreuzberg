...
/// <summary>
/// Custom converter for LateInteractionModelType sealed union with flattened variant fields.
/// </summary>
/// <remarks>
...
/// </remarks>
public sealed class LateInteractionModelTypeJsonConverter : JsonConverter<LateInteractionModelType>
{
    public override LateInteractionModelType Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        {            "preset" => JsonSerializer.Deserialize<LateInteractionModelType.Preset>(flatJson, options) ?? throw new JsonException("Failed to deserialize variant"),            "custom" => JsonSerializer.Deserialize<LateInteractionModelType.Custom>(flatJson, options) ?? throw new JsonException("Failed to deserialize variant"),            "plugin" => JsonSerializer.Deserialize<LateInteractionModelType.Plugin>(flatJson, options) ?? throw new JsonException("Failed to deserialize variant"),            // Unknown discriminator: forward compatibility for unknown variants
            _ => throw new JsonException($"Unknown LateInteractionModelType discriminator: {tagValue}")
        };
    }

    public override void Write(Utf8JsonWriter writer, LateInteractionModelType value, JsonSerializerOptions options)
    {
        ...
        {            case LateInteractionModelType.Preset v_preset:
            tag = "preset";                inner = v_preset;                break;            case LateInteractionModelType.Custom v_custom:
            tag = "custom";                inner = v_custom;                break;            case LateInteractionModelType.Plugin v_plugin:
            tag = "plugin";                inner = v_plugin;                break;            default:
            throw new JsonException($"Unknown LateInteractionModelType variant: {value.GetType().Name}");
        }
