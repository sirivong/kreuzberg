    ...
    public static PatternMatch FromJson(string json)
    {
        try
        {
            return JsonSerializer.Deserialize<PatternMatch>(json, JsonOptions)
                ?? throw new KreuzbergException($"Failed to parse PatternMatch from JSON: deserializer returned null");
        }
        ...
        {
            throw new KreuzbergException($"Failed to parse PatternMatch from JSON: {e.Message}", e);
        }
    }
