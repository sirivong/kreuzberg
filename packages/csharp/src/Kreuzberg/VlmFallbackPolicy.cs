...
public sealed class VlmFallbackPolicyJsonConverter : JsonConverter<VlmFallbackPolicy>
{
    public override VlmFallbackPolicy Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        {            "disabled" => new VlmFallbackPolicy.Disabled(),            "on_low_quality" => JsonSerializer.Deserialize<VlmFallbackPolicy.OnLowQuality>(flatJson, options) ?? throw new JsonException("Failed to deserialize variant"),            "always" => new VlmFallbackPolicy.Always(),            _ => throw new JsonException($"Unknown VlmFallbackPolicy discriminator: {tagValue}")
        };
    }
