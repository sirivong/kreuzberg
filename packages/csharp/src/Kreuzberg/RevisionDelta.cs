    ...
    public static RevisionDelta FromJson(string json)
    {
        try
        {
            return JsonSerializer.Deserialize<RevisionDelta>(json, JsonOptions)
                ?? throw new KreuzbergException($"Failed to parse RevisionDelta from JSON: deserializer returned null");
        }
        ...
        {
            throw new KreuzbergException($"Failed to parse RevisionDelta from JSON: {e.Message}", e);
        }
    }
