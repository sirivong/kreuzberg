...
public static class KreuzbergConverter
{
    ...
    public static PcmAudio DecodeAudioToPcm(byte[] bytes, ulong? maxBytes)
    {
        ArgumentNullException.ThrowIfNull(bytes);
        var bytesHandle = GCHandle.Alloc(bytes, GCHandleType.Pinned);
        try
        {
            var nativeResult = NativeMethods.DecodeAudioToPcm(
            ...
            var returnValue = new PcmAudio(nativeResult);
            return returnValue;
        }
        finally
        {
    ...
    /// </summary>
    /// <exception cref="KreuzbergException">Returns `WhisperModelError` on I/O failures, download failures, or when the
    /// model is unavailable and `allow_network` is `false`.</exception>
    /// <param name="model"></param>
    /// <param name="cacheDir">Optional.</param>
    /// <param name="allowNetwork"></param>
    /// <param name="verifyHash"></param>
    public static WhisperModelPaths EnsureWhisperModel(WhisperModel model, string? cacheDir, bool allowNetwork, bool verifyHash)
    {
        var modelHandle = (int)model;
            var nativeResult = NativeMethods.EnsureWhisperModel(
            modelHandle,
            cacheDir!,
            allowNetwork,
            verifyHash
        ...
        var returnValue = new WhisperModelPaths(nativeResult);
        return returnValue;
    }
    ...
    /// </summary>
    /// <param name="bytes"></param>
    public static AudioTags ReadAudioTags(byte[] bytes)
    {
        ArgumentNullException.ThrowIfNull(bytes);
        var bytesHandle = GCHandle.Alloc(bytes, GCHandleType.Pinned);
        try
        {
            var nativeResult = NativeMethods.ReadAudioTags(
            bytesHandle.AddrOfPinnedObject(),
            (UIntPtr)bytes.Length
            );
            if (NativeMethods.LastErrorCode() != 0)
            ...
            var returnValue = new AudioTags(nativeResult);
            return returnValue;
        }
        finally
        {
