...
public sealed class ChunkSizingJsonConverter : JsonConverter<ChunkSizing>
{
    public override ChunkSizing Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        {            "characters" => new ChunkSizing.Characters(),            "tokenizer" => JsonSerializer.Deserialize<ChunkSizing.Tokenizer>(flatJson, options) ?? throw new JsonException("Failed to deserialize variant"),            // Unknown discriminator: binding-excluded variant or forward compatibility
            _ => throw new JsonException($"Unknown ChunkSizing discriminator: {tagValue} (this may be a binding-excluded variant)")
        };
    }
