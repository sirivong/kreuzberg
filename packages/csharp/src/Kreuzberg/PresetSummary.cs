    ...
    public static PresetSummary FromJson(string json)
    {
        try
        {
            return JsonSerializer.Deserialize<PresetSummary>(json, JsonOptions)
                ?? throw new KreuzbergException($"Failed to parse PresetSummary from JSON: deserializer returned null");
        }
        ...
        {
            throw new KreuzbergException($"Failed to parse PresetSummary from JSON: {e.Message}", e);
        }
    }
