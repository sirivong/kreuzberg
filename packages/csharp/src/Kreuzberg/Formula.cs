    ...
    public static Formula FromJson(string json)
    {
        try
        {
            return JsonSerializer.Deserialize<Formula>(json, JsonOptions)
                ?? throw new KreuzbergException($"Failed to parse Formula from JSON: deserializer returned null");
        }
        ...
        {
            throw new KreuzbergException($"Failed to parse Formula from JSON: {e.Message}", e);
        }
    }
