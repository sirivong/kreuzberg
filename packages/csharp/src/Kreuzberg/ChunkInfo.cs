    ...
    public static ChunkInfo FromJson(string json)
    {
        try
        {
            return JsonSerializer.Deserialize<ChunkInfo>(json, JsonOptions)
                ?? throw new KreuzbergException($"Failed to parse ChunkInfo from JSON: deserializer returned null");
        }
        ...
        {
            throw new KreuzbergException($"Failed to parse ChunkInfo from JSON: {e.Message}", e);
        }
    }
