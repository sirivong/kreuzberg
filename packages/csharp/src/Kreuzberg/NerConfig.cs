    ...
    public static NerConfig FromJson(string json)
    {
        try
        {
            return JsonSerializer.Deserialize<NerConfig>(json, JsonOptions)
                ?? throw new KreuzbergException($"Failed to parse NerConfig from JSON: deserializer returned null");
        }
        ...
        {
            throw new KreuzbergException($"Failed to parse NerConfig from JSON: {e.Message}", e);
        }
    }
