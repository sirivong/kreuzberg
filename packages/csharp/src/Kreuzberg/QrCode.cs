    ...
    public static QrCode FromJson(string json)
    {
        try
        {
            return JsonSerializer.Deserialize<QrCode>(json, JsonOptions)
                ?? throw new KreuzbergException($"Failed to parse QrCode from JSON: deserializer returned null");
        }
        ...
        {
            throw new KreuzbergException($"Failed to parse QrCode from JSON: {e.Message}", e);
        }
    }
