...
pub const ExtractionOutput = struct {
    ...
    crawl: std.StringHashMap([]const u8),
};
...
pub fn extract_sync(input: []const u8, config: []const u8) XbergError![]u8 {
    ...
    const _result = c.xberg_extract_sync(input_handle, config_handle);
...
pub fn extract_batch_sync(inputs: []const u8, config: []const u8) XbergError![]u8 {
    ...
    if (config_handle == null) return _error_with_message(XbergError);
    defer c.xberg_extraction_config_free(config_handle);
    const _result = c.xberg_extract_batch_sync(inputs_z, config_handle);
    if (c.xberg_last_error_code() != 0) {
        return _error_with_message(XbergError);
    }
    if (_result == null) {
