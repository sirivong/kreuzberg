...
pub fn decode_audio_to_pcm(bytes: []const u8, max_bytes: ?u64) KreuzbergError!PcmAudio {
    const _result = c.kreuzberg_decode_audio_to_pcm(bytes.ptr, bytes.len, if (max_bytes) |v| v else std.math.maxInt(u64));
    if (c.kreuzberg_last_error_code() != 0) {
        return _error_with_message(KreuzbergError);
    }
    return PcmAudio{ ._handle = _result.? };
}
...
pub fn ensure_whisper_model(model: WhisperModel, cache_dir: ?[]const u8, allow_network: bool, verify_hash: bool) KreuzbergError!WhisperModelPaths {
    const cache_dir_z: ?[:0]u8 = if (cache_dir) |v| try std.fmt.allocPrintSentinel(
        std.heap.c_allocator, "{s}", .{v}, 0) else null;
    defer if (cache_dir_z) |z| std.heap.c_allocator.free(z);
    const _result = c.kreuzberg_ensure_whisper_model(model, if (cache_dir_z) |z| z.ptr else null, allow_network, verify_hash);
    if (c.kreuzberg_last_error_code() != 0) {
        return _error_with_message(KreuzbergError);
    }
    return WhisperModelPaths{ ._handle = _result.? };
}
...
pub fn read_audio_tags(bytes: []const u8) AudioTags {
    const _result = c.kreuzberg_read_audio_tags(bytes.ptr, bytes.len);
    return AudioTags{ ._handle = _result.? };
}
...
///
/// Since v5.0.0.
pub fn rerank_async(_query: []const u8, _documents: []const u8, _config: []const u8) KreuzbergError![]u8 {
    const _query_z = try std.fmt.allocPrintSentinel(
        std.heap.c_allocator, "{s}", .{_query}, 0);
    defer std.heap.c_allocator.free(_query_z);
    // Vec/Map parameters are passed as JSON strings across the FFI boundary.
    const _documents_z = try std.fmt.allocPrintSentinel(
        std.heap.c_allocator, "{s}", .{_documents}, 0);
    defer std.heap.c_allocator.free(_documents_z);
    const _config_z = try std.fmt.allocPrintSentinel(
        std.heap.c_allocator, "{s}", .{_config}, 0);
    defer std.heap.c_allocator.free(_config_z);
    const _config_handle = c.kreuzberg_reranker_config_from_json(_config_z);
    if (_config_handle == null) return _error_with_message(KreuzbergError);
    defer c.kreuzberg_reranker_config_free(_config_handle);
    const _result = c.kreuzberg_rerank_async(_query_z, _documents_z, _config_handle);
    if (c.kreuzberg_last_error_code() != 0) {
        return _error_with_message(KreuzbergError);
    }
    if (_result == null) {
    ...
    const _result_len = c.kreuzberg_rerank_async_len(_query_z, _documents_z, _config_handle);
    return blk: {
        if (_result == null) return _first_error(KreuzbergError);
        const slice = _result[0.._result_len];
        const owned = try std.heap.c_allocator.dupe(u8, slice);
...
pub const PcmAudio = struct {
    ...
    /// Release the underlying FFI handle. Safe to call once per instance.
    pub fn free(self: *PcmAudio) void {
        c.kreuzberg_pcm_audio_free(@as(*c.KREUZBERGPcmAudio, @ptrCast(self._handle)));
    }
};

/// On-disk paths for all files needed to load a Whisper model in an ORT session.
pub const WhisperModelPaths = struct {
    _handle: *anyopaque,

    /// Release the underlying FFI handle. Safe to call once per instance.
    pub fn free(self: *WhisperModelPaths) void {
        c.kreuzberg_whisper_model_paths_free(@as(*c.KREUZBERGWhisperModelPaths, @ptrCast(self._handle)));
    }
};
...
pub const AudioTags = struct {
    ...
    /// Release the underlying FFI handle. Safe to call once per instance.
    pub fn free(self: *AudioTags) void {
        c.kreuzberg_audio_tags_free(@as(*c.KREUZBERGAudioTags, @ptrCast(self._handle)));
    }
};
