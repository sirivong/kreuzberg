...
import pytest
from xberg import ExtractionConfig, PageConfig, XbergError

from langchain_xberg import XbergLoader
from tests.conftest import make_document, make_error, make_page, make_result
...
async def test_alazy_load_bytes_mode(mock_extract: AsyncMock) -> None:
    mock_extract.return_value = make_result([make_document(content="Bytes async content")])
...
async def test_alazy_load_per_page(mock_extract: AsyncMock) -> None:
    mock_extract.return_value = make_result(
        [
            make_document(
                pages=[make_page(1, "Page 1", is_blank=False), make_page(2, "Page 2", is_blank=False)],
                page_count=2,
            )
        ]
    )
...
async def test_aload_convenience(mock_extract: AsyncMock) -> None:
    mock_extract.return_value = make_result([make_document(content="Async doc")])
