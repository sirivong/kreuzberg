...
// alef:hash:46cde08adc07ae87ffab71296d23e14aeb1206f1bfd9aafcd87a1aef4eeb8c9d
...
impl JsFormatMetadata {
    pub fn docx(&self) -> Option<JsDocxMetadata> {
        if self.format_type_tag != "docx" {
            return None;
        }
        self._0
            .as_ref()
            .and_then(|s| serde_json::from_str::<JsDocxMetadata>(s).ok())
    }

    pub fn excel(&self) -> Option<JsExcelMetadata> {
        if self.format_type_tag != "excel" {
            return None;
        }
        self._0
            .as_ref()
            .and_then(|s| serde_json::from_str::<JsExcelMetadata>(s).ok())
    }

    
    pub fn email(&self) -> Option<JsEmailMetadata> {
        if self.format_type_tag != "email" {
            return None;
        }
        self._0
            .as_ref()
            .and_then(|s| serde_json::from_str::<JsEmailMetadata>(s).ok())
    }

    
    pub fn pptx(&self) -> Option<JsPptxMetadata> {
        if self.format_type_tag != "pptx" {
            return None;
        }
        self._0
            .as_ref()
            .and_then(|s| serde_json::from_str::<JsPptxMetadata>(s).ok())
    }

    
    pub fn archive(&self) -> Option<JsArchiveMetadata> {
        if self.format_type_tag != "archive" {
            return None;
        }
        self._0
            .as_ref()
            .and_then(|s| serde_json::from_str::<JsArchiveMetadata>(s).ok())
    }

    
    pub fn xml(&self) -> Option<JsXmlMetadata> {
        if self.format_type_tag != "xml" {
            return None;
        }
        self._0
            .as_ref()
            .and_then(|s| serde_json::from_str::<JsXmlMetadata>(s).ok())
    }

    
    pub fn text(&self) -> Option<JsTextMetadata> {
        if self.format_type_tag != "text" {
            return None;
        }
        self._0
            .as_ref()
            .and_then(|s| serde_json::from_str::<JsTextMetadata>(s).ok())
    }

    
    pub fn html(&self) -> Option<JsHtmlMetadata> {
        if self.format_type_tag != "html" {
            return None;
        }
        self._0
            .as_ref()
            .and_then(|s| serde_json::from_str::<JsHtmlMetadata>(s).ok())
    }

    
    pub fn ocr(&self) -> Option<JsOcrMetadata> {
        if self.format_type_tag != "ocr" {
            return None;
        }
        self._0
            .as_ref()
            .and_then(|s| serde_json::from_str::<JsOcrMetadata>(s).ok())
    }

    
    pub fn csv(&self) -> Option<JsCsvMetadata> {
        if self.format_type_tag != "csv" {
            return None;
        }
        self._0
            .as_ref()
            .and_then(|s| serde_json::from_str::<JsCsvMetadata>(s).ok())
    }

    
    pub fn bibtex(&self) -> Option<JsBibtexMetadata> {
        if self.format_type_tag != "bibtex" {
            return None;
        }
        self._0
            .as_ref()
            .and_then(|s| serde_json::from_str::<JsBibtexMetadata>(s).ok())
    }

    
    pub fn citation(&self) -> Option<JsCitationMetadata> {
        if self.format_type_tag != "citation" {
            return None;
        }
        self._0
            .as_ref()
            .and_then(|s| serde_json::from_str::<JsCitationMetadata>(s).ok())
    }

    
    pub fn fiction_book(&self) -> Option<JsFictionBookMetadata> {
        if self.format_type_tag != "fictionbook" {
            return None;
        }
        self._0
            .as_ref()
            .and_then(|s| serde_json::from_str::<JsFictionBookMetadata>(s).ok())
    }

    
    pub fn dbf(&self) -> Option<JsDbfMetadata> {
        if self.format_type_tag != "dbf" {
            return None;
        }
        self._0
            .as_ref()
            .and_then(|s| serde_json::from_str::<JsDbfMetadata>(s).ok())
    }

    
    pub fn jats(&self) -> Option<JsJatsMetadata> {
        if self.format_type_tag != "jats" {
            return None;
        }
        self._0
            .as_ref()
            .and_then(|s| serde_json::from_str::<JsJatsMetadata>(s).ok())
    }

    
    pub fn epub(&self) -> Option<JsEpubMetadata> {
        if self.format_type_tag != "epub" {
            return None;
        }
        self._0
            .as_ref()
            .and_then(|s| serde_json::from_str::<JsEpubMetadata>(s).ok())
    }

    
    pub fn pst(&self) -> Option<JsPstMetadata> {
        if self.format_type_tag != "pst" {
            return None;
        }
        self._0
            .as_ref()
            .and_then(|s| serde_json::from_str::<JsPstMetadata>(s).ok())
    }
}

// Standalone getter functions for JsFormatMetadata union variants.
// These wrap the inner impl methods to expose them via NAPI since
// #[napi(getter)] doesn't work on #[napi(object)] structs.

#[napi]
pub fn get_format_metadata_excel(fmt: &JsFormatMetadata) -> Option<JsExcelMetadata> {
    fmt.excel()
}

#[napi]
pub fn get_format_metadata_docx(fmt: &JsFormatMetadata) -> Option<JsDocxMetadata> {
    fmt.docx()
}

#[napi]
pub fn get_format_metadata_email(fmt: &JsFormatMetadata) -> Option<JsEmailMetadata> {
    fmt.email()
}

#[napi]
pub fn get_format_metadata_pptx(fmt: &JsFormatMetadata) -> Option<JsPptxMetadata> {
    fmt.pptx()
}

#[napi]
pub fn get_format_metadata_archive(fmt: &JsFormatMetadata) -> Option<JsArchiveMetadata> {
    fmt.archive()
}

#[napi]
pub fn get_format_metadata_xml(fmt: &JsFormatMetadata) -> Option<JsXmlMetadata> {
    fmt.xml()
}

#[napi]
pub fn get_format_metadata_text(fmt: &JsFormatMetadata) -> Option<JsTextMetadata> {
    fmt.text()
}

#[napi]
pub fn get_format_metadata_html(fmt: &JsFormatMetadata) -> Option<JsHtmlMetadata> {
    fmt.html()
}

#[napi]
pub fn get_format_metadata_ocr(fmt: &JsFormatMetadata) -> Option<JsOcrMetadata> {
    fmt.ocr()
}

#[napi]
pub fn get_format_metadata_csv(fmt: &JsFormatMetadata) -> Option<JsCsvMetadata> {
    fmt.csv()
}

#[napi]
pub fn get_format_metadata_bibtex(fmt: &JsFormatMetadata) -> Option<JsBibtexMetadata> {
    fmt.bibtex()
}

#[napi]
pub fn get_format_metadata_citation(fmt: &JsFormatMetadata) -> Option<JsCitationMetadata> {
    fmt.citation()
}

#[napi(js_name = "getFormatMetadataFictionBook")]
pub fn get_format_metadata_fiction_book(fmt: &JsFormatMetadata) -> Option<JsFictionBookMetadata> {
    fmt.fiction_book()
}

#[napi]
pub fn get_format_metadata_dbf(fmt: &JsFormatMetadata) -> Option<JsDbfMetadata> {
    fmt.dbf()
}

#[napi]
pub fn get_format_metadata_jats(fmt: &JsFormatMetadata) -> Option<JsJatsMetadata> {
    fmt.jats()
}

#[napi]
pub fn get_format_metadata_epub(fmt: &JsFormatMetadata) -> Option<JsEpubMetadata> {
    fmt.epub()
}

#[napi]
pub fn get_format_metadata_pst(fmt: &JsFormatMetadata) -> Option<JsPstMetadata> {
    fmt.pst()
}
