...
impl GlineBackend {
    #[allow(clippy::too_many_arguments)]
    #[must_use]
    #[pyo3(signature = (repo_id, model_path, tokenizer_path))]
    #[new]
    pub fn new(repo_id: String, model_path: String, tokenizer_path: String) -> Self {
        Self {
            repo_id,
            model_path,
            tokenizer_path,
    ...
    pub fn detect<'py>(
        ...
        Err(pyo3::exceptions::PyNotImplementedError::new_err(
            "Not implemented: GlineBackend.detect",
        ))
    }
    ...
    pub fn detect_with_custom<'py>(
        ...
        Err(pyo3::exceptions::PyNotImplementedError::new_err(
            "Not implemented: GlineBackend.detect_with_custom",
        ))
    }
    ...
    pub fn new(repo_id: Option<String>) -> PyResult<GlineBackend> {
        kreuzberg::GlineBackend::new(repo_id.as_deref())
            .map(Into::into)
            .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))
    }
...
pub struct TokenCounter {
    inner: Arc<std::sync::Mutex<kreuzberg::TokenCounter>>,
}

#[pymethods]
impl TokenCounter {
    #[pyo3(signature = (category, original))]
    pub fn next_token(&self, category: PiiCategory, original: String) -> String {
        let category_core: kreuzberg::PiiCategory = category.into();

        self.inner.lock().unwrap().next_token(&category_core, &original)
    }
    ...
    pub fn new() -> TokenCounter {
        Self {
            inner: Arc::new(std::sync::Mutex::new(kreuzberg::TokenCounter::new())),
        }
    }
}
...
impl Segment {
    #[allow(clippy::too_many_arguments)]
    #[must_use]
    #[pyo3(signature = (text, byte_start))]
    #[new]
    pub fn new(text: String, byte_start: usize) -> Self {
...
pub fn classify_pages<'py>(
    ...
    pyo3_async_runtimes::tokio::future_into_py(py, async move {
        let result = kreuzberg::text::classification::classify_pages(&result_core, &config_core)
            .await
            .map_err(|e| PyErr::new::<PyRuntimeError, _>(e.to_string()))?;
...
pub fn redact<'py>(py: Python<'py>, result: ExtractionResult, config: RedactionConfig) -> PyResult<Bound<'py, PyAny>> {
    ...
    pyo3_async_runtimes::tokio::future_into_py(py, async move {
        let result = kreuzberg::text::redaction::redact(&result_core, &config_core)
            .await
            .map_err(|e| PyErr::new::<PyRuntimeError, _>(e.to_string()))?;
...
#[pyfunction]
#[pyo3(signature = (strategy, original, category, counter))]
...
pub fn translate_result<'py>(
    ...
    pyo3_async_runtimes::tokio::future_into_py(py, async move {
        let result = kreuzberg::text::translation::translate_result(&result_core, &config_core)
            .await
            .map_err(|e| PyErr::new::<PyRuntimeError, _>(e.to_string()))?;
...
pub fn embed_texts(texts: Vec<String>, config: EmbeddingConfig) -> PyResult<Vec<Vec<f32>>> {
    ...
    kreuzberg::embed_texts(texts, &config_core).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))
}
...
impl From<GlineBackend> for kreuzberg::GlineBackend {
    fn from(val: GlineBackend) -> Self {
        Self {
            repo_id: val.repo_id,
            model_path: val.model_path.into(),
            tokenizer_path: val.tokenizer_path.into(),
        }
    }
}
...
impl From<kreuzberg::GlineBackend> for GlineBackend {
    fn from(val: kreuzberg::GlineBackend) -> Self {
        Self {
            repo_id: val.repo_id,
            model_path: val.model_path.to_string_lossy().to_string(),
            tokenizer_path: val.tokenizer_path.to_string_lossy().to_string(),
        }
    }
}
...
impl From<kreuzberg::chunking::semantic::merge::Segment> for Segment {
    fn from(val: kreuzberg::chunking::semantic::merge::Segment) -> Self {
        Self {
            text: val.text,
            byte_start: val.byte_start,
        }
...
pub fn _kreuzberg(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(apply_strategy, m)?)?;
    ...
    m.add_function(wrap_pyfunction!(embed_texts, m)?)?;
