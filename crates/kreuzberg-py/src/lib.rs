...
// alef:hash:0b3be86eceae530a477b1c0c3ecaf0a60a0d741a382fea3f27693f6b647eee3c
...
impl EmbedTextParams {
    #[must_use]
    #[pyo3(signature = (texts, preset=None, model=None, api_key=None, embedding_plugin=None))]
    #[new]
    pub fn new(
        texts: Vec<String>,
        preset: Option<String>,
...
impl PdfImage {
    #[allow(clippy::too_many_arguments)]
    #[must_use]
    #[pyo3(signature = (page_number, image_index, width, height, filters, data, decoded_format, color_space=None, bits_per_component=None, image_kind=None, kind_confidence=None, cluster_id=None))]
    #[new]
    pub fn new(
        page_number: usize,
        image_index: usize,
...
impl OutputFormat {
    ...
    fn plain(&self, py: Python<'_>) -> PyResult<Option<pyo3::Py<pyo3::types::PyDict>>> {
        ...
        // Check the tag field to see if this variant is active
        let tag_field = "tag";
        let tag_value = json.get(tag_field).and_then(|v| v.as_str()).unwrap_or("");
        if tag_value != "plain" {
            return Ok(None);
        }
        // Create a Python dict from the JSON
...
impl PdfBackend {
    fn __str__(&self) -> PyResult<String> {
        serde_json::to_value(self)
            ...
            .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(format!("Failed to serialize PdfBackend: {e}")))
    }
...
impl PoolError {
    fn __str__(&self) -> PyResult<String> {
        serde_json::to_value(self)
            ...
            .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(format!("Failed to serialize PoolError: {e}")))
    }
        ...
        Python::attach(|py| {
            self.inner
                ...
                .call_method1(
                    "should_process",
                    (
                        serde_json::to_string(_result).unwrap_or_default(),
                        serde_json::to_string(_config).unwrap_or_default(),
                    ),
                )
                .and_then(|v| v.extract::<bool>())
                .unwrap_or_default()
...
impl From<DocxMetadata> for kreuzberg::DocxMetadata {
    fn from(val: DocxMetadata) -> Self {
        Self {
            core_properties: val.core_properties.as_ref().and_then(|s| serde_json::from_str(s).ok()),
            app_properties: val.app_properties.as_ref().and_then(|s| serde_json::from_str(s).ok()),
...
impl From<kreuzberg::DocxMetadata> for DocxMetadata {
    fn from(val: kreuzberg::DocxMetadata) -> Self {
        Self {
            core_properties: val.core_properties.as_ref().map(ToString::to_string),
            app_properties: val.app_properties.as_ref().map(ToString::to_string),
