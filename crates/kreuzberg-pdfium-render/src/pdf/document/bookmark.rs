...
#[cfg(doc)]
use {
    crate::pdf::action::PdfActionType, crate::pdf::document::PdfDocument, crate::pdf::document::bookmarks::PdfBookmarks,
};
...
impl<'a> PdfBookmark<'a> {
    ...
    pub fn action(&self) -> Option<PdfAction<'a>> {
        ...
        if handle.is_null() {
            ...
            Some(PdfAction::from_pdfium(handle, self.document_handle, self.bindings))
        }
    }
    ...
    pub fn destination(&self) -> Option<PdfDestination<'a>> {
        ...
        if handle.is_null() {
            ...
            Some(PdfDestination::from_pdfium(self.document_handle, handle, self.bindings))
        }
    }
    ...
    pub fn parent(&self) -> Option<PdfBookmark<'a>> {
        self.parent
            .map(|parent_handle| PdfBookmark::from_pdfium(parent_handle, None, self.document_handle, self.bindings))
    }
    ...
    pub fn children_len(&self) -> usize {
        ...
        self.bindings.FPDFBookmark_GetCount(self.bookmark_handle).unsigned_abs() as usize
    }
        ...
        match self.parent {
            Some(parent_handle) => {
                ...
                PdfBookmarksIterator::new(
                    PdfBookmark::from_pdfium(parent_handle, None, self.document_handle, self.bindings).first_child(),
                    false,
    ...
    pub fn iter_direct_children(&self) -> PdfBookmarksIterator<'a> {
        PdfBookmarksIterator::new(self.first_child(), false, None, self.document_handle(), self.bindings())
    }
    ...
    pub fn iter_all_descendants(&self) -> PdfBookmarksIterator<'a> {
        PdfBookmarksIterator::new(self.first_child(), true, None, self.document_handle(), self.bindings())
    }
}
...
mod tests {
    ...
    fn test_bookmarks() -> Result<(), PdfiumError> {
        ...
        let section5_3_3_1 = document.bookmarks().find_first_by_title("Section 5.3.3.1")?;
