...
fn test_config_file_precedence_over_defaults() {
    ...
    let file_config: xberg::core::config::ExtractionConfig = serde_json::from_value(json).expect("Failed to parse");
...
fn test_output_format_deserialization() {
    ...
    let markdown: xberg::core::config::OutputFormat =
        serde_json::from_value(serde_json::json!("markdown")).expect("Failed to deserialize markdown");
    assert_eq!(markdown, xberg::core::config::OutputFormat::Markdown);

    let html: xberg::core::config::OutputFormat =
        serde_json::from_value(serde_json::json!("html")).expect("Failed to deserialize html");
    assert_eq!(html, xberg::core::config::OutputFormat::Html);
}
...
fn test_config_with_all_output_formats() {
    let formats = vec![
        xberg::core::config::OutputFormat::Plain,
        xberg::core::config::OutputFormat::Markdown,
        xberg::core::config::OutputFormat::Html,
    ];
...
fn test_unknown_output_format_accepted_as_custom() {
    ...
    let result: Result<xberg::core::config::ExtractionConfig, _> = serde_json::from_value(json);
    ...
    assert_eq!(
        ...
        xberg::core::config::OutputFormat::Custom("my_custom_renderer".to_string()),
