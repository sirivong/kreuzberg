...
//!
//! ```sh
//! XBERG_DEEPSEEK_OCR_MODEL_PATH=/path/to/deepseek-ocr \
//!   cargo test -p xberg-candle-ocr --features deepseek-ocr \
...
fn deepseek_ocr_extracts_text_from_sample_image() {
    let model_path = match std::env::var("XBERG_DEEPSEEK_OCR_MODEL_PATH") {
        ...
        _ => {
            println!(
                "XBERG_DEEPSEEK_OCR_MODEL_PATH not set — skipping integration test.\n\
