...
impl FilterPipeline {
    ...
    ///
    /// # Errors
    /// Returns a `XbergError::Validation` if any preserve patterns are invalid regex
    pub(crate) fn new(config: &Arc<TokenReductionConfig>, language: &str) -> Result<Self> {
        ...
        let preserve_patterns =
            preserve_patterns.map_err(|e| XbergError::validation(format!("Invalid regex pattern: {}", e)))?;
