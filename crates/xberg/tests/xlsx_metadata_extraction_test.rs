...
use xberg::extraction::excel::{excel_to_markdown, excel_to_text, read_excel_file};
...
/// The sheet dimension is set to "A1:XFD1048575", which could cause Xberg
...
fn test_xlsx_excel_solver_extreme_dimensions_no_oom() {
    let workspace_root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .parent()
        .expect("Operation failed");
    let test_file = workspace_root.join("test_documents/test_fixtures/xlsx-oom-repro/xberg-oom-repro.xlsx");
