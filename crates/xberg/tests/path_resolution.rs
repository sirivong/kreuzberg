...
async fn test_markdown_resolves_images() {
    ...
    let result = extract_uri_document(&path, None, &config).await.unwrap();
...
async fn test_markdown_bytes_no_resolution() {
    ...
    let config = config_with_images();
    let result = extract_bytes_document(&content, "text/markdown", &config)
        .await
        .unwrap();

    let image_count = result.images.as_ref().map_or(0, |imgs| imgs.len());
    assert_eq!(image_count, 0, "extract_bytes_document should not resolve local images");
}
...
async fn test_rst_resolves_images() {
    ...
    let config = config_with_images();
    let result = extract_uri_document(&path, Some("text/x-rst"), &config).await.unwrap();
...
async fn test_orgmode_resolves_images() {
    ...
    let config = config_with_images();
    let result = extract_uri_document(&path, Some("text/x-org"), &config).await.unwrap();
...
async fn test_typst_resolves_images() {
    ...
    let config = config_with_images();
    let result = extract_uri_document(&path, Some("application/x-typst"), &config)
        .await
        .unwrap();
...
async fn test_djot_resolves_images() {
    ...
    let config = config_with_images();
    let result = extract_uri_document(&path, Some("text/djot"), &config).await.unwrap();
...
async fn test_traversal_blocked() {
    ...
    let config = config_with_images();
    let result = extract_uri_document(&md_path, Some("text/markdown"), &config)
        .await
        .unwrap();
