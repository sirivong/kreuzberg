...
async fn test_ocr_chinese_with_chinese_model() {
    ...
    let cache_dir = std::env::temp_dir().join("xberg_paddle_multilang_test");
...
async fn test_ocr_concurrent_different_languages() {
    ...
    let cache_dir = std::env::temp_dir().join("xberg_paddle_multilang_concurrent");
