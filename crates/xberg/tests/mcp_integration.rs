...
fn test_config_merge_in_mcp_context() {
    ...
    let full_config: xberg::core::config::ExtractionConfig =
...
fn test_config_json_flexibility() {
    ...
    for config_json in configs {
        let config: Result<xberg::core::config::ExtractionConfig, _> = serde_json::from_value(config_json);
...
fn test_mcp_invalid_config_json_error() {
    ...
    let result: Result<xberg::core::config::ExtractionConfig, _> = serde_json::from_str(invalid_config);
...
fn test_mcp_config_round_trip_serialization() {
    let original_config = xberg::core::config::ExtractionConfig {
        use_cache: false,
        enable_quality_processing: true,
        force_ocr: true,
...
fn test_mcp_error_invalid_format_field() {
    ...
    assert_eq!(
        ...
        xberg::OutputFormat::Custom("invalid_format_that_does_not_exist".to_string())
    );
}
...
fn test_mcp_html_output_format() {
    ...
    assert_eq!(config.output_format, xberg::core::config::OutputFormat::Html);
}
...
fn test_mcp_config_partial_updates() {
    let mut base_config = xberg::core::config::ExtractionConfig::default();
