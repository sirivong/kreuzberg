...
async fn test_cache_instrumentation() {
    ...
    cache
        .set_default("test_key", b"test data".to_vec(), None)
        .expect("Operation failed");

    let _ = cache.get_default("test_key", None).expect("Value not found");
...
async fn test_registry_instrumentation() {
    use xberg::plugins::registry::DocumentExtractorRegistry;
