...
impl AllHeads {
    ...
    fn load(vb: VarBuilder<'_>, device: &Device) -> crate::candle::Result<Self> {
        let span_rep = span_rep::SpanRep::from_var_builder(&vb.pp("span_rep").pp("span_rep_layer"))
            .map_err(|e| crate::candle::GlinerCandleError::Backend(format!("span_rep load: {e}")))?;
        let count_lstm = count_lstm::CountLstmFixed::from_var_builder(&vb.pp("count_embed"), device)
            .map_err(|e| crate::candle::GlinerCandleError::Backend(format!("count_embed load: {e}")))?;
        let count_pred = count_pred::CountPred::from_var_builder(&vb.pp("count_pred"))
            .map_err(|e| crate::candle::GlinerCandleError::Backend(format!("count_pred load: {e}")))?;
