...
pub struct JsExtractionOutput {
    ...
    pub crawl: Option<HashMap<String, serde_json::Value>>,
}
...
pub fn extract_sync(input: Option<JsExtractInput>, config: Option<JsExtractionConfig>) -> Result<JsExtractionOutput> {
    ...
    xberg::extract_sync(input_core, &config_core)
        .map(|val| val.into())
        .map_err(|e| napi::Error::new(napi::Status::GenericFailure, e.to_string()))
}
...
pub fn extract_batch_sync(
    ...
    xberg::extract_batch_sync(inputs_core, &config_core)
        .map(|val| val.into())
        .map_err(|e| napi::Error::new(napi::Status::GenericFailure, e.to_string()))
}
        ...
        match result {
            ...
            Ok(val) => {
                ...
                match s.as_str() {
                    ...
                    _ => serde_json::from_str::<_>(&s).map_err(|_| {
                        xberg::XbergError::Other("Failed to parse return value for method 'extract_bytes'".to_string())
                    }),
                }
            }
        }
...
impl From<JsExtractionOutput> for xberg::ExtractionOutput {
    fn from(val: JsExtractionOutput) -> Self {
        ...
        if let Some(__v) = val.crawl {
            __result.crawl = __v.into_iter().map(|(k, v)| (k.into(), v)).collect();
        }
        __result
    }
}
...
impl From<xberg::ExtractionOutput> for JsExtractionOutput {
    fn from(val: xberg::ExtractionOutput) -> Self {
        Self {
            ...
            crawl: Some(val.crawl.into_iter().map(|(k, v)| (k.into(), v)).collect()),
        }
    }
}
