...
#[derive(Clone, serde::Serialize, serde::Deserialize, Default)]
#[php_class]
#[php(name = "Kreuzberg\\PcmAudio")]
#[serde(default, rename_all = "camelCase")]
pub struct PcmAudio {
    #[php(prop, name = "samples")]
    pub samples: Vec<f32>,
...
impl PcmAudio {
    #[php(constructor)]
    pub fn new(samples: Vec<f32>, sampleRateHz: u32, channels: u16, durationMs: i64) -> Self {
        Self {
            samples,
    ...
    pub fn get_sample_rate_hz(&self) -> u32 {
        self.sample_rate_hz.clone()
    }
...
impl WhisperModelPaths {
    ...
    pub fn new(
        ...
        Self {
            encoder,
            decoder,
            decoder_with_past: decoderWithPast,
            tokenizer,
            config,
            n_mels: nMels,
        }
    ...
    pub fn get_encoder(&self) -> String {
        self.encoder.clone()
    }

    pub fn get_decoder(&self) -> String {
        self.decoder.clone()
    }

    pub fn get_decoder_with_past(&self) -> String {
        self.decoder_with_past.clone()
    }
...
impl Default for AudioTags {
    fn default() -> Self {
        <kreuzberg::transcription::tags::AudioTags as Default>::default().into()
    }
}
...
impl KreuzbergApi {
     ...
     * @param mixed $pages Slice of page texts to classify. Each page is classified independently using the configured LLM, and results are aggregated.
     *
     * @param mixed $config Classification configuration including labels and LLM settings.
     *
     * @return A vector of `ClassificationLabel` entries representing the document's overall classification.
     *
     * @throws KreuzbergException Returns an error if `config.labels` is empty or if LLM calls fail.
     */
     ...
     */
    #[php(name = "decodeAudioToPcm")]
    pub fn decode_audio_to_pcm(bytes: PhpBytes, maxBytes: Option<i64>) -> PhpResult<PcmAudio> {
        let result = kreuzberg::transcription::decode::decode_audio_to_pcm(&bytes.0[..], maxBytes.map(|v| v as u64))
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))?;
        Ok(result.into())
    }
     ...
     * @throws KreuzbergException Returns `WhisperModelError` on I/O failures, download failures, or when the
     * model is unavailable and `allow_network` is `false`.
     */
    #[php(name = "ensureWhisperModel")]
    pub fn ensure_whisper_model(
        model: String,
        cacheDir: Option<String>,
        allowNetwork: bool,
        verifyHash: bool,
    ) -> PhpResult<WhisperModelPaths> {
        let model_core: kreuzberg::WhisperModel = model.clone().into();
        let result = kreuzberg::transcription::model::ensure_whisper_model(
            model_core,
            cacheDir.as_deref().map(std::path::Path::new),
            allowNetwork,
            verifyHash,
        )
        .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))?;
     ...
     */
    #[php(name = "readAudioTags")]
    pub fn read_audio_tags(bytes: PhpBytes) -> AudioTags {
        kreuzberg::transcription::tags::read_audio_tags(&bytes.0[..]).into()
    }

    #[cfg(all(feature = "captioning", feature = "tokio-runtime"))]
    /**
     * Caption a single image from bytes.
     * @param mixed $image_bytes The image data.
     *
     * @param mixed $llm_config LLM configuration for the VLM call.
     *
     * @param mixed $custom_prompt Optional custom caption prompt. Uses the default `RegionKind.Caption` prompt when `None`.
     *
     * @return The generated caption text.
     *
     * @throws KreuzbergException Returns an error if the VLM call fails or if image format detection fails.
     */
     ...
     * @param mixed $path Path to the image file.
     *
     * @param mixed $llm_config LLM configuration for the VLM call.
     *
     * @param mixed $custom_prompt Optional custom caption prompt. Uses the default `RegionKind.Caption` prompt when `None`.
     ...
     * @return The generated caption text.
     *
     * @throws KreuzbergException Returns an error if the file cannot be read, if image format detection fails,
...
pub extern "C" fn get_module() -> *mut ::ext_php_rs::zend::ModuleEntry {
    ...
    __EXT_PHP_RS_MODULE_ENTRY.get_or_init(|| {
        let builder = ::ext_php_rs::builders::ModuleBuilder::new("kreuzberg", "5.0.0-rc.16")
