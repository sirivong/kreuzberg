...
//! # CI gating
//!
//! These tests are always on. Set `KREUZBERG_HF_CACHE_DIR` to point at a
...
//! `KREUZBERG_SKIP_LIVE_HF=1` to skip these tests.

#![cfg(all(feature = "reranker", feature = "reranker-presets", feature = "tokio-runtime"))]

use std::path::PathBuf;

use kreuzberg::core::config::RerankerModelType;
use kreuzberg::{RerankerConfig, get_reranker_preset, rerank_async};
...
fn should_skip() -> bool {
    std::env::var("KREUZBERG_SKIP_LIVE_HF").ok().as_deref() == Some("1")
}

fn cache_dir() -> Option<PathBuf> {
    std::env::var("KREUZBERG_HF_CACHE_DIR").ok().map(PathBuf::from)
}
...
async fn run_preset_inference(
    preset_name: &str,
    suite: &FixtureSuite,
) -> kreuzberg::Result<Vec<kreuzberg::RerankedDocument>> {
...
fn assert_scores_in_unit_interval(results: &[kreuzberg::RerankedDocument], context: &str) {
    for r in results {
        assert!(
...
async fn bge_reranker_base_english_top_ranks_first() {
    if should_skip() {
        eprintln!("KREUZBERG_SKIP_LIVE_HF=1, skipping");
        return;
    }
