...
/// # Errors
///
/// - `KreuzbergError::Reranking` if the API call fails or returns unexpected data.
...
pub(crate) async fn rerank_via_llm(
    ...
    let response = client.rerank(request).await.map_err(|e| {
        crate::KreuzbergError::reranking(format!("LLM rerank request failed (model={}): {e}", config.model))
    })?;
