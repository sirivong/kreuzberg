...
//! `looks_like_ort_error`) from `crates/kreuzberg/src/embeddings/mod.rs`.
...
fn download_model_files(
    ...
    let api = hf_hub::api::sync::ApiBuilder::from_env()
        ...
        .with_progress(true)
        .build()
        .map_err(|e| crate::KreuzbergError::reranking(format!("Failed to create HF API client: {e}")))?;
    ...
    let tokenizer_path = repo
        .get("tokenizer.json")
        .map_err(|e| crate::KreuzbergError::reranking(format!("Failed to download tokenizer.json: {e}")))?;

    let config_path = repo
        .get("config.json")
        .map_err(|e| crate::KreuzbergError::reranking(format!("Failed to download config.json: {e}")))?;
...
fn load_tokenizer(
    ...
    let config: serde_json::Value = serde_json::from_slice(
        &std::fs::read(config_path)
            .map_err(|e| crate::KreuzbergError::reranking(format!("Failed to read config.json: {e}")))?,
    )
    .map_err(|e| crate::KreuzbergError::reranking(format!("Failed to parse config.json: {e}")))?;

    let tokenizer_config: serde_json::Value = serde_json::from_slice(
        &std::fs::read(tokenizer_config_path)
            .map_err(|e| crate::KreuzbergError::reranking(format!("Failed to read tokenizer_config.json: {e}")))?,
    )
    .map_err(|e| crate::KreuzbergError::reranking(format!("Failed to parse tokenizer_config.json: {e}")))?;

    let mut tokenizer = tokenizers::Tokenizer::from_file(tokenizer_path)
        .map_err(|e| crate::KreuzbergError::reranking(format!("Failed to load tokenizer: {e}")))?;
    ...
    tokenizer
        ...
        .map_err(|e| crate::KreuzbergError::reranking(format!("Failed to configure tokenizer: {e}")))?;
    ...
    // Slow path: write lock + initialization
    {
        ...
        let (model_path, tokenizer_path, config_path, special_tokens_path, tokenizer_config_path) =
            ...
            .map_err(|panic_payload| {
                ...
                if looks_like_ort_error(&panic_msg) {
                    ...
                    crate::KreuzbergError::reranking(format!("Model download panicked: {panic_msg}"))
                }
            })??;
        ...
        .map_err(|panic_payload| {
            ...
            if looks_like_ort_error(&panic_msg) {
                ...
                crate::KreuzbergError::reranking(format!("ONNX Runtime initialization panicked: {panic_msg}"))
            }
        })?
        .map_err(|e| {
            ...
            if looks_like_ort_error(&error_msg) {
                ...
                crate::KreuzbergError::reranking(format!("Failed to create ONNX session: {e}"))
            }
        })?;
    ...
    match model_type {
        crate::core::config::RerankerModelType::Preset { name } => {
            let preset = get_preset(name)
                .ok_or_else(|| crate::KreuzbergError::reranking(format!("Unknown reranker preset: {name}")))?;
            Ok((
                preset.model_repo,
                preset.model_file,
        ...
        crate::core::config::RerankerModelType::Llm { .. } => Err(crate::KreuzbergError::reranking(
            "LLM rerankers have no local model to warm or download — the provider serves them over HTTP.",
        )),
        crate::core::config::RerankerModelType::Plugin { .. } => Err(crate::KreuzbergError::reranking(
...
/// # Errors
///
/// - `KreuzbergError::Validation` if `query` is blank or empty after trimming.
/// - `KreuzbergError::MissingDependency` if ONNX Runtime is not installed (ONNX path).
/// - `KreuzbergError::Reranking` if the preset name is unknown or model download fails.
///
/// Since v5.0.0.
#[cfg(feature = "reranker")]
pub fn rerank(
    ...
    match &config.model {
        ...
        crate::core::config::RerankerModelType::Llm { llm } => {
            ...
            let result = if let Ok(handle) = tokio::runtime::Handle::try_current() {
                ...
                if matches!(handle.runtime_flavor(), tokio::runtime::RuntimeFlavor::CurrentThread) {
                    return Err(crate::KreuzbergError::reranking(
        ...
        crate::core::config::RerankerModelType::Llm { .. } => Err(crate::KreuzbergError::MissingDependency(
        ...
        | crate::core::config::RerankerModelType::Custom { .. } => {
            ...
            let logits = engine
                ...
                .map_err(|e| crate::KreuzbergError::reranking(format!("ONNX inference failed: {e}")))?;
...
pub async fn rerank_async(
    ...
    let _permit = RERANK_SEMAPHORE
        .acquire()
        .await
        .map_err(|_| crate::KreuzbergError::reranking("Reranker semaphore closed".to_string()))?;

    let config = std::sync::Arc::new(config.clone());
    tokio::task::spawn_blocking(move || rerank(query, documents, &config))
        .await
        .map_err(|e| crate::KreuzbergError::reranking(format!("Reranker task panicked: {e}")))?
}
