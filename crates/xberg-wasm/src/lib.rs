...
impl WasmExtractionOutput {
    ...
    #[wasm_bindgen(getter)]
    pub fn crawl(&self) -> JsValue {
        self.crawl.clone()
    }

    #[wasm_bindgen(setter)]
    pub fn set_crawl(&mut self, value: JsValue) {
        self.crawl = value;
    }
...
mod __alef_wasm_bridge_documentextractor {
    ...
    impl xberg::DocumentExtractor for WasmDocumentExtractorBridge {
        async fn extract_bytes(
            ...
            let func_val = js_sys::Reflect::get(&self.inner, &key)
                .map_err(|_| xberg::XbergError::Other(format!("Failed to get method '{}'", "extract_bytes")))?;

            let func: js_sys::Function = func_val
                .dyn_into()
                .map_err(|_| xberg::XbergError::Other(format!("Method '{}' is not a function", "extract_bytes")))?;
            ...
            let promise_val = func
                .apply(&self.inner, &args)
                .map_err(|_| xberg::XbergError::Other(format!("Failed to call method '{}'", "extract_bytes")))?;
            let promise: js_sys::Promise = promise_val.dyn_into().map_err(|_| {
                xberg::XbergError::Other(format!("Method '{}' did not return a Promise", "extract_bytes"))
            })?;
            let result = wasm_bindgen_futures::JsFuture::from(promise)
                .await
                .map_err(|e| xberg::XbergError::Other(format!("Promise rejected: {:?}", e)))?;
            ...
            result
                ...
                .ok_or_else(|| xberg::XbergError::Other("Failed to convert result".to_string()))
                .and_then(|s| {
                    serde_json::from_str::<xberg::InternalDocument>(&s)
                        .map_err(|e| xberg::XbergError::Other(format!("Failed to deserialize result: {}", e)))
                })
        }
...
impl From<WasmExtractionOutput> for xberg::ExtractionOutput {
    fn from(val: WasmExtractionOutput) -> Self {
        Self {
            ...
            crawl: serde_wasm_bindgen::from_value(val.crawl.clone()).unwrap_or_default(),
        }
    }
}
...
impl From<xberg::ExtractionOutput> for WasmExtractionOutput {
    fn from(val: xberg::ExtractionOutput) -> Self {
        Self {
            ...
            errors: val.errors.into_iter().map(Into::into).collect(),
            summary: val.summary.into(),
            crawl: js_sys::JSON::parse(&serde_json::to_string(&val.crawl).unwrap_or_default()).unwrap_or(JsValue::NULL),
        }
    }
}
