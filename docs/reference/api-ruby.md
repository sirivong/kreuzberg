...
## Ruby API Reference <span class="version-badge">v5.0.0-rc.36</span>
...
```ruby
result = extract_bytes("data", "value", ExtractionConfig.new)
```
...
```ruby
result = extract_file("value", mime_type: "value", ExtractionConfig.new)
```
...
```ruby
result = extract_file_sync("value", mime_type: "value", ExtractionConfig.new)
```
...
```ruby
result = batch_extract_files_sync([], ExtractionConfig.new)
```
...
```ruby
result = batch_extract_files([], ExtractionConfig.new)
```
...
```ruby
result = batch_extract_bytes([], ExtractionConfig.new)
```
...
```ruby
result = instance.extract_bytes("data", "value", ExtractionConfig.new)
```
...
```ruby
result = instance.extract_file("value", "value", ExtractionConfig.new)
```
...
| `qr_codes` | `Array<QrCode>?` | `\[\]` | QR codes decoded from this image, when QR detection is enabled. Populated by the QR post-processor (`crates/kreuzberg/src/extractors/qr.rs`) via the pure-Rust `rqrr` decoder. `nil` when QR detection is disabled; an empty `Some(\[\])` when detection ran but found nothing. |
...
| `model_cache_dir` | `String?` | `nil` | Override the directory used for Whisper model cache. When `nil`, uses the centralized resolver: `KREUZBERG_CACHE_DIR/whisper` or the platform default (`~/.cache/kreuzberg/whisper` on Linux, etc.). |
