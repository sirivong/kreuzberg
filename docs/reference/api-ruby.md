...
```ruby
def self.extract_sync(input, config)
```

**Example:**

```ruby
result = extract_sync(ExtractInput.new, ExtractionConfig.new)
```
...
```ruby
def self.extract_batch_sync(inputs, config)
```

**Example:**

```ruby
result = extract_batch_sync([], ExtractionConfig.new)
```
...
| `crawl` | `Hash{String=>Object}` | `{}` | Optional implementation-specific crawl metadata. |
