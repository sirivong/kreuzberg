...
## Kotlin (Android) API Reference <span class="version-badge">v5.0.0-rc.35</span>
...
| `modelCacheDir` | `Path?` | `null` | Override the directory used for Whisper model cache. When `null`, uses the centralized resolver: `KREUZBERG_CACHE_DIR/whisper` or the platform default (`~/.cache/kreuzberg/whisper` on Linux, etc.). |
