...
```kotlin
@Throws(Error::class)
fun extractSync(input: ExtractInput, config: ExtractionConfig): ExtractionOutput
```

**Example:**

```kotlin
val result = extractSync(ExtractInput(), ExtractionConfig())
```
...
```kotlin
@Throws(Error::class)
fun extractBatchSync(inputs: List<ExtractInput>, config: ExtractionConfig): ExtractionOutput
```

**Example:**

```kotlin
val result = extractBatchSync([], ExtractionConfig())
```
...
| `crawl` | `Map<String, Any>` | `{}` | Optional implementation-specific crawl metadata. |
