...
## WebAssembly API Reference <span class="version-badge">v5.0.0-rc.20</span>
...
```typescript
const result = await extractBytes(new Uint8Array([100, 97, 116, 97]), "value", new ExtractionConfig());
```
...
```typescript
const result = await instance.extractBytes(new Uint8Array([100, 97, 116, 97]), "value", new ExtractionConfig());
```
