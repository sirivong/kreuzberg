...
## R API Reference <span class="version-badge">v5.0.0-rc.35</span>
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `content` | `raw` | — | The content bytes to extract from |
| `mime_type` | `character` | — | MIME type of the content (e.g., "application/pdf", "text/html") |
| `config` | `FileExtractionConfig or NULL` | `NULL` | Per-item configuration overrides (None uses batch-level defaults) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `path` | `character` | — | Path to the file to extract from |
| `config` | `FileExtractionConfig or NULL` | `NULL` | Per-file configuration overrides (None uses batch-level defaults) |
...
| `entities` | `list or NULL` | `list()` | Named entities detected in `content` by the NER post-processor. `NULL` when no NER backend is configured. Populated by the gline-rs ONNX backend or the LLM-driven backend (see `crates/kreuzberg/src/text/ner/`). |
...
| `model` | `character or NULL` | `NULL` | Override the default model — only used by `NerBackendKind.Onnx`. `NULL` lets the backend pick its pinned default (`urchade/gliner_multi-v2.1` for gline-rs). |
...
| `cache_dir` | `character or NULL` | `NULL` | Custom cache directory for model files. Defaults to `~/.cache/kreuzberg/rerankers/` if not specified. |
...
| `model_cache_dir` | `character or NULL` | `NULL` | Override the directory used for Whisper model cache. When `NULL`, uses the centralized resolver: `KREUZBERG_CACHE_DIR/whisper` or the platform default (`~/.cache/kreuzberg/whisper` on Linux, etc.). |
