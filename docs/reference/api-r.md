...
## R API Reference <span class="version-badge">v1.0.0-rc.1</span>
...
```r
extract_batch_sync(inputs, config)
```

**Example:**

```r
result <- extract_batch_sync([], %{{}})
```
...
```r
result <- detect_mime_type_from_bytes(charToRaw("data"))
```
...
```r
detect_qr_codes(image_bytes, format_hint = NULL)
```

**Example:**

```r
result <- detect_qr_codes(charToRaw("data"), "value")
```
...
| `format_hint` | `character or NULL` | No | The  format hint |
...
- `Ok(list)` - Vector of post-processor names
...
```r
classify_document(pages, config)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `pages` | `list` | Yes | Slice of page texts to classify. Each page is classified independently |
...
|------|------|----------|-------------|
| `text` | `character` | Yes | The text |
| `categories` | `list` | Yes | The categories |
...
```r
summarize(text, language = NULL, max_tokens = NULL)
```
...
|------|------|----------|-------------|
| `text` | `character` | Yes | The text |
| `language` | `character or NULL` | No | The language |
...
```r
translate_result(result, config)
```
...
```r
find_footnote_anchors(markdown)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `markdown` | `character` | Yes | The markdown text to search |
...
```r
parse_footnote_definitions(markdown)
```
...
```r
find_inference_markers(markdown)
```
...
```r
verify_excerpt(excerpt, source_text)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `excerpt` | `character` | Yes | The text snippet to find |
| `source_text` | `character` | Yes | The full source text to search |
...
```r
chunk_for_rag(text, config)
```

**Example:**

```r
result <- chunk_for_rag("value", %{{}})
```
...
```r
extract_region_with_vlm(image_bytes, image_mime, region_kind, llm_config, custom_prompt = NULL)
```
...
```r
analyze_document(metadata, config, document_bytes = NULL)
```

**Example:**

```r
result <- analyze_document(%{{}}, %{{}}, charToRaw("data"))
```
...
| `document_bytes` | `raw or NULL` | No | The document bytes |
...
```r
analyze_with_user_chunks(user_ranges, total_pages, size_bytes, config)
```

**Example:**

```r
result <- analyze_with_user_chunks([], 42, 42, %{{}})
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `user_ranges` | `list` | Yes | The user ranges |
...
```r
score_confidence(signals, weights)
```
...
```r
check_format_limits(mime_type, sheet_count = NULL, workbook_cells = NULL, embedded_count = NULL, config)
```

**Example:**

```r
result <- check_format_limits("value", 42, 42, 42, %{{}})
```
...
| `sheet_count` | `integer or NULL` | No | The sheet count |
| `workbook_cells` | `integer or NULL` | No | The workbook cells |
| `embedded_count` | `integer or NULL` | No | The embedded count |
| `config` | `HeuristicsConfig` | Yes | The configuration options |
...
```r
boundaries_from_extraction_result(result, thresholds)
```

**Example:**

```r
result <- boundaries_from_extraction_result(%{{}}, %{{}})
```
...
```r
detect_boundaries(input, thresholds)
```
...
```r
choose_call_mode(input, t)
```

**Example:**

```r
result <- choose_call_mode(%{{}}, %{{}})
```
...
```r
calculate_chunk_plan(page_count, size_bytes, needs_ocr, config)
```

**Example:**

```r
result <- calculate_chunk_plan(42, 42, TRUE, %{{}})
```
...
| `needs_ocr` | `logical` | Yes | Whether OCR will be required |
...
```r
calculate_plan_from_overrides(user_chunks, total_pages, size_bytes, config)
```

**Example:**

```r
result <- calculate_plan_from_overrides([], 42, 42, %{{}})
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `user_chunks` | `list` | Yes | The user chunks |
| `total_pages` | `integer` | Yes | The total pages |
| `size_bytes` | `integer` | Yes | The size bytes |
| `config` | `HeuristicsConfig` | Yes | The configuration options |
...
```r
result <- fingerprint(charToRaw("data"))
```
...
```r
resolve(preset, custom_schema = NULL, context)
```
...
| `custom_schema` | `list or NULL` | No | The custom schema |
| `context` | `list` | Yes | The context |
...
```r
result <- extract_structured_json(charToRaw("data"), "value", "value", "value")
```
...
| `mime` | `character` | Yes | The mime |
| `preset_spec_json` | `character` | Yes | The preset spec json |
| `options_json` | `character` | Yes | The options json |
...
JSON-serialised `list` (a JSON array) on success.
...
```r
result <- split_and_extract_json(charToRaw("data"), "value", "value", "value")
```
...
```r
result <- render_pdf_page_to_png(charToRaw("data"), 42, 42, "value")
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `pdf_bytes` | `raw` | Yes | Raw PDF file bytes |
| `page_index` | `integer` | Yes | Zero-based page index |
| `dpi` | `integer or NULL` | No | Resolution in dots per inch (default: 150) |
| `password` | `character or NULL` | No | Optional password for encrypted PDFs |
...
```r
pdf_page_count(pdf_bytes, password = NULL)
```

**Example:**

```r
result <- pdf_page_count(charToRaw("data"), "value")
```
...
```r
caption_image(image_bytes, llm_config, custom_prompt = NULL)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `image_bytes` | `raw` | Yes | The image data. |
| `llm_config` | `LlmConfig` | Yes | LLM configuration for the VLM call. |
| `custom_prompt` | `character or NULL` | No | Optional custom caption prompt. Uses the default |
...
```r
caption_image_file(path, llm_config, custom_prompt = NULL)
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `path` | `character` | Yes | Path to the image file. |
| `llm_config` | `LlmConfig` | Yes | LLM configuration for the VLM call. |
| `custom_prompt` | `character or NULL` | No | Optional custom caption prompt. Uses the default |
...
| `check_exists` | `logical` | Yes | The check exists |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `duration_ms` | `integer or NULL` | `NULL` | Duration in milliseconds derived from the decoded audio stream. |
| `codec` | `character or NULL` | `NULL` | Audio codec (e.g. "mp3", "aac", "opus", "flac"). |
| `container` | `character or NULL` | `NULL` | Container format (e.g. "mpeg", "mp4", "ogg", "wav"). |
| `sample_rate_hz` | `integer or NULL` | `NULL` | Sample rate in Hz after decode (always 16000 when resampled for Whisper). |
| `channels` | `integer or NULL` | `NULL` | Number of audio channels (1 = mono, 2 = stereo). |
| `bitrate` | `integer or NULL` | `NULL` | Audio bitrate in kbps from the source file tags/properties. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `x1` | `numeric` | — | Left edge (x-coordinate of the top-left corner). |
| `y1` | `numeric` | — | Top edge (y-coordinate of the top-left corner). |
| `x2` | `numeric` | — | Right edge (x-coordinate of the bottom-right corner). |
| `y2` | `numeric` | — | Bottom edge (y-coordinate of the bottom-right corner). |
...
| `citation_keys` | `list` | `list()` | BibTeX citation keys (e.g. `"knuth1984"`) for all entries. |
| `authors` | `list` | `list()` | Author names collected across all bibliography entries. |
| `year_range` | `YearRange or NULL` | `NULL` | Earliest and latest publication years found in the bibliography. |
| `entry_types` | `list or NULL` | `list()` | Count of entries grouped by BibTeX entry type (e.g. `"article"` → 5). |
...
| `total_size_mb` | `numeric` | — | Combined size of all cache files in megabytes. |
| `available_space_mb` | `numeric` | — | Free disk space available on the cache volume, in megabytes. |
| `oldest_file_age_days` | `numeric` | — | Age of the oldest cache file in days (0.0 if the cache is empty). |
| `newest_file_age_days` | `numeric` | — | Age of the most recently written cache file in days (0.0 if the cache is empty). |
...
| `custom_prompt` | `character or NULL` | `NULL` | Optional custom prompt override forwarded to every `caption_image` call. `NULL` uses the default `RegionKind.Caption` prompt. |
...
| `from` | `character` | — | Value before the change. |
| `to` | `character` | — | Value after the change. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `label` | `character` | — | The label of the citation (e.g., "src1" in `\[^src1\]: ...`). |
| `source` | `character` | — | The source reference (path, URL, or identifier). |
| `locator` | `character or NULL` | `NULL` | Optional locator within the source (e.g., "page 3" or "section 2.1"). |
| `excerpt` | `character or NULL` | `NULL` | Optional excerpt — quoted text from the source. |
...
| `format` | `character or NULL` | `NULL` | Detected citation file format (e.g. `"ris"`, `"pubmed"`, `"endnote"`). |
| `authors` | `list` | `list()` | Author names collected across all citation records. |
| `year_range` | `YearRange or NULL` | `NULL` | Earliest and latest publication years found in the file. |
| `dois` | `list` | `list()` | DOI identifiers found in the citation records. |
| `keywords` | `list` | `list()` | Keywords collected from all citation records. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `label` | `character` | — | Label name as configured in `PageClassificationConfig.labels`. |
| `confidence` | `numeric or NULL` | `NULL` | Backend-reported confidence in `\[0.0, 1.0\]`. `NULL` when the backend (e.g. an LLM prompt without explicit confidence schema) did not report one. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `text_coverage` | `numeric` | — | Fraction of pages with usable text in `\[0, 1\]`. |
| `ocr_aggregate` | `numeric or NULL` | `NULL` | Mean OCR per-element recognition confidence; `NULL` when OCR did not run. |
...
```r
from_extraction_result(result, schema_compliance, text_coverage)
```

**Example:**

```r
result <- ConfidenceSignals.from_extraction_result(%{{}}, %{{}}, 0.5)
```
...
| `text_coverage` | `numeric` | Yes | The text coverage |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `text_coverage` | `numeric` | `0.3` | Weight assigned to `text_coverage`. Default 0.30. |
| `ocr_aggregate` | `numeric` | `0.3` | Weight assigned to `ocr_aggregate` when OCR ran. Default 0.30 — folds into `text_coverage` weight when OCR did not run. |
| `schema_compliance` | `numeric` | `0.4` | Weight assigned to `schema_compliance`. Default 0.40. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `character` | — | Contributor display name. |
| `role` | `character or NULL` | `NULL` | Contributor role (e.g. `"author"`, `"editor"`). |
...
| `delimiter` | `character or NULL` | `NULL` | Field delimiter character (e.g. `","` or `"\t"`). |
| `has_header` | `logical` | — | Whether the first row was treated as a header. |
| `column_types` | `list or NULL` | `list()` | Inferred data type for each column (e.g. `"string"`, `"integer"`, `"float"`). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `character` | — | Field (column) name. |
| `field_type` | `character` | — | dBASE field type character (e.g. `"C"` for character, `"N"` for numeric). |
...
| `fields` | `list` | `list()` | Descriptor for each field in the table schema. |
...
| `detections` | `list` | — | All layout detections on this page after postprocessing. |
...
| `lines` | `list` | — | Lines that make up this hunk. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `include_metadata` | `logical` | `true` | Include metadata changes in the diff. Default: `true`. |
| `include_embedded` | `logical` | `true` | Include embedded-children changes in the diff. Default: `true`. |
| `max_content_chars` | `integer or NULL` | `NULL` | Truncate content to this many characters before diffing. Useful for very large documents where only the first N characters matter. `NULL` means no truncation. |
...
| `confidence` | `numeric` | — | Confidence in this boundary, `\[0.0, 1.0\]`. |
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `content` | `raw` | Yes | Raw document bytes |
| `mime_type` | `character` | Yes | MIME type of the document (already validated) |
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `path` | `character` | Yes | Path to the document file |
| `mime_type` | `character` | Yes | MIME type of the document (already validated) |
| `config` | `ExtractionConfig` | Yes | Extraction configuration |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `mime_type` | `character` | — | MIME type of the document. |
| `size_bytes` | `integer` | — | File size in bytes. |
| `page_count` | `integer or NULL` | `NULL` | Page count (if known, e.g., from previous analysis). |
| `force_ocr` | `logical` | — | Whether OCR is forced regardless of text layer. |
| `user_chunk_config` | `UserChunkConfig or NULL` | `NULL` | User-provided chunk configuration overrides. |
| `chunking_enabled` | `logical` | — | Whether chunking is enabled for this job. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `revision_id` | `character` | — | Format-specific revision identifier. For DOCX this is the `w:id` attribute value on the change element (e.g. `"42"`). When the attribute is absent a synthetic fallback is generated (`"docx-ins-0"`, `"docx-del-3"`, …). |
| `author` | `character or NULL` | `NULL` | Display name of the author who made this change, when available. |
| `timestamp` | `character or NULL` | `NULL` | ISO-8601 timestamp of the change, when available. Stored as a plain string so this type remains FFI-friendly and unconditionally available without the `chrono` optional dep. DOCX populates this from the `w:date` attribute (e.g. `"2024-03-15T10:30:00Z"`). |
| `kind` | `RevisionKind` | — | Semantic kind of this revision. |
| `anchor` | `RevisionAnchor or NULL` | `NULL` | Best-effort document location for this revision. Resolution is format-dependent and may be `NULL` when the location cannot be determined (e.g. changes inside table cells before table-cell anchor support is added). |
...
| `node_types` | `list` | `list()` | Sorted, deduplicated list of node type names present in this document. Each value is the snake_case `node_type` tag of the corresponding `NodeContent` variant (e.g. `"paragraph"`, `"heading"`, `"table"`, …). Computed from `nodes` via `DocumentStructure.finalize_node_types`. Empty until that method is called (internal construction paths call it at the end of derivation). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `text` | `character` | — | Summary text (plain prose). |
| `strategy` | `SummaryStrategy` | — | Strategy that produced this summary. |
| `token_count` | `integer or NULL` | `NULL` | Approximate token count of the summary, when known. |
...
| `content` | `character` | — | Cleaned/processed text content. Aliased as `cleaned_text` for back-compat. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `path` | `character` | — | Archive-relative path identifying this entry. |
...
| `data` | `raw` | — | Raw file bytes from the embedded stream (already decompressed by lopdf). |
...
| `cache_dir` | `character or NULL` | `NULL` | Custom cache directory for model files Defaults to `~/.cache/xberg/embeddings/` if not specified. Allows full customization of model download location. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `character` | — | Short identifier for this preset (e.g. `"balanced"`, `"fast"`, `"quality"`). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `keywords` | `logical` | — | Run keyword extraction on the input text. When `true`, the enrichment backend identifies the most salient terms and returns them in `EnrichResult.keywords`. |
| `entities` | `logical` | — | Run named-entity recognition (NER) on the input text. When `true`, the enrichment backend identifies named entities (persons, organisations, locations, etc.) and returns them in `EnrichResult.entities`. |
| `labels` | `list` | `list()` | Custom labels to pass through to the result without modification. These are caller-supplied tags that the enrichment pipeline propagates verbatim into `EnrichResult.labels`. Useful for attaching project- or document-level metadata to every enrichment result. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `keywords` | `list` | `list()` | Salient terms extracted from the text. Populated when `EnrichOptions.keywords` was `true`. The ordering is backend-defined (typically by descending relevance score). |
| `entities` | `list` | `list()` | Named entities found in the text. Populated when `EnrichOptions.entities` was `true`. Uses the shared OSS entity schema (`Entity` / `EntityCategory`) so consumers can pattern-match on entity categories without JSON gymnastics. |
| `labels` | `list` | `list()` | Caller-supplied labels echoed from `EnrichOptions.labels`. |
...
| `text` | `character` | — | Raw mention text exactly as it appeared in the source. |
...
| `confidence` | `numeric or NULL` | `NULL` | Backend-reported confidence in `\[0.0, 1.0\]`. `NULL` when the backend does not expose confidence scores. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `coverage` | `character or NULL` | `NULL` | Dublin Core `coverage` field (geographic or temporal scope). |
| `dc_format` | `character or NULL` | `NULL` | Dublin Core `format` field (media type of the resource). |
| `relation` | `character or NULL` | `NULL` | Dublin Core `relation` field (related resource identifier). |
| `source` | `character or NULL` | `NULL` | Dublin Core `source` field (origin resource identifier). |
| `dc_type` | `character or NULL` | `NULL` | Dublin Core `type` field (nature or genre of the resource). |
| `cover_image` | `character or NULL` | `NULL` | Path or identifier of the cover image within the EPUB container. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `error_type` | `character` | — | Machine-readable error type identifier (e.g. "UnsupportedFormat"). |
| `message` | `character` | — | Human-readable error description. |
...
| `bytes` | `raw or NULL` | `NULL` | Raw bytes for `kind = "bytes"`. |
| `uri` | `character or NULL` | `NULL` | Local path, `file://` URI, or HTTP(S) URL for `kind = "uri"`. |
| `mime_type` | `character or NULL` | `NULL` | MIME type hint. |
| `filename` | `character or NULL` | `NULL` | Filename hint used for MIME detection and metadata. |
| `config` | `FileExtractionConfig or NULL` | `NULL` | Per-input extraction overrides. |
...
```r
bytes(bytes, mime_type, filename)
```

**Example:**

```r
result <- ExtractInput.bytes(charToRaw("data"), "value", "value")
```
...
| `bytes` | `raw` | Yes | The bytes |
| `mime_type` | `character` | Yes | The mime type |
| `filename` | `character or NULL` | No | The filename |
...
| `bounding_box` | `BoundingBox or NULL` | `NULL` | Bounding box of the image on the page (PDF coordinates: x0=left, y0=bottom, x1=right, y1=top). Only populated for PDF-extracted images when position data is available from the PDF extractor. |
...
| `kind_confidence` | `numeric or NULL` | `NULL` | Confidence score for `image_kind`, in the range 0.0 to 1.0. |
...
| `caption` | `character or NULL` | `NULL` | VLM-generated caption describing the image, when captioning is configured. Populated by the captioning post-processor (`crates/xberg/src/plugins/processor/builtin/captioning.rs`), which routes each image through `crate.llm.region_extractor.extract_region_with_vlm` in caption mode. `NULL` when captioning is disabled or the VLM declined to caption. |
| `qr_codes` | `list or NULL` | `list()` | QR codes decoded from this image, when QR detection is enabled. Populated by the QR post-processor (`crates/xberg/src/extractors/qr.rs`) via the pure-Rust `rqrr` decoder. `NULL` when QR detection is disabled; an empty `Some(\[\])` when detection ran but found nothing. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `text_coverage` | `numeric` | — | Fraction of pages with a usable text layer. |
| `ocr_aggregate` | `numeric or NULL` | `NULL` | Mean OCR per-element recognition confidence when OCR ran; `NULL` when it did not. |
| `schema_compliance` | `SchemaCompliance` | — | Whether the merged output validates against the preset schema. |
| `combined` | `numeric` | — | Weighted blend in `\[0, 1\]`.  The value compared against the fallback threshold. |
...
| `keywords` | `KeywordConfig or NULL` | `NULL` | Keyword extraction configuration (None = no keyword extraction) |
...
| `layout` | `LayoutDetectionConfig or NULL` | `NULL` | Layout detection configuration (None = layout detection disabled). When set, PDF pages and images are analyzed for document structure (headings, code, formulas, tables, figures, etc.) using RT-DETR models via ONNX Runtime. For PDFs, layout hints override paragraph classification in the markdown pipeline. For images, per-region OCR is performed with markdown formatting based on detected layout classes. Requires the `layout-detection` feature to run inference; the field is present whenever the `layout-types` feature is active (which includes `layout-detection` as well as the no-ORT target groups). |
| `transcription` | `TranscriptionConfig or NULL` | `NULL` | Transcription (speech-to-text) configuration for audio/video files. When set and `enabled`, files with audio/video MIME types (mp3, mp4, m4a, wav, webm, etc.) are routed to the Whisper-based transcription pipeline. The actual heavy dependencies are only active under the `transcription` feature; the field is visible under `transcription-types` (including on WASM and Android targets that use the no-ORT preset). Default: `NULL` (transcription disabled). This is an additive, non-breaking change. |
| `use_layout_for_markdown` | `logical` | `false` | Run layout detection on the non-OCR PDF markdown path. When `true` and `layout` is `Some(_)`, layout regions inform heading, table, list, and figure detection in the structure pipeline that would otherwise rely on font-clustering heuristics alone. Significantly improves SF1 (structural F1) at the cost of inference latency (~150-300ms/page CPU, ~20-50ms/page GPU). Default: `false`. Requires the `layout-detection` feature. |
...
| `email` | `EmailConfig or NULL` | `NULL` | Email extraction configuration (None = use defaults). Currently supports configuring the fallback codepage for MSG files that do not specify one. See `EmailConfig` for details. |
...
| `ner` | `NerConfig or NULL` | `NULL` | Named-entity recognition configuration. When set, the NER post-processor runs at the Middle stage and populates `ExtractionResult.entities`. |
| `redaction` | `RedactionConfig or NULL` | `NULL` | Redaction / anonymisation configuration. When set, the redaction post-processor runs at the Late stage and rewrites every textual field in `ExtractionResult`, emitting an audit trail in `ExtractionResult.redaction_report`. |
| `summarization` | `SummarizationConfig or NULL` | `NULL` | Summarisation configuration. When set, the summarisation post-processor runs at the Middle stage and populates `ExtractionResult.summary`. |
| `translation` | `TranslationConfig or NULL` | `NULL` | Translation configuration. When set, the translation post-processor runs at the Middle stage and populates `ExtractionResult.translation`. |
| `page_classification` | `PageClassificationConfig or NULL` | `NULL` | Per-page classification configuration. When set, the classification post-processor runs at the Middle stage and populates `ExtractionResult.page_classifications`. |
| `captioning` | `CaptioningConfig or NULL` | `NULL` | VLM captioning configuration for extracted images. When set, the captioning post-processor runs at the Middle stage and writes a caption into each `ExtractedImage.caption`. |
| `qr_codes` | `logical or NULL` | `NULL` | Enable QR-code detection in extracted images. When `true`, the QR post-processor runs at the Middle stage and populates `ExtractedImage.qr_codes`. |
...
| `error_type` | `character` | — | Stable snake_case error kind. |
| `source` | `character` | — | Best-effort source identifier. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `results` | `list` | `list()` | Extraction results in discovery order. |
| `errors` | `list` | `list()` | Non-fatal per-input errors. |
| `summary` | `ExtractionSummary` | — | Aggregate counts for the operation. |
| `crawl` | `list` | `list()` | Optional implementation-specific crawl metadata. |
...
```r
result <- ExtractionOutput.single(%{{}})
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `content` | `character` | — | Plain-text representation of the extracted document content. |
| `mime_type` | `character` | — | MIME type of the source document (e.g. `"application/pdf"`). |
| `metadata` | `Metadata` | — | Document-level metadata (author, title, dates, format-specific fields). |
| `extraction_method` | `ExtractionMethod or NULL` | `NULL` | Extraction strategy used to produce the returned text. Populated when the extractor can reliably distinguish native text extraction, OCR-only extraction, or mixed native/OCR output. |
| `tables` | `list` | `list()` | Tables extracted from the document, each with structured cell data. |
| `detected_languages` | `list or NULL` | `list()` | ISO 639-1 language codes detected in the document content. |
...
| `extracted_keywords` | `list or NULL` | `list()` | Extracted keywords when keyword extraction is enabled. When keyword extraction (RAKE or YAKE) is configured, this field contains the extracted keywords with scores, algorithm info, and position data. Previously stored in `metadata.additional\["keywords"\]`. |
...
| `code_intelligence` | `list or NULL` | `NULL` | Code intelligence results from tree-sitter analysis. Populated when extracting source code files with the `tree-sitter` feature. Contains metrics, structural analysis, imports/exports, comments, docstrings, symbols, diagnostics, and optionally chunked code segments. Stored as an opaque JSON value so that all language bindings (Go, Java, C#, …) can deserialize it as a raw JSON object rather than a typed struct. The underlying type is `tree_sitter_language_pack.ProcessResult`. |
| `llm_usage` | `list or NULL` | `list()` | LLM token usage and cost data for all LLM calls made during this extraction. Contains one entry per LLM call. Multiple entries are produced when VLM OCR, structured extraction, or LLM embeddings run during the same extraction. `NULL` when no LLM was used. |
| `entities` | `list or NULL` | `list()` | Named entities detected in `content` by the NER post-processor. `NULL` when no NER backend is configured. Populated by the `xberg-gliner` ONNX backend or the LLM-driven backend (see `crates/xberg/src/text/ner/`). |
| `summary` | `DocumentSummary or NULL` | `NULL` | Summary of `content` produced by the summarisation post-processor. `NULL` when summarisation is not configured. Populated by the TextRank extractive backend (deterministic, no external service) or by the liter-llm-driven abstractive backend. |
| `extraction_confidence` | `ExtractionConfidence or NULL` | `NULL` | Confidence score computed by the heuristics pipeline. Populated when the `heuristics` feature is enabled and confidence scoring has been performed.  Combines text-coverage, OCR aggregate confidence, and schema-compliance into a single `\[0, 1\]` value. `NULL` when confidence scoring is not configured or the feature is absent. |
| `translation` | `Translation or NULL` | `NULL` | Translation of `content` produced by the translation post-processor. `NULL` when translation is not configured. |
| `page_classifications` | `list or NULL` | `list()` | Per-page classifications produced by the page-classification post-processor. `NULL` when classification is not configured. |
| `redaction_report` | `RedactionReport or NULL` | `NULL` | Audit report of redactions applied by the redaction post-processor. The redaction processor rewrites `content`, `formatted_content`, every chunk's text, and the textual fields of `entities` / `summary` / `translation` / `page_classifications` in place. This report describes what was found and how it was replaced. `NULL` when redaction is not configured. |
| `formulas` | `list` | `list()` | Mathematical formulas recognized in the document. Populated by the layout-guided formula pipeline when the `layout-detection` feature is enabled and the document contains regions classified as formulas. Empty otherwise. |
| `form_fields` | `list` | `list()` | Form fields extracted from a PDF's AcroForm or XFA structure. Populated by the PDF extractor when `PdfConfig.extract_form_fields` is enabled (default) and the document is a fillable form. Empty otherwise. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `genres` | `list` | `list()` | Genre tags as declared in the FB2 `<genre>` elements. |
| `sequences` | `list` | `list()` | Book series (sequence) names, if any. |
| `annotation` | `character or NULL` | `NULL` | Short annotation / summary from the FB2 `<annotation>` element. |
...
| `keywords` | `KeywordConfig or NULL` | `NULL` | Override keyword extraction for this file. |
| `postprocessor` | `PostProcessorConfig or NULL` | `NULL` | Override post-processor for this file. |
| `result_format` | `ResultFormat or NULL` | `NULL` | Override result format for this file. |
| `output_format` | `OutputFormat or NULL` | `NULL` | Override output content format for this file. |
...
| `transcription` | `TranscriptionConfig or NULL` | `NULL` | Transcription configuration (see ExtractionConfig for docs). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `label` | `character` | — | The label of the footnote reference (e.g., "1" in `\[^1\]`). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `parse_citations` | `logical` | `true` | Whether to parse the structured citation block (default: true). When enabled, the parser will look for and extract citations from the block after `---` + `<!-- citations ... -->`. |
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `enabled` | `logical` | Yes | The enabled |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `label` | `character` | — | The label of the footnote (e.g., "1" in `\[^1\]: ...`). |
| `content` | `character` | — | The full content of the footnote definition. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `latex` | `character` | — | LaTeX source of the recognized formula, without surrounding `$$` delimiters. This field contains the raw LaTeX code as produced by the OCR backend. To render the formula in Markdown or other formats, wrap with `$$..$$` delimiters as needed. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enable_pdf_text_heuristics` | `logical` | `true` | Enable PDF text-layer detection heuristics. When `true`, PDFs with a substantial text layer will skip chunking. Default: `true`. |
| `text_layer_threshold` | `numeric` | `0.7` | Minimum fraction of pages that must have text to skip chunking. Range `0.0..=1.0`. Default: `0.7` (70 % of pages). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `extract_images` | `logical` | `true` | Extract images from documents |
...
| `inject_placeholders` | `logical` | `true` | Whether to inject image reference placeholders into markdown output. When `true` (default), image references like `!\[Image 1\](embedded:p1_i0)` are appended to the markdown. Set to `false` to extract images as data without polluting the markdown output. |
...
| `exif` | `list` | `list()` | EXIF metadata tags |
...
| `auto_rotate` | `logical` | `false` | Auto-detect and correct image rotation. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `copyright` | `character or NULL` | `NULL` | Copyright statement from the article's `<permissions>` element. |
| `license` | `character or NULL` | `NULL` | Open-access license URI from the article's `<license>` element. |
| `history_dates` | `list` | `list()` | Publication history dates keyed by event type (e.g. `"received"`, `"accepted"`). |
| `contributor_roles` | `list` | `list()` | Authors and contributors with their stated roles. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled` | `logical` | `true` | Enable language detection |
| `min_confidence` | `numeric` | `0.8` | Minimum confidence threshold (0.0-1.0) |
| `detect_multiple` | `logical` | `false` | Detect multiple languages in the document |
...
| `confidence` | `numeric` | — | Detection confidence score in `\[0.0, 1.0\]`. |
...
| `text` | `character` | Yes | The text |
| `categories` | `list` | Yes | The categories |
| `custom_labels` | `list` | Yes | The custom labels |
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `meta_schema_json` | `character` | Yes | The meta schema json |
...
```r
result <- instance.parse_preset("value", charToRaw("data"))
```
...
| `pages` | `list` | — | Per-page signals extracted from the PDF. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `density_shift_threshold` | `numeric` | `0.3` | Text density difference threshold for `DensityShift` detection. Default: 0.3. |
| `bigram_overlap_min` | `numeric` | `0.1` | Minimum bigram-overlap ratio below which a density shift is promoted to a `DensityShift` boundary.  Default: 0.1 (10 % overlap). |
...
| `categories` | `list` | `list()` | Entity categories to detect. Defaults to a sensible PERSON/ORG/LOCATION/EMAIL set when empty. |
| `model` | `character or NULL` | `NULL` | Override the default model — only used by `NerBackendKind.Onnx`. `NULL` lets the backend pick its pinned default xberg GLiNER model alias. |
| `llm` | `LlmConfig or NULL` | `NULL` | Optional LLM configuration — only used by `NerBackendKind.Llm`. Token usage for LLM backends is recorded in `ExtractionResult.llm_usage`. |
| `custom_labels` | `list` | `list()` | Arbitrary user-supplied entity labels for zero-shot detection. `xberg-gliner` natively supports zero-shot inference over caller-supplied labels. The LLM backend also honours these labels by including them in the structured-output schema. Custom labels surface as `EntityCategory.Custom` in the resulting `Entity` stream. Use this when you need domain-specific entity types (e.g. `"Treatment"`, `"Product"`, `"Vessel"`) without forking GLiNER's taxonomy. |
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `image_bytes` | `raw` | Yes | Raw image data (JPEG, PNG, TIFF, etc.) |
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `lang` | `character` | Yes | ISO 639-2/3 language code (e.g., "eng", "deu", "fra") |
...
| `language` | `list` | `list()` | Language code(s) for OCR recognition. Accepts either a single language code ("eng") or a list (\["eng", "deu"\]). Defaults to \["eng"\]. For Tesseract, languages are joined with "+". |
...
| `vlm_fallback` | `VlmFallbackPolicy` | `"disabled"` | Ergonomic VLM fallback policy. When set to anything other than `VlmFallbackPolicy.Disabled` and `OcrConfig.pipeline` is `NULL`, a multi-stage pipeline is synthesised automatically: - `VlmFallbackPolicy.OnLowQuality` → `\[classical_stage, vlm_stage\]` with the `quality_threshold` mapped onto `OcrQualityThresholds.pipeline_min_quality`. - `VlmFallbackPolicy.Always` → `\[vlm_stage\]` only. Requires `OcrConfig.vlm_config` to be `Some` when not `Disabled`. When `OcrConfig.pipeline` is explicitly set, this field is ignored. |
| `vlm_config` | `LlmConfig or NULL` | `NULL` | VLM (Vision Language Model) OCR configuration. Required when `backend` is `"vlm"` or when `vlm_fallback` is not `VlmFallbackPolicy.Disabled`. Uses liter-llm to send page images to a vision model for text extraction. |
...
| `table_rows` | `integer or NULL` | `NULL` | Number of rows in the detected table (if a single table was found). |
| `table_cols` | `integer or NULL` | `NULL` | Number of columns in the detected table (if a single table was found). |
...
| `language` | `list or NULL` | `/* serde(default) */` | Language override for this stage (None = use parent OcrConfig.language). Accepts either a single language code ("eng") or a list (\["eng", "deu"\]). |
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `path` | `character` | Yes | Path to cache directory |
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `enable` | `logical` | Yes | Whether to enable table detection |
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `enable` | `logical` | Yes | Whether to enable angle classification |
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `threshold` | `numeric` | Yes | Detection threshold (0.0-1.0) |
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `threshold` | `numeric` | Yes | Box threshold (0.0-1.0) |
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ratio` | `numeric` | Yes | Unclip ratio (typically 1.5-2.0) |
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `score` | `numeric` | Yes | Minimum confidence (0.0-1.0), text below this is dropped |
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `tier` | `character` | Yes | `"mobile"` (default, lightweight, faster) or `"server"` (high accuracy, GPU/complex documents) |
...
| `labels` | `list` | — | Labels assigned to the page. Single-label classification yields exactly one entry; multi-label classification yields any subset of the configured label set. |
...
- `list` enables zero-copy sharing of table data
- `list` enables zero-copy sharing of image data
...
| `speaker_notes` | `character or NULL` | `NULL` | Speaker notes for this slide (PPTX only). Contains the text from the slide's notes pane (`ppt/notesSlides/notesSlide{N}.xml`). Only populated when the source is a PPTX file and notes are present. |
| `section_name` | `character or NULL` | `NULL` | Section name this slide belongs to (PPTX only). PowerPoint sections group slides into logical chapters (`<p:sectionLst>` in `ppt/presentation.xml`). Only populated when the source is a PPTX file and the slide belongs to a named section. |
| `sheet_name` | `character or NULL` | `NULL` | Sheet name for this page (XLSX/ODS only). Each spreadsheet sheet maps to one `PageContent` entry. This field carries the sheet's display name as it appears in the workbook. `NULL` for all non-spreadsheet formats and for sheets with an empty name. |
...
| `text_excerpt` | `character` | — | First ~500 characters of extracted text. |
| `starts_with_letterhead_like` | `logical` | — | `true` if page starts with letterhead-like content (ALL CAPS line in first 5 lines or a logo-image bbox at top). |
| `has_page_number_one_marker` | `logical` | — | `true` if text contains "Page 1" or "1 of N" pattern. |
| `has_signature_block` | `logical` | — | `true` if text contains signature indicators ("Sincerely", "Signed") or a signature image bbox. |
| `layout_text_density` | `numeric` | — | Text density: characters per page area, normalised to `\[0.0, 1.0\]`. |
...
```r
from_page_text(page_number, text, layout_text_density)
```
...
| `text` | `character` | Yes | The text |
| `layout_text_density` | `numeric` | Yes | The layout text density |
...
| `text` | `character` | — | Matched substring (owned copy — pattern engine returns owned data so the caller can free the original text if needed before replacement). |
...
| `extract_tables` | `logical` | `true` | Extract tables from PDF. When `true` (default), runs pdf_oxide's native grid detector and, if it finds nothing, falls back to the heuristic text-layer reconstruction in `pdf.oxide.table.extract_tables_heuristic`. Set to `false` to skip both passes — `tables` will then be empty in the result. |
...
| `ocr_inline_images` | `logical` | `false` | Perform OCR on inline images extracted from PDF pages and attach the recognized text to each `ExtractedImage.ocr_result`. Requires Tesseract to be available; if `ExtractionConfig.ocr` is `NULL` the extractor falls back to `TesseractConfig.default()`. Per-image failures degrade gracefully (the image is returned without OCR text rather than failing the whole extraction). Default: `false`. |
| `extract_form_fields` | `logical` | `true` | Extract AcroForm and XFA form fields into `ExtractionResult.form_fields`. When `true` (default), reads the document's interactive form structure (field names, types, values, widget geometry). Cheap and strictly additive — non-form PDFs simply yield an empty list. Set to `false` to skip the form pass entirely. |
| `reading_order` | `logical` | `false` | Reorder extracted text by layout-detected reading order. When `true`, projects text spans onto layout-detected regions, performs column detection, and emits spans in natural reading order (important for multi-column academic PDFs). Requires the `layout-detection` feature; has no effect without it. Defaults to `false`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `character` | — | Partial field name (the leaf name within the field hierarchy). |
| `full_name` | `character` | — | Fully-qualified field name (dotted path from the form root). |
| `field_type` | `FormFieldType` | — | Classified field type. |
| `value` | `character or NULL` | `/* serde(default) */` | Current field value, if any. |
| `default_value` | `character or NULL` | `/* serde(default) */` | Default field value, if any. |
| `flags` | `integer` | `/* serde(default) */` | Raw field-flags bitmask (read-only, required, multiline, …). |
| `page` | `integer or NULL` | `/* serde(default) */` | 1-indexed page the field's widget appears on. Currently always `NULL` for AcroForm fields; page assignment is a deferred enhancement requiring spatial analysis of widget annotations per page. |
| `bbox` | `BoundingBox or NULL` | `/* serde(default) */` | Widget bounding box on its page, if known. |
| `max_length` | `integer or NULL` | `/* serde(default) */` | Maximum input length for text fields, if specified. |
| `tooltip` | `character or NULL` | `/* serde(default) */` | Tooltip / alternate field description, if present. |
...
| `enabled_set` | `list or NULL` | `NULL` | Pre-computed AHashSet for O(1) enabled processor lookup |
| `disabled_set` | `list or NULL` | `NULL` | Pre-computed AHashSet for O(1) disabled processor lookup |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `id` | `character` | — | Stable, URL-safe preset identifier (lowercase snake_case). |
| `version` | `character` | — | Monotonic version string (e.g. `v1`). |
| `schema_name` | `character` | — | Human-readable schema name forwarded to the LLM as the response/tool name. |
| `description` | `character` | — | One-line preset description shown in the registry UI. |
| `category` | `PresetCategory` | — | Top-level category for grouping in the playground. |
| `tags` | `list` | `/* serde(default) */` | Free-form tags used for search/filtering. May be empty. |
| `schema` | `list` | — | JSON Schema (Draft 2020-12) describing the structured output shape. |
| `system_prompt` | `character` | — | Instruction primer sent to the model. |
| `context_template` | `character or NULL` | `/* serde(default) */` | Optional mustache-style template merged with caller-supplied context. |
...
| `emit_citations` | `logical` | — | When true, the prompt asks the model to wrap each field as `{value, page, bbox, confidence}` for downstream citation overlays. |
| `sample` | `PresetSample or NULL` | `/* serde(default) */` | Optional bundled sample (input file + reference output) for preview. |
| `fingerprint` | `character` | `/* serde(default) */` | Stable sha256 fingerprint of the canonical preset file contents. Populated at registry load — not present in the on-disk JSON files. Used as a cache-invalidation token by the worker pipeline. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `input_path` | `character` | — | Path to the sample input file, relative to the preset directory. |
| `output_path` | `character` | — | Path to the reference structured output, relative to the preset directory. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `id` | `character` | — | Preset identifier matching `Preset.id`. |
| `version` | `character` | — | Preset version matching `Preset.version`. |
| `schema_name` | `character` | — | Schema name matching `Preset.schema_name`. |
| `description` | `character` | — | One-line preset description. |
| `category` | `PresetCategory` | — | Top-level category. |
| `tags` | `list` | — | Free-form tags. |
| `preferred_call_mode` | `CallMode` | — | Default call mode. |
| `emit_citations` | `logical` | — | Whether the preset prompts the model for citations. |
| `fingerprint` | `character` | — | Stable fingerprint matching `Preset.fingerprint`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `payload` | `character` | — | Decoded payload (text, URL, vCard string, …). |
| `confidence` | `numeric or NULL` | `NULL` | Detector-reported confidence in `\[0.0, 1.0\]`. `NULL` when the decoder does not expose confidence (the default `rqrr` backend always reports `Some` because successful decode implies high confidence). |
| `bbox` | `QrBoundingBox or NULL` | `NULL` | Bounding box of the QR code inside the source image, in pixel coordinates (`x`, `y` of the top-left corner; `width`, `height` of the rectangle). `NULL` if the decoder did not report a bounding box. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `categories` | `list` | `list()` | Categories to redact. Empty means "every category supported by the engine." |
| `strategy` | `RedactionStrategy` | `"mask"` | Strategy applied to every match. |
| `ner` | `NerConfig or NULL` | `NULL` | Optional NER backend — required to redact PERSON / ORGANIZATION / LOCATION categories (the pure-Rust pattern engine only covers regex-detectable PII). |
| `preserve_offsets` | `logical` | `true` | When `true`, chunk byte ranges are kept consistent with the rewritten content by adjusting `byte_start` / `byte_end` after replacement. When `false`, chunk byte ranges still refer to the *original* content offsets — useful when downstream consumers want to map findings back to the original document. |
| `custom_terms` | `list` | `list()` | Arbitrary user-supplied literal terms to redact. Each term is treated as a regex hit against the document, surfacing as `PiiCategory.Custom(label)` in `RedactionFinding` where `label` is the per-term label (defaulting to the literal value itself). Case-insensitive by default; set `RedactionTerm.case_sensitive` for exact match. Use this when you need to redact tenant-specific tokens (employee IDs, project codes, internal product names) without writing a custom plugin. |
| `custom_patterns` | `list` | `list()` | Arbitrary user-supplied regex patterns to redact. Same surfacing semantics as `custom_terms`: each hit becomes a `PiiCategory.Custom(label)` finding. Patterns are validated at config-construction time via `RedactionConfig.validate`. |
...
| `replacement_token` | `character` | — | String that replaced the original mention. Always present; for `Drop` the replacement is the empty string. |
...
| `case_sensitive` | `logical` | `serde(default = "default_case_sensitive")` | When `true`, match case-sensitively; otherwise prepend `(?i)` to the regex. |
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `label` | `character` | Yes | The label |
| `pattern` | `character` | Yes | The pattern |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `findings` | `list` | — | Individual redaction findings in original-source byte order. |
...
| `case_sensitive` | `logical` | `serde(default = "default_case_sensitive")` | When `true`, match the value as-is; otherwise match ASCII-case-insensitively. |
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `preset_id` | `character` | Yes | The preset id |
| `name` | `character` | Yes | The name |
...
| `score` | `numeric` | — | Relevance score in `\[0, 1\]`. Higher means more relevant to the query. |
| `document` | `character` | — | The document text. |
...
| `top_k` | `integer or NULL` | `NULL` | Return at most this many documents. `NULL` returns all. Applied after sorting by score, so the highest-scoring documents are kept. |
| `batch_size` | `integer` | `32` | Batch size for local ONNX cross-encoder inference. |
| `show_download_progress` | `logical` | `false` | Show model download progress (local ONNX path only). |
| `cache_dir` | `character or NULL` | `NULL` | Custom cache directory for model files. Defaults to `~/.cache/xberg/rerankers/` if not specified. |
| `acceleration` | `AccelerationConfig or NULL` | `NULL` | Hardware acceleration for the reranker ONNX model. Controls which execution provider (CPU, CUDA, CoreML, TensorRT) is used for local inference. Defaults to `NULL` (auto-select per platform). |
| `max_rerank_duration_secs` | `integer or NULL` | `NULL` | Maximum wall-clock duration (in seconds) for a single `rerank()` call when using `RerankerModelType.Plugin`. Applies only to the in-process plugin path — protects against hung host-language backends. On timeout, the dispatcher returns `Plugin` instead of blocking forever. `NULL` disables the timeout. The default (60 seconds) is conservative for common in-process inference; increase for large document sets on slow hardware. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `id` | `character` | — | Source preset identifier. |
| `version` | `character` | — | Source preset version. |
| `fingerprint` | `character` | — | Fingerprint of the source preset file, used as a cache token. |
| `schema_name` | `character` | — | Schema name forwarded to the LLM. |
| `schema` | `list` | — | Effective JSON Schema (caller override or the preset's own). |
| `system_prompt` | `character` | — | System prompt with rendered context appended. |
| `merge_mode` | `MergeMode` | — | Merge strategy for paginated outputs. |
| `preferred_call_mode` | `CallMode` | — | Preferred call mode. |
| `emit_citations` | `logical` | — | Whether the prompt asks for per-field citations. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `content` | `list` | `list()` | Line-level content changes for this revision. |
| `table_changes` | `list` | `list()` | Cell-level table changes for this revision. |
...
| `cors_origins` | `list` | `list()` | CORS allowed origins. Empty vector means allow all origins. If this is an empty listtor, the server will accept requests from any origin. If populated with specific origins (e.g., `"<https://example.com"`>), only those origins will be allowed. |
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `origin` | `character` | Yes | The origin to check (e.g., "<https://example.com">) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `content` | `character` | — | The extracted text content, formatted for readability. |
| `format` | `character` | — | The source format identifier (e.g. `"json"`, `"yaml"`, `"toml"`). |
| `metadata` | `list` | — | Key-value metadata extracted from recognized text fields. |
| `text_fields` | `list` | — | JSON paths of fields that were classified as text-bearing. |
...
| `schema_name` | `character` | `serde(default = "default_schema_name")` | Schema name passed to the LLM's structured output mode. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `mime_type` | `character` | — | MIME type, canonicalised to lowercase by the caller. |
| `page_count` | `integer` | — | Number of pages in the document. |
| `text_coverage` | `numeric` | — | Fraction of pages with a real text layer (0.0..=1.0). |
| `avg_chars_per_page` | `numeric` | — | Average extracted characters per page. |
| `embedded_image_count` | `integer` | — | Count of embedded images (figures, photos, signatures) discovered. |
| `user_force_vision` | `logical` | — | When `true`, promote the result to at least `StructuredCallMode.TextPlusVision`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `scan_max_coverage` | `numeric` | `0.1` | PDFs with `text_coverage` strictly below this are treated as scanned. **Conservative default: 0.10** — deployments override via their own config after measuring their document corpus. |
| `digital_min_coverage` | `numeric` | `0.9` | PDFs with `text_coverage` at or above this AND zero embedded images route to `StructuredCallMode.TextOnly`. **Conservative default: 0.90** — deployments override via their own config after measuring their document corpus. |
| `docx_text_min_density` | `numeric` | `200` | DOCX / HTML / text documents with `avg_chars_per_page` above this route to `StructuredCallMode.TextOnly`. **Conservative default: 200.0** — deployments override via their own config after measuring their document corpus. |
| `enable_vision_fallback` | `logical` | `false` | When `true`, emit `StructuredCallMode.TextOnlyWithVisionFallback` instead of `StructuredCallMode.TextOnly` so the orchestrator can escalate to vision on low confidence. **Conservative default: `false`** — must be explicitly enabled per deployment after bench validation; deployments override via their own config. |
...
| `max_tokens` | `integer or NULL` | `NULL` | Maximum summary length in tokens. `NULL` lets the backend pick a default. |
| `llm` | `LlmConfig or NULL` | `NULL` | LLM configuration for the abstractive backend. Ignored when `strategy = Extractive`. Required when `strategy = Abstractive`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `sanitize` | `logical` | `true` | Run SVG bytes through `usvg` sanitization (strips external `href` attributes, JavaScript event handlers, and `foreignObject` elements) even when the output format is `Native`.  Defaults to `true`. |
| `render_dpi` | `numeric` | `96` | Target DPI when rasterizing SVG to a pixel-based format (PNG, JPEG, WebP, HEIF).  The tree's viewBox is scaled by `render_dpi / 96.0` before the pixel buffer is allocated.  Defaults to `96.0` (1× CSS pixel density). |
...
| `cell_changes` | `list` | — | Cell-level changes within the table. |
...
| `cells` | `list` | `list()` | All cells in row-major order. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `language` | `list` | `list()` | Language code(s) for OCR recognition. Accepts either a single language code ("eng") or a list (\["eng", "deu"\]). For Tesseract backend, languages are joined with "+". |
...
| `language_hint` | `character or NULL` | `NULL` | ISO 639-1 language code hint for stopword selection (e.g. `"en"`, `"de"`). |
| `preserve_markdown` | `logical` | `false` | Preserve Markdown formatting tokens during reduction. |
| `preserve_code` | `logical` | `true` | Preserve code block contents unchanged. |
| `semantic_threshold` | `numeric` | `0.3` | Cosine similarity threshold below which sentences are considered dissimilar. |
| `enable_parallel` | `logical` | `true` | Use Rayon parallel iterators for multi-core processing. |
| `use_simd` | `logical` | `true` | Use SIMD-optimized text scanning where available. |
| `custom_stopwords` | `list or NULL` | `NULL` | Per-language custom stopword lists (`language_code → stopword_list`). |
| `preserve_patterns` | `list` | `list()` | Regex patterns whose matched text is always preserved unchanged. |
| `target_reduction` | `numeric or NULL` | `NULL` | Target fraction of text to retain (0.0–1.0); `NULL` = no fixed target. |
| `enable_semantic_clustering` | `logical` | `false` | Group semantically similar sentences and emit only one per cluster. |
...
| `preserve_important_words` | `logical` | `true` | Preserve important words (capitalized, technical terms) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled` | `logical` | `true` | Master switch. When false the block is ignored and audio files fall back to the normal "unsupported format" path. |
...
| `language` | `character or NULL` | `NULL` | Optional language hint (ISO-639-1 code, e.g. "en", "de"). When `NULL` (default), the current engine falls back to English. For deterministic production output, always set this explicitly. |
| `timestamps` | `logical` | `false` | Whether to request segment-level timestamps. Accepted for forward compatibility. The current engine always uses `<\|notimestamps\|>` and does not emit segment metadata yet. |
| `max_duration_ms` | `integer or NULL` | `NULL` | Hard safety limit on input duration (milliseconds). Files longer than this are rejected after decode, before model work. Default: 30 minutes. Set to `NULL` to disable (not recommended for untrusted input). |
| `max_bytes` | `integer or NULL` | `NULL` | Hard safety limit on input size (bytes). Default: 512 MiB. Protects against pathological or malicious uploads. |
| `timeout_ms` | `integer or NULL` | `NULL` | Wall-clock timeout for the entire transcription operation (ms). Default: 10 minutes. Reserved for timeout enforcement; the current extractor does not enforce this field yet. |
| `model_cache_dir` | `character or NULL` | `NULL` | Override the directory used for Whisper model cache. When `NULL`, uses the centralized resolver: `XBERG_CACHE_DIR/whisper` or the platform default (`~/.cache/xberg/whisper` on Linux, etc.). |
| `allow_network` | `logical` | `true` | Allow network access to download models from Hugging Face Hub. When `false`, only previously cached models may be used. Useful for air-gapped or fully offline deployments. |
| `verify_hash` | `logical` | `true` | Request SHA256 verification of downloaded model files. Reserved for the checksum table follow-up. The current resolver logs a warning and treats this as a no-op. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `target_lang` | `character` | — | BCP-47 language tag the translation was produced into (e.g. `"de"`, `"fr-CA"`). |
| `source_lang` | `character or NULL` | `NULL` | BCP-47 source language. `NULL` when the translation backend was asked to detect. |
| `content` | `character` | — | Translated plain-text body. Matches the shape of `ExtractionResult.content`. |
| `formatted_content` | `character or NULL` | `NULL` | Translated markup body (Markdown / HTML / etc.) when `preserve_markup` was enabled on the config. `NULL` otherwise. |
...
| `document_url_pattern` | `character or NULL` | `NULL` | Optional regex filter for document-discovered URLs. |
| `max_document_urls_per_result` | `integer or NULL` | `NULL` | Maximum URLs to follow per extraction result. |
| `max_total_urls` | `integer or NULL` | `NULL` | Maximum URLs followed across the whole extraction call. |
| `allow_local_file_inputs` | `logical` | `true` | Allow bare local filesystem path inputs. |
| `allow_file_uris` | `logical` | `true` | Allow local `file://` URI inputs. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `page_ranges` | `list or NULL` | `list()` | User-specified page ranges (overrides automatic chunking). |
| `pages_per_chunk` | `integer or NULL` | `NULL` | User-specified pages per chunk (overrides automatic calculation). |
| `force_chunking` | `logical` | — | Force chunking even for small documents. |
| `disable_chunking` | `logical` | — | Disable chunking even for large documents. |
...
| `custom` | Custom renderer registered via the RendererRegistry. The string is the renderer name (e.g., "docx", "latex"). — Fields: `0`: `character` |
...
| Value | Description |
|-------|-------------|
| `preset` | Use a preset cross-encoder model (recommended). — Fields: `name`: `character` |
| `custom` | Use a custom ONNX cross-encoder from HuggingFace. — Fields: `model_id`: `character`, `model_file`: `character`, `additional_files`: `list`, `max_length`: `integer` |
...
| `subscript` | Subscript text. |
| `superscript` | Superscript text. |
| `link` | Hyperlink annotation. — Fields: `url`: `character`, `title`: `character` |
...
| `url` | A URL or URI. |
| `custom` | A caller-supplied custom category label. — Fields: `0`: `character` |
...
| Value | Description |
|-------|-------------|
| `context` | Unchanged context line. — Fields: `0`: `character` |
| `added` | Line added in the "after" version. — Fields: `0`: `character` |
| `removed` | Line removed from the "before" version. — Fields: `0`: `character` |
...
| `sheet` | Spreadsheet cell or range, identified by sheet index and optional name. — Fields: `index`: `integer`, `name`: `character` |
...
| `failed` | Processing failed. — Fields: `error`: `character` |
...
| `use_overrides` | Use user-provided chunk overrides. — Fields: `user_chunks`: `list` |
...
| `text_layer_detected` | PDF has substantial text layer (OCR not needed). — Fields: `text_coverage`: `numeric`, `avg_chars_per_page`: `integer` |
| `format_not_chunkable` | Document format does not support chunking. — Fields: `mime_type`: `character` |
...
| `ocr_required` | PDF requires OCR and is large. — Fields: `page_count`: `integer`, `force_ocr`: `logical` |
