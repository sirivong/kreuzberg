...
## Java API Reference <span class="version-badge">v5.0.0-rc.36</span>
...
| `formatHint` | `Optional<String>` | No | The  format hint |
...
|------|------|----------|-------------|
| `text` | `String` | Yes | The text |
| `language` | `Optional<String>` | No | The language |
...
```java
public static List<Long> findInferenceMarkers(String markdown)
```
...
```java
public static ChunkingResult chunkForRag(String text, ChunkingConfig config) throws Error
```
...
```java
public static ChunkingDecision analyzeDocument(DocumentMetadata metadata, HeuristicsConfig config, byte[] documentBytes) throws Error
```

**Example:**

```java
var result = analyzeDocument(new DocumentMetadata(), new HeuristicsConfig(), "data".getBytes());
```
...
| `documentBytes` | `Optional<byte\[\]>` | No | The document bytes |
...
```java
public static ChunkingDecision analyzeWithUserChunks(List<PageRange> userRanges, int totalPages, long sizeBytes, HeuristicsConfig config)
```

**Example:**

```java
var result = analyzeWithUserChunks(List.of(), 42, 42, new HeuristicsConfig());
```
...
```java
public static Optional<String> checkFormatLimits(String mimeType, int sheetCount, long workbookCells, int embeddedCount, HeuristicsConfig config)
```
...
|------|------|----------|-------------|
| `mimeType` | `String` | Yes | The mime type |
| `sheetCount` | `Optional<Integer>` | No | The sheet count |
| `workbookCells` | `Optional<Long>` | No | The workbook cells |
| `embeddedCount` | `Optional<Integer>` | No | The embedded count |
| `config` | `HeuristicsConfig` | Yes | The configuration options |
...
```java
public static ChunkPlan calculateChunkPlan(int pageCount, long sizeBytes, boolean needsOcr, HeuristicsConfig config)
```
...
```java
public static ChunkPlan calculatePlanFromOverrides(List<PageRange> userChunks, int totalPages, long sizeBytes, HeuristicsConfig config)
```

**Example:**

```java
var result = calculatePlanFromOverrides(List.of(), 42, 42, new HeuristicsConfig());
```
...
```java
var result = fingerprint("data".getBytes());
```
...
```java
public static ResolvedPreset resolve(Preset preset, Object customSchema, Map<String, String> context) throws ResolveError
```

**Example:**

```java
var result = resolve(new Preset(), Map.of(), Map.of());
```
...
| `customSchema` | `Optional<Object>` | No | The custom schema |
| `context` | `Map<String, String>` | Yes | The context |

**Returns:** `ResolvedPreset`

**Errors:** Throws `ResolveErrorException`.
...
```java
public static String extractStructuredJson(byte[] bytes, String mime, String presetSpecJson, String optionsJson) throws Error
```

**Example:**

```java
var result = extractStructuredJson("data".getBytes(), "value", "value", "value");
```
...
```java
public static String splitAndExtractJson(byte[] bytes, String mime, String presetSpecJson, String optionsJson) throws Error
```

**Example:**

```java
var result = splitAndExtractJson("data".getBytes(), "value", "value", "value");
```
...
| `customPrompt` | `Optional<String>` | `null` | Optional custom prompt override forwarded to every `caption_image` call. `null` uses the default `RegionKind.Caption` prompt. |
...
| `chunks` | `List<ChunkInfo>` | `Collections.emptyList()` | Individual chunk information. |
...
```java
public static ChunkPlan defaultOptions()
```

**Example:**

```java
var result = ChunkPlan.defaultOptions();
```
...
| `ocrAggregate` | `Optional<Float>` | `null` | Mean OCR per-element recognition confidence; `null` when OCR did not run. |
...
```java
public static ConfidenceWeights defaultOptions()
```

**Example:**

```java
var result = ConfidenceWeights.defaultOptions();
```
...
| `changed` | `List<EmbeddedDiff>` | `Collections.emptyList()` | Children present in both but with differing content (matched by `path`). Each entry holds the diff of the nested `ExtractionResult`. |
...
| `labels` | `List<String>` | `Collections.emptyList()` | Custom labels to pass through to the result without modification. These are caller-supplied tags that the enrichment pipeline propagates verbatim into `EnrichResult.labels`. Useful for attaching project- or document-level metadata to every enrichment result. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `keywords` | `List<String>` | `Collections.emptyList()` | Salient terms extracted from the text. Populated when `EnrichOptions.keywords` was `true`. The ordering is backend-defined (typically by descending relevance score). |
| `entities` | `List<Entity>` | `Collections.emptyList()` | Named entities found in the text. Populated when `EnrichOptions.entities` was `true`. Uses the shared OSS entity schema (`Entity` / `EntityCategory`) so consumers can pattern-match on entity categories without JSON gymnastics. |
| `labels` | `List<String>` | `Collections.emptyList()` | Caller-supplied labels echoed from `EnrichOptions.labels`. |
...
| `caption` | `Optional<String>` | `null` | VLM-generated caption describing the image, when captioning is configured. Populated by the captioning post-processor (`crates/kreuzberg/src/plugins/processor/builtin/captioning.rs`), which routes each image through `crate.llm.region_extractor.extract_region_with_vlm` in caption mode. `null` when captioning is disabled or the VLM declined to caption. |
| `qrCodes` | `Optional<List<QrCode>>` | `Collections.emptyList()` | QR codes decoded from this image, when QR detection is enabled. Populated by the QR post-processor (`crates/kreuzberg/src/extractors/qr.rs`) via the pure-Rust `rqrr` decoder. `null` when QR detection is disabled; an empty `Some(\[\])` when detection ran but found nothing. |
...
| `ner` | `Optional<NerConfig>` | `null` | Named-entity recognition configuration. When set, the NER post-processor runs at the Middle stage and populates `ExtractionResult.entities`. |
| `redaction` | `Optional<RedactionConfig>` | `null` | Redaction / anonymisation configuration. When set, the redaction post-processor runs at the Late stage and rewrites every textual field in `ExtractionResult`, emitting an audit trail in `ExtractionResult.redaction_report`. |
| `summarization` | `Optional<SummarizationConfig>` | `null` | Summarisation configuration. When set, the summarisation post-processor runs at the Middle stage and populates `ExtractionResult.summary`. |
| `translation` | `Optional<TranslationConfig>` | `null` | Translation configuration. When set, the translation post-processor runs at the Middle stage and populates `ExtractionResult.translation`. |
| `pageClassification` | `Optional<PageClassificationConfig>` | `null` | Per-page classification configuration. When set, the classification post-processor runs at the Middle stage and populates `ExtractionResult.page_classifications`. |
...
| `model` | `Optional<String>` | `null` | Override the default model — only used by `NerBackendKind.Onnx`. `null` lets the backend pick its pinned default (`urchade/gliner_multi-v2.1` for gline-rs). |
| `llm` | `Optional<LlmConfig>` | `null` | Optional LLM configuration — only used by `NerBackendKind.Llm`. Token usage for LLM backends is recorded in `ExtractionResult.llm_usage`. |
| `customLabels` | `List<String>` | `Collections.emptyList()` | Arbitrary user-supplied entity labels for zero-shot detection. gline-rs natively supports zero-shot inference over caller-supplied labels — this is the primary value of GLiNER. The LLM backend also honours these labels by including them in the structured-output schema. Custom labels surface as `EntityCategory.Custom` in the resulting `Entity` stream. Use this when you need domain-specific entity types (e.g. `"Treatment"`, `"Product"`, `"Vessel"`) without forking GLiNER's taxonomy. |
...
| `cacheDir` | `Optional<String>` | `null` | Custom cache directory for model files. Defaults to `~/.cache/kreuzberg/rerankers/` if not specified. |
...
```java
public static StructuredThresholds defaultOptions()
```

**Example:**

```java
var result = StructuredThresholds.defaultOptions();
```
...
| `modelCacheDir` | `Optional<String>` | `null` | Override the directory used for Whisper model cache. When `null`, uses the centralized resolver: `KREUZBERG_CACHE_DIR/whisper` or the platform default (`~/.cache/kreuzberg/whisper` on Linux, etc.). |
...
| `USE_OVERRIDES` | Use user-provided chunk overrides. — Fields: `userChunks`: `List<PageRange>` |
