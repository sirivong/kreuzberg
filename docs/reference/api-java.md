...
## Java API Reference <span class="version-badge">v1.0.0-rc.1</span>
...
```java
public static ExtractionOutput extractSync(ExtractInput input, ExtractionConfig config) throws Error
```

**Example:**

```java
var result = extractSync(new ExtractInput(), new ExtractionConfig());
```
...
```java
public static ExtractionOutput extractBatchSync(List<ExtractInput> inputs, ExtractionConfig config) throws Error
```

**Example:**

```java
var result = extractBatchSync(List.of(), new ExtractionConfig());
```
...
```java
public static ExtractionOutput extract(ExtractInput input, ExtractionConfig config) throws Error
```
...
```java
public static ExtractionOutput extractBatch(List<ExtractInput> inputs, ExtractionConfig config) throws Error
```
...
```java
var result = ExtractInput.bytes("data".getBytes(), "value", "value");
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `results` | `List<ExtractionResult>` | `Collections.emptyList()` | Extraction results in discovery order. |
...
| `crawl` | `Map<String, Object>` | `Collections.emptyMap()` | Optional implementation-specific crawl metadata. |
...
| `maxDocumentUrlsPerResult` | `Optional<Integer>` | `null` | Maximum URLs to follow per extraction result. |
| `maxTotalUrls` | `Optional<Integer>` | `null` | Maximum URLs followed across the whole extraction call. |
