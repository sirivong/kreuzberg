...
## Gleam API Reference <span class="version-badge">v5.0.0-rc.1</span>
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `pdfBytes` | `BitArray` | Yes | Raw PDF file bytes |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `content` | `BitArray` | — | The content bytes to extract from |
...
| `additional` | `Dict(String, String)` | `{}` | Additional custom fields from postprocessors. Serialized as a nested `"additional"` object (not flattened at root level). Uses `Cow<'static, str>` keys so static string keys avoid allocation. |
