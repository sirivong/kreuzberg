...
## Zig API Reference <span class="version-badge">v5.0.0-rc.36</span>
...
```zig
const result = try extractBytes("data", "value", .{});
```
...
```zig
const result = try extractFile("value", "value", .{});
```
...
```zig
const result = try extractFileSync("value", "value", .{});
```
...
```zig
const result = try batchExtractFilesSync(&[_]u8{}, .{});
```
...
```zig
const result = try batchExtractFiles(&[_]u8{}, .{});
```
...
```zig
const result = try batchExtractBytes(&[_]u8{}, .{});
```
