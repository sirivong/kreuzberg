...
## Zig API Reference <span class="version-badge">v5.0.0-rc.15</span>
...
| `attributes` | `[]const [:0]const u8` | `/* serde(default) */` | Attributes mapped by element identifier (if present) |
...
| `qrCodes` | `[]const QrCode?` | `[]` | QR codes decoded from this image, when QR detection is enabled. Populated by the QR post-processor (`crates/kreuzberg/src/extractors/qr.rs`) via the pure-Rust `rqrr` decoder. `null` when QR detection is disabled; an empty `Some(vec![])` when detection ran but found nothing. |
...
| `concurrency` | `[:0]const u8?` | `null` | Concurrency limits for constrained environments (None = use defaults). Controls Rayon thread pool size, ONNX Runtime intra-op threads, and (when `max_concurrent_extractions` is unset) the batch concurrency semaphore. See `ConcurrencyConfig` for details. |
...
| `cancelToken` | `[:0]const u8?` | `null` | Cancellation token for this extraction (None = no external cancellation). Pass a `CancellationToken` clone here and call its `cancel()` from another thread / task to abort the extraction in progress. The extractor checks the token at safe checkpoints (before lock acquisition, between pages, between batch items) and returns `Cancelled` when set. The field is excluded from serialization because `CancellationToken` is a runtime handle, not a configuration value. |
...
| `colSpan` | `u32` | `/* serde(default) */` | Number of columns this cell spans. |
...
| `attributes` | `[]const []const [:0]const u8` | — | Additional attributes as key-value pairs |
...
| `tables` | `[]const Table` | `/* serde(default) */` | Tables found on this page (uses Arc for memory efficiency) Serializes as Vec<Table> for JSON compatibility while maintaining Arc semantics in-memory for zero-copy sharing. |
...
| `hyperlinks` | `[]const [:0]const u8` | `/* serde(default) */` | Hyperlinks discovered in slides as (url, optional_label) pairs. |
...
| `links` | `[]const []const [:0]const u8?` | `null` | Markdown links as (text, URL) tuples (Markdown files only) |
| `codeBlocks` | `[]const []const [:0]const u8?` | `null` | Code blocks as (language, code) tuples (Markdown files only) |
...
| `links` | `[]const []const [:0]const u8?` | `[]` | Markdown links as (text, url) tuples (for Markdown files) |
| `codeBlocks` | `[]const []const [:0]const u8?` | `[]` | Code blocks as (language, code) tuples (for Markdown files) |
...
| `language` | `[:0]const u8?` | `null` | Optional language hint (ISO-639-1 code, e.g. "en", "de"). When `null` (default) the engine may attempt auto-detection if supported. For deterministic production output, always set this explicitly. |
...
| `maxDurationMs` | `u64?` | `null` | Hard safety limit on input duration (milliseconds). Files longer than this are rejected *before* any decode or model work. Default: 30 minutes. Set to `null` to disable (not recommended for untrusted input). |
...
| `timeoutMs` | `u64?` | `null` | Wall-clock timeout for the entire transcription operation (ms). Includes model download (first time), decode, and inference. Default: 10 minutes. Uses `tokio.select!` so the async runtime is never blocked. |
| `modelCacheDir` | `[:0]const u8?` | `null` | Override the directory used for Whisper model cache. When `null`, uses the centralized resolver: `KREUZBERG_CACHE_DIR/transcription/whisper` or the platform default (`~/.cache/kreuzberg/transcription/whisper` on Linux, etc.). |
...
| `MetadataBlock` | Structured metadata block (email headers, YAML frontmatter, etc.). — Fields: `entries`: `[]const []const [:0]const u8` |
