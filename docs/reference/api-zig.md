...
## Zig API Reference <span class="version-badge">v1.0.0-rc.1</span>
...
```zig
pub fn extract_sync(input: ExtractInput, config: ExtractionConfig) Error!ExtractionOutput
```

**Example:**

```zig
const result = try extractSync(.{}, .{});
```
...
```zig
pub fn extract_batch_sync(inputs: []const ExtractInput, config: ExtractionConfig) Error!ExtractionOutput
```

**Example:**

```zig
const result = try extractBatchSync(&[_]u8{}, .{});
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `inputs` | `\[\]const ExtractInput` | Yes | The inputs |
| `config` | `ExtractionConfig` | Yes | The configuration options |
...
```zig
pub fn extract(input: ExtractInput, config: ExtractionConfig) Error!ExtractionOutput
```
...
```zig
pub fn extract_batch(inputs: []const ExtractInput, config: ExtractionConfig) Error!ExtractionOutput
```

**Example:**

```zig
const result = try extractBatch(&[_]u8{}, .{});
```
...
```zig
const result = try detectMimeTypeFromBytes("data");
```
...
```zig
const result = try getExtensionsForMime("value");
```
...
```zig
pub fn list_supported_formats() []const SupportedFormat
```
...
```zig
pub fn detect_qr_codes(image_bytes: []const u8, format_hint: ?[:0]const u8) []const QrCode
```

**Example:**

```zig
const result = detectQrCodes("data", "value");
```
...
```zig
pub fn clear_embedding_backends() Error!void
```
...
```zig
const result = try listEmbeddingBackends();
```
...
```zig
const result = try listDocumentExtractors();
```
...
```zig
pub fn clear_document_extractors() Error!void
```
...
```zig
const result = try listOcrBackends();
```
...
```zig
pub fn clear_ocr_backends() Error!void
```
...
```zig
pub fn register_builtin() Error!void
```
...
```zig
const result = try listPostProcessors();
```
...
```zig
pub fn clear_post_processors() Error!void
```
...
```zig
const result = try listRenderers();
```
...
```zig
pub fn clear_renderers() Error!void
```
...
```zig
pub fn clear_reranker_backends() Error!void
```
...
```zig
pub fn list_reranker_backends() Error![]const [:0]const u8
```

**Example:**

```zig
const result = try listRerankerBackends();
```
...
```zig
const result = try listValidators();
```
...
```zig
pub fn clear_validators() Error!void
```
...
```zig
pub fn classify_pages(result: ExtractionResult, config: PageClassificationConfig) Error!void
```
...
```zig
pub fn classify_text(text: [:0]const u8, config: PageClassificationConfig) Error![]const ClassificationLabel
```

**Example:**

```zig
const result = try classifyText("value", .{});
```
...
```zig
pub fn classify_document(pages: []const [:0]const u8, config: PageClassificationConfig) Error![]const ClassificationLabel
```

**Example:**

```zig
const result = try classifyDocument(&[_]u8{}, .{});
```
...
```zig
pub fn redact(result: ExtractionResult, config: RedactionConfig) Error!void
```
...
```zig
pub fn find_all(text: [:0]const u8) []const PatternMatch
```
...
```zig
pub fn scan_text(text: [:0]const u8, categories: []const PiiCategory) []const PatternMatch
```

**Example:**

```zig
const result = scanText("value", &[_]u8{});
```
...
```zig
pub fn summarize(text: [:0]const u8, language: ?[:0]const u8, max_tokens: ?u32) ?[:0]const u8
```
...
```zig
pub fn token_count(text: [:0]const u8) u32
```
...
```zig
pub fn translate_result(result: ExtractionResult, config: TranslationConfig) Error!void
```
...
```zig
pub fn chunk_for_rag(text: [:0]const u8, config: ChunkingConfig) Error!ChunkingResult
```

**Example:**

```zig
const result = try chunkForRag("value", .{});
```
...
```zig
pub fn extract_region_with_vlm(image_bytes: []const u8, image_mime: [:0]const u8, region_kind: RegionKind, llm_config: LlmConfig, custom_prompt: ?[:0]const u8) Error![:0]const u8
```

**Example:**

```zig
const result = try extractRegionWithVlm("data", "value", .{}, .{}, "value");
```
...
```zig
pub fn extract_keywords(text: [:0]const u8, config: KeywordConfig) Error![]const Keyword
```

**Example:**

```zig
const result = try extractKeywords("value", .{});
```
...
```zig
pub fn analyze_document(metadata: DocumentMetadata, config: HeuristicsConfig, document_bytes: ?[]const u8) Error!ChunkingDecision
```

**Example:**

```zig
const result = try analyzeDocument(.{}, .{}, "data");
```
...
| `documentBytes` | `\[\]const u8?` | No | The document bytes |
...
```zig
pub fn analyze_with_user_chunks(user_ranges: []const PageRange, total_pages: u32, size_bytes: u64, config: HeuristicsConfig) ChunkingDecision
```

**Example:**

```zig
const result = analyzeWithUserChunks(&[_]u8{}, 42, 42, .{});
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `userRanges` | `\[\]const PageRange` | Yes | The user ranges |
| `totalPages` | `u32` | Yes | The total pages |
| `sizeBytes` | `u64` | Yes | The size bytes |
| `config` | `HeuristicsConfig` | Yes | The configuration options |
...
```zig
pub fn check_format_limits(mime_type: [:0]const u8, sheet_count: ?u32, workbook_cells: ?u64, embedded_count: ?u32, config: HeuristicsConfig) ?[:0]const u8
```

**Example:**

```zig
const result = checkFormatLimits("value", 42, 42, 42, .{});
```
...
| `sheetCount` | `u32?` | No | The sheet count |
| `workbookCells` | `u64?` | No | The workbook cells |
| `embeddedCount` | `u32?` | No | The embedded count |
| `config` | `HeuristicsConfig` | Yes | The configuration options |
...
```zig
pub fn boundaries_from_extraction_result(result: ExtractionResult, thresholds: MultidocThresholds) []const DocumentBoundary
```

**Example:**

```zig
const result = boundariesFromExtractionResult(.{}, .{});
```
...
**Returns:** `[]const DocumentBoundary`
...
```zig
pub fn detect_boundaries(input: MultidocInput, thresholds: MultidocThresholds) []const DocumentBoundary
```
...
```zig
const result = calculateChunkPlan(42, 42, true, .{});
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `pageCount` | `u32` | Yes | Total number of pages in the document |
| `sizeBytes` | `u64` | Yes | File size in bytes |
...
```zig
pub fn calculate_plan_from_overrides(user_chunks: []const PageRange, total_pages: u32, size_bytes: u64, config: HeuristicsConfig) ChunkPlan
```

**Example:**

```zig
const result = calculatePlanFromOverrides(&[_]u8{}, 42, 42, .{});
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `userChunks` | `\[\]const PageRange` | Yes | The user chunks |
| `totalPages` | `u32` | Yes | The total pages |
| `sizeBytes` | `u64` | Yes | The size bytes |
| `config` | `HeuristicsConfig` | Yes | The configuration options |
...
```zig
pub fn fingerprint(raw: []const u8) [:0]const u8
```

**Example:**

```zig
const result = fingerprint("data");
```
...
```zig
pub fn resolve(preset: Preset, custom_schema: ?[:0]const u8, context: std.StringHashMap([:0]const u8)) ResolveError!ResolvedPreset
```
...
| `customSchema` | `\[:0\]const u8?` | No | The custom schema |
| `context` | `std.StringHashMap(\[:0\]const u8)` | Yes | The context |
...
```zig
pub fn extract_structured_json(bytes: []const u8, mime: [:0]const u8, preset_spec_json: [:0]const u8, options_json: [:0]const u8) Error![:0]const u8
```

**Example:**

```zig
const result = try extractStructuredJson("data", "value", "value", "value");
```
...
| `mime` | `\[:0\]const u8` | Yes | The mime |
| `presetSpecJson` | `\[:0\]const u8` | Yes | The preset spec json |
| `optionsJson` | `\[:0\]const u8` | Yes | The options json |
...
```zig
pub fn split_and_extract_json(bytes: []const u8, mime: [:0]const u8, preset_spec_json: [:0]const u8, options_json: [:0]const u8) Error![:0]const u8
```

**Example:**

```zig
const result = try splitAndExtractJson("data", "value", "value", "value");
```
...
```zig
const result = try renderPdfPageToPng("data", 42, 42, "value");
```
...
```zig
pub fn caption_image(image_bytes: []const u8, llm_config: LlmConfig, custom_prompt: ?[:0]const u8) Error![:0]const u8
```

**Example:**

```zig
const result = try captionImage("data", .{}, "value");
```
...
```zig
pub fn caption_image_file(path: [:0]const u8, llm_config: LlmConfig, custom_prompt: ?[:0]const u8) Error![:0]const u8
```

**Example:**

```zig
const result = try captionImageFile("value", .{}, "value");
```
...
```zig
const result = try detectMimeType("value", true);
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `totalFiles` | `u64` | — | Total number of files currently in the cache directory. |
| `totalSizeMb` | `f64` | — | Combined size of all cache files in megabytes. |
| `availableSpaceMb` | `f64` | — | Free disk space available on the cache volume, in megabytes. |
| `oldestFileAgeDays` | `f64` | — | Age of the oldest cache file in days (0.0 if the cache is empty). |
| `newestFileAgeDays` | `f64` | — | Age of the most recently written cache file in days (0.0 if the cache is empty). |
...
| `minImageArea` | `u32` | `serde(default = "default_min_image_area")` | Skip images whose `width * height` is below this threshold (in pixels). Default `1_000` filters out icons and decorations. |
...
| `estimatedTimeMs` | `u64` | — | Estimated processing time for this chunk in milliseconds. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `totalChunks` | `u32` | `0` | Total number of chunks. |
| `chunks` | `\[\]const ChunkInfo` | `\[\]` | Individual chunk information. |
| `totalEstimatedTimeMs` | `u64` | `0` | Estimated total processing time in milliseconds. |
...
```zig
pub fn totalPages(self: *const ChunkPlan) u32
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `textCoverage` | `f32` | — | Fraction of pages with usable text in `\[0, 1\]`. |
| `ocrAggregate` | `f32?` | `null` | Mean OCR per-element recognition confidence; `null` when OCR did not run. |
...
```zig
pub fn fromExtractionResult(result: ExtractionResult, schema_compliance: SchemaCompliance, text_coverage: f32) ConfidenceSignals
```

**Example:**

```zig
const result = ConfidenceSignals.fromExtractionResult(.{}, .{}, 0.5);
```
...
| `textCoverage` | `f32` | Yes | The text coverage |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `textCoverage` | `f32` | `0.3` | Weight assigned to `text_coverage`. Default 0.30. |
| `ocrAggregate` | `f32` | `0.3` | Weight assigned to `ocr_aggregate` when OCR ran. Default 0.30 — folds into `text_coverage` weight when OCR did not run. |
| `schemaCompliance` | `f32` | `0.4` | Weight assigned to `schema_compliance`. Default 0.40. |
...
```zig
pub fn isNormalized(self: *const ConfidenceWeights) bool
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `fromLine` | `u64` | — | Starting line number in the old content (0-indexed). |
...
| `maxContentChars` | `u64?` | `null` | Truncate content to this many characters before diffing. Useful for very large documents where only the first N characters matter. `null` means no truncation. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `startPage` | `u32` | — | 1-indexed start page (inclusive). |
| `endPage` | `u32` | — | 1-indexed end page (inclusive). |
...
```zig
const result = try instance.extractBytes("data", "value", .{});
```
...
```zig
const result = try instance.extractFile("value", "value", .{});
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `mimeType` | `\[:0\]const u8` | — | MIME type of the document. |
| `sizeBytes` | `u64` | — | File size in bytes. |
| `pageCount` | `u32?` | `null` | Page count (if known, e.g., from previous analysis). |
...
| `cacheDir` | `\[:0\]const u8?` | `null` | Custom cache directory for model files Defaults to `~/.cache/xberg/embeddings/` if not specified. Allows full customization of model download location. |
...
| `labels` | `\[\]const \[:0\]const u8` | `\[\]` | Custom labels to pass through to the result without modification. These are caller-supplied tags that the enrichment pipeline propagates verbatim into `EnrichResult.labels`. Useful for attaching project- or document-level metadata to every enrichment result. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `keywords` | `\[\]const \[:0\]const u8` | `\[\]` | Salient terms extracted from the text. Populated when `EnrichOptions.keywords` was `true`. The ordering is backend-defined (typically by descending relevance score). |
| `entities` | `\[\]const Entity` | `\[\]` | Named entities found in the text. Populated when `EnrichOptions.entities` was `true`. Uses the shared OSS entity schema (`Entity` / `EntityCategory`) so consumers can pattern-match on entity categories without JSON gymnastics. |
| `labels` | `\[\]const \[:0\]const u8` | `\[\]` | Caller-supplied labels echoed from `EnrichOptions.labels`. |
...
| `uri` | `\[:0\]const u8?` | `null` | Local path, `file://` URI, or HTTP(S) URL for `kind = "uri"`. |
| `mimeType` | `\[:0\]const u8?` | `null` | MIME type hint. |
| `filename` | `\[:0\]const u8?` | `null` | Filename hint used for MIME detection and metadata. |
...
```zig
pub fn bytes(bytes: []const u8, mime_type: [:0]const u8, filename: ?[:0]const u8) ExtractInput
```

**Example:**

```zig
const result = ExtractInput.bytes("data", "value", "value");
```
...
| `bytes` | `\[\]const u8` | Yes | The bytes |
| `mimeType` | `\[:0\]const u8` | Yes | The mime type |
| `filename` | `\[:0\]const u8?` | No | The filename |
...
```zig
pub fn uri(uri: [:0]const u8) ExtractInput
```
...
| `caption` | `\[:0\]const u8?` | `null` | VLM-generated caption describing the image, when captioning is configured. Populated by the captioning post-processor (`crates/xberg/src/plugins/processor/builtin/captioning.rs`), which routes each image through `crate.llm.region_extractor.extract_region_with_vlm` in caption mode. `null` when captioning is disabled or the VLM declined to caption. |
| `qrCodes` | `\[\]const QrCode?` | `\[\]` | QR codes decoded from this image, when QR detection is enabled. Populated by the QR post-processor (`crates/xberg/src/extractors/qr.rs`) via the pure-Rust `rqrr` decoder. `null` when QR detection is disabled; an empty `Some(\[\])` when detection ran but found nothing. |
| `dataBase64` | `\[:0\]const u8?` | `null` | Base64-encoded copy of `data`; populated when `ImageExtractionConfig.include_data_base64` is `true`. Omitted from JSON by default; use instead of `data` in JSON-only clients. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `textCoverage` | `f32` | — | Fraction of pages with a usable text layer. |
| `ocrAggregate` | `f32?` | `null` | Mean OCR per-element recognition confidence when OCR ran; `null` when it did not. |
...
```zig
pub fn needsImageData(self: *const ExtractionConfig) bool
```
...
| `errorType` | `\[:0\]const u8` | — | Stable snake_case error kind. |
| `source` | `\[:0\]const u8` | — | Best-effort source identifier. |
| `message` | `\[:0\]const u8` | — | Error message. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `results` | `\[\]const ExtractionResult` | `\[\]` | Extraction results in discovery order. |
| `errors` | `\[\]const ExtractionErrorItem` | `\[\]` | Non-fatal per-input errors. |
| `summary` | `ExtractionSummary` | — | Aggregate counts for the operation. |
| `crawl` | `std.StringHashMap(\[:0\]const u8)` | `{}` | Optional implementation-specific crawl metadata. |
...
```zig
const result = ExtractionOutput.single(.{});
```
...
```zig
const result = ExtractionResult.fromOcr(.{});
```
...
| `remoteUrls` | `u64` | — | Number of URI inputs that resolved to remote HTTP(S) URLs. |
| `pagesCrawled` | `u64` | — | Number of HTML pages crawled or scraped. |
| `documentsDownloaded` | `u64` | — | Number of downloaded non-HTML documents extracted from URLs. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `latex` | `\[:0\]const u8` | — | LaTeX source of the recognized formula, without surrounding `$$` delimiters. This field contains the raw LaTeX code as produced by the OCR backend. To render the formula in Markdown or other formats, wrap with `$$..$$` delimiters as needed. |
...
| `rowSpan` | `u32` | `serde(default = "default_span")` | Number of rows this cell spans. |
| `colSpan` | `u32` | `serde(default = "default_span")` | Number of columns this cell spans. |
...
| `textLayerThreshold` | `f32` | `0.7` | Minimum fraction of pages that must have text to skip chunking. Range `0.0..=1.0`. Default: `0.7` (70 % of pages). |
| `fileSizeThresholdBytes` | `u64` | `10485760` | File size threshold in bytes for considering chunking. Files smaller than this are processed without chunking. Default: 10 MiB (10 × 1 024 × 1 024). |
| `pageCountThreshold` | `u32` | `50` | Page count threshold for considering chunking. Documents with fewer pages are processed without chunking. Default: 50. |
| `targetPagesPerChunk` | `u32` | `10` | Target number of pages per chunk for optimal parallel processing. Default: 10. |
| `maxPagesPerChunk` | `u32` | `25` | Hard cap on pages per chunk. No chunk will exceed this limit. Must be ≥ `target_pages_per_chunk`. Default: 25. |
| `diskProcessingThresholdBytes` | `u64` | `52428800` | File size threshold for disk-based processing. Files larger than this are buffered to disk to prevent OOM. Default: 50 MiB (50 × 1 024 × 1 024). |
| `minCharsPerPage` | `u32` | `50` | Minimum characters per page to consider a page as having text. Default: 50. |
| `maxXlsxSheetCount` | `u32` | `200` | Maximum sheet count allowed in an XLSX workbook. Workbooks beyond this are rejected pre-extraction to avoid OOM / abusive billing inflation. Default: 200. |
| `maxXlsxWorkbookCells` | `u64` | `5000000` | Maximum cell count (sheets × rows × columns approximation) in an XLSX workbook. Default: 5 000 000 (≈ 200 sheets × 25 k cells). |
| `maxPptxEmbeddedCount` | `u32` | `50` | Maximum number of OLE-embedded objects extractable from a single PPTX or DOCX. Protects against zip-bomb-style nested-document abuse. Default: 50. |
...
```zig
pub fn validate(self: *const HeuristicsConfig) Error!void
```
...
| `minConfidence` | `f64` | `0.8` | Minimum confidence threshold (0.0-1.0) |
...
```zig
pub fn detect(self: *const LlmBackend, text: [:0]const u8, categories: []const EntityCategory) Error![]const Entity
```

**Example:**

```zig
const result = try instance.detect("value", &[_]u8{});
```
...
```zig
pub fn detectWithCustom(self: *const LlmBackend, text: [:0]const u8, categories: []const EntityCategory, custom_labels: []const [:0]const u8) Error![]const Entity
```

**Example:**

```zig
const result = try instance.detectWithCustom("value", &[_]u8{}, &[_]u8{});
```
...
```zig
pub fn compile(meta_schema_json: [:0]const u8) LoadError!MetaSchema
```

**Example:**

```zig
const result = try MetaSchema.compile("value");
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `metaSchemaJson` | `\[:0\]const u8` | Yes | The meta schema json |
...
```zig
pub fn parsePreset(self: *const MetaSchema, path: [:0]const u8, raw: []const u8) LoadError!Preset
```

**Example:**

```zig
const result = try instance.parsePreset("value", "data");
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `pageCount` | `u32` | — | Total number of pages in the PDF. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `densityShiftThreshold` | `f32` | `0.3` | Text density difference threshold for `DensityShift` detection. Default: 0.3. |
| `bigramOverlapMin` | `f32` | `0.1` | Minimum bigram-overlap ratio below which a density shift is promoted to a `DensityShift` boundary.  Default: 0.1 (10 % overlap). |
...
| `model` | `\[:0\]const u8?` | `null` | Override the default model — only used by `NerBackendKind.Onnx`. `null` lets the backend pick its pinned default xberg GLiNER model alias. |
...
| `customLabels` | `\[\]const \[:0\]const u8` | `\[\]` | Arbitrary user-supplied entity labels for zero-shot detection. `xberg-gliner` natively supports zero-shot inference over caller-supplied labels. The LLM backend also honours these labels by including them in the structured-output schema. Custom labels surface as `EntityCategory.Custom` in the resulting `Entity` stream. Use this when you need domain-specific entity types (e.g. `"Treatment"`, `"Product"`, `"Vessel"`) without forking GLiNER's taxonomy. |
...
```zig
const result = try instance.processImage("data", .{});
```
...
```zig
const result = try instance.processImageFile("value", .{});
```
...
```zig
pub fn emitsStructuredMarkdown(self: *const OcrBackend) bool
```
...
```zig
const result = try instance.processDocument("value", .{});
```
...
| `language` | `\[\]const \[:0\]const u8` | `\[\]` | Language code(s) for OCR recognition. Accepts either a single language code ("eng") or a list (\["eng", "deu"\]). Defaults to \["eng"\]. For Tesseract, languages are joined with "+". |
...
| `language` | `\[\]const \[:0\]const u8?` | `/* serde(default) */` | Language override for this stage (None = use parent OcrConfig.language). Accepts either a single language code ("eng") or a list (\["eng", "deu"\]). |
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `batchSize` | `u32` | Yes | Number of text regions to process simultaneously |
...
```zig
pub fn pageCount(self: *const PageRange) u32
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `pageNumber` | `u32` | — | 1-indexed page number. |
| `textExcerpt` | `\[:0\]const u8` | — | First ~500 characters of extracted text. |
...
| `layoutTextDensity` | `f32` | — | Text density: characters per page area, normalised to `\[0.0, 1.0\]`. |
...
```zig
pub fn fromPageText(page_number: u32, text: [:0]const u8, layout_text_density: f32) PageSignals
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `pageNumber` | `u32` | Yes | The page number |
| `text` | `\[:0\]const u8` | Yes | The text |
| `layoutTextDensity` | `f32` | Yes | The layout text density |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `\[:0\]const u8` | — | Partial field name (the leaf name within the field hierarchy). |
| `fullName` | `\[:0\]const u8` | — | Fully-qualified field name (dotted path from the form root). |
| `fieldType` | `FormFieldType` | — | Classified field type. |
| `value` | `\[:0\]const u8?` | `/* serde(default) */` | Current field value, if any. |
| `defaultValue` | `\[:0\]const u8?` | `/* serde(default) */` | Default field value, if any. |
...
| `maxLength` | `u32?` | `/* serde(default) */` | Maximum input length for text fields, if specified. |
| `tooltip` | `\[:0\]const u8?` | `/* serde(default) */` | Tooltip / alternate field description, if present. |
...
| `pageCount` | `u32?` | `null` | Total number of pages in the PDF document |
...
```zig
const result = instance.shouldProcess(.{}, .{});
```
...
```zig
const result = instance.estimatedDurationMs(.{});
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `id` | `\[:0\]const u8` | — | Stable, URL-safe preset identifier (lowercase snake_case). |
| `version` | `\[:0\]const u8` | — | Monotonic version string (e.g. `v1`). |
| `schemaName` | `\[:0\]const u8` | — | Human-readable schema name forwarded to the LLM as the response/tool name. |
| `description` | `\[:0\]const u8` | — | One-line preset description shown in the registry UI. |
| `category` | `PresetCategory` | — | Top-level category for grouping in the playground. |
| `tags` | `\[\]const \[:0\]const u8` | `/* serde(default) */` | Free-form tags used for search/filtering. May be empty. |
| `schema` | `\[:0\]const u8` | — | JSON Schema (Draft 2020-12) describing the structured output shape. |
| `systemPrompt` | `\[:0\]const u8` | — | Instruction primer sent to the model. |
| `contextTemplate` | `\[:0\]const u8?` | `/* serde(default) */` | Optional mustache-style template merged with caller-supplied context. |
...
| `fingerprint` | `\[:0\]const u8` | `/* serde(default) */` | Stable sha256 fingerprint of the canonical preset file contents. Populated at registry load — not present in the on-disk JSON files. Used as a cache-invalidation token by the worker pipeline. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `inputPath` | `\[:0\]const u8` | — | Path to the sample input file, relative to the preset directory. |
| `outputPath` | `\[:0\]const u8` | — | Path to the reference structured output, relative to the preset directory. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `id` | `\[:0\]const u8` | — | Preset identifier matching `Preset.id`. |
| `version` | `\[:0\]const u8` | — | Preset version matching `Preset.version`. |
| `schemaName` | `\[:0\]const u8` | — | Schema name matching `Preset.schema_name`. |
| `description` | `\[:0\]const u8` | — | One-line preset description. |
| `category` | `PresetCategory` | — | Top-level category. |
| `tags` | `\[\]const \[:0\]const u8` | — | Free-form tags. |
| `preferredCallMode` | `CallMode` | — | Default call mode. |
| `emitCitations` | `bool` | — | Whether the preset prompts the model for citations. |
| `fingerprint` | `\[:0\]const u8` | — | Stable fingerprint matching `Preset.fingerprint`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `messageCount` | `u64` | — | Total number of email messages found in the PST archive. |
...
```zig
pub fn validate(self: *const RedactionConfig) Error!void
```
...
```zig
pub fn labeled(label: [:0]const u8, pattern: [:0]const u8) RedactionPattern
```
...
```zig
pub fn literal(value: [:0]const u8) RedactionTerm
```
...
```zig
pub fn labeled(label: [:0]const u8, value: [:0]const u8) RedactionTerm
```
...
```zig
pub fn loadEmbedded() LoadError!Registry
```

**Example:**

```zig
const result = try Registry.loadEmbedded();
```
...
```zig
pub fn get(self: *const Registry, id: [:0]const u8) ?Preset
```
...
```zig
pub fn summaries(self: *const Registry) []const PresetSummary
```
...
```zig
pub fn sampleBytes(self: *const Registry, preset_id: [:0]const u8, name: [:0]const u8) ?[]const u8
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `presetId` | `\[:0\]const u8` | Yes | The preset id |
| `name` | `\[:0\]const u8` | Yes | The name |
...
```zig
pub fn extendFromDir(self: *const Registry, dir: [:0]const u8) LoadError!u64
```

**Example:**

```zig
const result = try instance.extendFromDir("value");
```
...
```zig
const result = try instance.render(.{});
```
...
```zig
pub fn rerank(self: *const RerankerBackend, query: [:0]const u8, documents: []const [:0]const u8) Error![]const f32
```
...
| `cacheDir` | `\[:0\]const u8?` | `null` | Custom cache directory for model files. Defaults to `~/.cache/xberg/rerankers/` if not specified. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `id` | `\[:0\]const u8` | — | Source preset identifier. |
| `version` | `\[:0\]const u8` | — | Source preset version. |
| `fingerprint` | `\[:0\]const u8` | — | Fingerprint of the source preset file, used as a cache token. |
| `schemaName` | `\[:0\]const u8` | — | Schema name forwarded to the LLM. |
| `schema` | `\[:0\]const u8` | — | Effective JSON Schema (caller override or the preset's own). |
| `systemPrompt` | `\[:0\]const u8` | — | System prompt with rendered context appended. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `maxArchiveSize` | `u64` | `524288000` | Maximum uncompressed size for archives (500 MB) |
| `maxCompressionRatio` | `u64` | `100` | Maximum compression ratio before flagging as potential bomb (100:1) |
| `maxFilesInArchive` | `u64` | `10000` | Maximum number of files in archive (10,000) |
| `maxNestingDepth` | `u64` | `1024` | Maximum nesting depth for structures (100) |
| `maxEntityLength` | `u64` | `1048576` | Maximum length of any single XML entity / attribute / token (1 MiB). This is a per-token cap, NOT a total cap — billion-laughs class attacks where a single entity expands to hundreds of MB are caught here, while normal long text content (a paragraph, a CDATA block) is caught by `max_content_size` instead. |
| `maxContentSize` | `u64` | `104857600` | Maximum string growth per document (100 MB) |
| `maxIterations` | `u64` | `10000000` | Maximum iterations per operation |
| `maxXmlDepth` | `u64` | `1024` | Maximum XML depth (100 levels) |
| `maxTableCells` | `u64` | `100000` | Maximum cells per table (100,000) |
...
| `schemaName` | `\[:0\]const u8` | `serde(default = "default_schema_name")` | Schema name passed to the LLM's structured output mode. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `mimeType` | `\[:0\]const u8` | — | MIME type, canonicalised to lowercase by the caller. |
| `pageCount` | `u32` | — | Number of pages in the document. |
| `textCoverage` | `f64` | — | Fraction of pages with a real text layer (0.0..=1.0). |
| `avgCharsPerPage` | `f64` | — | Average extracted characters per page. |
| `embeddedImageCount` | `u32` | — | Count of embedded images (figures, photos, signatures) discovered. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `scanMaxCoverage` | `f64` | `0.1` | PDFs with `text_coverage` strictly below this are treated as scanned. **Conservative default: 0.10** — deployments override via their own config after measuring their document corpus. |
| `digitalMinCoverage` | `f64` | `0.9` | PDFs with `text_coverage` at or above this AND zero embedded images route to `StructuredCallMode.TextOnly`. **Conservative default: 0.90** — deployments override via their own config after measuring their document corpus. |
| `docxTextMinDensity` | `f64` | `200` | DOCX / HTML / text documents with `avg_chars_per_page` above this route to `StructuredCallMode.TextOnly`. **Conservative default: 200.0** — deployments override via their own config after measuring their document corpus. |
...
| `maxTokens` | `u32?` | `null` | Maximum summary length in tokens. `null` lets the backend pick a default. |
...
| `renderDpi` | `f32` | `96` | Target DPI when rasterizing SVG to a pixel-based format (PNG, JPEG, WebP, HEIF).  The tree's viewBox is scaled by `render_dpi / 96.0` before the pixel buffer is allocated.  Defaults to `96.0` (1× CSS pixel density). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `language` | `\[\]const \[:0\]const u8` | `\[\]` | Language code(s) for OCR recognition. Accepts either a single language code ("eng") or a list (\["eng", "deu"\]). For Tesseract backend, languages are joined with "+". |
...
| `modelCacheDir` | `\[:0\]const u8?` | `null` | Override the directory used for Whisper model cache. When `null`, uses the centralized resolver: `XBERG_CACHE_DIR/whisper` or the platform default (`~/.cache/xberg/whisper` on Linux, etc.). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `pageRanges` | `\[\]const PageRange?` | `\[\]` | User-specified page ranges (overrides automatic chunking). |
| `pagesPerChunk` | `u32?` | `null` | User-specified pages per chunk (overrides automatic calculation). |
...
```zig
const result = instance.shouldValidate(.{}, .{});
```
...
| `Failed` | Processing failed. — Fields: `error`: `\[:0\]const u8` |
...
| `UseOverrides` | Use user-provided chunk overrides. — Fields: `userChunks`: `\[\]const PageRange` |
...
| Value | Description |
|-------|-------------|
| `SmallFile` | File is below size threshold. — Fields: `sizeBytes`: `u64`, `thresholdBytes`: `u64` |
| `FewPages` | Document has fewer pages than threshold. — Fields: `pageCount`: `u32`, `threshold`: `u32` |
| `TextLayerDetected` | PDF has substantial text layer (OCR not needed). — Fields: `textCoverage`: `f32`, `avgCharsPerPage`: `u32` |
| `FormatNotChunkable` | Document format does not support chunking. — Fields: `mimeType`: `\[:0\]const u8` |
...
| Value | Description |
|-------|-------------|
| `LargeFile` | File exceeds size threshold. — Fields: `sizeBytes`: `u64`, `thresholdBytes`: `u64` |
| `ManyPages` | Document has many pages. — Fields: `pageCount`: `u32`, `threshold`: `u32` |
| `OcrRequired` | PDF requires OCR and is large. — Fields: `pageCount`: `u32`, `forceOcr`: `bool` |
| `LargeAndManyPages` | Both size and page count exceed thresholds. — Fields: `sizeBytes`: `u64`, `pageCount`: `u32` |
