...
## TypeScript API Reference <span class="version-badge">v5.0.0-rc.35</span>
...
```typescript
const result = await extractBytes(Buffer.from("data"), "value", new ExtractionConfig());
```
...
```typescript
const result = await extractFile("value", "value", new ExtractionConfig());
```
...
```typescript
const result = extractFileSync("value", "value", new ExtractionConfig());
```
...
```typescript
const result = extractBytesSync(Buffer.from("data"), "value", new ExtractionConfig());
```
...
```typescript
const result = batchExtractFilesSync([], new ExtractionConfig());
```
...
```typescript
const result = await batchExtractFiles([], new ExtractionConfig());
```
...
```typescript
const result = await batchExtractBytes([], new ExtractionConfig());
```
...
```typescript
const result = detectMimeTypeFromBytes(Buffer.from("data"));
```
...
```typescript
const result = detectQrCodes(Buffer.from("data"), "value");
```
...
```typescript
const result = await extractRegionWithVlm(Buffer.from("data"), "value", new RegionKind(), new LlmConfig(), "value");
```
...
```typescript
const result = analyzeDocument(new DocumentMetadata(), new HeuristicsConfig(), Buffer.from("data"));
```
...
```typescript
const result = fingerprint(Buffer.from("data"));
```
...
```typescript
function extractStructuredJson(bytes: Buffer, mime: string, presetSpecJson: string, optionsJson: string): string
```

**Example:**

```typescript
const result = extractStructuredJson(Buffer.from("data"), "value", "value", "value");
```
...
```typescript
function splitAndExtractJson(bytes: Buffer, mime: string, presetSpecJson: string, optionsJson: string): string
```

**Example:**

```typescript
const result = splitAndExtractJson(Buffer.from("data"), "value", "value", "value");
```
...
```typescript
const result = renderPdfPageToPng(Buffer.from("data"), 42, 42, "value");
```
...
```typescript
const result = pdfPageCount(Buffer.from("data"), "value");
```
...
```typescript
const result = await captionImage(Buffer.from("data"), new LlmConfig(), "value");
```
...
```typescript
extractBytes(content: Buffer, mimeType: string, config: ExtractionConfig): Promise<InternalDocument>
```

**Example:**

```typescript
const result = await instance.extractBytes(Buffer.from("data"), "value", new ExtractionConfig());
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `content` | `Buffer` | Yes | Raw document bytes |
...
```typescript
const result = await instance.extractFile("value", "value", new ExtractionConfig());
```
...
| `qrCodes` | `Array<QrCode> \| null` | `\[\]` | QR codes decoded from this image, when QR detection is enabled. Populated by the QR post-processor (`crates/kreuzberg/src/extractors/qr.rs`) via the pure-Rust `rqrr` decoder. `null` when QR detection is disabled; an empty `Some(\[\])` when detection ran but found nothing. |
...
| `cacheDir` | `string \| null` | `null` | Custom cache directory for model files. Defaults to `~/.cache/kreuzberg/rerankers/` if not specified. |
...
| `modelCacheDir` | `string \| null` | `null` | Override the directory used for Whisper model cache. When `null`, uses the centralized resolver: `KREUZBERG_CACHE_DIR/whisper` or the platform default (`~/.cache/kreuzberg/whisper` on Linux, etc.). |
