...
## TypeScript API Reference <span class="version-badge">v1.0.0-rc.1</span>
...
```typescript
function extractSync(input: ExtractInput, config: ExtractionConfig): ExtractionOutput
```

**Example:**

```typescript
const result = extractSync(new ExtractInput(), new ExtractionConfig());
```
...
```typescript
function extractBatchSync(inputs: Array<ExtractInput>, config: ExtractionConfig): ExtractionOutput
```

**Example:**

```typescript
const result = extractBatchSync([], new ExtractionConfig());
```
...
```typescript
const result = ExtractInput.bytes(Buffer.from("data"), "value", "value");
```
...
| `crawl` | `Record<string, unknown>` | `{}` | Optional implementation-specific crawl metadata. |
