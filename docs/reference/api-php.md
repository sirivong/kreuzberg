...
## PHP API Reference <span class="version-badge">v5.0.0-rc.13</span>
...
| `qrCodes` | `?array<QrCode>` | `[]` | QR codes decoded from this image, when QR detection is enabled. Populated by the QR post-processor (`crates/kreuzberg/src/extractors/qr.rs`) via the pure-Rust `rqrr` decoder. `null` when QR detection is disabled; an empty `Some(vec![])` when detection ran but found nothing. |
...
| `links` | `?array<array<string>>` | `null` | Markdown links as (text, URL) tuples (Markdown files only) |
| `codeBlocks` | `?array<array<string>>` | `null` | Code blocks as (language, code) tuples (Markdown files only) |
...
| `codeBlocks` | `?array<array<string>>` | `[]` | Code blocks as (language, code) tuples (for Markdown files) |
