...
```php
public function extractBytes(string $content, string $mimeType, ExtractionConfig $config): string
```
...
```php
public function extractFile(string $path, string $mimeType, ExtractionConfig $config): string
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `coreProperties` | `?mixed` | `null` | Core properties from docProps/core.xml (Dublin Core metadata) Contains title, creator, subject, keywords, dates, etc. Shared format across DOCX/PPTX/XLSX documents. |
| `appProperties` | `?mixed` | `null` | Application properties from docProps/app.xml (Word-specific statistics) Contains word count, page count, paragraph count, editing time, etc. DOCX-specific variant of Office application properties. |
