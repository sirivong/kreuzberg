...
## PHP API Reference <span class="version-badge">v1.0.0-rc.1</span>
...
```php
public static function extractSync(ExtractInput $input, ExtractionConfig $config): ExtractionOutput
```

**Example:**

```php
$result = extractSync(new ExtractInput(), new ExtractionConfig());
```
...
```php
public static function extractBatchSync(array<ExtractInput> $inputs, ExtractionConfig $config): ExtractionOutput
```

**Example:**

```php
$result = extractBatchSync([], new ExtractionConfig());
```
...
```php
public static function extract(ExtractInput $input, ExtractionConfig $config): ExtractionOutput
```
...
```php
public static function extractBatch(array<ExtractInput> $inputs, ExtractionConfig $config): ExtractionOutput
```
...
```php
public static function findFootnoteAnchors(string $markdown): array<FootnoteAnchor>
```
...
```php
public static function parseFootnoteDefinitions(string $markdown): array<FootnoteDefinition>
```
...
```php
public static function findInferenceMarkers(string $markdown): array<int>
```
...
```php
public static function parseCitations(string $markdown): array<Citation>
```
...
```php
public static function chunkForRag(string $text, ChunkingConfig $config): ChunkingResult
```

**Example:**

```php
$result = chunkForRag("value", new ChunkingConfig());
```
...
```php
public static function analyzeDocument(DocumentMetadata $metadata, HeuristicsConfig $config, ?string $documentBytes = null): ChunkingDecision
```

**Example:**

```php
$result = analyzeDocument(new DocumentMetadata(), new HeuristicsConfig(), "data");
```
...
| `documentBytes` | `?string` | No | The document bytes |
...
```php
public static function analyzeWithUserChunks(array<PageRange> $userRanges, int $totalPages, int $sizeBytes, HeuristicsConfig $config): ChunkingDecision
```

**Example:**

```php
$result = analyzeWithUserChunks([], 42, 42, new HeuristicsConfig());
```
...
```php
public static function scoreConfidence(ConfidenceSignals $signals, ConfidenceWeights $weights): ExtractionConfidence
```

**Example:**

```php
$result = scoreConfidence(new ConfidenceSignals(), new ConfidenceWeights());
```
...
```php
public static function checkFormatLimits(string $mimeType, ?int $sheetCount = null, ?int $workbookCells = null, ?int $embeddedCount = null, HeuristicsConfig $config): ?string
```

**Example:**

```php
$result = checkFormatLimits("value", 42, 42, 42, new HeuristicsConfig());
```
...
```php
public static function boundariesFromExtractionResult(ExtractionResult $result, MultidocThresholds $thresholds): array<DocumentBoundary>
```

**Example:**

```php
$result = boundariesFromExtractionResult(new ExtractionResult(), new MultidocThresholds());
```
...
```php
public static function detectBoundaries(MultidocInput $input, MultidocThresholds $thresholds): array<DocumentBoundary>
```

**Example:**

```php
$result = detectBoundaries(new MultidocInput(), new MultidocThresholds());
```
...
```php
public static function chooseCallMode(StructuredInput $input, StructuredThresholds $t): StructuredCallMode
```

**Example:**

```php
$result = chooseCallMode(new StructuredInput(), new StructuredThresholds());
```
...
```php
public static function calculateChunkPlan(int $pageCount, int $sizeBytes, bool $needsOcr, HeuristicsConfig $config): ChunkPlan
```

**Example:**

```php
$result = calculateChunkPlan(42, 42, true, new HeuristicsConfig());
```
...
```php
public static function calculatePlanFromOverrides(array<PageRange> $userChunks, int $totalPages, int $sizeBytes, HeuristicsConfig $config): ChunkPlan
```

**Example:**

```php
$result = calculatePlanFromOverrides([], 42, 42, new HeuristicsConfig());
```
...
```php
public static function resolve(Preset $preset, ?mixed $customSchema = null, array<string, string> $context): ResolvedPreset
```

**Example:**

```php
$result = resolve(new Preset(), [], []);
```
...
| `customSchema` | `?mixed` | No | The custom schema |
| `context` | `array<string, string>` | Yes | The context |
...
```php
public static function extractStructuredJson(string $bytes, string $mime, string $presetSpecJson, string $optionsJson): string
```

**Example:**

```php
$result = extractStructuredJson("data", "value", "value", "value");
```
...
```php
public static function splitAndExtractJson(string $bytes, string $mime, string $presetSpecJson, string $optionsJson): string
```

**Example:**

```php
$result = splitAndExtractJson("data", "value", "value", "value");
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `imageBytes` | `string` | Yes | The image data. |
...
```php
public static function fromExtractionResult(ExtractionResult $result, SchemaCompliance $schemaCompliance, float $textCoverage): ConfidenceSignals
```

**Example:**

```php
$result = ConfidenceSignals::fromExtractionResult(new ExtractionResult(), new SchemaCompliance(), 0.5);
```
...
```php
public static function bytes(string $bytes, string $mimeType, string $filename): ExtractInput
```

**Example:**

```php
$result = ExtractInput::bytes("data", "value", "value");
```
...
| `crawl` | `array<string, mixed>` | `{}` | Optional implementation-specific crawl metadata. |
...
```php
$result = ExtractionOutput::single(new ExtractionResult());
```
...
| `ocrElements` | `?array<OcrElement>` | `/* serde(default) */` | Structured OCR elements with bounding boxes and confidence scores. Available when TSV output is requested or table detection is enabled. |
...
| `imageIndices` | `array<int>` | `/* serde(default) */` | Indices into `ExtractionResult.images` for images found on this page. Each value is a zero-based index into the top-level `images` collection. Only populated when `extract_images = true` in the extraction config. |
...
```php
public static function fromPageText(int $pageNumber, string $text, float $layoutTextDensity): PageSignals
```
...
| `textCoverage` | `float` | — | Fraction of pages with a real text layer (0.0..=1.0). |
| `avgCharsPerPage` | `float` | — | Average extracted characters per page. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `scanMaxCoverage` | `float` | `0.1` | PDFs with `text_coverage` strictly below this are treated as scanned. **Conservative default: 0.10** — deployments override via their own config after measuring their document corpus. |
| `digitalMinCoverage` | `float` | `0.9` | PDFs with `text_coverage` at or above this AND zero embedded images route to `StructuredCallMode::TextOnly`. **Conservative default: 0.90** — deployments override via their own config after measuring their document corpus. |
| `docxTextMinDensity` | `float` | `200` | DOCX / HTML / text documents with `avg_chars_per_page` above this route to `StructuredCallMode::TextOnly`. **Conservative default: 200.0** — deployments override via their own config after measuring their document corpus. |
...
| `OnLowQuality` | Try the classical OCR backend first. If the quality score is below `quality_threshold`, send the page to the VLM. `quality_threshold` is in the `\[0.0, 1.0\]` range produced by `calculate_quality_score`. A value of `0.5` is a reasonable starting point; calibrate with the Stage 0 benchmark harness. — Fields: `qualityThreshold`: `float` |
...
| `Custom` | Use a custom ONNX cross-encoder from HuggingFace. — Fields: `modelId`: `string`, `modelFile`: `string`, `additionalFiles`: `array<string>`, `maxLength`: `int` |
