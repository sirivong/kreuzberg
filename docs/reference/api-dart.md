...
## Dart API Reference <span class="version-badge">v5.0.0-rc.35</span>
...
```dart
final result = await extractBytes(Uint8List.fromList([100, 97, 116, 97]), "value", ExtractionConfig());
```
...
|------|------|----------|-------------|
| `content` | `Uint8List` | Yes | The byte array to extract |
...
```dart
final result = await extractFile("value", "value", ExtractionConfig());
```
...
```dart
final result = extractFileSync("value", "value", ExtractionConfig());
```
...
```dart
final result = batchExtractFilesSync([], ExtractionConfig());
```
...
```dart
final result = await batchExtractFiles([], ExtractionConfig());
```
...
```dart
final result = await batchExtractBytes([], ExtractionConfig());
```
...
```dart
final result = detectMimeTypeFromBytes(Uint8List.fromList([100, 97, 116, 97]));
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `content` | `Uint8List` | Yes | Raw file bytes |
...
```dart
final result = getExtensionsForMime("value");
```
...
```dart
List<SupportedFormat> listSupportedFormats()
```
...
```dart
List<QrCode> detectQrCodes(Uint8List imageBytes, [String? formatHint])
```

**Example:**

```dart
final result = detectQrCodes(Uint8List.fromList([100, 97, 116, 97]), "value");
```
...
```dart
List<String> listRerankerBackends()
```
...
```dart
void classifyPages(ExtractionResult result, PageClassificationConfig config)
```

**Example:**

```dart
await classifyPages(ExtractionResult(), PageClassificationConfig());
```
...
```dart
List<ClassificationLabel> classifyText(String text, PageClassificationConfig config)
```

**Example:**

```dart
final result = await classifyText("value", PageClassificationConfig());
```
...
```dart
List<ClassificationLabel> classifyDocument(List<String> pages, PageClassificationConfig config)
```

**Example:**

```dart
final result = await classifyDocument([], PageClassificationConfig());
```
...
```dart
void redact(ExtractionResult result, RedactionConfig config)
```

**Example:**

```dart
await redact(ExtractionResult(), RedactionConfig());
```
...
```dart
List<PatternMatch> findAll(String text)
```

**Example:**

```dart
final result = findAll("value");
```
...
```dart
List<PatternMatch> scanText(String text, List<PiiCategory> categories)
```

**Example:**

```dart
final result = scanText("value", []);
```
...
```dart
String? summarize(String text, [String? language, int? maxTokens])
```

**Example:**

```dart
final result = summarize("value", "value", 42);
```
...
```dart
void translateResult(ExtractionResult result, TranslationConfig config)
```

**Example:**

```dart
await translateResult(ExtractionResult(), TranslationConfig());
```
...
```dart
List<FootnoteAnchor> findFootnoteAnchors(String markdown)
```

**Example:**

```dart
final result = findFootnoteAnchors("value");
```
...
```dart
List<FootnoteDefinition> parseFootnoteDefinitions(String markdown)
```

**Example:**

```dart
final result = parseFootnoteDefinitions("value");
```
...
```dart
List<int> findInferenceMarkers(String markdown)
```

**Example:**

```dart
final result = findInferenceMarkers("value");
```
...
```dart
List<String> findUnmarkedClaims(String markdown)
```
...
```dart
List<Citation> parseCitations(String markdown)
```

**Example:**

```dart
final result = parseCitations("value");
```
...
```dart
bool verifyExcerpt(String excerpt, String sourceText)
```
...
```dart
ChunkingResult chunkForRag(String text, ChunkingConfig config)
```

**Example:**

```dart
final result = chunkForRag("value", ChunkingConfig());
```
...
```dart
ExtractionDiff compare(ExtractionResult a, ExtractionResult b, DiffOptions opts)
```

**Example:**

```dart
final result = compare(ExtractionResult(), ExtractionResult(), DiffOptions());
```
...
```dart
String extractRegionWithVlm(Uint8List imageBytes, String imageMime, RegionKind regionKind, LlmConfig llmConfig, [String? customPrompt])
```

**Example:**

```dart
final result = await extractRegionWithVlm(Uint8List.fromList([100, 97, 116, 97]), "value", RegionKind(), LlmConfig(), "value");
```
...
```dart
List<Keyword> extractKeywords(String text, KeywordConfig config)
```

**Example:**

```dart
final result = extractKeywords("value", KeywordConfig());
```
...
```dart
ChunkingDecision analyzeDocument(DocumentMetadata metadata, HeuristicsConfig config, [Uint8List? documentBytes])
```

**Example:**

```dart
final result = analyzeDocument(DocumentMetadata(), HeuristicsConfig(), Uint8List.fromList([100, 97, 116, 97]));
```
...
| `documentBytes` | `Uint8List?` | No | The document bytes |
...
```dart
ChunkingDecision analyzeWithUserChunks(List<PageRange> userRanges, int totalPages, int sizeBytes, HeuristicsConfig config)
```

**Example:**

```dart
final result = analyzeWithUserChunks([], 42, 42, HeuristicsConfig());
```
...
```dart
ExtractionConfidence scoreConfidence(ConfidenceSignals signals, ConfidenceWeights weights)
```

**Example:**

```dart
final result = scoreConfidence(ConfidenceSignals(), ConfidenceWeights());
```
...
```dart
String? checkFormatLimits(String mimeType, HeuristicsConfig config, [int? sheetCount, int? workbookCells, int? embeddedCount])
```

**Example:**

```dart
final result = checkFormatLimits("value", 42, 42, 42, HeuristicsConfig());
```
...
| `workbookCells` | `int?` | No | The workbook cells |
...
```dart
List<DocumentBoundary> boundariesFromExtractionResult(ExtractionResult result, MultidocThresholds thresholds)
```

**Example:**

```dart
final result = boundariesFromExtractionResult(ExtractionResult(), MultidocThresholds());
```
...
```dart
List<DocumentBoundary> detectBoundaries(MultidocInput input, MultidocThresholds thresholds)
```

**Example:**

```dart
final result = detectBoundaries(MultidocInput(), MultidocThresholds());
```
...
```dart
StructuredCallMode chooseCallMode(StructuredInput input, StructuredThresholds t)
```

**Example:**

```dart
final result = chooseCallMode(StructuredInput(), StructuredThresholds());
```
...
```dart
ChunkPlan calculateChunkPlan(int pageCount, int sizeBytes, bool needsOcr, HeuristicsConfig config)
```

**Example:**

```dart
final result = calculateChunkPlan(42, 42, true, HeuristicsConfig());
```
...
| `sizeBytes` | `int` | Yes | File size in bytes |
...
```dart
ChunkPlan calculatePlanFromOverrides(List<PageRange> userChunks, int totalPages, int sizeBytes, HeuristicsConfig config)
```

**Example:**

```dart
final result = calculatePlanFromOverrides([], 42, 42, HeuristicsConfig());
```
...
```dart
String fingerprint(Uint8List raw)
```

**Example:**

```dart
final result = fingerprint(Uint8List.fromList([100, 97, 116, 97]));
```
...
```dart
ResolvedPreset resolve(Preset preset, Map<String, String> context, [String? customSchema])
```

**Example:**

```dart
final result = resolve(Preset(), {}, {});
```
...
```dart
String extractStructuredJson(Uint8List bytes, String mime, String presetSpecJson, String optionsJson)
```

**Example:**

```dart
final result = extractStructuredJson(Uint8List.fromList([100, 97, 116, 97]), "value", "value", "value");
```
...
```dart
String splitAndExtractJson(Uint8List bytes, String mime, String presetSpecJson, String optionsJson)
```

**Example:**

```dart
final result = splitAndExtractJson(Uint8List.fromList([100, 97, 116, 97]), "value", "value", "value");
```
...
```dart
final result = renderPdfPageToPng(Uint8List.fromList([100, 97, 116, 97]), 42, 42, "value");
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `pdfBytes` | `Uint8List` | Yes | Raw PDF file bytes |
...
```dart
int pdfPageCount(Uint8List pdfBytes, [String? password])
```

**Example:**

```dart
final result = pdfPageCount(Uint8List.fromList([100, 97, 116, 97]), "value");
```
...
```dart
String captionImage(Uint8List imageBytes, LlmConfig llmConfig, [String? customPrompt])
```

**Example:**

```dart
final result = await captionImage(Uint8List.fromList([100, 97, 116, 97]), LlmConfig(), "value");
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `imageBytes` | `Uint8List` | Yes | The image data. |
...
```dart
String captionImageFile(String path, LlmConfig llmConfig, [String? customPrompt])
```

**Example:**

```dart
final result = await captionImageFile("value", LlmConfig(), "value");
```
...
```dart
final result = detectMimeType("value", true);
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `content` | `Uint8List` | — | The content bytes to extract from |
...
| `entryTypes` | `Map<String, int>?` | `{}` | Count of entries grouped by BibTeX entry type (e.g. `"article"` → 5). |
...
| `totalEstimatedTimeMs` | `int` | `0` | Estimated total processing time in milliseconds. |
...
```dart
static ChunkPlan default()
```

**Example:**

```dart
final result = ChunkPlan.default();
```
...
```dart
final result = instance.totalPages();
```
...
```dart
final result = ChunkingConfig.default();
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `textCoverage` | `double` | — | Fraction of pages with usable text in `\[0, 1\]`. |
| `ocrAggregate` | `double?` | `null` | Mean OCR per-element recognition confidence; `null` when OCR did not run. |
...
```dart
static ConfidenceSignals fromExtractionResult(ExtractionResult result, SchemaCompliance schemaCompliance, double textCoverage)
```

**Example:**

```dart
final result = ConfidenceSignals.fromExtractionResult(ExtractionResult(), SchemaCompliance(), 0.5);
```
...
| `textCoverage` | `double` | Yes | The text coverage |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `textCoverage` | `double` | `0.3` | Weight assigned to `text_coverage`. Default 0.30. |
| `ocrAggregate` | `double` | `0.3` | Weight assigned to `ocr_aggregate` when OCR ran. Default 0.30 — folds into `text_coverage` weight when OCR did not run. |
| `schemaCompliance` | `double` | `0.4` | Weight assigned to `schema_compliance`. Default 0.40. |
...
```dart
static ConfidenceWeights default()
```

**Example:**

```dart
final result = ConfidenceWeights.default();
```
...
```dart
final result = instance.isNormalized();
```
...
```dart
final result = ContentFilterConfig.default();
```
...
```dart
static DiffOptions default()
```

**Example:**

```dart
final result = DiffOptions.default();
```
...
```dart
final result = await instance.extractBytes(Uint8List.fromList([100, 97, 116, 97]), "value", ExtractionConfig());
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `content` | `Uint8List` | Yes | Raw document bytes |
...
```dart
final result = await instance.extractFile("value", "value", ExtractionConfig());
```
...
```dart
final result = DocumentStructure.default();
```
...
```dart
final result = EmbeddingConfig.default();
```
...
| `qrCodes` | `List<QrCode>?` | `\[\]` | QR codes decoded from this image, when QR detection is enabled. Populated by the QR post-processor (`crates/kreuzberg/src/extractors/qr.rs`) via the pure-Rust `rqrr` decoder. `null` when QR detection is disabled; an empty `Some(\[\])` when detection ran but found nothing. |
...
```dart
final result = ExtractionConfig.default();
```
...
```dart
final result = instance.needsImageData();
```
...
```dart
final result = instance.needsImageProcessing();
```
...
```dart
final result = ExtractionResult.fromOcr(OcrExtractionResult());
```
...
```dart
static FootnoteConfig default()
```

**Example:**

```dart
final result = FootnoteConfig.default();
```
...
```dart
FootnoteConfig withParseCitations(bool enabled)
```

**Example:**

```dart
final result = instance.withParseCitations(true);
```
...
```dart
static HeuristicsConfig default()
```

**Example:**

```dart
final result = HeuristicsConfig.default();
```
...
| `ocrCoverageThreshold` | `double?` | `null` | OCR coverage threshold for smart OCR triggering (0.0-1.0) Determines when OCR should be triggered based on text block coverage. OCR is triggered when text blocks cover less than this fraction of the page. Default: 0.5 (trigger OCR if less than 50% of page has text) |
...
```dart
final result = HierarchyConfig.default();
```
...
```dart
final result = HtmlOutputConfig.default();
```
...
```dart
final result = ImageExtractionConfig.default();
```
...
```dart
final result = ImagePreprocessingConfig.default();
```
...
```dart
final result = KeywordConfig.default();
```
...
```dart
final result = LanguageDetectionConfig.default();
```
...
```dart
final result = LayoutDetectionConfig.default();
```
...
```dart
static LlmBackend new(LlmConfig config)
```

**Example:**

```dart
final result = LlmBackend.new(LlmConfig());
```
...
```dart
final result = await instance.detect("value", []);
```
...
```dart
final result = await instance.detectWithCustom("value", [], []);
```
...
```dart
static MetaSchema compile(String metaSchemaJson)
```
...
```dart
Preset parsePreset(String path, Uint8List raw)
```
...
```dart
static MultidocThresholds default()
```

**Example:**

```dart
final result = MultidocThresholds.default();
```
...
```dart
final result = OcrConfig.default();
```
...
```dart
final result = OcrQualityThresholds.default();
```
...
| `detDbThresh` | `double` | — | Database threshold for text detection (default: 0.3) Range: 0.0-1.0, higher values require more confident detections |
| `detDbBoxThresh` | `double` | — | Box threshold for text bounding box refinement (default: 0.5) Range: 0.0-1.0 |
| `detDbUnclipRatio` | `double` | — | Unclip ratio for expanding text bounding boxes (default: 1.6) Controls the expansion of detected text regions |
...
| `dropScore` | `double` | — | Minimum recognition confidence score for text lines (default: 0.5). Text regions with recognition confidence below this threshold are discarded. Matches PaddleOCR Python's `drop_score` parameter. Range: 0.0-1.0 |
...
```dart
final result = instance.withCacheDir("value");
```
...
```dart
final result = instance.withTableDetection(true);
```
...
```dart
final result = instance.withAngleCls(true);
```
...
```dart
final result = instance.withDetDbThresh(0.5);
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `threshold` | `double` | Yes | Detection threshold (0.0-1.0) |
...
```dart
final result = instance.withDetDbBoxThresh(0.5);
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `threshold` | `double` | Yes | Box threshold (0.0-1.0) |
...
```dart
final result = instance.withDetDbUnclipRatio(0.5);
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ratio` | `double` | Yes | Unclip ratio (typically 1.5-2.0) |
...
```dart
final result = instance.withDetLimitSideLen(42);
```
...
```dart
final result = instance.withRecBatchNum(42);
```
...
```dart
final result = instance.withDropScore(0.5);
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `score` | `double` | Yes | Minimum confidence (0.0-1.0), text below this is dropped |
...
```dart
final result = instance.withPadding(42);
```
...
```dart
final result = instance.withModelTier("value");
```
...
```dart
final result = PaddleOcrConfig.default();
```
...
```dart
final result = PageConfig.default();
```
...
```dart
final result = instance.pageCount();
```
...
```dart
static PageSignals fromPageText(int pageNumber, String text, double layoutTextDensity)
```

**Example:**

```dart
final result = PageSignals.fromPageText(42, "value", 0.5);
```
...
| `text` | `String` | Yes | The text |
| `layoutTextDensity` | `double` | Yes | The layout text density |
...
| `topMarginFraction` | `double?` | `null` | Top margin fraction (0.0–1.0) of page height to exclude headers/running heads. Default: 0.06 (6%) |
| `bottomMarginFraction` | `double?` | `null` | Bottom margin fraction (0.0–1.0) of page height to exclude footers/page numbers. Default: 0.05 (5%) |
...
```dart
final result = PdfConfig.default();
```
...
```dart
await instance.process(ExtractionResult(), ExtractionConfig());
```
...
```dart
final result = instance.shouldProcess(ExtractionResult(), ExtractionConfig());
```
...
```dart
final result = instance.estimatedDurationMs(ExtractionResult());
```
...
```dart
final result = PostProcessorConfig.default();
```
...
```dart
final result = RakeParams.default();
```
...
```dart
static RedactionConfig default()
```

**Example:**

```dart
final result = RedactionConfig.default();
```
...
```dart
static RedactionPattern labeled(String label, String pattern)
```

**Example:**

```dart
final result = RedactionPattern.labeled("value", "value");
```
...
```dart
static RedactionTerm literal(String value)
```

**Example:**

```dart
final result = RedactionTerm.literal("value");
```
...
```dart
static RedactionTerm labeled(String label, String value)
```

**Example:**

```dart
final result = RedactionTerm.labeled("value", "value");
```
...
```dart
final result = instance.get("value");
```
...
```dart
Uint8List? sampleBytes(String presetId, String name)
```
...
```dart
final result = instance.render(InternalDocument());
```
...
```dart
static RerankerConfig default()
```

**Example:**

```dart
final result = RerankerConfig.default();
```
...
```dart
final result = SecurityLimits.default();
```
...
```dart
final result = instance.listenAddr();
```
...
```dart
final result = instance.corsAllowsAll();
```
...
```dart
final result = instance.maxRequestBodyMb();
```
...
```dart
final result = instance.maxMultipartFieldMb();
```
...
```dart
static StructuredThresholds default()
```

**Example:**

```dart
final result = StructuredThresholds.default();
```
...
```dart
static SvgOptions default()
```

**Example:**

```dart
final result = SvgOptions.default();
```
...
```dart
final result = TesseractConfig.default();
```
...
```dart
final result = TokenReductionConfig.default();
```
...
```dart
final result = TokenReductionOptions.default();
```
...
| `maxDurationMs` | `int?` | `null` | Hard safety limit on input duration (milliseconds). Files longer than this are rejected after decode, before model work. Default: 30 minutes. Set to `null` to disable (not recommended for untrusted input). |
| `maxBytes` | `int?` | `null` | Hard safety limit on input size (bytes). Default: 512 MiB. Protects against pathological or malicious uploads. |
| `timeoutMs` | `int?` | `null` | Wall-clock timeout for the entire transcription operation (ms). Default: 10 minutes. Reserved for timeout enforcement; the current extractor does not enforce this field yet. |
| `modelCacheDir` | `String?` | `null` | Override the directory used for Whisper model cache. When `null`, uses the centralized resolver: `KREUZBERG_CACHE_DIR/whisper` or the platform default (`~/.cache/kreuzberg/whisper` on Linux, etc.). |
...
```dart
static TranscriptionConfig default()
```

**Example:**

```dart
final result = TranscriptionConfig.default();
```
...
```dart
final result = TreeSitterConfig.default();
```
...
```dart
final result = TreeSitterProcessConfig.default();
```
...
```dart
await instance.validate(ExtractionResult(), ExtractionConfig());
```
...
```dart
final result = instance.shouldValidate(ExtractionResult(), ExtractionConfig());
```
...
```dart
final result = YakeParams.default();
```
...
| `Custom` | Use a custom ONNX cross-encoder from HuggingFace. — Fields: `modelId`: `String`, `modelFile`: `String`, `additionalFiles`: `List<String>`, `maxLength`: `int` |
