...
## Dart API Reference <span class="version-badge">v1.0.0-rc.1</span>
...
```dart
ExtractionOutput extractSync(ExtractInput input, ExtractionConfig config)
```

**Example:**

```dart
final result = extractSync(ExtractInput(), ExtractionConfig());
```
...
```dart
ExtractionOutput extractBatchSync(List<ExtractInput> inputs, ExtractionConfig config)
```

**Example:**

```dart
final result = extractBatchSync([], ExtractionConfig());
```
...
```dart
ExtractionOutput extract(ExtractInput input, ExtractionConfig config)
```
...
```dart
ExtractionOutput extractBatch(List<ExtractInput> inputs, ExtractionConfig config)
```
...
```dart
static ExtractInput default()
```

**Example:**

```dart
final result = ExtractInput.default();
```
...
```dart
static ExtractInput bytes(Uint8List bytes, String mimeType, [String? filename])
```

**Example:**

```dart
final result = ExtractInput.bytes(Uint8List.fromList([100, 97, 116, 97]), "value", "value");
```
...
```dart
static ExtractInput uri(String uri)
```

**Example:**

```dart
final result = ExtractInput.uri("value");
```
...
| `crawl` | `Map<String, String>` | `{}` | Optional implementation-specific crawl metadata. |
...
```dart
static ExtractionOutput single(ExtractionResult result)
```

**Example:**

```dart
final result = ExtractionOutput.single(ExtractionResult());
```
...
```dart
static UrlExtractionConfig default()
```

**Example:**

```dart
final result = UrlExtractionConfig.default();
```
