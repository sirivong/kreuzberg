...
## Elixir API Reference <span class="version-badge">v1.0.0-rc.8</span>
...
```elixir
@spec register_document_extractor(extractor) :: {:ok, term()} | {:error, term()}
def register_document_extractor(extractor)
```

**Example:**

```elixir
:ok = register_document_extractor(%{{}})
```
...
```elixir
@spec unregister_document_extractor(name) :: {:ok, term()} | {:error, term()}
def unregister_document_extractor(name)
```

**Example:**

```elixir
:ok = unregister_document_extractor("value")
```
...
```elixir
@spec register_renderer(renderer) :: {:ok, term()} | {:error, term()}
...
```elixir
@spec unregister_renderer(name) :: {:ok, term()} | {:error, term()}
def unregister_renderer(name)
```

**Example:**

```elixir
:ok = unregister_renderer("value")
```
