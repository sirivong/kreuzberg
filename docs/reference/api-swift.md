...
## Swift API Reference <span class="version-badge">v1.0.0-rc.1</span>
...
```swift
public static func extractSync(input: ExtractInput, config: ExtractionConfig) throws -> ExtractionOutput
```

**Example:**

```swift
let result = try extractSync(ExtractInput(), ExtractionConfig())
```
...
```swift
public static func extractBatchSync(inputs: [ExtractInput], config: ExtractionConfig) throws -> ExtractionOutput
```

**Example:**

```swift
let result = try extractBatchSync([], ExtractionConfig())
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `entryCount` | `UInt64` | — | Number of entries in the bibliography. |
...
| `entryTypes` | `\[String: UInt64\]?` | `{}` | Count of entries grouped by BibTeX entry type (e.g. `"article"` → 5). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `totalFiles` | `UInt64` | — | Total number of files currently in the cache directory. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `row` | `UInt64` | — | Zero-based row index. |
| `col` | `UInt64` | — | Zero-based column index. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `byteStart` | `UInt64` | — | Byte offset where this chunk starts in the original text (UTF-8 valid boundary). |
| `byteEnd` | `UInt64` | — | Byte offset where this chunk ends in the original text (UTF-8 valid boundary). |
| `tokenCount` | `UInt64?` | `null` | Number of tokens in this chunk (if available). This is calculated by the embedding model's tokenizer if embeddings are enabled. |
| `chunkIndex` | `UInt64` | — | Zero-based index of this chunk in the document. |
...
```swift
public static func default() -> ChunkPlan
```
...
```swift
public func totalPages() -> UInt32
```

**Example:**

```swift
let result = instance.totalPages()
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `maxCharacters` | `UInt64` | `1000` | Maximum size per chunk (in units determined by `sizing`). When `sizing` is `Characters` (default), this is the max character count. When using token-based sizing, this is the max token count. Default: 1000 |
| `overlap` | `UInt64` | `200` | Overlap between chunks (in units determined by `sizing`). Default: 200 |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `citationCount` | `UInt64` | — | Total number of citation records in the file. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `recordCount` | `UInt64` | — | Total number of data records in the DBF file. |
| `fieldCount` | `UInt64` | — | Number of field (column) definitions. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `fromLine` | `UInt64` | — | Starting line number in the old content (0-indexed). |
| `fromCount` | `UInt64` | — | Number of lines from the old content in this hunk. |
| `toLine` | `UInt64` | — | Starting line number in the new content (0-indexed). |
| `toCount` | `UInt64` | — | Number of lines from the new content in this hunk. |
...
| `elementIndex` | `UInt64?` | `null` | Position index in the element sequence |
...
| `size` | `UInt64?` | `null` | Size in bytes |
...
| `compressedSize` | `UInt64` | — | Compressed byte count of the original stream (before decompression). Used by callers to compute the decompression ratio and detect zip-bomb-style attacks that embed a tiny compressed stream expanding to gigabytes of data. |
...
| `batchSize` | `UInt64` | `32` | Batch size for embedding generation |
| `showDownloadProgress` | `Bool` | `false` | Show model download progress |
| `cacheDir` | `URL?` | `null` | Custom cache directory for model files Defaults to `~/.cache/xberg/embeddings/` if not specified. Allows full customization of model download location. |
...
| `labels` | `\[String\]` | `\[\]` | Custom labels to pass through to the result without modification. These are caller-supplied tags that the enrichment pipeline propagates verbatim into `EnrichResult.labels`. Useful for attaching project- or document-level metadata to every enrichment result. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `keywords` | `\[String\]` | `\[\]` | Salient terms extracted from the text. Populated when `EnrichOptions.keywords` was `true`. The ordering is backend-defined (typically by descending relevance score). |
| `entities` | `\[Entity\]` | `\[\]` | Named entities found in the text. Populated when `EnrichOptions.entities` was `true`. Uses the shared OSS entity schema (`Entity` / `EntityCategory`) so consumers can pattern-match on entity categories without JSON gymnastics. |
| `labels` | `\[String\]` | `\[\]` | Caller-supplied labels echoed from `EnrichOptions.labels`. |
...
| `rowCount` | `UInt64` | — | Number of rows |
| `colCount` | `UInt64` | — | Number of columns |
| `cellCount` | `UInt64` | — | Total number of non-empty cells |
...
```swift
public static func default() -> ExtractInput
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `index` | `UInt64` | — | Input index in the original request. |
...
| `crawl` | `\[String: String\]` | `{}` | Optional implementation-specific crawl metadata. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `inputs` | `UInt64` | — | Number of inputs submitted by the caller. |
| `results` | `UInt64` | — | Number of extraction results produced. |
| `errors` | `UInt64` | — | Number of per-input errors. |
| `remoteUrls` | `UInt64` | — | Number of URI inputs that resolved to remote HTTP(S) URLs. |
| `pagesCrawled` | `UInt64` | — | Number of HTML pages crawled or scraped. |
| `documentsDownloaded` | `UInt64` | — | Number of downloaded non-HTML documents extracted from URLs. |
...
| `offset` | `UInt64` | — | Byte offset of the anchor in the markdown text. |
...
```swift
public static func default() -> FootnoteConfig
```
...
```swift
public func withParseCitations(enabled: Bool) -> FootnoteConfig
```

**Example:**

```swift
let result = instance.withParseCitations(true)
```
...
| `offset` | `UInt64` | — | Byte offset of the definition line in the markdown text. |
...
```swift
public static func default() -> HeuristicsConfig
```
...
| `kClusters` | `UInt64` | `3` | Number of font size clusters to use for hierarchy levels (1-7) Default: 6, which provides H1-H6 heading levels with body text. Larger values create more fine-grained hierarchy levels. |
...
| `timeoutSecs` | `UInt64?` | `null` | Request timeout in seconds (default: 60). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `detModel` | `URL` | — | Path to the detection model directory. |
| `clsModel` | `URL` | — | Path to the classification model directory. |
| `recModel` | `URL` | — | Path to the recognition model directory. |
...
```swift
public static func default() -> MultidocThresholds
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `minTotalNonWhitespace` | `UInt64` | `64` | Minimum total non-whitespace characters to consider text substantive. |
| `minNonWhitespacePerPage` | `Double` | `32` | Minimum non-whitespace characters per page on average. |
| `minMeaningfulWordLen` | `UInt64` | `4` | Minimum character count for a word to be "meaningful". |
| `minMeaningfulWords` | `UInt64` | `3` | Minimum count of meaningful words before text is accepted. |
| `minAlnumRatio` | `Double` | `0.3` | Minimum alphanumeric ratio (non-whitespace chars that are alphanumeric). |
| `minGarbageChars` | `UInt64` | `5` | Minimum Unicode replacement characters (U+FFFD) to trigger OCR fallback. |
...
| `minWordsForAvgLengthCheck` | `UInt64` | `50` | Minimum word count before average word length check applies. |
| `minConsecutiveRepeatRatio` | `Double` | `0.08` | Minimum consecutive word repetition ratio to detect column scrambling. |
| `minWordsForRepeatCheck` | `UInt64` | `50` | Minimum word count before consecutive repetition check is applied. |
| `substantiveMinChars` | `UInt64` | `100` | Minimum character count for "substantive markdown" OCR skip gate. |
| `nonTextMinChars` | `UInt64` | `20` | Minimum character count for "non-text content" OCR skip gate. |
...
| `cacheDir` | `URL?` | `null` | Optional custom cache directory for model files |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `byteStart` | `UInt64` | — | Byte offset where this page starts in the content string (UTF-8 valid boundary, inclusive) |
| `byteEnd` | `UInt64` | — | Byte offset where this page ends in the content string (UTF-8 valid boundary, exclusive) |
| `pageNumber` | `UInt32` | — | Page number (1-indexed) |
...
```swift
public func pageCount() -> UInt32
```

**Example:**

```swift
let result = instance.pageCount()
```
...
```swift
public static func fromPageText(pageNumber: UInt32, text: String, layoutTextDensity: Float) -> PageSignals
```

**Example:**

```swift
let result = PageSignals.fromPageText(42, "value", 0.5)
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `start` | `UInt64` | — | Inclusive byte-offset start of the match in the source text. |
| `end` | `UInt64` | — | Exclusive byte-offset end of the match. |
...
| `slideCount` | `UInt64` | — | Total number of slides |
| `imageCount` | `UInt64` | — | Total number of embedded images |
| `tableCount` | `UInt64` | — | Total number of tables |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `messageCount` | `UInt64` | — | Total number of email messages found in the PST archive. |
...
```swift
public func extendFromDir(dir: URL) throws -> UInt64
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `index` | `UInt64` | — | Position of this document in the original input `documents` slice. |
...
| `topK` | `UInt64?` | `null` | Return at most this many documents. `null` returns all. Applied after sorting by score, so the highest-scoring documents are kept. |
| `batchSize` | `UInt64` | `32` | Batch size for local ONNX cross-encoder inference. |
| `showDownloadProgress` | `Bool` | `false` | Show model download progress (local ONNX path only). |
| `cacheDir` | `URL?` | `null` | Custom cache directory for model files. Defaults to `~/.cache/xberg/rerankers/` if not specified. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `maxArchiveSize` | `UInt64` | `524288000` | Maximum uncompressed size for archives (500 MB) |
| `maxCompressionRatio` | `UInt64` | `100` | Maximum compression ratio before flagging as potential bomb (100:1) |
| `maxFilesInArchive` | `UInt64` | `10000` | Maximum number of files in archive (10,000) |
| `maxNestingDepth` | `UInt64` | `1024` | Maximum nesting depth for structures (100) |
| `maxEntityLength` | `UInt64` | `1048576` | Maximum length of any single XML entity / attribute / token (1 MiB). This is a per-token cap, NOT a total cap — billion-laughs class attacks where a single entity expands to hundreds of MB are caught here, while normal long text content (a paragraph, a CDATA block) is caught by `max_content_size` instead. |
| `maxContentSize` | `UInt64` | `104857600` | Maximum string growth per document (100 MB) |
| `maxIterations` | `UInt64` | `10000000` | Maximum iterations per operation |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `fromIndex` | `UInt64` | — | Zero-based index of the table in both `a.tables` and `b.tables`. |
| `toIndex` | `UInt64` | — | Zero-based index in `b.tables` (equal to `from_index` for same-dimension tables). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `language` | `\[String\]` | `\[\]` | Language code(s) for OCR recognition. Accepts either a single language code ("eng") or a list (\["eng", "deu"\]). For Tesseract backend, languages are joined with "+". |
...
| `lineCount` | `UInt64` | — | Number of lines |
...
| `maxDurationMs` | `UInt64?` | `null` | Hard safety limit on input duration (milliseconds). Files longer than this are rejected after decode, before model work. Default: 30 minutes. Set to `null` to disable (not recommended for untrusted input). |
| `maxBytes` | `UInt64?` | `null` | Hard safety limit on input size (bytes). Default: 512 MiB. Protects against pathological or malicious uploads. |
| `timeoutMs` | `UInt64?` | `null` | Wall-clock timeout for the entire transcription operation (ms). Default: 10 minutes. Reserved for timeout enforcement; the current extractor does not enforce this field yet. |
| `modelCacheDir` | `URL?` | `null` | Override the directory used for Whisper model cache. When `null`, uses the centralized resolver: `XBERG_CACHE_DIR/whisper` or the platform default (`~/.cache/xberg/whisper` on Linux, etc.). |
...
| `cacheDir` | `URL?` | `null` | Custom cache directory for downloaded grammars. When `null`, uses the default: `~/.cache/tree-sitter-language-pack/v{version}/libs/`. |
| `languages` | `\[String\]?` | `null` | Languages to pre-download on init (e.g., `\["python", "rust"\]`). |
...
| `maxDocumentUrlsPerResult` | `UInt32?` | `null` | Maximum URLs to follow per extraction result. |
| `maxTotalUrls` | `UInt32?` | `null` | Maximum URLs followed across the whole extraction call. |
...
```swift
public static func default() -> UrlExtractionConfig
```
...
| `elementCount` | `UInt64` | — | Total number of XML elements processed |
...
| `Tokenizer` | Size measured in tokens from a HuggingFace tokenizer. — Fields: `model`: `String`, `cacheDir`: `URL` |
...
| `Custom` | Use a custom ONNX model from HuggingFace — Fields: `modelId`: `String`, `dimensions`: `UInt64` |
...
| Value | Description |
|-------|-------------|
| `Paragraph` | Body paragraph, identified by its zero-based index in the document flow. — Fields: `index`: `UInt64` |
| `TableCell` | Cell inside a table. — Fields: `row`: `UInt64`, `col`: `UInt64`, `tableIndex`: `UInt64` |
| `Page` | Page, identified by its zero-based index. — Fields: `index`: `UInt64` |
| `Slide` | Presentation slide, identified by its zero-based index. — Fields: `index`: `UInt64` |
| `Sheet` | Spreadsheet cell or range, identified by sheet index and optional name. — Fields: `index`: `UInt64`, `name`: `String` |
