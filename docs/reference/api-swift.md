...
## Swift API Reference <span class="version-badge">v5.0.0-rc.1</span>
...
|------|------|----------|-------------|
| `content` | `Data` | Yes | The byte array to extract |
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `path` | `URL` | Yes | Path to the file to extract |
...
| `relativeHeight` | `Int64?` | `null` | Relative height |
...
| `fileCount` | `UInt64` | — | Total number of files in the archive |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `content` | `Data` | — | The content bytes to extract from |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `path` | `URL` | — | Path to the file to extract from |
...
| `citationKeys` | `[String]` | `[]` | Citation keys |
| `authors` | `[String]` | `[]` | Authors |
| `yearRange` | `YearRange?` | `null` | Year range (year range) |
| `entryTypes` | `[String: UInt64]?` | `{}` | Entry types |
...
| `firstPage` | `UInt64?` | `null` | First page number this chunk spans (1-indexed). Only populated when page tracking is enabled in extraction configuration. |
| `lastPage` | `UInt64?` | `null` | Last page number this chunk spans (1-indexed, equal to first_page for single-page chunks). Only populated when page tracking is enabled in extraction configuration. |
...
| `chunkCount` | `UInt64` | — | Total number of chunks |
| `config` | `String` | — | Configuration used for chunking |
| `inputSizeBytes` | `UInt64` | — | Input text size in bytes |
...
| `maxCharacters` | `UInt64?` | `null` | Maximum characters per chunk (default: 2000) |
| `overlap` | `UInt64?` | `null` | Number of overlapping characters between chunks (default: 100) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `citationCount` | `UInt64` | — | Number of citations |
...
| `columnTypes` | `[String]?` | `[]` | Column types |
...
| `fieldCount` | `UInt64` | — | Number of fields |
| `fields` | `[DbfFieldInfo]` | `[]` | Fields |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `byteOffset` | `UInt64` | — | Byte offset of the start of the line in the original text. |
...
| `attributes` | `[String: String]?` | `null` | Format-specific key-value attributes. Extensible bag for data that doesn't warrant a typed field: CSS classes, LaTeX environment names, Excel cell formulas, slide layout names, etc. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `pageNumber` | `UInt64?` | `null` | Page number (1-indexed) |
...
| `dimensions` | `UInt64` | — | Dimensionality of the embeddings |
| `count` | `UInt64` | — | Number of embeddings generated |
...
| `data` | `Data` | — | Raw file bytes from the embedded stream. |
...
| `cacheDir` | `URL?` | `null` | Custom cache directory for model files Defaults to `~/.cache/kreuzberg/embeddings/` if not specified. Allows full customization of model download location. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `sheetCount` | `UInt64?` | `null` | Number of sheets in the workbook. |
...
| `imageIndex` | `UInt64` | — | Zero-indexed position of this image in the document/page |
| `pageNumber` | `UInt64?` | `null` | Page/slide number where image was found (1-indexed) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `data` | `Data` | — | Uses `bytes.Bytes` for cheap cloning of large buffers. |
| `format` | `String` | — | Format |
| `filename` | `String?` | `null` | Filename |
| `description` | `String?` | `null` | Human-readable description |
| `dimensions` | `[UInt32]?` | `null` | Dimensions |
...
| `forceOcrPages` | `[UInt64]?` | `null` | Force OCR on specific pages only (1-indexed page numbers, must be >= 1). When set, only the listed pages are OCR'd regardless of text layer quality. Unlisted pages use native text extraction. Ignored when `force_ocr` is `true`. Only applies to PDF documents. Duplicates are automatically deduplicated. An `ocr` config is recommended for backend/language selection; defaults are used if absent. |
...
| `extractionTimeoutSecs` | `UInt64?` | `null` | Default per-file timeout in seconds for batch extraction. When set, each file in a batch will be canceled after this duration unless overridden by `FileExtractionConfig.timeout_secs`. `null` means no timeout (unbounded extraction time). |
...
| `tables` | `[Table]` | `[]` | Tables extracted from the document |
...
| `uris` | `[Uri]?` | `[]` | URIs/links discovered during document extraction. Contains hyperlinks, image references, citations, email addresses, and other URI-like references found in the document. Always extracted when present in the source document. |
...
| `llmUsage` | `[LlmUsage]?` | `[]` | LLM token usage and cost data for all LLM calls made during this extraction. Contains one entry per LLM call. Multiple entries are produced when VLM OCR, structured extraction, and/or LLM embeddings all run during the same extraction. `null` when no LLM was used. |
...
| `forceOcrPages` | `[UInt64]?` | `[]` | Override force OCR pages for this file (1-indexed page numbers). |
...
| `boundaries` | `[PageBoundary]?` | `null` | Character byte boundaries per frame (for multi-frame TIFFs) |
| `pageContents` | `[PageContent]?` | `null` | Per-frame content information |
...
| `historyDates` | `[String: String]` | `{}` | History dates |
| `contributorRoles` | `[ContributorRole]` | `[]` | Contributor roles |
...
| `modelCount` | `UInt64` | — | Number of models in the manifest |
...
|-------|------|---------|-------------|
| `text` | `String` | — | Text |
| `byteStart` | `UInt64` | — | Byte start |
| `byteEnd` | `UInt64` | — | Byte end |
...
| `pageNumber` | `UInt64` | — | Page number (1-indexed). |
...
| `ocrElements` | `[OcrElement]?` | `null` | Structured OCR elements with bounding boxes and confidence scores. Available when TSV output is requested or table detection is enabled. |
...
| `tableCount` | `UInt64` | — | Number of tables detected |
| `tableRows` | `UInt64?` | `null` | Table rows |
| `tableCols` | `UInt64?` | `null` | Table cols |
...
| `pageNumber` | `UInt64` | — | Page number where the table was found (1-indexed) |
...
| `pageCount` | `Int32?` | `null` | Document statistics - page count (meta:page-count) |
| `wordCount` | `Int32?` | `null` | Document statistics - word count (meta:word-count) |
| `characterCount` | `Int32?` | `null` | Document statistics - character count (meta:character-count) |
| `paragraphCount` | `Int32?` | `null` | Document statistics - paragraph count (meta:paragraph-count) |
| `tableCount` | `Int32?` | `null` | Document statistics - table count (meta:table-count) |
| `imageCount` | `Int32?` | `null` | Document statistics - image count (meta:image-count) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `blockCount` | `UInt64` | — | Number of hierarchy blocks on this page |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `number` | `UInt64` | — | Page number (1-indexed) |
...
| `imageCount` | `UInt64?` | `null` | Number of images on this page |
| `tableCount` | `UInt64?` | `null` | Number of tables on this page |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `totalCount` | `UInt64` | — | Total number of pages/slides/sheets |
...
| `pageNumber` | `UInt64` | — | Page number where the annotation appears (1-indexed). |
...
| `officeMetadata` | `[String: String]` | — | Office metadata extracted from docProps/core.xml and docProps/app.xml. Contains keys like "title", "author", "created_by", "subject", "keywords", "modified_by", "created_at", "modified_at", etc. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `slideCount` | `UInt64` | — | Total number of slides in the presentation |
| `slideNames` | `[String]` | `[]` | Names of slides (if available) |
| `imageCount` | `UInt64?` | `null` | Number of embedded images |
| `tableCount` | `UInt64?` | `null` | Number of tables |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `messageCount` | `UInt64` | — | Number of messages |
...
| `maxEntityLength` | `UInt64` | `1048576` | Maximum length of any single XML entity / attribute / token (1 MiB). This is a per-token cap, NOT a cumulative cap — billion-laughs class attacks where a single entity expands to hundreds of MB are caught here, while normal long text content (a paragraph, a CDATA block) is caught by `max_content_size` instead. |
...
| `content` | `String` | — | The extracted text content |
| `format` | `String` | — | Format |
| `metadata` | `[String: String]` | — | Document metadata |
...
| `rowSpan` | `UInt64` | — | Row span (number of rows this cell spans) |
| `colSpan` | `UInt64` | — | Column span (number of columns this cell spans) |
| `isHeader` | `Bool` | — | Whether this is a header cell |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `lineCount` | `UInt64` | — | Number of lines in the document |
| `wordCount` | `UInt64` | — | Number of words |
| `characterCount` | `UInt64` | — | Number of characters |
...
| `customStopwords` | `[String: [String]]?` | `null` | Custom stopwords |
