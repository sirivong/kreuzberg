...
async function benchmarkExtractions(): Promise<void> {
  ...
  for (let i = 0; i < numRuns; i++) {
    await extract({ kind: "uri", uri: filePath }, config);
  }
  ...
  const tasks = Array(numRuns)
    .fill(null)
    .map(() => extract({ kind: "uri", uri: filePath }, config));
  ...
  start = performance.now();
  const _result1 = await extract({ kind: "uri", uri: filePath }, cacheConfig);
  ...
  start = performance.now();
  const _result2 = await extract({ kind: "uri", uri: filePath }, cacheConfig);
