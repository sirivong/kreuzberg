...
RT-DETR also runs through xberg's pure-Rust `tract` engine (no native ONNX Runtime library, CPU-only), matching ONNX Runtime within 5e-3 on its outputs. This is a step toward layout detection on targets that cannot link native ONNX Runtime (WASM, the Android x86_64 emulator); it is not yet enabled for those targets. PP-DocLayout-V3, TATR, and SLANeXT table structure models remain ONNX Runtime-only.
:::
