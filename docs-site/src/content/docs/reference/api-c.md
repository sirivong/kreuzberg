...
## C API Reference <span class="version-badge">v1.0.0-rc.34</span>
...
```c
XbergExtractionResult* xberg_extract(XbergExtractInput input, XbergExtractionConfig config);
```

**Example:**

```c
XbergExtractionResult *result = xberg_extract((XbergExtractInput){0}, NULL);
```
...
```c
XbergExtractionResult* xberg_extract_batch(XbergExtractInput* inputs, XbergExtractionConfig config);
```

**Example:**

```c
XbergExtractionResult *result = xberg_extract_batch(NULL, NULL);
```
...
```c
XbergMapResult* xberg_map_url(const char* uri, XbergUrlExtractionConfig config);
```

**Example:**

```c
XbergMapResult *result = xberg_map_url("value", NULL);
```
...
| `config` | `XbergUrlExtractionConfig` | Yes | The configuration options |

**Returns:** `XbergMapResult`
...
```c
void xberg_clear_tokenizer_backends();
```
...
```c
void xberg_classify_chunks(XbergExtractedDocument result, XbergChunkClassificationConfig config);
```

**Example:**

```c
xberg_classify_chunks(NULL, NULL);
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `result` | `XbergExtractedDocument` | Yes | The extracted document |
| `config` | `XbergChunkClassificationConfig` | Yes | The configuration options |
...
```c
XbergSparseEmbedding* xberg_embed_sparse_async(const char** texts, XbergSparseEmbeddingConfig config);
```

**Example:**

```c
XbergSparseEmbedding* result = xberg_embed_sparse_async(NULL, NULL);
```
...
| `config` | `XbergSparseEmbeddingConfig` | Yes | The configuration options |

**Returns:** `XbergSparseEmbedding*`
...
```c
double xberg_max_sim_score(XbergMultiVectorEmbedding query, XbergMultiVectorEmbedding doc);
```

**Example:**

```c
double result = xberg_max_sim_score(NULL, NULL);
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `query` | `XbergMultiVectorEmbedding` | Yes | The multi vector embedding |
| `doc` | `XbergMultiVectorEmbedding` | Yes | The multi vector embedding |
...
```c
XbergLateInteractionMatch* xberg_max_sim_rank(XbergMultiVectorEmbedding query, XbergMultiVectorEmbedding* docs);
```

**Example:**

```c
XbergLateInteractionMatch* result = xberg_max_sim_rank(NULL, NULL);
```
...
|------|------|----------|-------------|
| `query` | `XbergMultiVectorEmbedding` | Yes | The multi vector embedding |
| `docs` | `XbergMultiVectorEmbedding*` | Yes | The docs |

**Returns:** `XbergLateInteractionMatch*`
...
```c
XbergMultiVectorEmbedding* xberg_embed_multi_vector_async(const char** texts, XbergLateInteractionConfig config, bool is_query);
```

**Example:**

```c
XbergMultiVectorEmbedding* result = xberg_embed_multi_vector_async(NULL, NULL, true);
```
...
|------|------|----------|-------------|
| `texts` | `const char**` | Yes | The texts |
| `config` | `XbergLateInteractionConfig` | Yes | The configuration options |
| `is_query` | `bool` | Yes | The is query |

**Returns:** `XbergMultiVectorEmbedding*`
...
|------|------|----------|-------------|
| `texts` | `const char**` | Yes | The  texts |
| `config` | `XbergSparseEmbeddingConfig` | Yes | The sparse embedding config |
...
|------|------|----------|-------------|
| `texts` | `const char**` | Yes | The  texts |
| `config` | `XbergLateInteractionConfig` | Yes | The late interaction config |
| `is_query` | `bool` | Yes | The  is query |
...
| `result` | `XbergExtractedDocument` | — | Full extraction result for this file. |
...
| `definitions` | `XbergChunkClassificationDefinition*` | — | The set of label definitions the classifier may emit. Must contain at least one entry. |
| `llm` | `XbergLlmConfig` | — | LLM configuration used for classification. |
| `batch_size` | `uintptr_t` | `serde(default = "default_batch_size")` | Number of chunks batched into a single LLM request. Larger batches amortize the fixed prompt cost (definitions block) across more chunks, at the risk of exceeding the model's context window for very large taxonomies or chunk texts. Defaults to `DEFAULT_BATCH_SIZE`. |
| `max_concurrency` | `uintptr_t` | `serde(default = "default_max_concurrency")` | Maximum number of in-flight batch requests. Bounds concurrency against the configured LLM provider. Defaults to `DEFAULT_MAX_CONCURRENCY`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `config` | `XbergChunkClassificationConfig` | — | Label-definition set and LLM/batching settings for the chunk-classification stage. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `kind` | `XbergCodeDataNodeKind` | — | Whether this node is a key/value pair, XML element, or sequence item. |
...
| `attributes` | `XbergCodeDataAttribute*` | `/* serde(default) */` | Attributes on element-shape nodes (XML `STag` attributes). Empty for all other kinds. |
| `children` | `XbergCodeDataNode*` | `/* serde(default) */` | Children for nested containers and XML element bodies. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `chunks` | `XbergCodeChunkInfo*` | `NULL` | Structural code chunks (function/class/module boundaries). |
| `data` | `XbergCodeDataNode*` | `NULL` | Hierarchical key/value data tree extracted from data-format source (JSON, YAML, TOML, XML, CSV, etc.), when data extraction was enabled. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `heading_style` | `XbergHeadingStyle` | `XBERG_XBERG_ATX` | Heading style to use in Markdown output (ATX `#` or Setext underline). |
| `list_indent_type` | `XbergListIndentType` | `XBERG_XBERG_SPACES` | How to indent nested list items (spaces or tab). |
...
| `highlight_style` | `XbergHighlightStyle` | `XBERG_XBERG_DOUBLE_EQUAL` | Style used for `<mark>` / highlighted text (e.g. `==text==`). |
...
```c
XbergExtractedDocument xberg_extract(XbergExtractInput input, XbergExtractionConfig config);
```

**Example:**

```c
XbergExtractedDocument *result = xberg_extract(instance, (XbergExtractInput){0}, NULL);
```
...
| `changed` | `XbergEmbeddedDiff*` | `NULL` | Children present in both but with differing content (matched by `path`). Each entry holds the diff of the nested `ExtractedDocument`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `model` | `XbergEmbeddingModelType` | `XBERG_XBERG_PRESET` | The embedding model to use (defaults to "gte-modernbert-base" preset if not specified) |
...
```c
XbergExtractInput xberg_from_bytes(const uint8_t* bytes, const char* mime_type, const char* filename);
```

**Example:**

```c
XbergExtractInput *result = xberg_from_bytes((const uint8_t *)"data", "value", "value");
```
...
```c
XbergExtractInput xberg_from_uri(const char* uri);
```

**Example:**

```c
XbergExtractInput *result = xberg_from_uri("value");
```
...
| `counts` | `XbergDocumentCounts` | — | Cheap structural counts (pages, tables, images). Always populated by the extraction pipeline, even when the `pages` / `images` collections are `NULL`. See `DocumentCounts`. |
...
| `ocr_result` | `XbergExtractedDocument*` | `NULL` | Nested OCR extraction result (if image was OCRed) When OCR is performed on this image, the result is embedded here rather than in a separate collection, making the relationship explicit. |
...
| `ocr_strategy` | `XbergOcrStrategy` | `XBERG_XBERG_AUTO` | Which pages get OCR'd when neither `force_ocr` nor `force_ocr_pages` applies. Defaults to `OcrStrategy.Auto`, which OCRs only pages whose native text fails a quality check. Only applies to PDF documents. Cannot be `OcrStrategy.ScannedPages` while `disable_ocr` is `true`. |
...
| `html_options` | `XbergConversionOptions*` | `NULL` | HTML to Markdown conversion options (None = use defaults) Configure how HTML documents are converted to Markdown, including heading styles, list formatting, code block styles, and preprocessing options. |
...
| `structured_extraction` | `XbergStructuredExtractionConfig*` | `NULL` | Structured extraction via LLM (None = disabled). When set, the extracted document content is sent to an LLM with the provided JSON schema. The structured response is stored in `ExtractedDocument.structured_output`. |
| `ner` | `XbergNerConfig*` | `NULL` | Named-entity recognition configuration. When set, the NER post-processor runs at the Middle stage and populates `ExtractedDocument.entities`. |
| `redaction` | `XbergRedactionConfig*` | `NULL` | Redaction / anonymisation configuration. When set, the redaction post-processor runs at the Late stage and rewrites every textual field in `ExtractedDocument`, emitting an audit trail in `ExtractedDocument.redaction_report`. |
| `summarization` | `XbergSummarizationConfig*` | `NULL` | Summarisation configuration. When set, the summarisation post-processor runs at the Middle stage and populates `ExtractedDocument.summary`. |
| `translation` | `XbergTranslationConfig*` | `NULL` | Translation configuration. When set, the translation post-processor runs at the Middle stage and populates `ExtractedDocument.translation`. |
| `page_classification` | `XbergPageClassificationConfig*` | `NULL` | Per-page classification configuration. When set, the classification post-processor runs at the Middle stage and populates `ExtractedDocument.page_classifications`. |
| `chunk_classification` | `XbergChunkClassificationConfig*` | `NULL` | Per-chunk multi-label classification configuration. When set, the chunk-classification post-processor runs at the Middle stage (after chunking) and populates `ChunkMetadata.classifications` on every chunk. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `results` | `XbergExtractedDocument*` | `NULL` | Extracted documents in discovery order. |
...
```c
XbergExtractionResult xberg_single(XbergExtractedDocument result);
```

**Example:**

```c
XbergExtractionResult *result = xberg_single((XbergExtractedDocument){0});
```
...
| `ocr_strategy` | `XbergOcrStrategy*` | `NULL` | Override the OCR page-selection strategy for this file. |
...
| `html_output` | `XbergHtmlOutputConfig*` | `NULL` | Override styled HTML output configuration for this file. |
...
| `url` | `XbergUrlExtractionConfig*` | `NULL` | Override URL ingestion and crawl configuration for this file. |
| `ner` | `XbergNerConfig*` | `NULL` | Override named-entity recognition configuration for this file. |
| `redaction` | `XbergRedactionConfig*` | `NULL` | Override redaction configuration for this file. |
| `summarization` | `XbergSummarizationConfig*` | `NULL` | Override summarization configuration for this file. |
| `translation` | `XbergTranslationConfig*` | `NULL` | Override translation configuration for this file. |
| `page_classification` | `XbergPageClassificationConfig*` | `NULL` | Override per-page classification configuration for this file. |
| `chunk_classification` | `XbergChunkClassificationConfig*` | `NULL` | Override per-chunk classification configuration for this file. |
| `captioning` | `XbergCaptioningConfig*` | `NULL` | Override VLM captioning configuration for this file. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `model` | `XbergLateInteractionModelType` | `XBERG_XBERG_PRESET` | The late-interaction model to use (defaults to the "gte-moderncolbert" preset). |
| `batch_size` | `uintptr_t` | — | Batch size for local ONNX inference. ColBERT emits a `\[seq, dim\]` multi-vector embedding per document, so memory scales with batch size — keep this modest. |
| `max_length` | `uintptr_t` | — | Maximum token sequence length for the tokenizer (documents). |
| `query_max_length` | `uintptr_t` | — | Fixed padded length for query augmentation. ColBERT queries are padded (with the mask token, kept attention-live) to exactly this many tokens rather than truncated/left as-is — this is the "query augmentation" trick from the ColBERT paper. |
...
| `acceleration` | `XbergAccelerationConfig*` | `NULL` | Hardware acceleration for the late-interaction ONNX model. |
| `max_embed_duration_secs` | `uint64_t*` | `NULL` | Maximum wall-clock duration (in seconds) for a single embed call when using `LateInteractionModelType.Plugin`. `NULL` disables the timeout. |
...
```c
XbergLateInteractionConfig xberg_default();
```

**Example:**

```c
XbergLateInteractionConfig *result = xberg_default();
```

**Returns:** `XbergLateInteractionConfig`
...
| `table_overlap_preference` | `XbergTableOverlapPreference` | `XBERG_XBERG_CONTENT` | How to resolve overlapping native vs layout tables. When a native oxide table and a layout (TATR/SLANeXT) table overlap on the same region, this controls which one is kept. Defaults to `TableOverlapPreference.Content` (historical behavior: keep the table with more content). Set to `TableOverlapPreference.Native` to favor source reading order (higher text F1) over the model's cell reflow. |
...
| `headers` | `void**` | `NULL` | Extra HTTP headers sent with every request to the provider. Mirrors liter-llm's `ClientConfigBuilder.header`, for gateways or providers that require custom auth/routing headers. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `urls` | `XbergSitemapUrl*` | `NULL` | The list of discovered URLs. |
...
```c
bool xberg_is_well_formed();
```

**Example:**

```c
bool result = xberg_is_well_formed(instance);
```
...
| `llm` | `XbergLlmConfig*` | `NULL` | Optional LLM configuration — only used by `NerBackendKind.Llm`. Token usage for LLM backends is recorded in `ExtractedDocument.llm_usage`. |
...
```c
XbergExtractedDocument xberg_process_image(const uint8_t* image_bytes, XbergOcrConfig config);
```

**Example:**

```c
XbergExtractedDocument *result = xberg_process_image(instance, (const uint8_t *)"data", NULL);
```
...
```c
XbergExtractedDocument xberg_process_image_file(const char* path, XbergOcrConfig config);
```

**Example:**

```c
XbergExtractedDocument *result = xberg_process_image_file(instance, "value", NULL);
```
...
```c
XbergExtractedDocument xberg_process_document(const char* path, XbergOcrConfig config);
```

**Example:**

```c
XbergExtractedDocument *result = xberg_process_document(instance, "value", NULL);
```
...
| `backend` | `const char*` | — | OCR backend: tesseract, paddleocr, paddle-ocr, or vlm |
...
| `metadata` | `void*` | `NULL` | OCR processing metadata (confidence scores, language, etc.) |
| `tables` | `XbergOcrTable*` | `NULL` | Tables detected and extracted via OCR |
| `ocr_elements` | `XbergOcrElement**` | `NULL` | Structured OCR elements with bounding boxes and confidence scores. Available when TSV output is requested or table detection is enabled. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `backend` | `const char*` | — | Backend name: "tesseract", "paddleocr", "paddle-ocr", "vlm", or a custom registered name. |
...
| `model_tier` | `const char*` | — | Model tier controlling detection/recognition model size and accuracy trade-off. For PP-OCRv5 (`model_version = "pp-ocrv5"`): - `"mobile"` (default): Lightweight models (~4.5MB detection, ~16.5MB recognition), fast download and inference - `"server"`: Large, high-accuracy models (~88MB detection, ~84MB recognition), best for GPU or complex documents For PP-OCRv6 (`model_version = "pp-ocrv6"`): `"medium"` (default), `"small"`, or `"tiny"`. A legacy `"mobile"`/`"server"` tier under v6 falls back to `"medium"`. |
| `model_version` | `const char*` | — | Model generation: `"pp-ocrv6"` (default) or `"pp-ocrv5"`. PP-OCRv6 adds a unified CJK+Latin+JA/KO recognition model with `medium`/`small`/`tiny` tiers (see `model_tier`). Scripts outside the v6 unified coverage (Arabic, Cyrillic, Devanagari, Greek, Tamil, Telugu, Thai) transparently fall back to the PP-OCRv5 per-script recognition models. Defaults to `"pp-ocrv6"`; the default `model_tier` (`"mobile"`) resolves to the v6 `"medium"` tier. Select `"pp-ocrv5"` to pin the legacy per-script/unified fleet. |
...
```c
XbergPaddleOcrConfig xberg_with_model_version(const char* version);
```

**Example:**

```c
XbergPaddleOcrConfig *result = xberg_with_model_version(instance, "value");
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `version` | `const char*` | Yes | `"pp-ocrv6"` (default) or `"pp-ocrv5"`. Under `"pp-ocrv6"`, `model_tier` |
...
| `bbox` | `XbergBoundingBox*` | `NULL` | Bounding box on this page, if known. |
...
| `scanned_pages` | `uint32_t**` | `NULL` | Pages that look like scans (1-indexed), using the default confidence threshold. `NULL` when the document could not be inspected; empty when no page qualifies. |
...
```c
void xberg_process(XbergExtractedDocument result, XbergExtractionConfig config);
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `result` | `XbergExtractedDocument` | Yes | Mutable reference to the extraction result to process |
...
```c
bool xberg_should_process(XbergExtractedDocument result, XbergExtractionConfig config);
```
...
```c
uint64_t xberg_estimated_duration_ms(XbergExtractedDocument result);
```
...
| `preset` | `XbergPreprocessingPreset` | `XBERG_XBERG_STANDARD` | Preprocessing preset level (Minimal, Standard, Aggressive) |
...
```c
const char* xberg_render_result(XbergExtractedDocument result);
```

**Example:**

```c
const char *result = xberg_render_result(instance, NULL);
```
...
| `property_changes` | `XbergPropertyChange*` | `NULL` | Formatting or metadata property changes for this revision. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `indices` | `uint32_t*` | — | Vocabulary token ids with non-zero weight, ascending. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `model` | `XbergSparseEmbeddingModelType` | `XBERG_XBERG_PRESET` | The sparse-embedding model to use (defaults to the "opensearch-v3-distill" preset). |
| `batch_size` | `uintptr_t` | — | Batch size for local ONNX inference. SPLADE emits a `\[seq, vocab\]` logit tensor per document, so memory scales with batch size — keep this modest. |
| `max_length` | `uintptr_t` | — | Maximum token sequence length for the tokenizer. |
| `show_download_progress` | `bool` | `false` | Show model download progress (local ONNX path only). |
| `cache_dir` | `const char**` | `NULL` | Optional alternate Hugging Face cache root for model files. When unset, hf-hub follows the standard Hugging Face environment and platform cache conventions. |
| `acceleration` | `XbergAccelerationConfig*` | `NULL` | Hardware acceleration for the sparse-embedding ONNX model. |
| `max_embed_duration_secs` | `uint64_t*` | `NULL` | Maximum wall-clock duration (in seconds) for a single embed call when using `SparseEmbeddingModelType.Plugin`. `NULL` disables the timeout. |
...
```c
XbergSparseEmbeddingConfig xberg_default();
```

**Example:**

```c
XbergSparseEmbeddingConfig *result = xberg_default();
```

**Returns:** `XbergSparseEmbeddingConfig`
...
```c
uintptr_t xberg_count_tokens(const char* text);
```

**Example:**

```c
uintptr_t result = xberg_count_tokens(instance, "value");
```
...
```c
void xberg_validate(XbergExtractedDocument result, XbergExtractionConfig config);
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `result` | `XbergExtractedDocument` | Yes | The extraction result to validate |
| `config` | `XbergExtractionConfig` | Yes | Extraction configuration |
...
```c
bool xberg_should_validate(XbergExtractedDocument result, XbergExtractionConfig config);
```
...
| `XBERG_NATIVE` | Prefer the native oxide table when it overlaps a layout table. Native tables preserve the source reading order, which scores higher on text F1 for documents where the layout model's cell reflow diverges from the ground truth. |
...
| `XBERG_CUSTOM` | Use a custom ONNX cross-encoder from HuggingFace. — Fields: `model_id`: `const char*`, `model_file`: `const char*`, `additional_files`: `const char**`, `max_length`: `int64_t`, `head`: `XbergRerankerHead` |
...
| Value | Description |
|-------|-------------|
| `XBERG_INLINE` | Inline links: `\[text\](url)`. Default. |
