...
## C API Reference <span class="version-badge">v1.0.0-rc.16</span>
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `model` | `XbergLateInteractionModelType` | `XBERG_XBERG_PRESET` | The late-interaction model to use (defaults to the "colbert" preset). |
...
| `model_version` | `const char*` | — | Model generation: `"pp-ocrv5"` (default) or `"pp-ocrv6"`. PP-OCRv6 adds a unified CJK+Latin+JA/KO recognition model with `medium`/`small`/`tiny` tiers (see `model_tier`). Scripts outside the v6 unified coverage (Arabic, Cyrillic, Devanagari, Greek, Tamil, Telugu, Thai) transparently fall back to the PP-OCRv5 per-script recognition models. Defaults to `"pp-ocrv5"` so existing configs are unchanged. |
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `version` | `const char*` | Yes | `"pp-ocrv5"` (default) or `"pp-ocrv6"`. Under `"pp-ocrv6"`, `model_tier` |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `model` | `XbergSparseEmbeddingModelType` | `XBERG_XBERG_PRESET` | The sparse-embedding model to use (defaults to the "splade" preset). |
...
| `XBERG_CODE` | Code (tree-sitter analyzable source). The structured analysis result is exposed via `ExtractedDocument.code_intelligence`; this variant only tags the format. |
