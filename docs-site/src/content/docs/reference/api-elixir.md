...
## Elixir API Reference <span class="version-badge">v1.0.0-rc.16</span>
...
```elixir
def from_bytes(bytes, mime_type, filename)
```

**Example:**

```elixir
{:ok, result} = ExtractInput.from_bytes(<<100, 97, 116, 97>>, "value", "value")
```
...
```elixir
{:ok, result} = ExtractInput.from_uri("value")
```
...
```elixir
{:ok, result} = ExtractionResult.single(%{{}})
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `model` | `LateInteractionModelType` | `:preset` | The late-interaction model to use (defaults to the "colbert" preset). |
| `batch_size` | `integer()` | — | Batch size for local ONNX inference. ColBERT emits a `\[seq, dim\]` multi-vector embedding per document, so memory scales with batch size — keep this modest. |
| `max_length` | `integer()` | — | Maximum token sequence length for the tokenizer (documents). |
| `query_max_length` | `integer()` | — | Fixed padded length for query augmentation. ColBERT queries are padded (with the mask token, kept attention-live) to exactly this many tokens rather than truncated/left as-is — this is the "query augmentation" trick from the ColBERT paper. |
| `show_download_progress` | `boolean()` | `false` | Show model download progress (local ONNX path only). |
| `cache_dir` | `String.t() \| nil` | `nil` | Custom cache directory for model files. Defaults to `~/.cache/xberg/late-interaction/` if not specified. |
| `acceleration` | `AccelerationConfig \| nil` | `nil` | Hardware acceleration for the late-interaction ONNX model. |
| `max_embed_duration_secs` | `integer() \| nil` | `nil` | Maximum wall-clock duration (in seconds) for a single embed call when using `LateInteractionModelType.Plugin`. `nil` disables the timeout. |
...
```elixir
{:ok, result} = LateInteractionConfig.default()
```
...
| `table_overlap_preference` | `TableOverlapPreference` | `:content` | How to resolve overlapping native vs layout tables. When a native oxide table and a layout (TATR/SLANeXT) table overlap on the same region, this controls which one is kept. Defaults to `TableOverlapPreference.Content` (historical behavior: keep the table with more content). Set to `TableOverlapPreference.Native` to favor source reading order (higher text F1) over the model's cell reflow. |
...
```elixir
{:ok, result} = instance.is_well_formed()
```
...
| `backend` | `String.t()` | — | OCR backend: tesseract, paddleocr, paddle-ocr, or vlm |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `backend` | `String.t()` | — | Backend name: "tesseract", "paddleocr", "paddle-ocr", "vlm", or a custom registered name. |
...
| `model_tier` | `String.t()` | — | Model tier controlling detection/recognition model size and accuracy trade-off. For PP-OCRv5 (`model_version = "pp-ocrv5"`): - `"mobile"` (default): Lightweight models (~4.5MB detection, ~16.5MB recognition), fast download and inference - `"server"`: Large, high-accuracy models (~88MB detection, ~84MB recognition), best for GPU or complex documents For PP-OCRv6 (`model_version = "pp-ocrv6"`): `"medium"` (default), `"small"`, or `"tiny"`. A legacy `"mobile"`/`"server"` tier under v6 falls back to `"medium"`. |
| `model_version` | `String.t()` | — | Model generation: `"pp-ocrv5"` (default) or `"pp-ocrv6"`. PP-OCRv6 adds a unified CJK+Latin+JA/KO recognition model with `medium`/`small`/`tiny` tiers (see `model_tier`). Scripts outside the v6 unified coverage (Arabic, Cyrillic, Devanagari, Greek, Tamil, Telugu, Thai) transparently fall back to the PP-OCRv5 per-script recognition models. Defaults to `"pp-ocrv5"` so existing configs are unchanged. |
...
```elixir
def with_model_version(version)
```

**Example:**

```elixir
{:ok, result} = instance.with_model_version("value")
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `version` | `String.t()` | Yes | `"pp-ocrv5"` (default) or `"pp-ocrv6"`. Under `"pp-ocrv6"`, `model_tier` |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `model` | `SparseEmbeddingModelType` | `:preset` | The sparse-embedding model to use (defaults to the "splade" preset). |
| `batch_size` | `integer()` | — | Batch size for local ONNX inference. SPLADE emits a `\[seq, vocab\]` logit tensor per document, so memory scales with batch size — keep this modest. |
| `max_length` | `integer()` | — | Maximum token sequence length for the tokenizer. |
| `show_download_progress` | `boolean()` | `false` | Show model download progress (local ONNX path only). |
| `cache_dir` | `String.t() \| nil` | `nil` | Custom cache directory for model files. Defaults to `~/.cache/xberg/sparse-embeddings/` if not specified. |
| `acceleration` | `AccelerationConfig \| nil` | `nil` | Hardware acceleration for the sparse-embedding ONNX model. |
| `max_embed_duration_secs` | `integer() \| nil` | `nil` | Maximum wall-clock duration (in seconds) for a single embed call when using `SparseEmbeddingModelType.Plugin`. `nil` disables the timeout. |
...
```elixir
{:ok, result} = SparseEmbeddingConfig.default()
```
