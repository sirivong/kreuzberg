...
## Ruby API Reference <span class="version-badge">v1.0.0-rc.34</span>
...
```ruby
def self.classify_chunks(result, config)
```

**Example:**

```ruby
classify_chunks(ExtractedDocument.new, ChunkClassificationConfig.new)
```
...
```ruby
def self.from_bytes(bytes, mime_type, filename)
```

**Example:**

```ruby
result = ExtractInput.from_bytes("data", "value", filename: "value")
```
...
```ruby
result = ExtractionResult.single(ExtractedDocument.new)
```
...
| `max_embed_duration_secs` | `Integer?` | `nil` | Maximum wall-clock duration (in seconds) for a single embed call when using `LateInteractionModelType.Plugin`. `nil` disables the timeout. |
...
```ruby
result = instance.is_well_formed()
```
...
```ruby
result = instance.with_model_version("value")
```
...
| `max_embed_duration_secs` | `Integer?` | `nil` | Maximum wall-clock duration (in seconds) for a single embed call when using `SparseEmbeddingModelType.Plugin`. `nil` disables the timeout. |
