...
## Ruby API Reference <span class="version-badge">v1.0.0-rc.16</span>
...
| `cache_dir` | `String?` | `nil` | Custom cache directory for model files. Defaults to `~/.cache/xberg/late-interaction/` if not specified. |
...
| `show_download_progress` | `Boolean` | `false` | Show model download progress (local ONNX path only). |
| `cache_dir` | `String?` | `nil` | Custom cache directory for model files. Defaults to `~/.cache/xberg/sparse-embeddings/` if not specified. |
