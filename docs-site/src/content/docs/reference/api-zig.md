...
## Zig API Reference <span class="version-badge">v1.0.0-rc.34</span>
...
```zig
pub fn extract(input: ExtractInput, config: ExtractionConfig) Error!ExtractionResult
```
...
```zig
pub fn extract_batch(inputs: []const ExtractInput, config: ExtractionConfig) Error!ExtractionResult
```
...
```zig
pub fn map_url(uri: [:0]const u8, config: UrlExtractionConfig) Error!MapResult
```

**Example:**

```zig
const result = try mapUrl("value", .{});
```
...
```zig
pub fn register_document_extractor(extractor: DocumentExtractor) Error!void
```

**Example:**

```zig
try registerDocumentExtractor(.{});
```
...
```zig
pub fn unregister_document_extractor(name: [:0]const u8) Error!void
```

**Example:**

```zig
try unregisterDocumentExtractor("value");
```
...
```zig
pub fn register_renderer(renderer: Renderer) Error!void
```

**Example:**

```zig
try registerRenderer(.{});
```
...
```zig
pub fn unregister_renderer(name: [:0]const u8) Error!void
```

**Example:**

```zig
try unregisterRenderer("value");
```
...
```zig
pub fn clear_tokenizer_backends() Error!void
```
...
```zig
pub fn list_tokenizer_backends() Error![]const [:0]const u8
```

**Example:**

```zig
const result = try listTokenizerBackends();
```
...
```zig
pub fn classify_chunks(result: ExtractedDocument, config: ChunkClassificationConfig) Error!void
```
...
```zig
pub fn max_sim_rank(query: MultiVectorEmbedding, docs: []const MultiVectorEmbedding) []const LateInteractionMatch
```

**Example:**

```zig
const result = maxSimRank(.{}, &[_]u8{});
```
...
|------|------|----------|-------------|
| `query` | `MultiVectorEmbedding` | Yes | The multi vector embedding |
| `docs` | `\[\]const MultiVectorEmbedding` | Yes | The docs |

**Returns:** `[]const LateInteractionMatch`
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `promptTemplate` | `\[:0\]const u8?` | `null` | Minijinja prompt template. Receives `{{ definitions }}` (rendered label + description list) and `{{ chunks }}` (a numbered list of chunk texts in the current batch) variables. `null` lets the backend pick a sensible default. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `label` | `\[:0\]const u8` | — | Label name returned in `ChunkMetadata.classifications`. |
| `description` | `\[:0\]const u8` | — | Semantic description of when this label applies. Injected verbatim into the classification prompt next to the label name. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `text` | `\[:0\]const u8` | — | The raw source text of this chunk. |
| `contextPath` | `\[\]const \[:0\]const u8` | — | Hierarchical path of enclosing structural items (e.g. `\["MyClass", "my_method"\]`). |
| `nodeTypes` | `\[\]const \[:0\]const u8` | — | Tree-sitter node kinds that appear at the top level of this chunk (e.g. `"function_definition"`, `"class_definition"`). |
| `byteStart` | `u64` | — | Inclusive start byte offset of this chunk in the original source. |
| `byteEnd` | `u64` | — | Exclusive end byte offset of this chunk in the original source. |
...
| `byteStart` | `u64` | — | Inclusive start byte offset of the `name="value"` attribute token. |
| `byteEnd` | `u64` | — | Exclusive end byte offset of the `name="value"` attribute token. |
...
| `key` | `\[:0\]const u8?` | `/* serde(default) */` | Key, attribute name, tag name, or positional index (`"0"`, `"1"`, …). `null` at the document root. |
| `value` | `\[:0\]const u8?` | `/* serde(default) */` | Leaf scalar value, if any. `null` for containers (objects, arrays, XML elements with child elements). |
...
| `byteStart` | `u64` | — | Inclusive start byte offset of this node in the original source. |
| `byteEnd` | `u64` | — | Exclusive end byte offset of this node in the original source. |
...
| `documentMaxSize` | `u64?` | `52428800` | Maximum size in bytes for document downloads. Defaults to 50 MB. |
...
```zig
pub fn extract(self: *const DocumentExtractor, input: ExtractInput, config: ExtractionConfig) Error!ExtractedDocument
```
...
| `cacheDir` | `\[:0\]const u8?` | `null` | Optional alternate Hugging Face cache root for model files. When unset, hf-hub follows `HF_HUB_CACHE`, `HUGGINGFACE_HUB_CACHE`, `HF_HOME`, XDG, and platform defaults. Prefer those environment variables when configuring the cache process-wide. |
...
```zig
pub fn fromBytes(bytes: []const u8, mime_type: [:0]const u8, filename: ?[:0]const u8) ExtractInput
```

**Example:**

```zig
const result = ExtractInput.fromBytes("data", "value", "value");
```
...
```zig
pub fn fromUri(uri: [:0]const u8) ExtractInput
```

**Example:**

```zig
const result = ExtractInput.fromUri("value");
```
...
| `maxConcurrentExtractions` | `u64?` | `null` | Maximum concurrent document extractions in batch operations. This is a ceiling within the configured total thread budget, not an independent pool size. When unset, the scheduler derives document and per-document concurrency from `ConcurrencyConfig.max_threads`. |
...
| `crawlFinalUrls` | `\[\]const \[:0\]const u8` | `\[\]` | Final URLs reached after redirects during URL ingestion. |
| `crawlRedirectCount` | `u64` | — | Total redirects followed while fetching or crawling URLs. |
| `crawlUniqueNormalizedUrls` | `\[\]const \[:0\]const u8` | `\[\]` | Unique normalized URLs discovered by crawls. |
...
```zig
const result = ExtractionResult.single(.{});
```
...
| `language` | `\[:0\]const u8?` | `"en"` | Language code for stopword filtering (e.g., "en", "de", "fr"). If None, no stopword filtering is applied. |
...
| `batchSize` | `u64` | — | Batch size for local ONNX inference. ColBERT emits a `\[seq, dim\]` multi-vector embedding per document, so memory scales with batch size — keep this modest. |
| `maxLength` | `u64` | — | Maximum token sequence length for the tokenizer (documents). |
| `queryMaxLength` | `u64` | — | Fixed padded length for query augmentation. ColBERT queries are padded (with the mask token, kept attention-live) to exactly this many tokens rather than truncated/left as-is — this is the "query augmentation" trick from the ColBERT paper. |
| `showDownloadProgress` | `bool` | `false` | Show model download progress (local ONNX path only). |
| `cacheDir` | `\[:0\]const u8?` | `null` | Optional alternate Hugging Face cache root for model files. When unset, hf-hub follows the standard Hugging Face environment and platform cache conventions. |
| `acceleration` | `AccelerationConfig?` | `null` | Hardware acceleration for the late-interaction ONNX model. |
| `maxEmbedDurationSecs` | `u64?` | `null` | Maximum wall-clock duration (in seconds) for a single embed call when using `LateInteractionModelType.Plugin`. `null` disables the timeout. |
...
```zig
const result = LateInteractionConfig.default();
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `\[:0\]const u8` | — | Stable preset name referenced from config. |
| `modelRepo` | `\[:0\]const u8` | — | HuggingFace repository hosting the ONNX model. |
| `modelFile` | `\[:0\]const u8` | — | Path to the ONNX file within the repo. |
| `additionalFiles` | `\[\]const \[:0\]const u8` | — | Sibling files that must be downloaded alongside `model_file`. |
| `maxLength` | `u64` | — | Maximum document token sequence length. |
| `queryMaxLength` | `u64` | — | Fixed padded query length (ColBERT query augmentation). |
...
| `timeoutSecs` | `u64?` | `null` | Request timeout in seconds. When `null`, liter-llm's built-in 60s default applies, except the VLM OCR path which uses a 300s default (a single page image transcription routinely exceeds 60s). Set explicitly to override. |
...
| `headers` | `std.StringHashMap(\[:0\]const u8)?` | `{}` | Extra HTTP headers sent with every request to the provider. Mirrors liter-llm's `ClientConfigBuilder.header`, for gateways or providers that require custom auth/routing headers. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `detModel` | `\[:0\]const u8` | — | Exact path to the detection ONNX model in the Hugging Face snapshot. |
| `clsModel` | `\[:0\]const u8` | — | Exact path to the classification ONNX model in the Hugging Face snapshot. |
| `recModel` | `\[:0\]const u8` | — | Exact path to the recognition ONNX model in the Hugging Face snapshot. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `numTokens` | `u32` | — | Number of attention-live token rows (padding rows are dropped, not zeroed — see `engine.normalize_tokens`). |
...
```zig
pub fn isWellFormed(self: *const MultiVectorEmbedding) bool
```

**Example:**

```zig
const result = instance.isWellFormed();
```
...
```zig
pub fn processImage(self: *const OcrBackend, image_bytes: []const u8, config: OcrConfig) Error!ExtractedDocument
```
...
```zig
pub fn processImageFile(self: *const OcrBackend, path: [:0]const u8, config: OcrConfig) Error!ExtractedDocument
```
...
```zig
pub fn processDocument(self: *const OcrBackend, path: [:0]const u8, config: OcrConfig) Error!ExtractedDocument
```
...
| `backend` | `\[:0\]const u8` | — | OCR backend: tesseract, paddleocr, paddle-ocr, or vlm |
...
| `ocrElements` | `\[\]const OcrElement?` | `\[\]` | Structured OCR elements with bounding boxes and confidence scores. Available when TSV output is requested or table detection is enabled. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `backend` | `\[:0\]const u8` | — | Backend name: "tesseract", "paddleocr", "paddle-ocr", "vlm", or a custom registered name. |
...
| `minProvenanceFallbackRatio` | `f64` | — | Minimum fraction of a page's non-whitespace characters with `MappingProvenance.Fallback` provenance before the page is treated as having a fabricated text layer and routed to OCR (issue #1254). Gated by `min_total_non_whitespace` so a short page with a stray fallback character cannot trip it. Only used when `enable_provenance_ocr_routing` is `true`. |
...
| `cacheDir` | `\[:0\]const u8?` | `null` | Optional Hugging Face Hub cache root for model files. When unset, the standard `HF_HUB_CACHE`, legacy `HUGGINGFACE_HUB_CACHE`, and `HF_HOME` conventions are used. |
...
| `modelTier` | `\[:0\]const u8` | — | Model tier controlling detection/recognition model size and accuracy trade-off. For PP-OCRv5 (`model_version = "pp-ocrv5"`): - `"mobile"` (default): Lightweight models (~4.5MB detection, ~16.5MB recognition), fast download and inference - `"server"`: Large, high-accuracy models (~88MB detection, ~84MB recognition), best for GPU or complex documents For PP-OCRv6 (`model_version = "pp-ocrv6"`): `"medium"` (default), `"small"`, or `"tiny"`. A legacy `"mobile"`/`"server"` tier under v6 falls back to `"medium"`. |
| `modelVersion` | `\[:0\]const u8` | — | Model generation: `"pp-ocrv6"` (default) or `"pp-ocrv5"`. PP-OCRv6 adds a unified CJK+Latin+JA/KO recognition model with `medium`/`small`/`tiny` tiers (see `model_tier`). Scripts outside the v6 unified coverage (Arabic, Cyrillic, Devanagari, Greek, Tamil, Telugu, Thai) transparently fall back to the PP-OCRv5 per-script recognition models. Defaults to `"pp-ocrv6"`; the default `model_tier` (`"mobile"`) resolves to the v6 `"medium"` tier. Select `"pp-ocrv5"` to pin the legacy per-script/unified fleet. |
...
```zig
pub fn withModelVersion(self: *const PaddleOcrConfig, version: [:0]const u8) PaddleOcrConfig
```

**Example:**

```zig
const result = instance.withModelVersion("value");
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `version` | `\[:0\]const u8` | Yes | `"pp-ocrv6"` (default) or `"pp-ocrv5"`. Under `"pp-ocrv6"`, `model_tier` |
...
| `imageIndices` | `\[\]const u32` | `/* serde(default) */` | Indices into `ExtractedDocument.images` for images found on this page. Each value is a zero-based index into the top-level `images` collection. Only populated when `extract_images = true` in the extraction config. |
...
| `scannedConfidence` | `f32?` | `null` | How strongly the document's most scan-like page resembles a scan, in `\[0.0, 1.0\]`. `null` when the document could not be inspected. A full-page raster with no visible text scores at least `0.85`; a born-digital slide with a full-bleed background image scores `0.50`. |
| `scannedPages` | `\[\]const u32?` | `\[\]` | Pages that look like scans (1-indexed), using the default confidence threshold. `null` when the document could not be inspected; empty when no page qualifies. |
...
```zig
pub fn process(self: *const PostProcessor, result: ExtractedDocument, config: ExtractionConfig) Error!void
```
...
```zig
pub fn shouldProcess(self: *const PostProcessor, result: ExtractedDocument, config: ExtractionConfig) bool
```
...
```zig
pub fn estimatedDurationMs(self: *const PostProcessor, result: ExtractedDocument) u64
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | `\[:0\]const u8` | — | Property name, such as `"bold"`, `"italic"`, `"font_size"`, or `"font_color"`. |
| `from` | `\[:0\]const u8?` | `null` | Value before the change, when available. |
| `to` | `\[:0\]const u8?` | `null` | Value after the change, when available. |
...
```zig
pub fn renderResult(self: *const Renderer, result: ExtractedDocument) Error![:0]const u8
```

**Example:**

```zig
const result = try instance.renderResult(.{});
```
...
| `maxRerankDurationSecs` | `u64?` | `60` | Maximum wall-clock duration (in seconds) for a single `rerank()` call when using `RerankerModelType.Plugin`. Applies only to the in-process plugin path — protects against hung host-language backends. On timeout, the dispatcher returns `Plugin` instead of blocking forever. `null` disables the timeout. The default (60 seconds) is conservative for common in-process inference; increase for large document sets on slow hardware. |
...
| `propertyChanges` | `\[\]const PropertyChange` | `\[\]` | Formatting or metadata property changes for this revision. |
...
| `batchSize` | `u64` | — | Batch size for local ONNX inference. SPLADE emits a `\[seq, vocab\]` logit tensor per document, so memory scales with batch size — keep this modest. |
| `maxLength` | `u64` | — | Maximum token sequence length for the tokenizer. |
| `showDownloadProgress` | `bool` | `false` | Show model download progress (local ONNX path only). |
| `cacheDir` | `\[:0\]const u8?` | `null` | Optional alternate Hugging Face cache root for model files. When unset, hf-hub follows the standard Hugging Face environment and platform cache conventions. |
| `acceleration` | `AccelerationConfig?` | `null` | Hardware acceleration for the sparse-embedding ONNX model. |
| `maxEmbedDurationSecs` | `u64?` | `null` | Maximum wall-clock duration (in seconds) for a single embed call when using `SparseEmbeddingModelType.Plugin`. `null` disables the timeout. |
...
```zig
const result = SparseEmbeddingConfig.default();
```
...
| `modelFile` | `\[:0\]const u8` | — | Path to the ONNX file within the repo. |
| `additionalFiles` | `\[\]const \[:0\]const u8` | — | Sibling files that must be downloaded alongside `model_file`. |
| `maxLength` | `u64` | — | Maximum token sequence length. |
| `description` | `\[:0\]const u8` | — | Human-readable description. |
...
```zig
pub fn countTokens(self: *const TokenizerBackend, text: [:0]const u8) u64
```
...
| `modelCacheDir` | `\[:0\]const u8?` | `null` | Optional alternate Hugging Face cache root for Whisper models. When unset, hf-hub follows `HF_HUB_CACHE`, `HUGGINGFACE_HUB_CACHE`, `HF_HOME`, XDG, and platform defaults. Files remain in the standard content-addressed snapshot layout and are not copied into an Xberg cache. |
...
| `content` | `\[:0\]const u8` | — | Translated plain-text body. Matches the shape of `ExtractedDocument.content`. |
...
| `maxDocumentUrlsPerResult` | `u32?` | `100` | Maximum URLs to follow per extraction result. |
| `maxTotalUrls` | `u32?` | `1000` | Maximum URLs followed across the whole extraction call. |
...
```zig
pub fn validate(self: *const Validator, result: ExtractedDocument, config: ExtractionConfig) Error!void
```
...
```zig
pub fn shouldValidate(self: *const Validator, result: ExtractedDocument, config: ExtractionConfig) bool
```
...
| Value | Description |
|-------|-------------|
| `Preset` | Use a preset ColBERT model (recommended). — Fields: `name`: `\[:0\]const u8` |
| `Custom` | Use a custom ColBERT ONNX model from HuggingFace. — Fields: `modelId`: `\[:0\]const u8`, `modelFile`: `\[:0\]const u8`, `additionalFiles`: `\[\]const \[:0\]const u8`, `maxLength`: `i64` |
| `Plugin` | In-process late-interaction backend registered via the plugin system. — Fields: `name`: `\[:0\]const u8` |
...
| `Tokenizer` | Size measured in tokens from a HuggingFace tokenizer or a registered tokenizer backend. — Fields: `model`: `\[:0\]const u8`, `cacheDir`: `\[:0\]const u8` |
...
| `Custom` | Use a custom ONNX cross-encoder from HuggingFace. — Fields: `modelId`: `\[:0\]const u8`, `modelFile`: `\[:0\]const u8`, `additionalFiles`: `\[\]const \[:0\]const u8`, `maxLength`: `i64`, `head`: `RerankerHead` |
...
| Value | Description |
|-------|-------------|
| `Preset` | Use a preset SPLADE model (recommended). — Fields: `name`: `\[:0\]const u8` |
| `Custom` | Use a custom SPLADE (`BertForMaskedLM`) ONNX model from HuggingFace. — Fields: `modelId`: `\[:0\]const u8`, `modelFile`: `\[:0\]const u8`, `additionalFiles`: `\[\]const \[:0\]const u8`, `maxLength`: `i64` |
| `Plugin` | In-process sparse-embedding backend registered via the plugin system. — Fields: `name`: `\[:0\]const u8` |
