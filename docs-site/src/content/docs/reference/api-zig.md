...
## Zig API Reference <span class="version-badge">v1.0.0-rc.22</span>
...
| `cacheDir` | `\[:0\]const u8?` | `null` | Custom cache directory for model files. Defaults to `~/.cache/xberg/late-interaction/` if not specified. |
...
| `showDownloadProgress` | `bool` | `false` | Show model download progress (local ONNX path only). |
| `cacheDir` | `\[:0\]const u8?` | `null` | Custom cache directory for model files. Defaults to `~/.cache/xberg/sparse-embeddings/` if not specified. |
