...
## PHP API Reference <span class="version-badge">v1.0.0-rc.16</span>
...
```php
public static function fromBytes(string $bytes, string $mimeType, string $filename): ExtractInput
```

**Example:**

```php
$result = ExtractInput::fromBytes("data", "value", "value");
```
...
```php
$result = ExtractionResult::single(new ExtractedDocument());
```
