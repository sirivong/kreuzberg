...
## WebAssembly API Reference <span class="version-badge">v1.0.0-rc.16</span>
...
```typescript
static fromBytes(bytes: Buffer, mimeType: string, filename: string): ExtractInput
```

**Example:**

```typescript
const result = ExtractInput.fromBytes(new Uint8Array([100, 97, 116, 97]), "value", "value");
```
...
```typescript
static fromUri(uri: string): ExtractInput
```
...
```typescript
static single(result: ExtractedDocument): ExtractionResult
```

**Example:**

```typescript
const result = ExtractionResult.single(new ExtractedDocument());
```
...
| `batchSize` | `number` | — | Batch size for local ONNX inference. ColBERT emits a `\[seq, dim\]` multi-vector embedding per document, so memory scales with batch size — keep this modest. |
| `maxLength` | `number` | — | Maximum token sequence length for the tokenizer (documents). |
| `queryMaxLength` | `number` | — | Fixed padded length for query augmentation. ColBERT queries are padded (with the mask token, kept attention-live) to exactly this many tokens rather than truncated/left as-is — this is the "query augmentation" trick from the ColBERT paper. |
| `showDownloadProgress` | `boolean` | `false` | Show model download progress (local ONNX path only). |
| `cacheDir` | `string \| null` | `null` | Custom cache directory for model files. Defaults to `~/.cache/xberg/late-interaction/` if not specified. |
| `acceleration` | `AccelerationConfig \| null` | `null` | Hardware acceleration for the late-interaction ONNX model. |
| `maxEmbedDurationSecs` | `number \| null` | `null` | Maximum wall-clock duration (in seconds) for a single embed call when using `LateInteractionModelType.Plugin`. `null` disables the timeout. |
...
```typescript
static default(): LateInteractionConfig
```
...
```typescript
withModelVersion(version: string): PaddleOcrConfig
```
...
| `batchSize` | `number` | — | Batch size for local ONNX inference. SPLADE emits a `\[seq, vocab\]` logit tensor per document, so memory scales with batch size — keep this modest. |
| `maxLength` | `number` | — | Maximum token sequence length for the tokenizer. |
| `showDownloadProgress` | `boolean` | `false` | Show model download progress (local ONNX path only). |
| `cacheDir` | `string \| null` | `null` | Custom cache directory for model files. Defaults to `~/.cache/xberg/sparse-embeddings/` if not specified. |
| `acceleration` | `AccelerationConfig \| null` | `null` | Hardware acceleration for the sparse-embedding ONNX model. |
| `maxEmbedDurationSecs` | `number \| null` | `null` | Maximum wall-clock duration (in seconds) for a single embed call when using `SparseEmbeddingModelType.Plugin`. `null` disables the timeout. |
...
```typescript
static default(): SparseEmbeddingConfig
```
