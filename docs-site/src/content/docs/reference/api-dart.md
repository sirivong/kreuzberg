...
## Dart API Reference <span class="version-badge">v1.0.0-rc.16</span>
...
```dart
ExtractionResult extract(ExtractInput input, ExtractionConfig config)
```
...
```dart
ExtractionResult extractBatch(List<ExtractInput> inputs, ExtractionConfig config)
```
...
```dart
MapResult mapUrl(String uri, UrlExtractionConfig config)
```
...
```dart
void registerDocumentExtractor(DocumentExtractor extractor)
```
...
```dart
void unregisterDocumentExtractor(String name)
```
...
```dart
void unregisterRenderer(String name)
```
...
```dart
List<String> listTokenizerBackends()
```
...
```dart
double maxSimScore(MultiVectorEmbedding query, MultiVectorEmbedding doc)
```
...
```dart
List<LateInteractionMatch> maxSimRank(MultiVectorEmbedding query, List<MultiVectorEmbedding> docs)
```
...
```dart
static ExtractInput fromBytes(Uint8List bytes, String mimeType, [String? filename])
```

**Example:**

```dart
final result = ExtractInput.fromBytes(Uint8List.fromList([100, 97, 116, 97]), "value", "value");
```
...
```dart
static ExtractInput fromUri(String uri)
```

**Example:**

```dart
final result = ExtractInput.fromUri("value");
```
...
```dart
static ExtractionResult single(ExtractedDocument result)
```

**Example:**

```dart
final result = ExtractionResult.single(ExtractedDocument());
```
...
| `maxEmbedDurationSecs` | `int?` | `null` | Maximum wall-clock duration (in seconds) for a single embed call when using `LateInteractionModelType.Plugin`. `null` disables the timeout. |
...
```dart
static LateInteractionConfig default()
```

**Example:**

```dart
final result = LateInteractionConfig.default();
```
...
```dart
final result = instance.isWellFormed();
```
...
```dart
ExtractedDocument processImage(Uint8List imageBytes, OcrConfig config)
```
...
```dart
PaddleOcrConfig withModelVersion(String version)
```

**Example:**

```dart
final result = instance.withModelVersion("value");
```
...
```dart
int estimatedDurationMs(ExtractedDocument result)
```
...
| `maxEmbedDurationSecs` | `int?` | `null` | Maximum wall-clock duration (in seconds) for a single embed call when using `SparseEmbeddingModelType.Plugin`. `null` disables the timeout. |
...
```dart
static SparseEmbeddingConfig default()
```

**Example:**

```dart
final result = SparseEmbeddingConfig.default();
```
