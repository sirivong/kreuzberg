...
## Swift API Reference <span class="version-badge">v1.0.0-rc.16</span>
...
| `documentMaxSize` | `UInt64?` | `52428800` | Maximum size in bytes for document downloads. Defaults to 50 MB. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `pages` | `UInt64` | — | Total pages in the source document (`0` when not page-addressable). |
| `tables` | `UInt64` | — | Tables detected in the document. |
| `images` | `UInt64` | — | Images detected in the document. |
...
| `crawlRedirectCount` | `UInt64` | — | Total redirects followed while fetching or crawling URLs. |
...
| `batchSize` | `UInt64` | — | Batch size for local ONNX inference. ColBERT emits a `\[seq, dim\]` multi-vector embedding per document, so memory scales with batch size — keep this modest. |
| `maxLength` | `UInt64` | — | Maximum token sequence length for the tokenizer (documents). |
| `queryMaxLength` | `UInt64` | — | Fixed padded length for query augmentation. ColBERT queries are padded (with the mask token, kept attention-live) to exactly this many tokens rather than truncated/left as-is — this is the "query augmentation" trick from the ColBERT paper. |
| `showDownloadProgress` | `Bool` | `false` | Show model download progress (local ONNX path only). |
| `cacheDir` | `URL?` | `null` | Custom cache directory for model files. Defaults to `~/.cache/xberg/late-interaction/` if not specified. |
| `acceleration` | `AccelerationConfig?` | `null` | Hardware acceleration for the late-interaction ONNX model. |
| `maxEmbedDurationSecs` | `UInt64?` | `null` | Maximum wall-clock duration (in seconds) for a single embed call when using `LateInteractionModelType.Plugin`. `null` disables the timeout. |
...
```swift
public static func default() -> LateInteractionConfig
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `index` | `UInt64` | — | Position of this document in the original input slice. |
...
| `maxLength` | `UInt64` | — | Maximum document token sequence length. |
| `queryMaxLength` | `UInt64` | — | Fixed padded query length (ColBERT query augmentation). |
| `dim` | `UInt64` | — | Per-token embedding dimensionality. |
...
```swift
let result = instance.isWellFormed()
```
...
| `modelVersion` | `String` | — | Model generation: `"pp-ocrv5"` (default) or `"pp-ocrv6"`. PP-OCRv6 adds a unified CJK+Latin+JA/KO recognition model with `medium`/`small`/`tiny` tiers (see `model_tier`). Scripts outside the v6 unified coverage (Arabic, Cyrillic, Devanagari, Greek, Tamil, Telugu, Thai) transparently fall back to the PP-OCRv5 per-script recognition models. Defaults to `"pp-ocrv5"` so existing configs are unchanged. |
...
```swift
public func withModelVersion(version: String) -> PaddleOcrConfig
```

**Example:**

```swift
let result = instance.withModelVersion("value")
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `version` | `String` | Yes | `"pp-ocrv5"` (default) or `"pp-ocrv6"`. Under `"pp-ocrv6"`, `model_tier` |
...
| `batchSize` | `UInt64` | — | Batch size for local ONNX inference. SPLADE emits a `\[seq, vocab\]` logit tensor per document, so memory scales with batch size — keep this modest. |
| `maxLength` | `UInt64` | — | Maximum token sequence length for the tokenizer. |
| `showDownloadProgress` | `Bool` | `false` | Show model download progress (local ONNX path only). |
| `cacheDir` | `URL?` | `null` | Custom cache directory for model files. Defaults to `~/.cache/xberg/sparse-embeddings/` if not specified. |
| `acceleration` | `AccelerationConfig?` | `null` | Hardware acceleration for the sparse-embedding ONNX model. |
| `maxEmbedDurationSecs` | `UInt64?` | `null` | Maximum wall-clock duration (in seconds) for a single embed call when using `SparseEmbeddingModelType.Plugin`. `null` disables the timeout. |
...
```swift
public static func default() -> SparseEmbeddingConfig
```
...
| `modelFile` | `String` | — | Path to the ONNX file within the repo. |
| `additionalFiles` | `\[String\]` | — | Sibling files that must be downloaded alongside `model_file`. |
| `maxLength` | `UInt64` | — | Maximum token sequence length. |
| `description` | `String` | — | Human-readable description. |
...
```swift
public func countTokens(text: String) -> UInt64
```
