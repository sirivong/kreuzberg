...
## Swift API Reference <span class="version-badge">v1.0.0-rc.34</span>
...
```swift
public static func classifyChunks(result: ExtractedDocument, config: ChunkClassificationConfig) throws
```

**Example:**

```swift
try classifyChunks(ExtractedDocument(), ChunkClassificationConfig())
```
...
| `byteStart` | `UInt64` | — | Inclusive start byte offset of this chunk in the original source. |
| `byteEnd` | `UInt64` | — | Exclusive end byte offset of this chunk in the original source. |
...
| `byteStart` | `UInt64` | — | Inclusive start byte offset of the `name="value"` attribute token. |
| `byteEnd` | `UInt64` | — | Exclusive end byte offset of the `name="value"` attribute token. |
...
| `byteStart` | `UInt64` | — | Inclusive start byte offset of this node in the original source. |
| `byteEnd` | `UInt64` | — | Exclusive end byte offset of this node in the original source. |
...
| `maxConcurrentExtractions` | `UInt64?` | `null` | Maximum concurrent document extractions in batch operations. This is a ceiling within the configured total thread budget, not an independent pool size. When unset, the scheduler derives document and per-document concurrency from `ConcurrencyConfig.max_threads`. |
...
| `minProvenanceFallbackRatio` | `Double` | — | Minimum fraction of a page's non-whitespace characters with `MappingProvenance.Fallback` provenance before the page is treated as having a fabricated text layer and routed to OCR (issue #1254). Gated by `min_total_non_whitespace` so a short page with a stray fallback character cannot trip it. Only used when `enable_provenance_ocr_routing` is `true`. |
