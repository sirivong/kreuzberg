...
| `MaxConcurrency` | `nuint` | `serde(default = "default_max_concurrency")` | Maximum number of in-flight batch requests. Bounds concurrency against the configured LLM provider. Defaults to `DEFAULT_MAX_CONCURRENCY`. |
...
| `ByteStart` | `nuint` | — | Inclusive start byte offset of the `name="value"` attribute token. |
| `ByteEnd` | `nuint` | — | Exclusive end byte offset of the `name="value"` attribute token. |
...
| `ByteStart` | `nuint` | — | Inclusive start byte offset of this node in the original source. |
| `ByteEnd` | `nuint` | — | Exclusive end byte offset of this node in the original source. |
...
| `MaxConcurrentExtractions` | `nuint?` | `null` | Maximum concurrent document extractions in batch operations. This is a ceiling within the configured total thread budget, not an independent pool size. When unset, the scheduler derives document and per-document concurrency from `ConcurrencyConfig.max_threads`. |
