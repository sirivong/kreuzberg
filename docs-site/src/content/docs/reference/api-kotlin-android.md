...
## Kotlin (Android) API Reference <span class="version-badge">v1.0.0-rc.16</span>
...
```kotlin
@JvmStatic
fun fromBytes(bytes: ByteArray, mimeType: String, filename: String? = null): ExtractInput
```

**Example:**

```kotlin
val result = ExtractInput.fromBytes("data".toByteArray(), "value", "value")
```
...
```kotlin
@JvmStatic
fun fromUri(uri: String): ExtractInput
```

**Example:**

```kotlin
val result = ExtractInput.fromUri("value")
```
...
```kotlin
@JvmStatic
fun single(result: ExtractedDocument): ExtractionResult
```

**Example:**

```kotlin
val result = ExtractionResult.single(ExtractedDocument())
```
...
| `cacheDir` | `Path?` | `null` | Custom cache directory for model files. Defaults to `~/.cache/xberg/late-interaction/` if not specified. |
| `acceleration` | `AccelerationConfig?` | `null` | Hardware acceleration for the late-interaction ONNX model. |
| `maxEmbedDurationSecs` | `Long?` | `null` | Maximum wall-clock duration (in seconds) for a single embed call when using `LateInteractionModelType.Plugin`. `null` disables the timeout. |
...
```kotlin
@JvmStatic
fun default(): LateInteractionConfig
```

**Example:**

```kotlin
val result = LateInteractionConfig.default()
```
...
```kotlin
val result = instance.isWellFormed()
```
...
```kotlin
val result = instance.withModelVersion("value")
```
...
| `showDownloadProgress` | `Boolean` | `false` | Show model download progress (local ONNX path only). |
| `cacheDir` | `Path?` | `null` | Custom cache directory for model files. Defaults to `~/.cache/xberg/sparse-embeddings/` if not specified. |
| `acceleration` | `AccelerationConfig?` | `null` | Hardware acceleration for the sparse-embedding ONNX model. |
| `maxEmbedDurationSecs` | `Long?` | `null` | Maximum wall-clock duration (in seconds) for a single embed call when using `SparseEmbeddingModelType.Plugin`. `null` disables the timeout. |
...
```kotlin
@JvmStatic
fun default(): SparseEmbeddingConfig
```

**Example:**

```kotlin
val result = SparseEmbeddingConfig.default()
```
