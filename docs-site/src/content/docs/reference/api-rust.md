...
## Rust API Reference <span class="version-badge">v1.0.0-rc.16</span>
...
| `cache_dir` | `Option<PathBuf>` | `None` | Custom cache directory for model files. Defaults to `~/.cache/xberg/late-interaction/` if not specified. |
...
| `model_version` | `String` | — | Model generation: `"pp-ocrv5"` (default) or `"pp-ocrv6"`. PP-OCRv6 adds a unified CJK+Latin+JA/KO recognition model with `medium`/`small`/`tiny` tiers (see `model_tier`). Scripts outside the v6 unified coverage (Arabic, Cyrillic, Devanagari, Greek, Tamil, Telugu, Thai) transparently fall back to the PP-OCRv5 per-script recognition models. Defaults to `"pp-ocrv5"` so existing configs are unchanged. |
...
| `show_download_progress` | `bool` | `false` | Show model download progress (local ONNX path only). |
| `cache_dir` | `Option<PathBuf>` | `None` | Custom cache directory for model files. Defaults to `~/.cache/xberg/sparse-embeddings/` if not specified. |
