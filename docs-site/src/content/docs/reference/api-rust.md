...
## Rust API Reference <span class="version-badge">v1.0.0-rc.34</span>
...
```rust
pub fn register_document_extractor(extractor: DocumentExtractor) -> Result<(), Error>
```
...
```rust
pub fn unregister_document_extractor(name: &str) -> Result<(), Error>
```
...
```rust
pub fn clear_tokenizer_backends() -> Result<(), Error>
```
...
```rust
pub fn list_tokenizer_backends() -> Result<Vec<String>, Error>
```
...
```rust
pub async fn classify_chunks(result: ExtractedDocument, config: ChunkClassificationConfig) -> Result<(), Error>
```

**Example:**

```rust
classify_chunks(ExtractedDocument::default(), ChunkClassificationConfig::default()).await?;
```
...
```rust
pub async fn embed_sparse_async(texts: Vec<String>, config: SparseEmbeddingConfig) -> Result<Vec<SparseEmbedding>, Error>
```
...
|------|------|----------|-------------|
| `query` | `MultiVectorEmbedding` | Yes | The multi vector embedding |
| `docs` | `Vec<MultiVectorEmbedding>` | Yes | The docs |
...
```rust
pub async fn embed_multi_vector_async(texts: Vec<String>, config: LateInteractionConfig, is_query: bool) -> Result<Vec<MultiVectorEmbedding>, Error>
```
...
| `output_format` | `OutputFormat` | `OutputFormat::Plain` | Target output format (Markdown, plain text, etc.). |
...
| `tier_strategy` | `String` | `Auto` | Which conversion tier to use. - `TierStrategy::Auto` (default) — automatically choose the best path. - `TierStrategy::Tier2` — always use the Tier-2 DOM-walk path. - `TierStrategy::Tier1` — always attempt Tier-1 (testkit only). |
...
```rust
pub async fn extract(&self, input: ExtractInput, config: ExtractionConfig) -> Result<ExtractedDocument, Error>
```
...
```rust
pub fn from_bytes(bytes: &[u8], mime_type: &str, filename: Option<String>) -> ExtractInput
```
...
```rust
pub fn from_uri(uri: &str) -> ExtractInput
```
...
| `concurrency` | `Option<String>` | `None` | Concurrency limits for constrained environments (None = use defaults). Controls Rayon thread pool size, ONNX Runtime intra-op threads, and the combined document/inner-task budget for batch extraction. See `ConcurrencyConfig` for details. |
...
```rust
pub async fn process_image(&self, image_bytes: &[u8], config: OcrConfig) -> Result<ExtractedDocument, Error>
```
...
```rust
pub async fn process_image_file(&self, path: PathBuf, config: OcrConfig) -> Result<ExtractedDocument, Error>
```
...
```rust
pub async fn process_document(&self, path: PathBuf, config: OcrConfig) -> Result<ExtractedDocument, Error>
```
...
```rust
pub fn with_model_version(&self, version: &str) -> PaddleOcrConfig
```
...
```rust
pub async fn process(&self, result: ExtractedDocument, config: ExtractionConfig) -> Result<(), Error>
```

**Example:**

```rust
instance.process(ExtractedDocument::default(), ExtractionConfig::default()).await?;
```
...
```rust
pub fn render_result(&self, result: ExtractedDocument) -> Result<String, Error>
```
...
```rust
pub async fn validate(&self, result: ExtractedDocument, config: ExtractionConfig) -> Result<(), Error>
```
