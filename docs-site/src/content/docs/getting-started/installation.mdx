...
with `await initWasm()` (or the default `init()` export) before calling `extract`.
:::

:::caution[Wasm Platform Limitations]
The Wasm binding does not support:

- **Layout detection** (RT-DETR model inference requires ONNX Runtime, unavailable in WebAssembly; RT-DETR also runs on the pure-Rust `tract` engine, but layout detection is not yet enabled for the `wasm-target` feature set)
- **PaddleOCR, embeddings, reranking, and transcription inference** (all require ONNX Runtime)
- **Auto-rotate document-orientation detection** (runs through ONNX Runtime natively; a pure-Rust `tract` path exists via `auto-rotate-tract` and is already used on the Android x86_64 emulator, but WASM wiring is not complete yet)
...
Pure-Rust extraction formats, OCR via Tesseract WASM, chunking, metadata, tables, language detection, SVG handling, redaction, summarization, QR-code detection, and image extraction work in WASM. See the [WASM API Reference](/reference/api-wasm/) for details.
:::
