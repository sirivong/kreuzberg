...
- **build/elixir-nif**: regenerated `packages/elixir/native/xberg_nif/Cargo.lock`, which was stale — it pinned `orp 0.1.1` / `xberg-gliner 0.2.0` while xberg rc.35 requires `orp ^0.1.2` / `gliner ^0.2.1`, making `cargo build` unsatisfiable (`failed to select a version for orp`). The lock now resolves orp 0.1.2 + gliner 0.2.1 (and syncs tree-sitter-language-pack to 1.10.9). (`packages/elixir/native/xberg_nif/Cargo.lock`)
...
- **WASM package now publishes its `pkg/` entry points.** The `wasm-bundle` artifact omitted
  ...
  published `@xberg-io/xberg-wasm` tarball was missing its entry points. `pkg/**` + `package.json` are now
  included. (`.github/workflows/publish.yaml`)
- **npm provenance restored.** The main `@xberg-io/xberg` package, its platform sub-packages, and the
...
- **Node main package waits for its platform sub-packages to index.** `publish-node` now
  `wait-for-package`s on `@xberg-io/xberg-linux-{x64,arm64}-gnu` before publishing the main package,
  closing the window where `npm install @xberg-io/xberg` could fail on unresolved optionalDependencies.
...
- **Swift publish now substitutes the XCFramework checksum and creates the `release/swift/<version>`
  ...
  e2e/test-app pins (`.package(url: …, branch: "release/swift/<version>")`), so SwiftPM could not
  resolve the package. A new `update-swift-package-manifest` job downloads the bundle checksum,
  ...
  (`.github/workflows/publish.yaml`)
- **bindings/node**: the npm package now ships **musl** (Alpine/static-libc) native binaries for `linux-x64-musl` and `linux-arm64-musl`, built in an Alpine container (`docker/Dockerfile.musl-node`, system ONNX Runtime + libheif, napi-rs cross via the musl Rust target). napi-rs already selects gnu vs musl at runtime via each platform package's `libc` field, and `crates/xberg-node/package.json` already declared the `@xberg-io/xberg-linux-{x64,arm64}-musl` optionalDependencies — they were just never built. Alpine/musl Node users now get a working native binding instead of a missing optional dependency. (Go and Dart musl are not yet shipped — Go cgo cannot select libc flavor at the consumer's build, and Dart's musl path needs alef-generator changes to its native-lib downloader.)
...
- **net/tls (android)**: the `native-tls` switch from #1146 pulled `openssl-sys` into the dependency graph for every non-Windows target, which has no working Android NDK cross-compile (no target sysroot), breaking `android-target` builds (Android cargo check, Kotlin/Dart Android natives). On Android the `ureq`/`hf-hub` dependencies now use ring-backed `rustls` + `webpki-roots` (which cross-compiles cleanly) instead of `native-tls`; feature unification means this also keeps `openssl-sys` out of the `tree-sitter-language-pack` `download` path. Desktop/server (Linux, macOS) and Windows keep `native-tls` for OS-certificate-store / `SSL_CERT_FILE` trust (#1146); corporate-CA trust is a desktop/server concern, mobile apps ship their own trust configuration.
- **build/heic**: backed out the embedded-libheif source build introduced earlier this cycle; the `heic` feature again uses a system libheif (pkg-config on Linux/macOS, vcpkg on Windows) provisioned by the `install-system-deps` CI action. The embedded build (libheif 1.23.0 from source via cmake) failed to compile its bundled dav1d plugin against current `libdav1d` (`Dav1dSettings.n_threads` was renamed to `n_tile_threads`), breaking the Python-arm wheel build, and disabling the libheif crate's default features stripped the required libheif API version feature on Windows, breaking the Node windows-msvc build. Restoring system provisioning fixes both; CI/publish jobs that compile `heic` must run `install-system-deps` (or, in cibuildwheel containers, install libheif via `CIBW_BEFORE_ALL`).
- **chunking**: a single-page very wide PDF no longer emits a spurious `Page boundary 0 has byte_end=… which exceeds text length …` validation warning (#1148). When `recompute_boundaries_from_pages` cannot locate a page inside the rendered content (wide-page whitespace layout breaks the substring match), the pipeline falls back to `metadata.pages.boundaries`, whose offsets are computed against the longer raw extractor text and overshoot the rendered `chunk_input`. Page boundaries are now clamped to a valid char boundary within the chunked text before validation, so provenance stays best-effort without the warning; already-valid boundaries are unchanged.
...
- **layout-detection**: `PpDocLayoutV3Model` `im_shape` input now correctly passes the resized input dimensions `[800.0, 800.0]` instead of the original image dimensions. PaddleDetection convention is that `im_shape` is the H×W of the tensor fed in — the model divides output box coordinates by `scale_factor` to map them back to original-image pixel space. Passing the original image size caused output bounding boxes to be ×scale larger than the image, breaking GLM-OCR paired mode (crops were out-of-bounds). Two new unit tests (`preprocess_single_im_shape_is_resized_dimensions_not_original`, `preprocess_single_scale_factor_is_resized_over_original`) verify the convention.
- **candle-ocr**: GLM-OCR paired mode no longer fails the entire page when a layout crop has an extreme aspect ratio (>200:1). Inline formula regions detected by PP-DocLayout-V3 can be very wide and thin (e.g. 843:1), causing the GLM-OCR preprocessor to reject them with `UnsupportedConfig`. The region is now skipped with a `tracing::warn!` and processing continues for the remaining regions on the page.
- **benchmark-harness**: `field-quality --mode formula` now applies a 600-second extraction timeout (was inheriting the 60-second default). GLM-OCR + PP-DocLayout-V3 on CPU requires substantially more than 60 seconds for multi-page documents; the 60-second default caused silent timeouts on the `code_and_formula` fixture.
- **layout-detection**: `PpDocLayoutV3Model` now implements the real PP-DocLayout-V3 ONNX contract. The previous speculative implementation passed a single `images` input at 960×960 with letterbox + ImageNet normalization and parsed `boxes`/`scores`/`labels` tensors — none of which match the actual ONNX. The real model requires three named inputs (`im_shape`, `image`, `scale_factor`) at 800×800 with /255-only normalization (no ImageNet mean/std), returns NMS output as `fetch_name_0` (`[N, 7]` f32, column layout `[class_id, score, x1, y1, x2, y2, _unused]`) and `fetch_name_1` (`[batch]` i32 per-image bbox counts), and unscales boxes to original-image pixel coordinates internally — no manual letterbox inversion is needed. Class mapping updated from the speculative 8-class scheme to the real 25-class PP-DocLayout-V3 taxonomy (0 abstract … 24 vision_footnote), with formula IDs 5/11/15 → `Formula` and chart ID 3 → `Chart`, enabling GLM-OCR paired-mode formula and chart extraction. SHA256 and size for the staged model file (`93d1197e…`, 131 731 131 bytes) are now recorded in `LayoutModelManager`. The `test_ensure_all_models_with_cache` test is fixed to pre-populate all 7 model entries (was pre-populating only rtdetr+tatr, causing download-dependent failures for the other five).
- **pdf**: Layout-guided reading-order reconstruction (`reading_order: true`) now reorders both plain-text and markdown/Djot/HTML outputs. Previously, only the plain-text path honored reading-order; markdown paths ignored the setting entirely and extracted blocks in native PDF order. Now segments are reordered at the structure level (via `reorder_segments_by_layout()`) before they feed the markdown/Djot/HTML rendering pipeline, ensuring multi-column PDFs produce columns in natural reading order (left-to-right, top-to-bottom) rather than PDF extraction order. Verified by a unit test (column-major ordering independent of layout-hint order) and benchmark single-column-preservation (order score stays 1.0 on single-column pages); the existing benchmark ground truth is captured in native extraction order, so it cannot quantify the multi-column gain. Requires `layout: Some(default)` and `use_layout_for_markdown: true` (layout hints must be computed).
...
- **ocr**: Tesseract no longer allocates its native FFI handle eagerly. `TesseractBackend::new()` is now infallible and defers `OcrProcessor` construction (Tesseract/Leptonica handle) to the first `process()` call via `OnceCell`, so building the OCR backend registry — which happens on some non-OCR paths — no longer pays the Tesseract init cost. (#1129)
- **core**: The `core::extractor` test module is now gated on `tokio-runtime`. Its tests call the `tokio-runtime`-only batch/sync helpers (`batch_extract_files`, `batch_extract_bytes`, `extract_file_sync`, `batch_extract_files_sync`), so `cargo test` failed to compile for feature sets without `tokio-runtime` (e.g. `heuristics`). Likewise gated the `llm::client` path-normalization test on `api` (uses `axum`) and the `service::extraction` tempfile test on `mcp` (uses `file_with_mime`) so `cargo test --features structured` compiles.
...
- **bindings**: All language bindings regenerated against alef 0.25.37, exposing the structured-extraction JSON bridge (`extract_structured_json` / `split_and_extract_json`), the RAG chunker (`chunk_for_rag` returning a now-bindable `ChunkingResult`), and multi-document boundary detection (`detect_boundaries` / `boundaries_from_extraction_result` over the serde-bindable `DocumentBoundary`/`BoundaryReason`/`MultidocInput`/`PageSignals`/`MultidocThresholds`). The structured engine's typed Rust surface (`StructuredOptions`, `StructuredOutput`, `PresetSpec`, `VisionCallCache`, the vision/chunk/prompt mechanism types, and `StructuredError`) stays Rust-only — bindings consume the JSON-string bridge.
- **types**: `Formula { latex, bbox, page }` and `ExtractionResult.formulas` — public surface for layout-guided formula extraction (populated under `layout-detection`). New `PdfFormField`/`FormFieldType` types and `ExtractionResult.form_fields` for AcroForm/XFA form extraction. `ChunkMetadata.heading_path: Vec<String>` — flattened RAG-shaped heading breadcrumb derived from `heading_context`. All additive (empty/`Vec::new()` by default).
- **config**: `LayoutDetectionConfig.enable_chart_understanding` (default `false`) routes chart-classified regions to the chart OCR task. `PdfConfig.extract_form_fields` (default `true`) and `PdfConfig.reading_order` (default `false`) gate form-field extraction and layout-ordered text emission.
- **layout/ocr**: GLM-OCR paired mode now extracts `LayoutClass::Formula` regions into `ExtractionResult.formulas` (raw LaTeX + bbox). New internal `LayoutClass::Chart` (PP-DocLayout class 5) routes to the chart task only when `enable_chart_understanding` is set; otherwise it is treated identically to `Picture`, so default behavior is unchanged.
- **pdf**: Form fields are extracted into `ExtractionResult.form_fields` via `pdf_oxide`'s form API, carried through the pipeline on `InternalDocument` (no metadata round-trip). Both layers are supported: AcroForm (canonical) plus XFA via the `pdf_oxide::xfa` parser; XFA fields whose names already exist in the AcroForm layer are de-duplicated so hybrid documents are not double-counted. Controlled by `PdfConfig.extract_form_fields`.
- **pdf**: Layout-guided reading-order reconstruction (`PdfConfig.reading_order`, default `false`, requires `layout-detection`). Projects text spans onto detected regions, performs column detection, and emits spans in natural reading order — corrects column scrambling on multi-column PDFs. Default-off is a byte-identical no-op.
- **chunking**: `chunk_for_rag` composer produces retrieval-shaped chunks with `ChunkMetadata.heading_path` populated from the resolved heading context.
...
- **Note: npm trusted publishing remains a per-package external config on
  `npmjs.org` for `@xberg-io/xberg`, the 8 platform sub-packages, and
  `@xberg-io/xberg-wasm`.** Until that binding is registered, the Publish Node
...
- **`@xberg-io/xberg-linux-x64-musl` and `@xberg-io/xberg-linux-arm64-musl` placeholder publishes.**
...
- **ner-onnx**: switched to the `xberg-gliner` fork with ONNX Runtime 2.0.0-rc.12. NER backend is now functional and coexists with workspace ORT version.
...
- **#826**: WASM loading on Next.js / Turbopack — `@xberg-io/xberg-wasm` now bundles cleanly under webpack 5, Turbopack, and Next.js's app router. Dynamic imports of Node built-ins and the pdfium-js subsystem carry `/* webpackIgnore: true */` markers so bundlers stop trying to inline platform-specific binaries.
...
- **Node bindings build uses a path-based pnpm filter** (`pnpm --filter ./crates/xberg-node`) instead of the never-resolved `pnpm --filter @xberg-io/xberg`. `crates/xberg-node/package.json`'s name is `xberg`, so the scoped filter never matched. Updated three sites (workflow + two scripts).
...
- **WASM benchmark module resolution** — WASM benchmark script failed to load `@xberg-io/xberg-wasm` through pnpm virtual store due to `import.meta.url` resolution issues in tsx. Changed to direct import from local build path.
...
- TypeScript/Node.js package renamed from `xberg` to `@xberg-io/xberg`.
...
- Binary wheels only.
- TypeScript/Node.js package renamed to `@xberg-io/xberg`.
