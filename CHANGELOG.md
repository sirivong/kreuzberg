...
- **pdf/chunking**: Fix `firstPage`/`lastPage` null on chunks extracted from multi-page PDFs. `extract_text_with_tracking` stored raw `page_text` in `PageContent.content` while `result.content` contained the cleaned version (after `fix_pdf_control_chars` removed/replaced PDF control characters U+0001–U+001F). `recompute_boundaries_from_pages` then failed the exact substring search for affected pages, leaving their chunks without page metadata. Fix: store `cleaned.into_owned()` in `PageContent.content` so both match exactly.
