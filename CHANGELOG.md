...
- **ocr**: `candle-trocr` feature enabling real BEiT+RoBERTa TrOCR inference (transformer-based printed/handwritten text recognition) via candle. Wires TrOCR encoder-decoder model behind new `candle-trocr` aggregate feature; backend selectable via `--ocr-backend candle-trocr`. Phase 3a complete: weight downloading, safetensors parsing, model loading on CPU/CUDA/Metal. Phase 3b (sequence generation) pending. Sub-features on `kreuzberg-candle-ocr` for future VLM backends (GOT-OCR 2.0, GLM-OCR, PaddleOCR-VL).
- **ocr**: `candle-paddleocr-vl` feature enabling PaddleOCR-VL 0.9B vision-language model (multi-task: OCR, tables, formulas, charts) via candle. Emits markdown directly from the VLM; extraction pipeline skips layout-reconstruction stages for VLM output. Supports 109+ languages. Selectable via `--ocr-backend candle-paddleocr-vl` with task selection via backend options (`{"task": "table"}`). Phase 3b complete.
- **ocr**: `candle-got-ocr` feature enabling GOT-OCR 2.0 end-to-end vision-language model (0.7B parameters) for structured document OCR with markdown output. VitDet vision encoder + Qwen-0.5B decoder architecture. Emits markdown tables, formulas, and complex layouts directly. Supports English and Chinese with graceful fallback to other languages. Selectable via `--ocr-backend candle-got-ocr` with output format selection via backend options (`{"output_format": "markdown" | "latex"}`). Phase 3c complete: full model architecture, HF Hub weight loading, image preprocessing, model initialization, and placeholder inference pipeline.
- **ocr**: `candle-glm-ocr` feature enabling GLM-OCR 0.9B lightweight vision-language model optimized for multi-lingual document OCR. CogViT 400M parameter encoder + GLM-0.5B decoder with cross-attention. Achieves SOTA on OmniDocBench V1.5 (94.62 accuracy, March 2026). Emits structured markdown for tables, formulas, and complex layouts. Supports 8+ languages (English, Chinese, French, Spanish, Russian, German, Japanese, Korean). Selectable via `--ocr-backend candle-glm-ocr` with device selection via backend options (`{"device": "auto" | "cpu" | "cuda" | "metal"}`). Phase 3d complete: CogViT encoder port, GLM text decoder port, model loading, tokenizer integration, end-to-end inference.
- **benchmark-harness**: Candle OCR pipeline variants (`candle-trocr`, `candle-paddleocr-vl`, `candle-got-ocr`, `candle-glm-ocr`) wired into benchmark harness for comparative evaluation against tesseract and paddle baselines.
