#!/usr/bin/env python3
"""Vendor xberg core crate into R package
Used by: ci-r.yaml - Vendor xberg core crate step
...
def main() -> None:
    ...
    crate_names = ["xberg", "xberg-ffi", "xberg-tesseract", "xberg-paddle-ocr"]
    ...
    # First handle xberg-ffi's dependency on xberg
    if "xberg-ffi" in copied_crates:
    ...
    if "xberg" in copied_crates:
        ...
        if xberg_toml.exists():
            ...
            if "xberg-tesseract" in copied_crates:
                content = re.sub(
                    r'xberg-tesseract = \{ version = "[^"]*", optional = true \}',
            ...
            if "xberg-paddle-ocr" in copied_crates:
                content = re.sub(
                    r'xberg-paddle-ocr = \{ version = "[^"]*", optional = true \}',
