...
describe("Batch API Tests - Sync", () => {
	it("should batch extract multiple files synchronously", () => {
		const paths = [resolveDocument("text/report.txt"), resolveDocument("data_formats/simple.json")];
	...
	it("should batch extract bytes synchronously", () => {
		...
		const bytes = [getDocumentBytes("text/report.txt"), getDocumentBytes("data_formats/simple.json")];
	...
	it("should batch extract bytes with correct MIME mapping", () => {
		...
		const bytes = [getDocumentBytes("pdfs/fake_memo.pdf"), getDocumentBytes("office/document.docx")];
