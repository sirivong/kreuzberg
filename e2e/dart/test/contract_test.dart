...
void main() {
  ...
  test('Tests async batch bytes extraction with per-file configs (batch_extract_bytes with file_configs parameter)', () async {
    final _config = await createConfigFromJson(json: '{"output_format":"markdown"}');
  ...
  test('Tests markdown chunker prepends heading hierarchy to chunk content', () async {
    final _config = await createConfigFromJson(json: '{"chunking":{"chunker_type":"markdown","max_chars":300,"max_overlap":50,"prepend_heading_context":true}}');
  ...
  test('Tests document structure with DOCX heading-driven nesting', () async {
    final _config = await createConfigFromJson(json: '{"include_document_structure":true}');
    final result = await KreuzbergBridge.extractBytesSync(File('docx/fake.docx').readAsBytesSync(), 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', _config);
  ...
  test('Tests element-based result format with element type assertions on DOCX', () async {
    final _config = await createConfigFromJson(json: '{"result_format":"element_based"}');
  ...
  test('Tests that extraction_timeout_secs config field is accepted and does not affect fast extractions', () async {
    final _config = await createConfigFromJson(json: '{"extraction_timeout_secs":300}');
  ...
  test('Tests keyword extraction via YAKE algorithm', () async {
    final _config = await createConfigFromJson(json: '{"keywords":{"algorithm":"yake","max_keywords":10}}');
    final result = await KreuzbergBridge.extractBytesSync(File('pdf/fake_memo.pdf').readAsBytesSync(), 'application/pdf', _config);
    expect(result.mimeType.toString().trim(), equals('application/pdf'.toString().trim()));
    expect(result.content.length, greaterThanOrEqualTo(10));
  ...
  test('Tests page extraction and page marker configuration', () async {
    final _config = await createConfigFromJson(json: '{"pages":{"extract_pages":true,"insert_page_markers":true}}');
    final result = await KreuzbergBridge.extractBytesSync(File('pdf/fake_memo.pdf').readAsBytesSync(), 'application/pdf', _config);
    expect(result.mimeType.toString().trim(), equals('application/pdf'.toString().trim()));
    expect(result.content.length, greaterThanOrEqualTo(10));
  ...
  test('Tests quality scoring produces a score value in [0.0, 1.0]', () async {
    final _config = await createConfigFromJson(json: '{"enable_quality_processing":true}');
    final result = await KreuzbergBridge.extractBytesSync(File('pdf/fake_memo.pdf').readAsBytesSync(), 'application/pdf', _config);
    expect(result.mimeType.toString().trim(), equals('application/pdf'.toString().trim()));
    expect(result.content.length, greaterThanOrEqualTo(10));
  ...
  test('Tests archive extraction with custom security limits', () async {
    final _config = await createConfigFromJson(json: '{"security_limits":{"max_archive_size":104857600,"max_compression_ratio":50,"max_files_in_archive":100}}');
  ...
  test('Tests tree-sitter configuration round-trip', () async {
    final _config = await createConfigFromJson(json: '{"tree_sitter":{"groups":["web"],"languages":["python","rust"],"process":{"comments":false,"diagnostics":false,"docstrings":false,"exports":true,"imports":true,"structure":true,"symbols":false}}}');
