...
// alef:hash:833c1a93cddf114142d8cd0b629272d9c25b3782e6b3e7a12e8cce1810d33ed7
...
void main() {
  ...
  test('Smoke test: PNG image (without OCR, metadata only)', () async {
    final result = await KreuzbergBridge.extractBytes(File('images/sample.png').readAsBytesSync(), mimeType: 'image/png', ExtractionConfig(useCache: true, enableQualityProcessing: true, forceOcr: false, disableOcr: true, resultFormat: ResultFormat.unified, outputFormat: OutputFormat.plain(), includeDocumentStructure: false, useLayoutForMarkdown: false, maxArchiveDepth: 3));
    expect(result.mimeType.toString().trim(), equals('image/png'.toString().trim()));
    expect(result.metadata.format.toString().trim(), equals('PNG'.toString().trim()));
  });
