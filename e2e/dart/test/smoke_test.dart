...
void main() {
  ...
  test('Smoke test: PNG image (without OCR, metadata only)', () async {
    final _config = await createConfigFromJson(json: '{"disable_ocr":true}');
    final result = await KreuzbergBridge.extractBytes(File('images/sample.png').readAsBytesSync(), 'image/png', _config);
    expect(result.mimeType.toString().trim(), equals('image/png'.toString().trim()));
  });
