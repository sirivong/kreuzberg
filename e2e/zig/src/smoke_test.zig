...
test "ocr_image_png" {
    ...
    try testing.expect(result.object.get("content").?.string.len >= @as(usize, 1));
    try testing.expect(
...
test "smoke_xlsx_basic" {
    ...
    try testing.expect(result.object.get("content").?.string.len >= @as(usize, 100));
    {
    ...
    try testing.expect(result.object.get("metadata").?.object.get("format").?.object.get("sheet_count").?.integer >= @as(i64, 2));
    {
        const _jva0 = result.object.get("metadata").?.object.get("format").?.object.get("sheet_names").?;
        const _jsa0 = if (_jva0 == .string) _jva0.string else try std.json.Stringify.valueAlloc(std.heap.c_allocator, _jva0, .{});
        defer if (_jva0 != .string) std.heap.c_allocator.free(_jsa0);
