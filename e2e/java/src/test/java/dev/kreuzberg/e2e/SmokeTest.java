...
// alef:hash:4221d2da2122aaf08a016a9417777b690040701ba962a48b622e1f2944c970fb
...
assertEquals("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", result.mimeType().trim());assertTrue(result.content().length() >= 100, "expected length >= 100");assertTrue(result.content().contains("Team"), "expected to contain: " + "Team");assertTrue(result.content().contains("Location"), "expected to contain: " + "Location");assertTrue(result.content().contains("Stanley Cups"), "expected to contain: " + "Stanley Cups");assertTrue(result.content().contains("Blues"), "expected to contain: " + "Blues");assertTrue(result.content().contains("Flyers"), "expected to contain: " + "Flyers");assertTrue(result.content().contains("Maple Leafs"), "expected to contain: " + "Maple Leafs");assertTrue(result.content().contains("STL"), "expected to contain: " + "STL");assertTrue(result.content().contains("PHI"), "expected to contain: " + "PHI");assertTrue(result.content().contains("TOR"), "expected to contain: " + "TOR");assertTrue(result.tables().size() >= 1, "expected at least 1 elements");assertTrue(java.util.Optional.ofNullable(result.metadata().format().excel().sheetCount()).orElse(0L) >= 2, "expected >= 2");assertTrue(result.metadata().format().excel().sheetNames().toString().contains("Stanley Cups"), "expected to contain: " + "Stanley Cups");
    }
