...
// alef:hash:ff68e328c6be45bbfdc9c8b0ce612274b3a3e801fc91f2acc8be4feca876483b
...
final class FormatSpecificTests: XCTestCase {
    ...
    func testFormatDocxStandalone() throws {
        ...
        let result = try Kreuzberg.extractBytesSync(content: contentVec, mime_type: "application/vnd.openxmlformats-officedocument.wordprocessingml.document", config: configObj)
    ...
    func testFormatHwpxStandalone() throws {
        ...
        for _byte in contentData { contentVec.push(value: _byte) }
        let configObj = try extractionConfigFromJson("{}")
        let result = try Kreuzberg.extractBytesSync(content: contentVec, mime_type: "application/haansofthwpx", config: configObj)
        XCTAssertGreaterThanOrEqual(result.content.count, 20)
    ...
    func testFormatPdfText() throws {
        ...
        for _byte in contentData { contentVec.push(value: _byte) }
        let configObj = try extractionConfigFromJson("{}")
        let result = try Kreuzberg.extractBytesSync(content: contentVec, mime_type: "application/pdf", config: configObj)
    ...
    func testFormatPptx() throws {
        ...
        let configObj = try extractionConfigFromJson("{}")
        let result = try Kreuzberg.extractFileSync(path: "pptx/simple.pptx", mime_type: "application/vnd.openxmlformats-officedocument.presentationml.presentation", config: configObj)
    }

    func testFormatXlsx() throws {
        // XLSX spreadsheet extraction using extract_file_sync
        let configObj = try extractionConfigFromJson("{}")
        let result = try Kreuzberg.extractFileSync(path: "xlsx/stanley_cups.xlsx", mime_type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", config: configObj)
    }
