...
// alef:hash:4705d4774a4077562adb000ccd0623299964f0fb63d7aeef504ef0785979d466
...
func Test_OcrImagePng(t *testing.T) {
	...
	{
		found := false
		if strings.Contains(fmt.Sprint(result.Content), `Hello`) {
			found = true
		}
		if strings.Contains(fmt.Sprint(result.Content), `World`) {
			found = true
		}
		if strings.Contains(fmt.Sprint(result.Content), `hello`) {
			found = true
...
func Test_SmokeDocxBasic(t *testing.T) {
	...
	{
		found := false
		if strings.Contains(fmt.Sprint(result.Content), `Lorem`) {
			found = true
		}
		if strings.Contains(fmt.Sprint(result.Content), `ipsum`) {
			found = true
		}
		if strings.Contains(fmt.Sprint(result.Content), `document`) {
			found = true
		}
...
func Test_SmokePdfBasic(t *testing.T) {
	...
	assert.GreaterOrEqual(t, len(result.Content), 50, "expected length >= 50")
	{
		...
		if strings.Contains(fmt.Sprint(result.Content), `To Whom it May Concern`) {
			found = true
		}
		if !found {
			t.Errorf("expected to contain at least one of the specified values")
...
func Test_SmokeXlsxBasic(t *testing.T) {
	...
	if !strings.Contains(fmt.Sprint(result.Content), `Team`) {
		t.Errorf("expected to contain %s", `Team`)
	}
	if !strings.Contains(fmt.Sprint(result.Content), `Location`) {
		t.Errorf("expected to contain %s", `Location`)
	}
	if !strings.Contains(fmt.Sprint(result.Content), `Stanley Cups`) {
		t.Errorf("expected to contain %s", `Stanley Cups`)
	}
	if !strings.Contains(fmt.Sprint(result.Content), `Blues`) {
		t.Errorf("expected to contain %s", `Blues`)
	}
	if !strings.Contains(fmt.Sprint(result.Content), `Flyers`) {
		t.Errorf("expected to contain %s", `Flyers`)
	}
	if !strings.Contains(fmt.Sprint(result.Content), `Maple Leafs`) {
		t.Errorf("expected to contain %s", `Maple Leafs`)
	}
	if !strings.Contains(fmt.Sprint(result.Content), `STL`) {
		t.Errorf("expected to contain %s", `STL`)
	}
	if !strings.Contains(fmt.Sprint(result.Content), `PHI`) {
		t.Errorf("expected to contain %s", `PHI`)
	}
	if !strings.Contains(fmt.Sprint(result.Content), `TOR`) {
		t.Errorf("expected to contain %s", `TOR`)
	}
	assert.GreaterOrEqual(t, len(result.Tables), 1, "expected at least 1 elements")
	if result.Metadata.Format != nil {
