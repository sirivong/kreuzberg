...
// alef:hash:5977fb22aae0e33d3108024f20c37214e1ee9c1937c7b878299583ccacd4bb7d
...
async fn test_batch_extract_bytes_happy() {
    ...
    let items_json = serde_json::json!([{"content": [72, 101, 108, 108, 111, 44, 32, 119, 111, 114, 108, 100, 33], "mime_type": "text/plain"}, {"content": [60, 104, 116, 109, 108, 62, 60, 98, 111, 100, 121, 62, 84, 101, 115, 116, 60, 47, 98, 111, 100, 121, 62, 60, 47, 104, 116, 109, 108, 62], "mime_type": "text/html"}]);
    let items = serde_json::from_value::<Vec<BatchBytesItem>>(items_json).unwrap();
    let config = Default::default();
...
async fn test_batch_extract_bytes_mixed_format() {
    ...
    let items_json = serde_json::json!([{"content": [80, 68, 70, 32, 112, 108, 97, 99, 101, 104, 111, 108, 100, 101, 114], "mime_type": "application/x-unknown"}]);
    let items = serde_json::from_value::<Vec<BatchBytesItem>>(items_json).unwrap();
    let config = Default::default();
...
fn test_batch_extract_bytes_sync_invalid_mime() {
    ...
    let items_json = serde_json::json!([{"content": [100, 97, 116, 97], "mime_type": "application/x-unknown"}]);
    let items = serde_json::from_value::<Vec<BatchBytesItem>>(items_json).unwrap();
    let config = Default::default();
    let _ = batch_extract_bytes_sync(items, &config).expect("should succeed");
}
