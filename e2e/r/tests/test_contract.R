...
test_that("config_quality_enabled: Tests quality scoring produces a score value in [0.0, 1.0]", {
  ...
  expect_equal(trimws(result$mime_type), "application/pdf")
  expect_true(nchar(result$content) >= 10)
  expect_true(if (is.character(result$quality_score)) length(result$quality_score) > 0 && any(nchar(result$quality_score) > 0) else length(result$quality_score) > 0)
  expect_true(result$quality_score >= 0.0)
