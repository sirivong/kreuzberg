...
// alef:hash:81456cab53e73b9808f285f61fc6fe4ce9d24f5ff30811e97ef98422578edbd0
...
class ContractTest {
    ...
    fun testApiBatchBytesAsync() = runBlocking {
        ...
        val result = Kreuzberg.extractFile(java.nio.file.Path.of("pdf/fake_memo.pdf"), null, null)
    ...
    fun testApiBatchBytesWithConfigsAsync() = runBlocking {
        ...
        val result = Kreuzberg.extractFile(java.nio.file.Path.of("pdf/fake_memo.pdf"), null, "{\"output_format\":\"markdown\"}")
        assertEquals("application/pdf", result.mimeType.trim())
        assertTrue(result.content.length >= 10, "expected length >= 10")
    ...
    fun testConfigChunkingPrependHeadingContext() = runBlocking {
        ...
        val result = Kreuzberg.extractFileSync(java.nio.file.Path.of("markdown/extraction_test.md"), null, config)
        assertTrue(result.content.length >= 10, "expected length >= 10")
        assertTrue(chunks.size >= 2, "expected >= 2 chunks")
        assertTrue(result.chunksHaveContent, "expected true")
        assertTrue(result.chunksHaveHeadingContext, "expected true")
        assertTrue(result.firstChunkStartsWithHeading, "expected true")
    }
    ...
    fun testConfigElementTypes() = runBlocking {
        ...
        val result = Kreuzberg.extractFileSync(java.nio.file.Path.of("docx/unit_test_headers.docx"), null, config)
    ...
    fun testConfigKeywords() = runBlocking {
        ...
        assertEquals("application/pdf", result.mimeType.trim())
        assertTrue(result.content.length >= 10, "expected length >= 10")
        assertFalse(result.keywords.isEmpty(), "expected non-empty value")
        assertTrue(result.keywords.size >= 1, "expected at least 1 elements")
    }
    ...
    fun testConfigTreeSitter() = runBlocking {
        ...
        val result = Kreuzberg.extractFileSync(java.nio.file.Path.of("code/hello.py"), null, config)
