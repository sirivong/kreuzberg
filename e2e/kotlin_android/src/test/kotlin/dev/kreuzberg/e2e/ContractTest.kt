...
class ContractTest {
    ...
    fun testApiBatchBytesWithConfigsAsync() = runBlocking {
        ...
        val config_Config = MAPPER.readValue("{\"output_format\":\"markdown\"}", ExtractionConfig::class.java)
        val result = Kreuzberg.extractFile("pdf/fake_memo.pdf", null, config_Config)
        assertEquals("application/pdf", result.mimeType.trim())
        assertTrue(result.content.length >= 10, "expected length >= 10")
    ...
    fun testOutputFormatBytesMarkdown() = runBlocking {
        ...
        val result = Kreuzberg.extractBytesSync(java.nio.file.Files.readAllBytes(java.nio.file.Paths.get("pdf/fake_memo.pdf")), "application/pdf", config)
        assertEquals("application/pdf", result.mimeType.trim())
        assertTrue(result.content.length >= 10, "expected length >= 10")
        assertEquals("markdown", result.metadata.outputFormat.orEmpty().trim())
    }
