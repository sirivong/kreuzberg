...
import { describe, expect, it } from 'vitest';import { extractFile, embedTextsAsync, type JsExtractionConfig } from 'kreuzberg';
...
describe('embed_async_pending', () => {  it('embed_texts_async_empty_input: embed_texts_async: empty text list', async () => {    const result = await embedTextsAsync([], undefined);    expect(result.length).toBe(0);
  ...
  it('embed_texts_async_preset_switch: embed_texts_async: preset override', async () => {    await embedTextsAsync(["Text"], { model: { name: "balanced", type: "preset" } } as JsExtractionConfig);
  }, 600000);
});
