...
import { describe, expect, it } from 'vitest';import { extractFile, extractBytesSync, type JsExtractionConfig } from 'kreuzberg';
...
describe('error', () => {  it('error_empty_bytes: Graceful handling of empty bytes (should not error)', async () => {    const _content_content = await (await import('node:fs/promises')).readFile('text/empty.txt');    extractBytesSync(_content_content, "text/plain", undefined);
  ...
  it('error_extract_bytes_conflicting_ocr: extract_bytes force+disable OCR', async () => {    await expect(async () => {      const _content_content = await (await import('node:fs/promises')).readFile('text/fake_text.txt');      await extractBytesSync(_content_content, "text/plain", { disableOcr: true, forceOcr: true } as JsExtractionConfig);    }).rejects.toThrow();
  }, 30000);
