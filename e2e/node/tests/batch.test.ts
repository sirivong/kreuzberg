...
import { describe, expect, it } from 'vitest';import { extractFile, batchExtractBytesSync, batchExtractBytes, batchExtractFiles, batchExtractFilesSync, BatchBytesItem, BatchFileItem, type JsExtractionConfig } from 'kreuzberg';
...
describe('batch', () => {  it('batch_bytes_invalid_mime: batch_extract_bytes_sync invalid MIME', () => {    batchExtractBytesSync([{ content: [72, 101, 108, 108, 111], mimeType: "application/x-nonexistent" }], undefined);
  }, 30000);
  it('batch_extract_bytes_happy: batch_extract_bytes: happy path with mixed inputs', async () => {    const result = await batchExtractBytes([{ content: [72, 101, 108, 108, 111, 44, 32, 119, 111, 114, 108, 100, 33], mimeType: "text/plain" }, { content: [60, 104, 116, 109, 108, 62, 60, 98, 111, 100, 121, 62, 84, 101, 115, 116, 60, 47, 98, 111, 100, 121, 62, 60, 47, 104, 116, 109, 108, 62], mimeType: "text/html" }], undefined);    expect(result.length).toBeGreaterThanOrEqual(1);

  }, 30000);
  it('batch_extract_bytes_mixed_format: batch_extract_bytes: handles unsupported MIME gracefully', async () => {    await batchExtractBytes([{ content: [80, 68, 70, 32, 112, 108, 97, 99, 101, 104, 111, 108, 100, 101, 114], mimeType: "application/x-unknown" }], undefined);
  }, 30000);
  ...
  it('batch_extract_bytes_sync_invalid_mime: batch_extract_bytes_sync: unsupported MIME', () => {    batchExtractBytesSync([{ content: [100, 97, 116, 97], mimeType: "application/x-unknown" }], undefined);
  }, 30000);
