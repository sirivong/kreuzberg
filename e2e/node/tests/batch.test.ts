...
describe("batch", () => {
  it("extract_batch_bytes_happy: extract_batch: happy path with mixed inputs", async () => {
    const result = await extractBatch(
      [
        { bytes: [72, 101, 108, 108, 111, 44, 32, 119, 111, 114, 108, 100, 33], kind: "bytes", mimeType: "text/plain" },
        {
          bytes: [
            60, 104, 116, 109, 108, 62, 60, 98, 111, 100, 121, 62, 84, 101, 115, 116, 60, 47, 98, 111, 100, 121, 62, 60,
  ...
  it("extract_batch_bytes_invalid_mime: extract_batch with invalid bytes MIME type", async () => {
    await extractBatch(
      [{ bytes: [72, 101, 108, 108, 111], kind: "bytes", mimeType: "application/x-nonexistent" }],
      undefined,
    );
  }, 30000);
  ...
  it("extract_batch_uri_all_missing: extract_batch with missing URI inputs", async () => {
    await extractBatch(
      [
        ...
        { kind: "uri", uri: "/nonexistent/b.txt" },
      ],
      undefined,
    );
  }, 30000);
  it("extract_batch_uri_basic: extract_batch over URI inputs", async () => {
    await extractBatch(
      [
        { kind: "uri", uri: "pdf/fake_memo.pdf" },
        { kind: "uri", uri: "text/fake_text.txt" },
      ],
      undefined,
    );
  }, 30000);
  ...
  it("extract_batch_uri_partial_failure: extract_batch with mixed valid and missing URI inputs", async () => {
    await extractBatch(
      [
        { kind: "uri", uri: "text/plain.txt" },
        { kind: "uri", uri: "/nonexistent/missing.pdf" },
      ],
      undefined,
    );
  }, 30000);
