...
describe("rerank_async_pending", () => {

	it("async_rerank: rerank_async: plugin model with no backend registered returns an error", async () => {
		await expect(async () => {
			await rerankAsync("neural networks", ["Neural networks are a class of machine learning models inspired by biological brains.", "The Eiffel Tower is located in Paris.", "Convolutional neural networks excel at image recognition tasks."], { model: { name: "async_rerank", type: "plugin" } } as ExtractionConfig);
		}).rejects.toThrow();
	}, 30000);
});
