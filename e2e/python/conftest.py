...
# alef:hash:28a8ee2da303e7eef2028c32dcf0f6dae43dab2bb22a183aa29fdf03d0fc72d2
...
def _find_pdfium_dir() -> str | None:
    ...
    for _candidate in sorted(
        _REPO_ROOT.glob("target/*/release/build/*/out/libpdfium*")
    ):
...
if _pdfium_dir is not None:
    for _var in ("DYLD_LIBRARY_PATH", "LD_LIBRARY_PATH"):
        ...
        if _pdfium_dir not in _existing:
            os.environ[_var] = (
                f"{_pdfium_dir}:{_existing}" if _existing else _pdfium_dir
            )
