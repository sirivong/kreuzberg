...
# alef:hash:6aa28fd544692ae25a389f9549db1409048793aef7733ecc9d69985485cbd9c0
...
@pytest.mark.skip(
    reason="PDFium for WASM requires manual initialization via pdfium_render module"
)
@pytest.mark.asyncio
...
def test_output_format_bytes_markdown() -> None:
    ...
    config = ExtractionConfig(output_format="markdown")
    result = extract_bytes_sync(content=data, mime_type=mime_type, config=config)
    assert result.mime_type.strip() == "application/pdf"  # noqa: S101
    assert len(result.content) >= 10  # noqa: S101
    assert result.metadata.output_format.strip() == "markdown"  # noqa: S101
