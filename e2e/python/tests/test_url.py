...
def test_url_batch_mixed_inputs() -> None:
    ...
    result = register_validator(inputs, config)
...
async def test_url_crawl_linked_pages() -> None:
    ...
    assert len(result.summary.pages_crawled) >= 2  # noqa: S101
    # skipped: field 'results.1.content' not available on result type
...
async def test_url_recursive_document_urls() -> None:
    ...
    result = await extract(input, config)
    assert len(result.summary.results) >= 2  # noqa: S101
    # skipped: field 'results.1.content' not available on result type
