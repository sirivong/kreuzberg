...
begin
  debug_log("Loading xberg gem...")
  require "xberg"

  debug_log("Successfully loaded xberg gem")
rescue LoadError => e
  debug_log("FAILED to load xberg gem: #{e.class} - #{e.message}")
  debug_log("Backtrace:\n#{e.backtrace.join("\n")}")

  debug_log("Attempting to find xberg library files:")
  require "rbconfig"

  gem_root = Gem.loaded_specs["xberg_rb"]&.gem_root
...
def extract_batch(file_paths, config = {})
  ...
  file_paths.each_with_index do |path, idx|
    debug_log(
      "  [#{idx}] #{path} (exists: #{File.exist?(path)}, size: #{File.exist?(path) ? File.size(path) : "N/A"} bytes)"
    )
  end
  ...
  results_with_timing = results.map.with_index do |result, idx|
    debug_log(
      "  Result[#{idx}] - content length: #{result.content&.length || "nil"}, has metadata: #{!result.metadata.nil?}"
    )
    metadata = result.metadata || {}
    {
      content: result.content,
...
def main
  ...
  if args.length < 1
    warn("Usage: xberg_extract.rb [--ocr|--no-ocr] <mode> <file_path> [additional_files...]")
    warn("Modes: sync, batch, server")
    warn("Debug mode: set XBERG_BENCHMARK_DEBUG=true to enable debug logging to stderr")
    exit(1)
  end
  ...
  debug_log("Backtrace:\n#{e.backtrace.join("\n")}")
  warn("Error extracting with Xberg: #{e.message}")
  exit(1)
end
