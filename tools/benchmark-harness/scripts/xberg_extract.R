...
DEBUG <- identical(Sys.getenv('XBERG_BENCHMARK_DEBUG'), 'true')
...
main <- function() {
  ...
  if (length(parsed_args) < 1) {
    cat("Usage: xberg_extract.R [--ocr|--no-ocr] <mode> <file_path> [additional_files...]\n", file = stderr())
    cat("Modes: sync, batch, server\n", file = stderr())
    cat("Debug mode: set XBERG_BENCHMARK_DEBUG=true to enable debug logging to stderr\n", file = stderr())
    quit(status = 1)
  }
