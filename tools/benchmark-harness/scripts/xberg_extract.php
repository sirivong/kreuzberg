...
function main(): void
{
    ...
    if (count($args) < 1) {
        fwrite(STDERR, "Usage: xberg_extract.php [--ocr|--no-ocr] <mode> <file_path> [additional_files...]\n");
        fwrite(STDERR, "Modes: sync, batch, server\n");
        fwrite(STDERR, "Debug mode: set XBERG_BENCHMARK_DEBUG=true to enable debug logging to stderr\n");
        exit(1);
    }
